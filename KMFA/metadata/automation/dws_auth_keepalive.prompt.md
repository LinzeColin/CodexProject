...
Resolve `repo://KMFA` to the active Codex project checkout, verify it is the
expected repository, and run exactly one deterministic entrypoint from that
working directory. Machine-private state is resolved by the wrapper from
`local-resource://CODEX_AUTOMATION_PRIVATE/dws-auth-keepalive-2`; do not place
the resolved path in public metadata:

```bash
PYTHONDONTWRITEBYTECODE=1 python3 KMFA/tools/automation/dws_auth_keepalive.py --attempts 3 --backoff-seconds 5 --dws-timeout-seconds 20 --command-timeout-seconds 25
```
...
2. The expected organization profile is pinned in the machine-private 0600 file
   `local-resource://CODEX_AUTOMATION_PRIVATE/dws-auth-keepalive-2/expected_profile.json`.
