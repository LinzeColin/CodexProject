...
description: Use when operating, reviewing, migrating, or modifying KMFA DingTalk daily routine check automation for payments, production management, routine deliverables, and role-based cash OCR monitoring.
---
...
`<resolved-zip-parent>/DWS_Outputs/` folder
...
- Do not commit real-person names, private identifiers, or machine-local paths; resolve `ROLE::...` and `ENV::...` references only from private runtime configuration.
- Missing private identity, input, runtime, archive, or notification bindings must fail closed before private I/O or delivery.
...
- ROLE::CASH_REPORT_SENDER：资金账户明细表
- ROLE::CASH_REPORT_SENDER：资金流水明细 / 资金明细

Weekly Monday:

- ROLE::CASH_REPORT_SENDER：现存票据
- ROLE::CASH_REPORT_SENDER：保证金回款表
- ROLE::CASH_REPORT_SENDER：回款明细表

Monthly:

- ROLE::TAX_REPORT_SENDER：纳税申报表
- ROLE::TAX_REPORT_SENDER：汇算清缴
...
- ROLE::PRODUCTION_REPORT_SENDER_PRIMARY or ROLE::PRODUCTION_REPORT_SENDER_BACKUP：每日人员表

Weekly Thursday:

- ROLE::PRODUCTION_REPORT_SENDER_PRIMARY or ROLE::PRODUCTION_REPORT_SENDER_BACKUP：资金计划

Weekly Friday:

- ROLE::PRODUCTION_REPORT_SENDER_PRIMARY：工人工时表
- ROLE::PRODUCTION_REPORT_SENDER_PRIMARY：应付表

### Cash Monitoring

- Sender: ROLE::CASH_REPORT_SENDER
- Group: 付款请示群
...
- Missing/late/low-confidence/OCR conflict: notify ROLE::OWNER
...
```text
ENV::KMFA_DAILY_ROUTINE_ARCHIVE_ROOT
```
...
Every run log must include `run_at_beijing`, `check_date`, `trigger_window`, `rules_evaluated`, and `rules_skipped`. Missing or stale upstream DWS output must be recorded as `SOURCE_MISSING` or `SOURCE_STALE` and surfaced to ROLE::OWNER through the notification policy.
