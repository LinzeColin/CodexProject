...
class S16StageReviewEvidenceTests(unittest.TestCase):
    ...
    def test_cross_phase_connections_and_findings_are_closed(self) -> None:
        ...
        self.assertEqual(
            ...
            ["S16REV-F001", "S16REV-F002", "S16REV-F003"],
        )
        self.assertTrue(
            all(
    ...
    def test_pending_manifest_stops_before_s17(self) -> None:
        current = builder.manifest()
        self.assertEqual(current["phase_acceptance_status"], "PENDING_FINAL_VALIDATION")
        self.assertEqual(current["decision"], "REMAIN_IN_S16_STAGE_REVIEW")
        self.assertTrue(current["s16_stage_review_started"])
        self.assertFalse(current["s16_stage_review_performed"])
        self.assertFalse(current["s17_entry_allowed"])
        self.assertFalse(current["s17_p1_entry_allowed"])
        self.assertFalse(current["s17_p1_started"])
        self.assertFalse(current["github_upload_performed"])
        self.assertFalse(current["app_reinstall_performed"])
        self.assertEqual(current["overall_accepted_phase_count"], 46)
