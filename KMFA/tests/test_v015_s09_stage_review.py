...
class V015S09StageReviewTests(unittest.TestCase):
    def test_source_and_predecessor_accounting(self) -> None:
        self.assertEqual(builder.source_contract()["s09_counts"], {"phases": 3, "tasks": 9})
        self.assertEqual(
    ...
    def test_findings_fixed_and_risks_routed(self) -> None:
        self.assertEqual([row["finding_id"] for row in builder.findings()], ["S09REV-F001", "S09REV-F002", "S09REV-F003"])
    ...
    def test_manifest_pending_and_final_truth(self) -> None:
        ...
        self.assertEqual((pending["decision"], pending["stage_acceptance_status"]), ("REMAIN_IN_S09_STAGE_REVIEW", "PENDING"))
        self.assertFalse(pending["s10_p1_entry_allowed"])
        sample = [
            {
                "status": "PASS",
                "exit_code": 0,
        ...
        self.assertEqual((final["decision"], final["stage_acceptance_status"]), ("GO_TO_S10_P1_ONLY", "PASSED"))
        self.assertTrue(final["s10_p1_entry_allowed"])
        self.assertFalse(final["s10_p1_started"])
        for row in (pending, final):
            self.assertEqual(row["overall_accepted_phase_count"], 25)
    ...
    def test_materialized_files_parse(self) -> None:
        for name, expected in (("stage9_review_findings_public_safe.csv", 3), ("open_risk_register_public_safe.csv", 5)):
