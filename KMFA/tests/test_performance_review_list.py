...
class PerformanceReviewListTests(unittest.TestCase):
    ...
    def test_performance_fact_table_uses_public_safe_refs_and_statuses_only(self) -> None:
        ...
        for row in fact_rows:
            ...
            binding = row["fact_binding_summary"]
            self.assertEqual(binding["required_slot_count"], len(REQUIRED_PERFORMANCE_REVIEW_FIELDS))
            self.assertEqual(binding["private_binding_revalidation_required_slot_count"], 2)
            self.assertEqual(binding["manual_review_required_slot_count"], 4)
            self.assertEqual(binding["binding_status"], "PRIVATE_BINDING_REVALIDATION_REQUIRED")
            self.assertFalse(binding["private_digest_values_committed"])
            self.assertNotIn("fact_status_by_field", row)
            self.assertNotIn("fact_hash_refs_by_field", row)
