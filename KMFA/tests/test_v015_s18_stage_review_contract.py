...
class S18StageReviewContractTests(unittest.TestCase):
    ...
    def test_public_verification_has_exact_accounting(self) -> None:
        ...
        self.assertEqual(
            value["accounting"], {"total": 246, "passed": 246, "failed": 0}
        )
        self.assertEqual(value["failed_checks"], [])

    def test_both_fix_markers_are_bound_to_live_code(self) -> None:
        roots = (
            contract.runtime.__file__,
            contract.p1.__file__,
            contract.p3.__file__,
        )
        code = contract.render_html() + "\n" + "\n".join(
            Path(path).read_text(encoding="utf-8") for path in roots
        )
    ...
    def test_technical_audit_has_no_open_issue(self) -> None:
        ...
        self.assertEqual(
            ...
            {"money_consistency", "scope_isolation", "cash_safety", "report_honesty", "human_usability"},
        )
