...
class V014S03P1FileRegistrationTests(unittest.TestCase):
    def test_build_registration_keeps_raw_names_and_hashes_private(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertNotIn(str(raw), public_text)
            self.assertEqual(public["raw_root_status"]["raw_root_id"], "PRIMARY_RAW_ROOT")
            self.assertIsNone(public["raw_root_status"]["raw_root_path"])
            self.assertIsNone(public["raw_root_status"]["public_path_value"])
            self.assertTrue(public["raw_root_status"]["private_registry_ref"].endswith("private_root_policy.json"))
