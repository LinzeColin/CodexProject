...
FORMULA_ID = "FORM-KMFA-V015-S19-STAGE-REVIEW-001"


class V015S19StageReviewGovernanceTests(unittest.TestCase):
    def test_registry_bindings(self) -> None:
        ...
        selected = [
            ...
            and 2726 <= int(row["parameter_id"].rsplit("-", 1)[-1]) <= 2745
        ]
        ...
        for token in (
            ...
            "public_check_count == 278",
            "fixed_review_finding_count == 2",
            "open_review_finding_count == 0",
            "navigation_dead_end_count == 0",
            "tampered_event_accept_count == 0",
            "scope_leak_count == 0",
            "raw_root_access_count == 0",
        ):
            self.assertIn(token, formula)
        self.assertIn("kmfa_v015_s19_stage_review:", model)
        self.assertIn("kmfa_v015_s19_stage_review:", metadata_model)
    ...
    def test_project_mirrors_name_pending_review(self) -> None:
        for relative in (
            ...
            for token in (
                ...
                "s19_stage_review_performed: false",
    ...
    def test_traceability_version_and_assurance(self) -> None:
        governance = builder.PROJECT_ROOT / "docs/governance"
        self.assertIn(
            "REQ-KMFA-V015-S19-STAGE-REVIEW",
        ...
        self.assertIn(f'kmfa_v015_s19_stage_review: "{builder.VERSION}"', version)
        assurance = (governance / "ASSURANCE_STATUS.yaml").read_text(encoding="utf-8")
        self.assertIn("total_active_parameters: 2360", assurance)
        self.assertIn("total_active_formulas: 392", assurance)
