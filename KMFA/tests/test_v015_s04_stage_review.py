...
class V015S04StageReviewTests(unittest.TestCase):
    def test_source_and_predecessor_accounting(self) -> None:
        self.assertEqual(builder.source_contract()["s04_counts"], {"phases": 3, "tasks": 9})
        self.assertEqual(builder.phase_evidence()["accounting"], {
            "phase_count": 3, "phase_passed_count": 3, "task_count": 9,
            "task_accepted_count": 9, "predecessor_receipt_count": 45,
        })

    def test_cross_phase_contracts_pass(self) -> None:
        result = builder.cross_phase_contracts()
        self.assertEqual(result["accounting"], {"total": 14, "passed": 14, "failed": 0, "blocking_failed": 0})
    ...
    def test_findings_and_risks_are_closed_or_routed(self) -> None:
        self.assertEqual([row["finding_id"] for row in builder.findings()], ["S04REV-F001", "S04REV-F002"])
    ...
    def test_manifest_truth_in_both_receipt_states(self) -> None:
        pending = builder.manifest(final_validation=False)
        self.assertEqual(pending["decision"], "REMAIN_IN_S04_STAGE_REVIEW")
        self.assertEqual(pending["stage_acceptance_status"], "PENDING")
        self.assertFalse(pending["s05_p1_entry_allowed"])
        sample = [{"status": "PASS", "validation_head": "a" * 40, "validation_run_id": "run"}]
        final = builder.manifest(final_validation=True, receipts=sample)
        self.assertEqual(final["decision"], "GO_TO_S05_P1_ONLY")
        self.assertEqual(final["stage_acceptance_status"], "PASSED")
        self.assertTrue(final["s05_p1_entry_allowed"])
        self.assertFalse(final["s05_p1_started"])
        for row in (pending, final):
            self.assertEqual(row["raw_root_access_count"], 0)
            self.assertEqual(row["actual_business_lineage_record_count"], 0)
    ...
    def test_materialized_csvs_parse(self) -> None:
        for name, expected in (("stage4_review_findings_public_safe.csv", 2), ("open_risk_register_public_safe.csv", 5)):
