...
class QualityRulesGovernanceTests(unittest.TestCase):
    ...
    def test_hard_gate_and_release_boundaries_are_explicit(self) -> None:
        manifest = json.loads(builder.MANIFEST_PATH.read_text(encoding="utf-8"))
        self.assertTrue(manifest["high_score_critical_failure_blocked"])
        self.assertFalse(manifest["score_can_override_hard_gate"])
