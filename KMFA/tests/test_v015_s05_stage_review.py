...
class V015S05StageReviewTests(unittest.TestCase):
    def test_source_and_predecessor_accounting(self) -> None:
        self.assertEqual(builder.source_contract()["s05_counts"], {"phases": 3, "tasks": 9})
        self.assertEqual(builder.phase_evidence()["accounting"], {
            "phase_count": 3, "phase_passed_count": 3, "task_count": 9,
            "task_accepted_count": 9, "predecessor_receipt_count": 52,
        })

    def test_cross_phase_contracts_pass(self) -> None:
        result = builder.cross_phase_contracts()
        self.assertEqual(result["accounting"], {"total": 16, "passed": 16, "failed": 0, "blocking_failed": 0})
    ...
    def test_findings_and_risks_are_closed_or_routed(self) -> None:
        self.assertEqual([row["finding_id"] for row in builder.findings()], ["S05REV-F001", "S05REV-F002"])
    ...
    def test_manifest_truth_in_pending_and_final_states(self) -> None:
        ...
        self.assertEqual((pending["decision"], pending["stage_acceptance_status"]), ("REMAIN_IN_S05_STAGE_REVIEW", "PENDING"))
        self.assertFalse(pending["s06_p1_entry_allowed"])
        ...
        self.assertEqual((final["decision"], final["stage_acceptance_status"]), ("GO_TO_S06_P1_ONLY", "PASSED"))
        self.assertTrue(final["s06_p1_entry_allowed"])
        self.assertFalse(final["s06_p1_started"])
    ...
    def test_materialized_csvs_parse(self) -> None:
        for name, expected in (("stage5_review_findings_public_safe.csv", 2), ("open_risk_register_public_safe.csv", 5)):
