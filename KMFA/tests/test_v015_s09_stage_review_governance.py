...
FORMULA_ID = "FORM-KMFA-V015-S09-STAGE-REVIEW-001"


class V015S09StageReviewGovernanceTests(unittest.TestCase):
    def test_registry_bindings(self) -> None:
        ...
        selected = [
            ...
            and 2075 <= int(row["parameter_id"].rsplit("-", 1)[-1]) <= 2081
        ]
        ...
        self.assertIn("predecessor_receipt_count == 60", formula)
        self.assertIn("binding_check_pass_count == 30", formula)
        self.assertIn("cross_phase_contract_pass_count == 24", formula)
        self.assertIn("kmfa_v015_s09_stage_review:", model)
        self.assertIn("kmfa_v015_s09_stage_review:", metadata_model)

    def test_traceability_version_and_assurance(self) -> None:
        governance = builder.PROJECT_ROOT / "docs/governance"
        self.assertIn("REQ-KMFA-V015-S09-STAGE-REVIEW", (governance / "TRACEABILITY_MATRIX.csv").read_text(encoding="utf-8"))
        ...
        self.assertIn("total_active_parameters: 1696", assurance)
        self.assertIn("total_active_formulas: 352", assurance)
