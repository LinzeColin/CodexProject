...
FORMULA_ID = "FORM-KMFA-V015-S08-STAGE-REVIEW-001"


class V015S08StageReviewGovernanceTests(unittest.TestCase):
    def test_registry_bindings(self) -> None:
        ...
        selected = [row for row in rows if row["parameter_id"].startswith("PARAM-KMFA-") and 2040 <= int(row["parameter_id"].rsplit("-", 1)[-1]) <= 2045]
        ...
        self.assertIn("binding_check_pass_count == 20", formula)
        self.assertIn("cross_phase_contract_pass_count == 22", formula)
        self.assertIn("kmfa_v015_s08_stage_review:", model)
        self.assertIn("kmfa_v015_s08_stage_review:", metadata_model)

    def test_traceability_version_and_assurance(self) -> None:
        governance = builder.PROJECT_ROOT / "docs/governance"
        self.assertIn("REQ-KMFA-V015-S08-STAGE-REVIEW", (governance / "TRACEABILITY_MATRIX.csv").read_text(encoding="utf-8"))
        ...
        self.assertIn("total_active_parameters: 1660", assurance)
        self.assertIn("total_active_formulas: 348", assurance)
