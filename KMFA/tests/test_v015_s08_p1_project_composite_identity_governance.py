...
FORMULA_ID = "FORM-KMFA-V015-S08-P1-PROJECT-COMPOSITE-IDENTITY-001"
PARAMETER_IDS = [f"PARAM-KMFA-{number}" for number in range(2014, 2022)]


class ProjectCompositeIdentityGovernanceTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_project_governance_matches_phase_state(self) -> None:
        status = "PASSED" if self.final else "PENDING_FINAL_VALIDATION"
        decision = "CONTINUE_TO_S08_P2_ONLY" if self.final else "REMAIN_IN_S08_P1_FINAL_VALIDATION"
        for relative in ("docs/governance/project.yaml", "metadata/project/project.yaml", "docs/governance/roadmap.yaml"):
            text = (builder.PROJECT_ROOT / relative).read_text(encoding="utf-8")
            for token in (
                "V015_S08_P1_PROJECT_COMPOSITE_IDENTITY",
                "KMFA-V015-S08-P1-PROJECT-COMPOSITE-IDENTITY-20260715",
                "ACC-KMFA-V015-S08-P1-PROJECT-COMPOSITE-IDENTITY",
                ...
                f"s08_p2_entry_allowed: {str(self.final).lower()}",
    ...
    def test_formula_model_and_parameters_are_registered_once(self) -> None:
        ...
        self.assertEqual(sum(row.get("status") == "active" for row in rows), 1636)

    def test_chinese_entry_files_describe_real_boundary(self) -> None:
        for relative in ("功能清单.md", "开发记录.md", "模型参数文件.md", "HANDOFF.md"):
            text = (builder.PROJECT_ROOT / relative).read_text(encoding="utf-8")
            self.assertIn("S08-P1", text)
            self.assertIn("128", text)
            self.assertIn("6", text)
        handoff = (builder.PROJECT_ROOT / "HANDOFF.md").read_text(encoding="utf-8")
        self.assertLess(handoff.index("S08-P1"), handoff.index("S07 整体复审"))

    def test_later_scope_and_release_remain_closed(self) -> None:
        self.assertFalse(self.manifest["s08_p2_started"])
        self.assertFalse(self.manifest["s08_p3_entry_allowed"])
