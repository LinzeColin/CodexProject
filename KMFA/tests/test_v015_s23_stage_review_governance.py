...
FORMULA_ID = "FORM-KMFA-V015-S23-STAGE-REVIEW-001"


class Stage23ReviewGovernanceTests(unittest.TestCase):
    def test_registry_bindings(self) -> None:
        ...
        selected = [row for row in rows if row["parameter_id"].startswith("PARAM-KMFA-") and 3046 <= int(row["parameter_id"].rsplit("-", 1)[-1]) <= 3065]
        ...
        self.assertIn("kmfa_v015_s23_stage_review:", model)
        self.assertIn("kmfa_v015_s23_stage_review:", metadata_model)
    ...
    def test_project_mirrors_match_review_lifecycle(self) -> None:
        ...
        for relative in ("docs/governance/project.yaml", "metadata/project/project.yaml"):
            ...
            for token in (
                ...
                "active_formula_count: 408",
                "active_parameter_count: 2680",
                'current_parameter_range: "PARAM-KMFA-3046..3065"',
                "s23_stage_review_started: true",
                f"s23_stage_review_performed: {str(passed).lower()}",
                f"s24_entry_allowed: {str(passed).lower()}",
                f"s24_p1_entry_allowed: {str(passed).lower()}",
                "s24_started: false",
            ):
                self.assertIn(token, text, relative)
            if passed:
                self.assertIn(f's23_stage_review_validation_run_id: "{manifest["validation_run_id"]}"', text)
                self.assertIn(f's23_stage_review_validation_head: "{manifest["validation_head"]}"', text)

    def test_traceability_version_and_assurance(self) -> None:
        governance = builder.PROJECT_ROOT / "docs/governance"
        self.assertIn("REQ-KMFA-V015-S23-STAGE-REVIEW", (governance / "TRACEABILITY_MATRIX.csv").read_text(encoding="utf-8"))
        version = (governance / "VERSION_MATRIX.yaml").read_text(encoding="utf-8")
        self.assertIn(f'{MODEL_ID}: "{builder.VERSION}"', version)
        self.assertIn(f'kmfa_v015_s23_stage_review: "{builder.VERSION}"', version)
        assurance = (governance / "ASSURANCE_STATUS.yaml").read_text(encoding="utf-8")
        self.assertIn("total_active_parameters: 2680", assurance)
        self.assertIn("total_active_formulas: 408", assurance)
