...
class CheckBoardInterfaceGovernanceTests(unittest.TestCase):
    def test_manifest_governance_is_current_and_review_is_closed_until_acceptance(self) -> None:
        ...
        self.assertEqual(manifest["s11_p2_acceptance_status"], "PASSED")
        self.assertTrue(manifest["s11_p3_started"])
    ...
    def test_interface_and_release_boundaries_are_explicit(self) -> None:
        manifest = json.loads(builder.MANIFEST_PATH.read_text(encoding="utf-8"))
        self.assertEqual(manifest["frontend_status_write_count"], 0)
        self.assertFalse(manifest["frontend_status_mutation_allowed"])
        self.assertEqual(manifest["large_yellow_surface_count"], 0)
        self.assertEqual(manifest["large_status_color_surface_count"], 0)
