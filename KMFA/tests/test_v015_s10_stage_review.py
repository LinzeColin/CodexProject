...
class V015S10StageReviewTests(unittest.TestCase):
    def test_source_and_predecessor_accounting(self) -> None:
        self.assertEqual(builder.source_contract()["s10_counts"], {"phases": 3, "tasks": 9})
        self.assertEqual(builder.phase_evidence()["accounting"], {"phase_count": 3, "phase_passed_count": 3, "task_count": 9, "task_accepted_count": 9, "predecessor_receipt_count": 57})

    def test_cross_phase_contracts_pass(self) -> None:
        result = builder.cross_phase_contracts()
        self.assertEqual(result["accounting"], {"total": 24, "passed": 24, "failed": 0, "blocking_failed": 0})

    def test_findings_fixed_and_risks_routed(self) -> None:
        self.assertEqual([row["finding_id"] for row in builder.findings()], ["S10REV-F001", "S10REV-F002", "S10REV-F003"])
    ...
    def test_manifest_pending_and_final_truth(self) -> None:
        ...
        self.assertEqual((pending["decision"], pending["stage_acceptance_status"]), ("REMAIN_IN_S10_STAGE_REVIEW", "PENDING"))
        self.assertFalse(pending["s11_p1_entry_allowed"])
        ...
        self.assertEqual((final["decision"], final["stage_acceptance_status"]), ("GO_TO_S11_P1_ONLY", "PASSED"))
        self.assertTrue(final["s11_p1_entry_allowed"])
        self.assertFalse(final["s11_p1_started"])
        for row in (pending, final):
            self.assertEqual(row["overall_accepted_phase_count"], 28)
    ...
    def test_materialized_files_parse(self) -> None:
        for name, expected in (("stage10_review_findings_public_safe.csv", 3), ("open_risk_register_public_safe.csv", 5)):
