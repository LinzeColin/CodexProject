...
class S19StageReviewEvidenceTests(unittest.TestCase):
    ...
    def test_cross_phase_connections_and_findings_are_closed(self) -> None:
        ...
        self.assertEqual(
            cross["accounting"],
            {"total": 44, "passed": 44, "failed": 0, "blocking_failed": 0},
        )
        self.assertEqual(
            [row["finding_id"] for row in builder.contract.REVIEW_FINDINGS],
            ["S19REV-F001", "S19REV-F002"],
        )
        self.assertTrue(
            all(
    ...
    def test_pending_manifest_stops_before_s20(self) -> None:
        current = builder.manifest()
        self.assertEqual(current["phase_acceptance_status"], "PENDING_FINAL_VALIDATION")
        self.assertEqual(current["decision"], "REMAIN_IN_S19_STAGE_REVIEW")
        self.assertTrue(current["s19_stage_review_started"])
        self.assertFalse(current["s19_stage_review_performed"])
        self.assertFalse(current["s20_entry_allowed"])
        self.assertFalse(current["s20_p1_entry_allowed"])
        self.assertFalse(current["s20_p1_started"])
        self.assertFalse(current["github_upload_performed"])
        self.assertFalse(current["app_reinstall_performed"])
        self.assertEqual(current["overall_accepted_phase_count"], 55)
