...
class V015S07StageReviewTests(unittest.TestCase):
    def test_source_and_predecessor_accounting(self) -> None:
        self.assertEqual(builder.source_contract()["s07_counts"], {"phases": 3, "tasks": 9})
        self.assertEqual(builder.phase_evidence()["accounting"], {
            "phase_count": 3, "phase_passed_count": 3, "task_count": 9,
            "task_accepted_count": 9, "predecessor_receipt_count": 54,
        })
    ...
    def test_findings_and_risks_are_closed_or_routed(self) -> None:
        self.assertEqual([row["finding_id"] for row in builder.findings()], ["S07REV-F001", "S07REV-F002"])
    ...
    def test_manifest_truth_in_pending_and_final_states(self) -> None:
        ...
        self.assertEqual((pending["decision"], pending["stage_acceptance_status"]), ("REMAIN_IN_S07_STAGE_REVIEW", "PENDING"))
        self.assertFalse(pending["s08_p1_entry_allowed"])
        ...
        self.assertEqual((final["decision"], final["stage_acceptance_status"]), ("GO_TO_S08_P1_ONLY", "PASSED"))
        self.assertTrue(final["s08_p1_entry_allowed"])
        self.assertFalse(final["s08_p1_started"])
    ...
    def test_materialized_csvs_parse(self) -> None:
        for name, expected in (("stage7_review_findings_public_safe.csv", 2), ("open_risk_register_public_safe.csv", 5)):
