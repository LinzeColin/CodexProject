...
class S20StageReviewEvidenceTests(unittest.TestCase):
    ...
    def test_cross_phase_connections_and_findings_are_closed(self) -> None:
        ...
        self.assertEqual([row["finding_id"] for row in builder.contract.REVIEW_FINDINGS], ["S20REV-F001", "S20REV-F002"])
        self.assertTrue(all(row["status"] == "FIXED_VALIDATED" for row in builder.contract.REVIEW_FINDINGS))

    def test_pending_manifest_stops_before_s21(self) -> None:
        current = builder.manifest()
        self.assertEqual(current["phase_acceptance_status"], "PENDING_FINAL_VALIDATION")
        self.assertEqual(current["decision"], "REMAIN_IN_S20_STAGE_REVIEW")
        self.assertTrue(current["s20_stage_review_started"])
        self.assertFalse(current["s20_stage_review_performed"])
        self.assertFalse(current["s21_entry_allowed"])
        self.assertFalse(current["s21_p1_entry_allowed"])
        self.assertFalse(current["s21_p1_started"])
        self.assertFalse(current["github_upload_performed"])
        self.assertFalse(current["app_reinstall_performed"])
        self.assertEqual(current["overall_accepted_phase_count"], 58)
