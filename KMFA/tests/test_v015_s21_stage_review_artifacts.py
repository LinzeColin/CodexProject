...
class Stage21ReviewArtifactTests(unittest.TestCase):
    ...
    def test_manifest_stops_before_s22_until_formal_acceptance(self) -> None:
        manifest = json.loads(builder.MANIFEST_PATH.read_text(encoding="utf-8"))
        self.assertFalse(manifest["s22_p1_started"])
        self.assertEqual(manifest["overall_phase_accepted_count"], 61)
