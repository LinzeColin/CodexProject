#!/usr/bin/env python3
"""执行并记录 KMFA v1.5 S15-P2 唯一一组正式验收。"""
...
from KMFA.tools import check_v015_s15_p2_identity_roles as checker
...
def _require_clean_subject() -> None:
    for args in (("diff", "--quiet"), ("diff", "--cached", "--quiet")):
        if subprocess.run(["git", *args], cwd=REPO_ROOT, check=False).returncode:
            raise ValidationError("validation requires a clean tracked S15-P2 implementation commit")
    untracked = [
...
def run() -> list[dict[str, Any]]:
    ...
    if (
        ...
        or subject.get("s15_p3_entry_allowed") is not False
        or subject.get("s15_p3_started") is not False
        ...
        raise ValidationError("validation subject must be pending S15-P2 with later and release work closed")
    ...
    for name, command in checker.EXPECTED_VALIDATIONS:
        ...
        receipt = {
            "schema_version": "kmfa.v015.s15p2.validation_receipt.v1",
...
def main() -> int:
    try:
        ...
        print(f"FAIL: {error}")
        return 1
    print(f"PASS: S15-P2 validation receipts run={rows[0]['validation_run_id']} count={len(rows)} head={rows[0]['validation_head']}")
    return 0
