#!/usr/bin/env python3
"""Run one exact formal validation set for the KMFA v1.5 S12 review."""
...
from KMFA.tools import check_v015_s12_stage_review as checker
...
def run() -> list[dict[str, Any]]:
    ...
    if (
        ...
        or subject.get("s12_stage_review_acceptance_status") != "PENDING_FINAL_VALIDATION"
        or subject.get("s13_p1_entry_allowed") is not False
        or subject.get("s13_p1_started") is not False
        or subject.get("github_upload_performed") is not False
        or subject.get("app_reinstall_performed") is not False
    ):
        raise ValidationError("validation subject must be pending S12 review with S13 and release actions closed")
    ...
    for name, command in checker.EXPECTED_VALIDATIONS:
        ...
        receipt = {
            "schema_version": "kmfa.v015.s12_stage_review.validation_receipt.v1",
