#!/usr/bin/env python3
"""严格检查 KMFA v1.5 S23 整体复审及正式验收绑定。"""
...
P3_BROWSER_EVIDENCE = "KMFA/stage_artifacts/V015_S23_P3_STABILITY_USABILITY/machine/browser_acceptance.json"

EXPECTED_VALIDATIONS = (
    ("review_contract", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B KMFA/tools/v015_s23_stage_review_contract.py"),
    ("focused_contract_tests", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B -m unittest KMFA.tests.test_v015_s23_stage_review_contract"),
    ("focused_browser_evidence_tests", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B -m unittest KMFA.tests.test_v015_s23_stage_review_browser"),
    ("focused_artifact_tests", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B -m unittest KMFA.tests.test_v015_s23_stage_review_artifacts"),
    ("focused_governance_tests", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B -m unittest KMFA.tests.test_v015_s23_stage_review_governance"),
    ("s23_p1_dependency", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B -c 'from KMFA.tools.v015_s23_p1_end_to_end_business_flow import public_verification; raise SystemExit(public_verification()[\"fail_count\"])'"),
    ("s23_p2_dependency", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B KMFA/tools/v015_s23_p2_precision_stress_extreme.py"),
    ("s23_p3_dependency", f"PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B KMFA/tools/v015_s23_p3_stability_usability.py --browser-evidence {P3_BROWSER_EVIDENCE}"),
    ("s23_p1_kernel_regression", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B -m unittest KMFA.tests.test_v015_s23_p1_end_to_end_business_flow"),
    ("s23_p1_runtime_regression", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B -m unittest KMFA.tests.test_v015_s23_p1_end_to_end_business_flow_runtime"),
    ("s23_p2_kernel_regression", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B -m unittest KMFA.tests.test_v015_s23_p2_precision_stress_extreme"),
    ("s23_p3_kernel_regression", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B -m unittest KMFA.tests.test_v015_s23_p3_stability_usability"),
    ("s23_p1_browser_regression", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B KMFA/tools/run_v015_s23_p1_browser_tests.py"),
    ("s23_p3_browser_regression", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B KMFA/tools/run_v015_s23_p3_browser_tests.py"),
    ("integrated_review_consistency", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B KMFA/tools/check_v015_s23_stage_review.py --integrated-review-check"),
    ("builder_exact_rebuild", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B KMFA/tools/build_v015_s23_stage_review.py --check"),
    ("stage_checker_pre_final", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B KMFA/tools/check_v015_s23_stage_review.py --pre-final --skip-validation-receipts --skip-clean-commit"),
    ...
    ("roadmap_sync_pending", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B KMFA/tools/v015_roadmap_governance_sync.py --check --validation-state S23_STAGE_REVIEW_PENDING_FINAL_VALIDATION"),
    ...
    ("governance_sync", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B KMFA/tools/check_v015_s23_stage_review.py --clean-governance-sync-check"),
    ...
    ("taskpack_source", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B KMFA/tools/check_v015_s23_stage_review.py --taskpack-source-check"),
    ("public_boundary", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B KMFA/tools/check_v015_s23_stage_review.py --public-boundary-check"),
...
ALLOWED_PREFIXES = (
    ...
    "KMFA/metadata/stage_status.jsonl", "KMFA/stage_artifacts/V015_S23_STAGE_REVIEW/",
    "KMFA/stage_artifacts/V015_S23_P3_STABILITY_USABILITY/exports/screenshots/01_management_task.png",
    "KMFA/stage_artifacts/V015_S23_P3_STABILITY_USABILITY/exports/screenshots/02_finance_task.png",
    ...
    "KMFA/tests/test_v015_s23_p3_stability_usability_browser.py",
    "KMFA/tests/test_v015_s23_stage_review_artifacts.py", "KMFA/tests/test_v015_s23_stage_review_browser.py",
    "KMFA/tests/test_v015_s23_stage_review_contract.py", "KMFA/tests/test_v015_s23_stage_review_governance.py",
    "KMFA/tools/build_v015_s23_stage_review.py", "KMFA/tools/check_v015_s23_stage_review.py",
    "KMFA/tools/run_v015_s23_stage_review_validations.py", "KMFA/tools/v015_roadmap_governance_sync.py",
    "KMFA/tools/v015_s23_stage_review_contract.py", "KMFA/功能清单.md", "KMFA/开发记录.md", "KMFA/模型参数文件.md",
)
...
def _check_scope() -> None:
    if subprocess.run(["git", "merge-base", "--is-ancestor", builder.REVIEW_BASE_COMMIT, "HEAD"], cwd=REPO_ROOT, check=False).returncode:
        raise CheckError("S23 review base is not an ancestor of HEAD")
    changed: set[str] = set()
    for args in (
...
def _check_taskpack_source() -> None:
    ...
    if source.get("stage_id") != "S23" or source.get("phase_ids") != ["S23-P1", "S23-P2", "S23-P3"] or source.get("source_integrity_status") != "PASS":
        raise CheckError("tracked S23 source contract drift")
...
def _check_public_boundary() -> None:
    ...
    roots = (builder.OUTPUT_ROOT, REPO_ROOT / "KMFA/tools/v015_s23_stage_review_contract.py")
    for root in roots:
        for path in ([root] if root.is_file() else [value for value in root.rglob("*") if value.is_file()]):
...
def _check_evidence(*, require_receipts: bool) -> None:
    ...
    if require_receipts and manifest.get("stage_acceptance_status") != "PASSED":
        raise CheckError("S23 stage review is not passed")
    if not require_receipts and manifest.get("stage_acceptance_status") not in {"PENDING", "PASSED"}:
        raise CheckError("S23 stage review pending manifest is invalid")
    ...
    limitations = _json(builder.LIMITATIONS_PATH)
    risks = _csv(builder.RISKS_PATH)
    if phase.get("accounting") != {"phase_count": 3, "phase_passed_count": 3, "task_count": 9, "task_accepted_count": 9, "predecessor_public_check_count": 156, "predecessor_receipt_count": 60}:
        raise CheckError("predecessor phase evidence drift")
    if integrated.get("integration_binding_count") != 40 or integrated.get("integration_binding_failed_count") != 0 or not integrated.get("stage_acceptance_ready"):
        raise CheckError("integrated review failed")
    if verification.get("public_check_count") != 40 or verification.get("public_check_failed_count") != 0:
    ...
    if len(findings) != 2 or any(row.get("status") != "FIXED_VALIDATED" or row.get("blocks_stage_acceptance") != "false" for row in findings):
        raise CheckError("review findings not closed")
    if len(limitations.get("limitations", [])) != 2 or limitations.get("open_blocking_count") != 0 or risks:
        raise CheckError("limitation or open-risk accounting drift")
    browser = _json(builder.BROWSER_CONTRACT_PATH)
    if browser.get("business_target_assertion_count") != 11 or browser.get("business_target_assertion_fail_count") != 0 or browser.get("visual_evidence_count") != 15:
        raise CheckError("browser review evidence drift")
    ...
    if manifest.get("s24_started") or manifest.get("s24_p1_started") or manifest.get("github_upload_performed") or manifest.get("app_reinstall_performed"):
        raise CheckError("out-of-scope lifecycle action recorded")
    if not require_receipts and manifest.get("s24_entry_allowed"):
        raise CheckError("S24 opened before formal validation")
    if require_receipts:
...
def _check_clean_governance_sync() -> None:
    ...
    try:
        ...
        result = subprocess.run([sys.executable, "-B", "scripts/validate_governance_sync.py", "--changed-only", "--base-ref", builder.REVIEW_BASE_COMMIT, "--enforce-sync"], cwd=REPO_ROOT, env=env, capture_output=True, text=True, check=False)
        if result.returncode:
...
def main() -> int:
    ...
    try:
        ...
        return 0
    except (CheckError, contract.StageReviewError, builder.BuildError, OSError, ValueError, KeyError, json.JSONDecodeError) as error:
