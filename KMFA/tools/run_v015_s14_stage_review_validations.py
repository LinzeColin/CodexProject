#!/usr/bin/env python3
"""Run one exact formal validation set for the KMFA v1.5 S14 review."""
...
from KMFA.tools import check_v015_s14_stage_review as checker
...
def run() -> list[dict[str, Any]]:
    ...
    if (
        ...
        or subject.get("s14_stage_review_acceptance_status") != "PENDING_FINAL_VALIDATION"
        or subject.get("s15_p1_entry_allowed") is not False
        or subject.get("s15_p1_started") is not False
        or subject.get("github_upload_performed") is not False
        or subject.get("app_reinstall_performed") is not False
    ):
        raise ValidationError("validation subject must be pending S14 review with S15 and release actions closed")
    ...
    for name, command in checker.EXPECTED_VALIDATIONS:
        ...
        receipt = {
            "schema_version": "kmfa.v015.s14_stage_review.validation_receipt.v1",
