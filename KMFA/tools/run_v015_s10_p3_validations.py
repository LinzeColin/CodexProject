#!/usr/bin/env python3
"""Run and receipt one exact validation set for KMFA v1.5 S10-P3."""
...
def _require_clean_subject() -> None:
    for args in (("diff", "--quiet"), ("diff", "--cached", "--quiet")):
        if subprocess.run(["git", *args], cwd=REPO_ROOT, check=False).returncode:
            raise ValidationError("validation requires a clean tracked S10-P3 implementation commit")
    untracked = [
        line
...
def run() -> list[dict[str, Any]]:
    ...
    if (
        ...
        or subject.get("s10_p3_started") is not True
        or subject.get("s10_stage_review_entry_allowed") is not False
        or subject.get("s10_stage_review_started") is not False
        or subject.get("automatic_connector_enabled_count") != 0
        or subject.get("github_upload_performed") is not False
        or subject.get("app_reinstall_performed") is not False
    ):
        raise ValidationError("validation subject must be pending S10-P3 with review, connectors and release work closed")
    ...
    for name, command in checker.EXPECTED_VALIDATIONS:
        ...
        receipt = {
            "schema_version": "kmfa.v015.s10p3.validation_receipt.v1",
...
def main() -> int:
    try:
        ...
        return 1
    print(
        "PASS: S10-P3 validation receipts "
