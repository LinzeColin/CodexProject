#!/usr/bin/env python3
"""Run and receipt one exact validation set for KMFA v1.5 S07-P1."""
...
from KMFA.tools import check_v015_s07_p1_zero_delta_validator as checker
...
def run() -> list[dict[str, Any]]:
    ...
    if (
        ...
        or subject.get("money_tolerance_cents") != 0
        or subject.get("private_zero_difference") is not True
        or subject.get("s07_p2_entry_allowed") is not False
    ):
        raise ValidationError("validation subject must be pending S07-P1 with S07-P2 closed")
    ...
    for name, command in checker.EXPECTED_VALIDATIONS:
        ...
        receipt = {
            "schema_version": "kmfa.v015.s07p1.validation_receipt.v1",
...
def main() -> int:
    try:
        ...
        return 1
    print(
        "PASS: S07-P1 validation receipts "
