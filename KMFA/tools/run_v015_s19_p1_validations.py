#!/usr/bin/env python3
"""执行并记录 KMFA v1.5 S19-P1 唯一一组正式验收。"""
...
from KMFA.tools import check_v015_s19_p1_tax_invoice_facts as checker
...
def _require_clean_subject() -> None:
    for args in (("diff", "--quiet"), ("diff", "--cached", "--quiet")):
        if subprocess.run(["git", *args], cwd=REPO_ROOT, check=False).returncode:
            raise ValidationError("validation requires a clean tracked S19-P1 implementation commit")
    untracked = [
        line
...
def run() -> list[dict[str, Any]]:
    ...
    if (
        ...
        or subject.get("s18_stage_review_acceptance_status") != "PASSED"
        or subject.get("s19_p1_started") is not True
        or subject.get("s19_p1_completed") is not False
        or subject.get("s19_p2_entry_allowed") is not False
        or subject.get("s19_p2_started") is not False
        ...
        raise ValidationError("validation subject must be pending S19-P1 with S19-P2 and release work closed")
    ...
    for name, command in checker.EXPECTED_VALIDATIONS:
        ...
        receipt = {
            "schema_version": "kmfa.v015.s19p1.validation_receipt.v1",
...
def main() -> int:
    try:
        ...
        return 1
    print(
        "PASS: S19-P1 validation receipts "
