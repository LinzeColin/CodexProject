#!/usr/bin/env python3
"""执行并记录 KMFA v1.5 S20-P1 唯一一组正式验收。"""
...
from KMFA.tools import check_v015_s20_p1_data_update as checker
...
def _require_clean_subject() -> None:
    for args in (("diff", "--quiet"), ("diff", "--cached", "--quiet")):
        if subprocess.run(["git", *args], cwd=REPO_ROOT, check=False).returncode:
            raise ValidationError("validation requires a clean tracked S20-P1 implementation commit")
    untracked = [
...
def run() -> list[dict[str, Any]]:
    ...
    if (
        ...
        or subject.get("s19_stage_review_acceptance_status") != "PASSED"
        or subject.get("s20_p1_started") is not True
        or subject.get("s20_p1_completed") is not False
        or subject.get("s20_p2_entry_allowed") is not False
        or subject.get("s20_p2_started") is not False
        ...
        raise ValidationError("validation subject must be pending S20-P1 with S20-P2 and release work closed")
    ...
    for name, command in checker.EXPECTED_VALIDATIONS:
        ...
        receipt = {
            "schema_version": "kmfa.v015.s20p1.validation_receipt.v1",
...
def main() -> int:
    try:
        ...
        return 1
    print(
        f"PASS: S20-P1 validation receipts run={rows[0]['validation_run_id']} "
