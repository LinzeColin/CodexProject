#!/usr/bin/env python3
"""Run and receipt one exact validation set for KMFA v1.5 S06-P3."""
...
from KMFA.tools import check_v015_s06_p3_baseline_coverage_boundary as checker
from KMFA.tools import v015_roadmap_governance_sync as roadmap
...
def run() -> list[dict[str, Any]]:
    ...
    if (
        ...
        or subject.get("fixture_consistent_with_golden") is not True
        or subject.get("open_item_count") != 147
        or subject.get("s06_stage_review_entry_allowed") is not False
    ):
        raise ValidationError("validation subject must be pending S06-P3 with Stage review closed")
    ...
    for name, command in checker.EXPECTED_VALIDATIONS:
        ...
        receipt = {
            "schema_version": "kmfa.v015.s06p3.validation_receipt.v1",
    ...
    roadmap.run(check=False, validation_state="PASSED")
    builder.write_outputs()
    builder.check_outputs()
    roadmap.run(check=True, validation_state="PASSED")
    return receipts
...
def main() -> int:
    try:
        ...
        return 1
    print(
        "PASS: S06-P3 validation receipts "
