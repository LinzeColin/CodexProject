#!/usr/bin/env python3
"""Strict receipt-bound checker for the KMFA v1.5 S12 stage review."""
...
EXPECTED_VALIDATIONS = (
    (
        "python_compile",
        "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B -c \"import ast,pathlib; [ast.parse(pathlib.Path(p).read_text(encoding='utf-8')) for p in ('KMFA/tools/v015_s12_stage_review_contract.py','KMFA/tools/build_v015_s12_stage_review.py','KMFA/tools/check_v015_s12_stage_review.py','KMFA/tools/run_v015_s12_stage_review_validations.py','KMFA/tools/v015_roadmap_governance_sync.py')]\"",
    ),
    ("focused_contract_tests", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B -m unittest KMFA.tests.test_v015_s12_stage_review_contract"),
    ("focused_review_tests", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B -m unittest KMFA.tests.test_v015_s12_stage_review"),
    ("focused_governance_tests", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B -m unittest KMFA.tests.test_v015_s12_stage_review_governance"),
    ("s12_p1_dependency", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B KMFA/tools/build_v015_s12_p1_project_cost_facts.py --check"),
    ("s12_p2_dependency", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B KMFA/tools/build_v015_s12_p2_core_calculations.py --check"),
    ("s12_p3_dependency", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B KMFA/tools/build_v015_s12_p3_engineering_logic.py --check"),
    ...
    ("s12_p3_kernel_regression", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B -m unittest KMFA.tests.test_v015_s12_p3_engineering_logic"),
    ("integrated_review_consistency", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B KMFA/tools/check_v015_s12_stage_review.py --integrated-review-check"),
    ("builder_exact_rebuild", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B KMFA/tools/build_v015_s12_stage_review.py --check"),
    ("stage_checker_pre_final", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B KMFA/tools/check_v015_s12_stage_review.py --pre-final --skip-validation-receipts --skip-clean-commit"),
    ...
    ("roadmap_sync_pending", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B KMFA/tools/v015_roadmap_governance_sync.py --check --validation-state S12_STAGE_REVIEW_PENDING_FINAL_VALIDATION"),
    ...
    ("governance_sync", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B KMFA/tools/check_v015_s12_stage_review.py --clean-governance-sync-check"),
    ...
    ("taskpack_source", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B -c \"from KMFA.tools.build_v015_s12_stage_review import source_contract; assert source_contract()['source_integrity_status'] == 'PASS'\""),
    ("public_boundary", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B KMFA/tools/check_v015_s12_stage_review.py --public-boundary-check"),
...
ALLOWED_PREFIXES = (
    ...
    "KMFA/stage_artifacts/V015_S12_STAGE_REVIEW/",
    "KMFA/tests/test_v015_roadmap_governance_sync.py",
    "KMFA/tests/test_v015_s12_stage_review.py",
    "KMFA/tests/test_v015_s12_stage_review_contract.py",
    "KMFA/tests/test_v015_s12_stage_review_governance.py",
    "KMFA/tools/build_v015_s12_stage_review.py",
    "KMFA/tools/check_v015_s12_stage_review.py",
    "KMFA/tools/run_v015_s12_stage_review_validations.py",
    "KMFA/tools/v015_roadmap_governance_sync.py",
    "KMFA/tools/v015_s12_stage_review_contract.py",
    "KMFA/功能清单.md",
    "KMFA/开发记录.md",
    "KMFA/模型参数文件.md",
)
...
FINAL_MUTABLE_PATHS = frozenset({
    ...
    "KMFA/metadata/project/project.yaml",
    "KMFA/metadata/stage_status.jsonl",
    "KMFA/stage_artifacts/V015_S12_STAGE_REVIEW/human/stage12_review_report_zh.md",
    "KMFA/stage_artifacts/V015_S12_STAGE_REVIEW/human/test_results_zh.md",
    "KMFA/stage_artifacts/V015_S12_STAGE_REVIEW/machine/s12_stage_review_manifest.json",
    "KMFA/stage_artifacts/V015_S12_STAGE_REVIEW/machine/validation_results.jsonl",
    "KMFA/功能清单.md",
    "KMFA/开发记录.md",
    "KMFA/模型参数文件.md",
})
...
def _check_scope() -> None:
    if subprocess.run(["git", "merge-base", "--is-ancestor", builder.REVIEW_BASE_COMMIT, "HEAD"], cwd=REPO_ROOT, check=False).returncode:
        raise CheckError("S12 review base is not an ancestor of HEAD")
    changed: set[str] = set()
    for args in (
...
def _check_public_boundary() -> None:
    ...
    roots = [builder.OUTPUT_ROOT, builder.PROJECT_ROOT / "tools/v015_s12_stage_review_contract.py"]
    for root in roots:
...
def _check_predecessors() -> None:
    ...
    if any(
        ...
        or row.get("validation_receipt_count") != 21
...
def _check_review_evidence() -> None:
    ...
    if verification.get("accounting") != {"total": 68, "passed": 68, "failed": 0}:
        raise CheckError("live review verification drift")
    integrated = _json(builder.MACHINE_ROOT / "integrated_review_public_safe.json")
    contract.validate_integrated_review(integrated)
    if (
        integrated["fact_projection"]["cost_conservation_delta_cents"] != 0
        or integrated["excluded_candidate_leak_count"] != 0
        or integrated["review_explanation_consistency"]["mismatch_count"] != 0
    ):
        raise CheckError("integrated review invariant drift")
    findings = _csv(builder.MACHINE_ROOT / "stage12_review_findings_public_safe.csv")
    ...
    if len(risks) != 6 or any(
        row.get("status") != "ROUTED_RESIDUAL"
        or row.get("plan_complete") != "true"
        or row.get("blocks_s12_stage_acceptance") != "false"
        for row in risks
    ):
...
def _check_manifest(*, pre_final: bool) -> dict[str, Any]:
    ...
    required = {
        ...
        "decision": "REMAIN_IN_S12_STAGE_REVIEW" if pre_final else "GO_TO_S13_P1_ONLY",
        "s12_stage_review_started": True,
        "s12_stage_review_performed": not pre_final,
        "s12_stage_review_acceptance_status": "PENDING_FINAL_VALIDATION" if pre_final else "PASSED",
        "s13_entry_allowed": not pre_final,
        "s13_p1_entry_allowed": not pre_final,
...
def _check_governance(*, pre_final: bool) -> None:
    ...
    for relative in ("docs/governance/project.yaml", "metadata/project/project.yaml", "docs/governance/roadmap.yaml"):
        ...
        tokens = (
            ...
            f'decision: "{decision}"',
            "s12_stage_review_started: true",
            f"s12_stage_review_performed: {str(not pre_final).lower()}",
            f"s13_p1_entry_allowed: {str(not pre_final).lower()}",
            "s13_p1_started: false",
            "github_upload_performed: false",
            "app_reinstall_performed: false",
        )
