#!/usr/bin/env python3
"""执行并记录 KMFA v1.5 S19-P3 唯一一组正式验收。"""
...
from KMFA.tools import check_v015_s19_p3_tax_policy_reporting as checker
...
def _require_clean_subject() -> None:
    for args in (("diff", "--quiet"), ("diff", "--cached", "--quiet")):
        if subprocess.run(["git", *args], cwd=REPO_ROOT, check=False).returncode:
            raise ValidationError("validation requires a clean tracked S19-P3 implementation commit")
    untracked = [
...
def run() -> list[dict[str, Any]]:
    ...
    if (
        ...
        or subject.get("s19_p1_acceptance_status") != "PASSED"
        or subject.get("s19_p2_acceptance_status") != "PASSED"
        or subject.get("s19_p3_started") is not True
        or subject.get("s19_p3_completed") is not False
        or subject.get("s19_stage_review_entry_allowed") is not False
        or subject.get("s19_stage_review_started") is not False
        ...
        raise ValidationError("validation subject must be pending S19-P3 with Stage Review and release work closed")
    ...
    for name, command in checker.EXPECTED_VALIDATIONS:
        ...
        receipt = {
            "schema_version": "kmfa.v015.s19p3.validation_receipt.v1",
...
def main() -> int:
    try:
        ...
        return 1
    print(
        f"PASS: S19-P3 validation receipts run={rows[0]['validation_run_id']} "
