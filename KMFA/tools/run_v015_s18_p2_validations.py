#!/usr/bin/env python3
"""执行并记录 KMFA v1.5 S18-P2 唯一一组正式验收。"""
...
from KMFA.tools import check_v015_s18_p2_funds_accounts as checker
...
def _require_clean_subject() -> None:
    for args in (("diff", "--quiet"), ("diff", "--cached", "--quiet")):
        if subprocess.run(["git", *args], cwd=REPO_ROOT, check=False).returncode:
            raise ValidationError("validation requires a clean tracked S18-P2 implementation commit")
    untracked = [
        line
...
def run() -> list[dict[str, Any]]:
    ...
    if (
        ...
        or subject.get("s18_p1_acceptance_status") != "PASSED"
        or subject.get("s18_p2_started") is not True
        or subject.get("s18_p2_completed") is not False
        or subject.get("s18_p3_entry_allowed") is not False
        ...
        or subject.get("bank_operation_count") != 0
        or subject.get("payment_button_count") != 0
        ...
        raise ValidationError("validation subject must be pending S18-P2 with later and release work closed")
    ...
    for name, command in checker.EXPECTED_VALIDATIONS:
        ...
        receipt = {
            "schema_version": "kmfa.v015.s18p2.validation_receipt.v1",
...
def main() -> int:
    try:
        ...
        return 1
    print(
        f"PASS: S18-P2 validation receipts run={rows[0]['validation_run_id']} "
