#!/usr/bin/env python3
"""Strict receipt-bound checker for KMFA v1.5 S10-P1."""
...
EXPECTED_VALIDATIONS = (
    (
        "python_compile",
        "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B -c \"import ast,pathlib; [ast.parse(pathlib.Path(p).read_text(encoding='utf-8')) for p in ('KMFA/tools/v015_s10_p1_general_import.py','KMFA/tools/build_v015_s10_p1_general_import.py','KMFA/tools/check_v015_s10_p1_general_import.py','KMFA/tools/run_v015_s10_p1_validations.py','KMFA/tools/v015_roadmap_governance_sync.py')]\"",
    ),
    ("focused_kernel_tests", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B -m unittest KMFA.tests.test_v015_s10_p1_general_import"),
    ("focused_artifact_tests", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B -m unittest KMFA.tests.test_v015_s10_p1_general_import_artifacts"),
    ("focused_governance_tests", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B -m unittest KMFA.tests.test_v015_s10_p1_general_import_governance"),
    ("s09_stage_review_regression", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B -m unittest KMFA.tests.test_v015_s09_stage_review_contract KMFA.tests.test_v015_s09_stage_review"),
    ("deterministic_evidence", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B KMFA/tools/build_v015_s10_p1_general_import.py --check"),
    ("pre_final_phase_checker", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B KMFA/tools/check_v015_s10_p1_general_import.py --pre-final --skip-validation-receipts"),
    ("s09_stage_review_dependency", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B KMFA/tools/check_v015_s10_p1_general_import.py --dependency-check"),
    ...
    ("roadmap_sync_pending", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B KMFA/tools/v015_roadmap_governance_sync.py --check --validation-state S10_P1_PENDING_FINAL_VALIDATION"),
    ...
    (
        ...
        "KMFA/tools/check_v015_s10_p1_general_import.py --clean-governance-sync-check",
    ),
    ...
    ("taskpack_source", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B KMFA/tools/check_v015_s10_p1_general_import.py --taskpack-source-check"),
    ("public_boundary", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B KMFA/tools/check_v015_s10_p1_general_import.py --public-boundary-check"),
...
ALLOWED_PHASE_PREFIXES = (
    ...
    "KMFA/metadata/imports/v015_s10_p1_general_import_policy_public_safe.json",
    "KMFA/metadata/model_registry.yaml",
    "KMFA/metadata/project/project.yaml",
    "KMFA/metadata/protocol/v015_s10_p1_import_preview_protocol_public_safe.json",
    "KMFA/metadata/protocol/v015_s10_p1_import_resume_protocol_public_safe.json",
    "KMFA/metadata/stage_status.jsonl",
    "KMFA/stage_artifacts/V015_S10_P1_GENERAL_IMPORT/",
    "KMFA/tests/test_v015_roadmap_governance_sync.py",
    "KMFA/tests/test_v015_s10_p1_general_import.py",
    "KMFA/tests/test_v015_s10_p1_general_import_artifacts.py",
    "KMFA/tests/test_v015_s10_p1_general_import_governance.py",
    "KMFA/tools/build_v015_s10_p1_general_import.py",
    "KMFA/tools/check_v015_s10_p1_general_import.py",
    "KMFA/tools/run_v015_s10_p1_validations.py",
    "KMFA/tools/v015_roadmap_governance_sync.py",
    "KMFA/tools/v015_s10_p1_general_import.py",
    "KMFA/功能清单.md",
    "KMFA/开发记录.md",
    "KMFA/模型参数文件.md",
)
...
def _check_scope() -> None:
    if subprocess.run(["git", "merge-base", "--is-ancestor", builder.PHASE_BASE_COMMIT, "HEAD"], cwd=REPO_ROOT, check=False).returncode:
        raise CheckError("S10-P1 base commit is not an ancestor of HEAD")
    changed = set()
    for args in (
    ...
    if unexpected:
        raise CheckError("unexpected S10-P1 path(s): " + ", ".join(unexpected))


def _check_dependency() -> None:
    value = builder.dependency()
    if value.get("acceptance_status") != "PASSED" or value.get("s10_p1_entry_allowed") is not True:
        raise CheckError("S09 review dependency is not accepted")
    head = str(value.get("validation_head") or "")
    if not re.fullmatch(r"[0-9a-f]{40}", head):
        raise CheckError("S09 review validation head is invalid")
    if subprocess.run(["git", "merge-base", "--is-ancestor", head, "HEAD"], cwd=REPO_ROOT, check=False).returncode:
        raise CheckError("S09 review validation head is not reachable")
...
def _check_taskpack_source() -> None:
    ...
    if (stage or {}).get("name") != "文件型数据源适配与导入管线" or (phase or {}).get("name") != "通用导入":
        raise CheckError("S10-P1 source phase drift")
    if [row.get("id") for row in (phase or {}).get("tasks", [])] != ["T01", "T02", "T03"]:
        raise CheckError("S10-P1 source task drift")


def _check_public_boundary() -> None:
    paths = [path for path in builder.OUTPUT_ROOT.rglob("*") if path.is_file()]
    paths.extend((builder.POLICY_PATH, builder.PREVIEW_PROTOCOL_PATH, builder.RESUME_PROTOCOL_PATH))
    text = "\n".join(path.read_text(encoding="utf-8") for path in paths)
    forbidden = (r"/Users/", r"/Volumes/", r"/home/", r"file://", r"KMFA_MetaData", r"private://")
    for pattern in forbidden:
        if re.search(pattern, text, re.IGNORECASE):
            raise CheckError(f"public S10-P1 evidence contains forbidden material: {pattern}")
    for path in paths:
        if path.suffix == ".json":
            _json(path)
...
def _check_governance_sync_in_clean_worktree() -> None:
    """Validate committed S10-P1 changes without unrelated user worktree files."""
    with tempfile.TemporaryDirectory(prefix="kmfa-s10p1-governance-") as temp_dir:
...
def _check_evidence() -> None:
    ...
    formats = _json(builder.FORMAT_MATRIX_PATH)
    preview = _json(builder.PREVIEW_CONTRACT_PATH)
    recovery = _json(builder.RECOVERY_VERIFICATION_PATH)
    policy = _json(builder.POLICY_PATH)
    resume = _json(builder.RESUME_PROTOCOL_PATH)
    tasks = _json(builder.TASK_MATRIX_PATH)
    if source.get("roadmap_phase_id") != "S10-P1" or source.get("task_count") != 3:
        raise CheckError("source contract drift")
    if formats.get("format_category_count") != 6 or formats.get("extension_count") != 8:
        raise CheckError("format matrix coverage drift")
    if len(formats.get("archive_rejections", [])) != 8:
        raise CheckError("archive rejection matrix drift")
    if preview.get("processing_before_confirmation_allowed") is not False or preview.get("preview_writes_raw") is not False:
        raise CheckError("preview confirmation/raw boundary drift")
    if preview.get("changed_source_requires_new_preview") is not True or len(preview.get("human_visible_fields", [])) != 6:
        raise CheckError("preview human field/binding drift")
    if resume.get("partial_commit_visible") is not False or resume.get("exact_replay_reuses_record") is not True:
        raise CheckError("resume/idempotency protocol drift")
    if policy.get("source_read_only") is not True or policy.get("private_derived_storage_only") is not True:
        raise CheckError("source/private storage policy drift")
    if recovery.get("accounting") != {"total": 32, "passed": 32, "failed": 0}:
        raise CheckError("public verification accounting drift")
    if recovery.get("raw_root_access_count") != 0 or recovery.get("github_upload_performed") is not False:
        raise CheckError("public verification boundary drift")
...
def _check_manifest(*, pre_final: bool) -> dict[str, Any]:
    ...
    expected = {
        ...
        "overall_accepted_phase_count": 25 if pre_final else 26,
        "stage_lifecycle_status": "IN_PROGRESS",
        "stage_acceptance_status": "PENDING",
        "stage_execution_percentage": 33,
        "decision": "REMAIN_IN_S10_P1_FINAL_VALIDATION" if pre_final else "CONTINUE_TO_S10_P2_ONLY",
        "supported_format_category_count": 6,
        "supported_extension_count": 8,
        "preview_required_field_count": 6,
        "partial_commit_visible": False,
        "raw_root_access_count": 0,
        "s10_p1_started": True,
        "s10_p1_acceptance_status": acceptance,
        "s10_p2_entry_allowed": not pre_final,
    ...
    if mismatches:
        raise CheckError("S10-P1 manifest mismatch: " + ", ".join(mismatches))
    return manifest
...
def _check_governance(*, pre_final: bool) -> None:
    ...
    for relative in ("docs/governance/project.yaml", "metadata/project/project.yaml", "docs/governance/roadmap.yaml"):
        ...
        for token in (
            ...
            f"s10_p2_entry_allowed: {str(not pre_final).lower()}",
            "s10_p2_started: false",
            "s10_p3_entry_allowed: false",
            "github_upload_performed: false",
            "app_reinstall_performed: false",
    ...
    for text in (registry_text, mirror_text):
        if "kmfa_v015_s10_p1_general_import" not in text or "MOD-KMFA-FILE-IMPORT-001" not in text:
            raise CheckError("S10-P1 model registry entry missing")
    if "FORM-KMFA-V015-S10-P1-GENERAL-IMPORT-001" not in formula_text:
        raise CheckError("S10-P1 formula registry entry missing")
    for number in range(2082, 2094):
        if f"PARAM-KMFA-{number}" not in parameter_text:
            raise CheckError(f"S10-P1 parameter missing: PARAM-KMFA-{number}")
...
def _check_receipts(manifest: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise CheckError("S10-P1 manifest validation binding drift")
    if _git("rev-parse", "HEAD^") != head:
        raise CheckError("final S10-P1 evidence commit must be the immediate child of validation head")
...
def main() -> int:
    ...
    try:
        ...
        return 1
    print("PASS: S10-P1 strict receipt-bound checks")
    return 0
