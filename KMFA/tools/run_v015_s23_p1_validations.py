#!/usr/bin/env python3
"""Run and bind the single formal KMFA v1.5 S23-P1 validation set."""
...
from KMFA.tools import check_v015_s23_p1_end_to_end_business_flow as checker
...
class ValidationError(RuntimeError):
    """Formal S23-P1 validation cannot continue."""
...
def _require_clean_subject() -> None:
    ...
    for args in (("diff", "--quiet"), ("diff", "--cached", "--quiet")):
        if subprocess.run(["git", *args], cwd=REPO_ROOT, check=False).returncode:
            raise ValidationError("validation requires a clean tracked S23-P1 implementation commit")
    untracked = [
...
def run() -> list[dict[str, Any]]:
    ...
    if (
        ...
        or subject.get("overall_accepted_phase_count") != 64
        or subject.get("s23_p1_started") is not True
        or subject.get("s23_p1_completed") is not False
        or subject.get("s23_p2_entry_allowed") is not False
        or subject.get("s23_p2_started") is not False
        ...
        or subject.get("raw_write_count") != 0
        or subject.get("external_network_request_count") != 0
        or subject.get("github_upload_performed") is not False
        or subject.get("app_reinstall_performed") is not False
    ):
        raise ValidationError("validation subject must be pending S23-P1 with later phases and release actions closed")
    ...
    for name, command in checker.EXPECTED_VALIDATIONS:
        ...
        receipt = {
            "schema_version": "kmfa.v015.s23p1.validation_receipt.v1",
    ...
    sync = subprocess.run(
        [str(BUNDLED_PYTHON), "-B", "KMFA/tools/v015_roadmap_governance_sync.py", "--write", "--validation-state", "S23_P1_PASSED"],
        cwd=REPO_ROOT,
        env=environment,
        capture_output=True,
        text=True,
...
def main() -> int:
    try:
        ...
        print(f"FAIL: {error}")
        return 1
    print(f"PASS: S23-P1 validation receipts run={rows[0]['validation_run_id']} count={len(rows)} head={rows[0]['validation_head']}")
    return 0
