#!/usr/bin/env python3
"""Run one exact validation receipt set for KMFA v1.5 S09 review."""
...
from KMFA.tools import check_v015_s09_stage_review as checker
...
def run() -> list[dict[str, Any]]:
    ...
    if (
        ...
        or subject.get("s09_stage_review_acceptance_status") != "PENDING_FINAL_VALIDATION"
        or subject.get("s10_p1_entry_allowed") is not False
        or subject.get("s10_p1_started") is not False
        or subject.get("github_upload_performed") is not False
        or subject.get("app_reinstall_performed") is not False
    ):
        raise ValidationError("validation subject must be pending S09 review with S10 and release actions closed")
    ...
    for name, command in checker.EXPECTED_VALIDATIONS:
        ...
        receipt = {
            "schema_version": "kmfa.v015.s09_stage_review.validation_receipt.v1",
...
def main() -> int:
    try:
        ...
        return 1
    print(
        "PASS: S09 Stage Review validation receipts "
