#!/usr/bin/env python3
"""Run one exact validation receipt set for the KMFA v1.5 S04 review."""
...
from KMFA.tools import check_v015_s04_stage_review as checker
...
def run() -> list[dict[str, Any]]:
    ...
    if subject.get("stage_acceptance_status") != "PENDING" or subject.get("s04_stage_review_acceptance_status") != "PENDING_FINAL_VALIDATION":
        raise ValidationError("validation subject must be the pending S04 review state")
    ...
    for name, command in checker.EXPECTED_VALIDATIONS:
        ...
        receipt = {
            "schema_version": "kmfa.v015.s04_stage_review.validation_receipt.v1",
...
def main() -> int:
    try:
        ...
        return 1
    print(f"PASS: S04 Stage Review validation receipts run={receipts[0]['validation_run_id']} count={len(receipts)} head={receipts[0]['validation_head']}")
    return 0
