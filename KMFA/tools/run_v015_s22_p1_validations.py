#!/usr/bin/env python3
"""Run and bind the single formal KMFA v1.5 S22-P1 validation set."""
...
from KMFA.tools import check_v015_s22_p1_notifications as checker
...
class ValidationError(RuntimeError):
    """Formal S22-P1 validation cannot continue."""
...
def _require_clean_subject() -> None:
    ...
    for args in (("diff", "--quiet"), ("diff", "--cached", "--quiet")):
        if subprocess.run(["git", *args], cwd=REPO_ROOT, check=False).returncode:
            raise ValidationError("validation requires a clean tracked S22-P1 implementation commit")
    untracked = [
...
def run() -> list[dict[str, Any]]:
    ...
    zero_keys = (
        ...
        "external_email_delivery_count", "full_report_body_count", "amount_detail_count",
        "attachment_count", "credential_field_count", "duplicate_dispatch_count",
        "idempotency_conflict_accept_count",
    )
    if (
        subject.get("phase_acceptance_status") != "PENDING_FINAL_VALIDATION"
        or subject.get("phase_task_accepted_count") != 0
        or subject.get("overall_accepted_phase_count") != 61
        or subject.get("s22_p1_started") is not True
        or subject.get("s22_p1_completed") is not False
        or subject.get("s22_p2_entry_allowed") is not False
        or subject.get("s22_p2_started") is not False
        or subject.get("s22_p3_started") is not False
        or subject.get("unconfirmed_rule_enabled_count") != 0
        ...
        raise ValidationError("validation subject must be pending S22-P1 with S22-P2 raw email GitHub and App closed")
    ...
    for name, command in checker.EXPECTED_VALIDATIONS:
        ...
        receipt = {
            "schema_version": "kmfa.v015.s22p1.validation_receipt.v1",
    ...
    sync = subprocess.run(
        [str(BUNDLED_PYTHON), "-B", "KMFA/tools/v015_roadmap_governance_sync.py", "--write", "--validation-state", "S22_P1_PASSED"],
        cwd=REPO_ROOT, env={**environment, "PYTHONDONTWRITEBYTECODE": "1", "PYTHONPATH": "."},
        capture_output=True, text=True, check=False,
    )
    if sync.returncode:
...
def main() -> int:
    try:
        ...
        print(f"FAIL: {error}")
        return 1
    print(f"PASS: S22-P1 validation receipts run={rows[0]['validation_run_id']} count={len(rows)} head={rows[0]['validation_head']}")
    return 0
