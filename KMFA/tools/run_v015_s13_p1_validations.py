#!/usr/bin/env python3
"""执行并记录 KMFA v1.5 S13-P1 唯一一组正式验收。"""
...
from KMFA.tools import check_v015_s13_p1_indicator_registry as checker
...
def _require_clean_subject() -> None:
    for args in (("diff", "--quiet"), ("diff", "--cached", "--quiet")):
        if subprocess.run(["git", *args], cwd=REPO_ROOT, check=False).returncode:
            raise ValidationError("validation requires a clean tracked S13-P1 implementation commit")
    untracked = [
        line
...
def run() -> list[dict[str, Any]]:
    ...
    if (
        ...
        or subject.get("s13_p2_entry_allowed") is not False
        or subject.get("s13_p2_started") is not False
        or subject.get("health_score_computed") is not False
        ...
        raise ValidationError("validation subject must be pending S13-P1 with later and release work closed")
    ...
    for name, command in checker.EXPECTED_VALIDATIONS:
        ...
        receipt = {
            "schema_version": "kmfa.v015.s13p1.validation_receipt.v1",
...
def main() -> int:
    try:
        ...
        return 1
    print(f"PASS: S13-P1 validation receipts run={rows[0]['validation_run_id']} count={len(rows)} head={rows[0]['validation_head']}")
    return 0
