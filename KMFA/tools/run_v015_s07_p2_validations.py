#!/usr/bin/env python3
"""Run and receipt one exact validation set for KMFA v1.5 S07-P2."""
...
from KMFA.tools import check_v015_s07_p2_conflict_classification as checker
...
def run() -> list[dict[str, Any]]:
    ...
    if (
        ...
        or subject.get("automatic_source_selection_allowed") is not False
        or subject.get("system_problem_assigned_to_user_count") != 0
        or subject.get("s07_p3_entry_allowed") is not False
    ):
        raise ValidationError("validation subject must be pending S07-P2 with S07-P3 closed")
    ...
    for name, command in checker.EXPECTED_VALIDATIONS:
        ...
        receipt = {
            "schema_version": "kmfa.v015.s07p2.validation_receipt.v1",
...
def main() -> int:
    try:
        ...
        return 1
    print(
        "PASS: S07-P2 validation receipts "
