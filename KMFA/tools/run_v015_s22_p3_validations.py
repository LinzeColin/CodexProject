#!/usr/bin/env python3
"""Run and bind the single formal KMFA v1.5 S22-P3 validation set."""
...
from KMFA.tools import check_v015_s22_p3_operations_governance as checker
...
class ValidationError(RuntimeError):
    """Formal S22-P3 validation cannot continue."""
...
def _require_clean_subject() -> None:
    if not BUNDLED_PYTHON.is_file() or not Path(sys.executable).samefile(
        BUNDLED_PYTHON
    ):
        raise ValidationError(
            f"run formal validation with bundled Python: {BUNDLED_PYTHON}"
        )
    for args in (("diff", "--quiet"), ("diff", "--cached", "--quiet")):
        if subprocess.run(["git", *args], cwd=REPO_ROOT, check=False).returncode:
            raise ValidationError(
                "validation requires a clean tracked S22-P3 implementation commit"
            )
    untracked = [
        line
        for line in _git(
    ...
    if untracked:
        raise ValidationError(
            "unexpected untracked file before validation: " + ", ".join(untracked)
        )
...
def run() -> list[dict[str, Any]]:
    ...
    if (
        ...
        or subject.get("overall_accepted_phase_count") != 63
        or subject.get("s22_p3_started") is not True
        or subject.get("s22_p3_completed") is not False
        or subject.get("s22_stage_review_entry_allowed") is not False
        or subject.get("s22_stage_review_started") is not False
        or subject.get("s23_started") is not False
        ...
        raise ValidationError(
            "validation subject must be pending S22-P3 with review S23 raw external GitHub and App closed"
        )
    ...
    environment["PATH"] = (
        f"{BUNDLED_PYTHON.parent}:{environment.get('PATH', '')}"
    )
    ...
    for name, command in checker.EXPECTED_VALIDATIONS:
        ...
        receipt = {
            "schema_version": "kmfa.v015.s22p3.validation_receipt.v1",
...
def main() -> int:
    try:
        ...
        return 1
    print(
        "PASS: S22-P3 validation receipts "
        f"run={rows[0]['validation_run_id']} count={len(rows)} "
        f"head={rows[0]['validation_head']}"
    )
    return 0
