#!/usr/bin/env python3
"""Execute and bind every KMFA v1.5 S03-P1 validation receipt.

Commands are fixed by the evidence builder and are executed without a shell.
Only output hashes are retained; command output itself is never published.
"""
...
class RunnerError(RuntimeError):
    """Raised when runner provenance or its private output boundary fails."""
...
def parse_command(command: str) -> Tuple[dict[str, str], list[str]]:
    """Parse fixed leading ENV=value tokens and argv without invoking a shell."""
...
def build_receipts(
    ...
    *,
    ...
    """Run all commands and return receipts plus aggregate integrity status."""
    ...
    for sequence, (validation_id, command) in enumerate(commands.items(), start=1):
        ...
        command_pass = exit_code == 0 and runner_integrity
        effective_exit = exit_code if exit_code != 0 else (0 if runner_integrity else 98)
        receipt = {
            "schema_version": builder.VALIDATION_RECEIPT_SCHEMA_VERSION,
            "run_id": receipt_run_id,
            "validation_id": validation_id,
            "command": command,
            "result": "PASS" if command_pass else "FAIL",
        ...
        all_pass = all_pass and command_pass
        if not command_pass:
            stop_reason = f"STOP_AFTER_{validation_id}"
    return receipts, all_pass


def _open_directory_no_follow(path: Path) -> int:
    absolute = Path(os.path.abspath(os.path.normpath(os.fspath(path))))
    if not absolute.is_absolute():
        raise RunnerError("private receipt parent must be absolute")
...
def write_private_receipts(path: Path, rows: Sequence[Mapping[str, Any]]) -> None:
    """Atomically write mode-0600 JSONL through a no-follow parent dirfd."""
    ...
    if target != fixed:
        raise RunnerError("private validation receipt path is not the fixed output")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if arguments:
        print("FAIL: S03-P1 validation runner accepts no arguments", file=sys.stderr)
        return 2
    try:
        ...
        pending_rows = [
            ...
            for sequence, (validation_id, command) in enumerate(
                builder.EXPECTED_VALIDATION_RECEIPTS.items(),
                start=1,
            )
        ]
        write_private_receipts(OUTPUT_PATH, pending_rows)
        ...
        print(f"{'PASS' if all_pass else 'FAIL'}: S03-P1 validations {passed}/{len(rows)}")
        return 0 if all_pass else 1
    except (RunnerError, builder.BuildError, OSError, ValueError) as error:
        print("FAIL: S03-P1 validation runner integrity error", file=sys.stderr)
