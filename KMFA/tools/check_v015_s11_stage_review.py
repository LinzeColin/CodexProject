...
EXPECTED_VALIDATIONS = (
    ("python_compile", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B -c \"import ast,pathlib; [ast.parse(pathlib.Path(p).read_text(encoding='utf-8')) for p in ('KMFA/tools/v015_s11_stage_review_contract.py','KMFA/tools/build_v015_s11_stage_review.py','KMFA/tools/check_v015_s11_stage_review.py','KMFA/tools/run_v015_s11_stage_review_validations.py','KMFA/tools/v015_roadmap_governance_sync.py')]\""),
    ("focused_contract_tests", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B -m unittest KMFA.tests.test_v015_s11_stage_review_contract"),
    ("focused_review_tests", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B -m unittest KMFA.tests.test_v015_s11_stage_review"),
    ("focused_governance_tests", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B -m unittest KMFA.tests.test_v015_s11_stage_review_governance"),
    ("s11_p1_dependency", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B KMFA/tools/build_v015_s11_p1_quality_rules.py --check"),
    ("s11_p2_dependency", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B KMFA/tools/build_v015_s11_p2_check_board_data_model.py --check"),
    ("s11_p3_dependency", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B KMFA/tools/build_v015_s11_p3_check_board_interface.py --check"),
    ("s11_p3_browser_regression", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B KMFA/tools/run_v015_s11_p3_browser_tests.py"),
    ("s11_p1_kernel_regression", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B -m unittest KMFA.tests.test_v015_s11_p1_quality_rules"),
    ("s11_p2_kernel_regression", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B -m unittest KMFA.tests.test_v015_s11_p2_check_board_data_model"),
    ("s11_p3_kernel_regression", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B -m unittest KMFA.tests.test_v015_s11_p3_check_board_interface"),
    ("builder_exact_rebuild", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B KMFA/tools/build_v015_s11_stage_review.py --check"),
    ("stage_checker_pre_final", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B KMFA/tools/check_v015_s11_stage_review.py --pre-final --skip-validation-receipts --skip-clean-commit"),
    ...
    ("roadmap_sync_pending", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B KMFA/tools/v015_roadmap_governance_sync.py --check --validation-state S11_STAGE_REVIEW_PENDING_FINAL_VALIDATION"),
    ...
    ("governance_sync", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B KMFA/tools/check_v015_s11_stage_review.py --clean-governance-sync-check"),
    ...
    ("taskpack_source", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B -c \"from KMFA.tools.build_v015_s11_stage_review import source_contract; assert source_contract()['source_integrity_status'] == 'PASS'\""),
    ("public_boundary", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B KMFA/tools/check_v015_s11_stage_review.py --public-boundary-check"),
...
ALLOWED_PREFIXES = (
    ...
    "KMFA/metadata/stage_status.jsonl", "KMFA/stage_artifacts/V015_S11_STAGE_REVIEW/",
    "KMFA/tests/test_v015_roadmap_governance_sync.py", "KMFA/tests/test_v015_s11_stage_review.py",
    "KMFA/tests/test_v015_s11_stage_review_contract.py", "KMFA/tests/test_v015_s11_stage_review_governance.py",
    "KMFA/tools/build_v015_s11_stage_review.py", "KMFA/tools/check_v015_s11_stage_review.py",
    "KMFA/tools/run_v015_s11_stage_review_validations.py", "KMFA/tools/v015_roadmap_governance_sync.py",
    "KMFA/tools/v015_s11_stage_review_contract.py", "KMFA/功能清单.md", "KMFA/开发记录.md", "KMFA/模型参数文件.md",
)
...
FINAL_MUTABLE_PATHS = frozenset({
    ...
    "KMFA/stage_artifacts/V015_S11_STAGE_REVIEW/human/stage11_review_report_zh.md",
    "KMFA/stage_artifacts/V015_S11_STAGE_REVIEW/human/test_results_zh.md",
    "KMFA/stage_artifacts/V015_S11_STAGE_REVIEW/machine/s11_stage_review_manifest.json",
    "KMFA/stage_artifacts/V015_S11_STAGE_REVIEW/machine/validation_results.jsonl",
...
def _check_scope() -> None:
    if subprocess.run(["git", "merge-base", "--is-ancestor", builder.REVIEW_BASE_COMMIT, "HEAD"], cwd=REPO_ROOT, check=False).returncode:
        raise CheckError("S11 review base is not an ancestor of HEAD")
    changed: set[str] = set()
    for args in (
...
def _check_public_boundary() -> None:
    ...
    roots = [builder.OUTPUT_ROOT, builder.PROJECT_ROOT / "tools/v015_s11_stage_review_contract.py"]
    for root in roots:
...
def _check_clean_governance_sync() -> None:
    ...
    try:
        ...
        result = subprocess.run(
            [
                "python3", "-B", "scripts/validate_governance_sync.py", "--changed-only",
                "--base-ref", builder.REVIEW_BASE_COMMIT, "--enforce-sync",
            ],
            cwd=REPO_ROOT,
            env=env,
            capture_output=True,
...
def _check_predecessors() -> None:
    ...
    expected_receipts = {"S11-P1": 19, "S11-P2": 19, "S11-P3": 20}
    if any(row.get("acceptance_status") != "PASSED" or row.get("validation_receipt_count") != expected_receipts.get(row.get("roadmap_phase_id")) for row in evidence["phases"]):
...
def _check_review_evidence() -> None:
    ...
    if contracts.get("accounting") != {"total": 28, "passed": 28, "failed": 0, "blocking_failed": 0}:
        raise CheckError("cross-part contract accounting drift")
    verification = _json(builder.MACHINE_ROOT / "cross_phase_verification_public_safe.json")
    if verification.get("accounting") != {"total": 45, "passed": 45, "failed": 0}:
        raise CheckError("live review verification drift")
    findings = _csv(builder.MACHINE_ROOT / "stage11_review_findings_public_safe.csv")
    if [row.get("finding_id") for row in findings] != ["S11REV-F001", "S11REV-F002", "S11REV-F003"]:
    ...
    if len(risks) != 5 or any(row.get("status") != "ROUTED_RESIDUAL" or row.get("plan_complete") != "true" or row.get("blocks_s11_stage_acceptance") != "false" for row in risks):
...
def _check_manifest(*, pre_final: bool) -> dict[str, Any]:
    ...
    required = {
        ...
        "decision": "REMAIN_IN_S11_STAGE_REVIEW" if pre_final else "GO_TO_S12_P1_ONLY",
        "s11_stage_review_started": True,
        "s11_stage_review_performed": not pre_final,
        "s11_stage_review_acceptance_status": "PENDING_FINAL_VALIDATION" if pre_final else "PASSED",
        "s12_entry_allowed": not pre_final,
        "s12_p1_entry_allowed": not pre_final,
...
def _check_governance(*, pre_final: bool) -> None:
    ...
    for relative in ("docs/governance/project.yaml", "metadata/project/project.yaml", "docs/governance/roadmap.yaml"):
        ...
        tokens = (
            ...
            "active_formula_count: 360", "active_parameter_count: 1796",
            'current_parameter_range: "PARAM-KMFA-2173..2181"',
            f'stage_lifecycle_status: "{lifecycle}"', f'stage_acceptance_status: "{status}"',
            f'decision: "{decision}"', "s11_stage_review_started: true",
            f"s11_stage_review_performed: {str(not pre_final).lower()}",
            f"s12_p1_entry_allowed: {str(not pre_final).lower()}", "s12_p1_started: false",
