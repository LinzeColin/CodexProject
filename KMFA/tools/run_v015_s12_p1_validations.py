#!/usr/bin/env python3
"""Run and receipt one exact validation set for KMFA v1.5 S12-P1."""
...
def _require_clean_subject() -> None:
    for args in (("diff", "--quiet"), ("diff", "--cached", "--quiet")):
        if subprocess.run(["git", *args], cwd=REPO_ROOT, check=False).returncode:
            raise ValidationError("validation requires a clean tracked S12-P1 implementation commit")
    untracked = [
        line
...
def run() -> list[dict[str, Any]]:
    ...
    if (
        ...
        or subject.get("s12_p2_entry_allowed") is not False
        or subject.get("s12_p2_started") is not False
        or subject.get("github_upload_performed") is not False
        or subject.get("app_reinstall_performed") is not False
    ):
        raise ValidationError("validation subject must be pending S12-P1 with later and release work closed")
    ...
    for name, command in checker.EXPECTED_VALIDATIONS:
        ...
        receipt = {
            "schema_version": "kmfa.v015.s12p1.validation_receipt.v1",
...
def main() -> int:
    try:
        ...
        return 1
    print(f"PASS: S12-P1 validation receipts run={rows[0]['validation_run_id']} count={len(rows)} head={rows[0]['validation_head']}")
    return 0
