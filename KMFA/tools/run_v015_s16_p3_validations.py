#!/usr/bin/env python3
"""执行并记录 KMFA v1.5 S16-P3 唯一一组正式验收。"""
...
from KMFA.tools import check_v015_s16_p3_homepage_usability as checker
    ...
    for args in (("diff", "--quiet"), ("diff", "--cached", "--quiet")):
        if subprocess.run(["git", *args], cwd=REPO_ROOT, check=False).returncode:
            raise ValidationError(
                "validation requires a clean tracked S16-P3 implementation commit"
            )
    untracked = [
        line
        for line in _git(
...
def run() -> list[dict[str, Any]]:
    ...
    if (
        ...
        or subject.get("s16_p3_started") is not True
        or subject.get("s16_stage_review_entry_allowed") is not False
        or subject.get("s16_stage_review_started") is not False
        ...
        raise ValidationError(
            "validation subject must be pending S16-P3 with later and release work closed"
        )
    ...
    for name, command in checker.EXPECTED_VALIDATIONS:
        ...
        receipt = {
            "schema_version": "kmfa.v015.s16p3.validation_receipt.v1",
...
def main() -> int:
    try:
        ...
        return 1
    print(
        f"PASS: S16-P3 validation receipts run={rows[0]['validation_run_id']} "
