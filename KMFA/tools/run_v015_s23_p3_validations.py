#!/usr/bin/env python3
"""Run and bind the single formal KMFA v1.5 S23-P3 validation set."""
...
from KMFA.tools import check_v015_s23_p3_stability_usability as checker
...
class ValidationError(RuntimeError):
    """Formal S23-P3 validation cannot continue."""
...
def _require_clean_subject() -> None:
    ...
    for args in (("diff", "--quiet"), ("diff", "--cached", "--quiet")):
        if subprocess.run(["git", *args], cwd=REPO_ROOT, check=False).returncode:
            raise ValidationError("validation requires a clean tracked S23-P3 implementation commit")
...
def run() -> list[dict[str, Any]]:
    ...
    if (
        ...
        or subject.get("overall_accepted_phase_count") != 66
        or subject.get("s23_p3_started") is not True
        or subject.get("s23_p3_completed") is not False
        or subject.get("s23_stage_review_entry_allowed") is not False
        or subject.get("s23_stage_review_started") is not False
        or subject.get("s24_started") is not False
        or any(subject.get(key) != 0 for key in ("raw_root_access_count", "raw_write_count", "external_network_request_count", "stage_review_execution_count", "s24_execution_count", "silent_error_count", "idempotency_failure_count", "queue_leak_count", "accessibility_fail_count"))
        or subject.get("github_upload_performed") is not False
        or subject.get("app_reinstall_performed") is not False
    ):
        raise ValidationError("validation subject must be pending S23-P3 with stability and later-phase boundaries closed")
    ...
    environment.setdefault("KMFA_LOCAL_AUTH_KEY", secrets.token_hex(32))
    environment.setdefault("KMFA_SESSION_SIGNING_KEY", secrets.token_hex(32))
    ...
    for name, command in checker.EXPECTED_VALIDATIONS:
        ...
        receipt = {
            "schema_version": "kmfa.v015.s23p3.validation_receipt.v1",
    ...
    sync = subprocess.run(
        [str(BUNDLED_PYTHON), "-B", "KMFA/tools/v015_roadmap_governance_sync.py", "--write", "--validation-state", "S23_P3_PASSED"],
...
def main() -> int:
    try:
        ...
        print(f"FAIL: {error}")
        return 1
    print(f"PASS: S23-P3 validation receipts run={rows[0]['validation_run_id']} count={len(rows)} head={rows[0]['validation_head']}")
    return 0
