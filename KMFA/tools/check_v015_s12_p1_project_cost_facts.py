#!/usr/bin/env python3
"""Strict receipt-bound checker for KMFA v1.5 S12-P1."""
...
EXPECTED_VALIDATIONS = (
    (
        "python_compile",
        "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B -c \"import ast,pathlib; [ast.parse(pathlib.Path(p).read_text(encoding='utf-8')) for p in ('KMFA/tools/v015_s12_p1_project_cost_facts.py','KMFA/tools/build_v015_s12_p1_project_cost_facts.py','KMFA/tools/check_v015_s12_p1_project_cost_facts.py','KMFA/tools/run_v015_s12_p1_validations.py','KMFA/tools/v015_roadmap_governance_sync.py')]\"",
    ),
    ("focused_kernel_tests", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B -m unittest KMFA.tests.test_v015_s12_p1_project_cost_facts"),
    ("focused_artifact_tests", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B -m unittest KMFA.tests.test_v015_s12_p1_project_cost_facts_artifacts"),
    ("focused_governance_tests", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B -m unittest KMFA.tests.test_v015_s12_p1_project_cost_facts_governance"),
    ("legacy_fact_layer_regression", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B -m unittest KMFA.tests.test_project_cost_fact_layer"),
    ...
    ("s11_stage_review_regression", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B -m unittest KMFA.tests.test_v015_s11_stage_review_contract KMFA.tests.test_v015_s11_stage_review"),
    ("s11_stage_review_dependency", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B KMFA/tools/check_v015_s12_p1_project_cost_facts.py --dependency-check"),
    ("deterministic_evidence", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B KMFA/tools/build_v015_s12_p1_project_cost_facts.py --check"),
    ("pre_final_phase_checker", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B KMFA/tools/check_v015_s12_p1_project_cost_facts.py --pre-final --skip-validation-receipts"),
    ...
    ("roadmap_sync_pending", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B KMFA/tools/v015_roadmap_governance_sync.py --check --validation-state S12_P1_PENDING_FINAL_VALIDATION"),
    ...
    ("governance_sync", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B KMFA/tools/check_v015_s12_p1_project_cost_facts.py --clean-governance-sync-check"),
    ...
    ("taskpack_source", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B KMFA/tools/check_v015_s12_p1_project_cost_facts.py --taskpack-source-check"),
    ("public_boundary", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B KMFA/tools/check_v015_s12_p1_project_cost_facts.py --public-boundary-check"),
...
ALLOWED_PHASE_PREFIXES = (
    ...
    "KMFA/metadata/lineage/v015_s12_p1_",
    "KMFA/metadata/model_registry.yaml",
    "KMFA/metadata/project/project.yaml",
    "KMFA/metadata/stage_status.jsonl",
    "KMFA/stage_artifacts/V015_S12_P1_PROJECT_COST_FACTS/",
    "KMFA/tests/test_v015_roadmap_governance_sync.py",
    "KMFA/tests/test_v015_s12_p1_project_cost_facts.py",
    "KMFA/tests/test_v015_s12_p1_project_cost_facts_artifacts.py",
    "KMFA/tests/test_v015_s12_p1_project_cost_facts_governance.py",
    "KMFA/tools/build_v015_s12_p1_project_cost_facts.py",
    "KMFA/tools/check_v015_s12_p1_project_cost_facts.py",
    "KMFA/tools/run_v015_s12_p1_validations.py",
    "KMFA/tools/v015_roadmap_governance_sync.py",
    "KMFA/tools/v015_s12_p1_project_cost_facts.py",
    "KMFA/功能清单.md",
    "KMFA/开发记录.md",
    "KMFA/模型参数文件.md",
)
...
def _check_scope() -> None:
    if subprocess.run(["git", "merge-base", "--is-ancestor", builder.PHASE_BASE_COMMIT, "HEAD"], cwd=REPO_ROOT, check=False).returncode:
        raise CheckError("S12-P1 base commit is not an ancestor of HEAD")
    changed: set[str] = set()
    for args in (
    ...
    if unexpected:
        raise CheckError("unexpected S12-P1 path(s): " + ", ".join(unexpected))


def _check_dependency() -> None:
    value = builder.dependency()
    if value.get("acceptance_status") != "PASSED" or value.get("validation_receipt_count") != 24:
        raise CheckError("S11 Stage Review dependency is not accepted")
    if value.get("s12_p1_entry_allowed") is not True or value.get("s12_p1_started") is not False:
        raise CheckError("S11 Stage Review did not open exactly S12-P1")
...
def _check_taskpack_source() -> None:
    ...
    if (stage or {}).get("name") != "项目成本事实层与计算引擎" or (phase or {}).get("name") != "事实层":
        raise CheckError("S12-P1 source phase drift")
    tasks = (phase or {}).get("tasks", [])
    if [row.get("id") for row in tasks] != ["T01", "T02", "T03"]:
        raise CheckError("S12-P1 source task drift")
    if [row.get("stop") for row in tasks] != ["口径未知不得合并。", "未知成本进入未归集池。", "总成本不守恒即失败。"]:
        raise CheckError("S12-P1 source stop condition drift")


def _check_public_boundary() -> None:
    paths = [path for path in builder.OUTPUT_ROOT.rglob("*") if path.is_file()]
    paths.extend((builder.INCOME_CONTRACT_PATH, builder.COST_CONTRACT_PATH, builder.POOL_CONTRACT_PATH))
    text = "\n".join(path.read_text(encoding="utf-8") for path in paths)
    for pattern in (r"/Users/", r"/Volumes/", r"/home/", r"file://", r"KMFA_MetaData", r"private://"):
        if re.search(pattern, text, re.IGNORECASE):
            raise CheckError(f"public S12-P1 evidence contains forbidden material: {pattern}")
    for forbidden in ("raw_value", "original_value", "plaintext_value", "private_hash", "bank_account_number", "identity_document_number"):
        if forbidden in text:
            raise CheckError(f"public S12-P1 evidence contains forbidden field: {forbidden}")
    for path in paths:
        if path.suffix == ".json":
            _json(path)
...
def _check_governance_sync_in_clean_worktree() -> None:
    with tempfile.TemporaryDirectory(prefix="kmfa-s12p1-governance-") as temp_dir:
...
def _check_evidence() -> None:
    ...
    income = _json(builder.INCOME_CONTRACT_PATH)
    cost = _json(builder.COST_CONTRACT_PATH)
    pool = _json(builder.POOL_CONTRACT_PATH)
    verification = _json(builder.VERIFICATION_PATH)
    conservation = _json(builder.CONSERVATION_PATH)
    tasks = _json(builder.TASK_MATRIX_PATH)
    if source.get("roadmap_phase_id") != "S12-P1" or source.get("task_count") != 3:
        raise CheckError("source contract drift")
    if income.get("fact_contract", {}).get("income_layers") != list(kernel.INCOME_LAYERS):
        raise CheckError("income layer contract drift")
    if income.get("fact_contract", {}).get("unknown_basis_combination_allowed") is not False:
        raise CheckError("unknown income basis merge gate drift")
    if cost.get("fact_contract", {}).get("cost_categories") != list(kernel.COST_CATEGORIES):
        raise CheckError("cost category contract drift")
    if cost.get("fact_contract", {}).get("traceability_fields") != list(kernel.TRACEABILITY_FIELDS):
        raise CheckError("cost traceability contract drift")
    pool_contract = pool.get("pool_contract", {})
    if any(pool_contract.get(key) is not False for key in ("dropped_cost_allowed", "average_allocation_allowed", "silent_classification_allowed")):
        raise CheckError("unallocated pool safety contract drift")
    if verification.get("accounting") != {"total": 63, "passed": 63, "failed": 0} or verification.get("failed_checks") != []:
        raise CheckError("fact verification accounting drift")
    expected_conservation = {
        "input_cost_fact_count": 13,
        "allocated_cost_fact_count": 10,
        "unallocated_cost_pool_count": 3,
        "input_cost_cents": 70000,
        "allocated_cost_cents": 55000,
        "unallocated_cost_cents": 15000,
        "conservation_delta_cents": 0,
        "dropped_cost_fact_count": 0,
        "average_allocation_count": 0,
        "silent_classification_count": 0,
        "money_tolerance_cents": 0,
    }
    mismatches = [key for key, value in expected_conservation.items() if conservation.get(key) != value]
    if mismatches:
        raise CheckError("cost conservation evidence drift: " + ", ".join(mismatches))
...
def _check_manifest(*, pre_final: bool) -> dict[str, Any]:
    ...
    expected = {
        ...
        "overall_accepted_phase_count": 31 if pre_final else 32,
        "decision": "REMAIN_IN_S12_P1_FINAL_VALIDATION" if pre_final else "CONTINUE_TO_S12_P2_ONLY",
        "income_layer_count": 5,
        "income_fact_count": 7,
        "income_unknown_basis_count": 1,
        "unknown_income_merge_allowed": False,
        "cross_layer_income_merge_allowed": False,
        "cost_category_count": 10,
        "traceability_field_count": 7,
        "allocated_cost_fact_count": 10,
        "unallocated_cost_pool_count": 3,
        "conservation_delta_cents": 0,
        "dropped_cost_fact_count": 0,
        ...
        "s12_p2_entry_allowed": not pre_final,
    ...
    if mismatches:
        raise CheckError("S12-P1 manifest mismatch: " + ", ".join(mismatches))
    return manifest
...
def _check_governance(*, pre_final: bool) -> None:
    ...
    for relative in ("docs/governance/project.yaml", "metadata/project/project.yaml", "docs/governance/roadmap.yaml"):
        ...
        for token in (
            ...
            f"s12_p2_entry_allowed: {str(not pre_final).lower()}",
            "s12_p2_started: false",
            "s12_p3_entry_allowed: false",
            "s12_stage_review_entry_allowed: false",
            "github_upload_performed: false",
    ...
    for text in (registry_text, mirror_text):
        if "kmfa_v015_s12_p1_project_cost_facts" not in text or "MOD-KMFA-COST-001" not in text:
            raise CheckError("S12-P1 model registry entry missing")
    if "FORM-KMFA-V015-S12-P1-PROJECT-COST-FACTS-001" not in formula_text:
        raise CheckError("S12-P1 formula registry entry missing")
    for number in range(2182, 2196):
        if f"PARAM-KMFA-{number}" not in parameter_text:
            raise CheckError(f"S12-P1 parameter missing: PARAM-KMFA-{number}")
...
def _check_receipts(manifest: dict[str, Any]) -> None:
    ...
    if manifest.get("validation_head") != head or manifest.get("validation_run_id") != run_id or manifest.get("validation_receipt_count") != len(rows):
        raise CheckError("S12-P1 manifest validation binding drift")
    if _git("rev-parse", "HEAD^") != head:
        raise CheckError("final S12-P1 evidence commit must be the immediate child of validation head")
...
def main() -> int:
    ...
    try:
        ...
        return 1
    print("PASS: S12-P1 strict receipt-bound checks")
    return 0
