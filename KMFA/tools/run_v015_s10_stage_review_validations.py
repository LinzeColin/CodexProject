#!/usr/bin/env python3
"""为 KMFA v1.5 S10 整体复审生成一组精确绑定的验证回执。"""
...
from KMFA.tools import check_v015_s10_stage_review as checker
...
def run() -> list[dict[str, Any]]:
    ...
    if (
        ...
        or subject.get("s10_stage_review_acceptance_status") != "PENDING_FINAL_VALIDATION"
        or subject.get("s11_p1_entry_allowed") is not False
        or subject.get("s11_p1_started") is not False
        or subject.get("github_upload_performed") is not False
        or subject.get("app_reinstall_performed") is not False
    ):
        raise ValidationError("validation subject must be pending S10 review with S11 and release actions closed")
    ...
    for name, command in checker.EXPECTED_VALIDATIONS:
        ...
        receipt = {
            "schema_version": "kmfa.v015.s10_stage_review.validation_receipt.v1",
