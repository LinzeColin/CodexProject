#!/usr/bin/env python3
"""Run and bind the single formal KMFA v1.5 S21-P2 validation set."""
...
from KMFA.tools import check_v015_s21_p2_report_generation as checker
...
class ValidationError(RuntimeError):
    """Formal S21-P2 validation cannot continue."""
...
def _require_clean_subject() -> None:
    ...
    for args in (("diff", "--quiet"), ("diff", "--cached", "--quiet")):
        if subprocess.run(["git", *args], cwd=REPO_ROOT, check=False).returncode:
            raise ValidationError("validation requires a clean tracked S21-P2 implementation commit")
...
def run() -> list[dict[str, Any]]:
    ...
    zero_keys = (
        ...
        "external_network_request_count", "approval_or_publication_count", "s21_p3_execution_count",
    )
    if (
        subject.get("phase_acceptance_status") != "PENDING_FINAL_VALIDATION"
        or subject.get("phase_task_accepted_count") != 0
        or subject.get("s21_p1_acceptance_status") != "PASSED"
        or subject.get("s21_p2_started") is not True or subject.get("s21_p2_completed") is not False
        or subject.get("s21_p3_entry_allowed") is not False or subject.get("s21_p3_started") is not False
        or subject.get("cross_format_difference_integer") != 0
        ...
        raise ValidationError("validation subject must be pending S21-P2 with downstream and release work closed")
    ...
    for name, command in checker.EXPECTED_VALIDATIONS:
        ...
        receipt = {
            "schema_version": "kmfa.v015.s21p2.validation_receipt.v1",
    ...
    sync = subprocess.run(
        [str(BUNDLED_PYTHON), "-B", "KMFA/tools/v015_roadmap_governance_sync.py", "--write", "--validation-state", "S21_P2_PASSED"],
...
def main() -> int:
    try:
        ...
        print(f"FAIL: {error}")
        return 1
    print(f"PASS: S21-P2 validation receipts run={rows[0]['validation_run_id']} count={len(rows)} head={rows[0]['validation_head']}")
    return 0
