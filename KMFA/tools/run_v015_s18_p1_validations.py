#!/usr/bin/env python3
"""执行并记录 KMFA v1.5 S18-P1 唯一一组正式验收。"""
...
from KMFA.tools import check_v015_s18_p1_receivables_collections as checker
...
def _require_clean_subject() -> None:
    for args in (("diff", "--quiet"), ("diff", "--cached", "--quiet")):
        if subprocess.run(["git", *args], cwd=REPO_ROOT, check=False).returncode:
            raise ValidationError("validation requires a clean tracked S18-P1 implementation commit")
    untracked = [
        line
...
def run() -> list[dict[str, Any]]:
    ...
    if (
        ...
        or subject.get("s17_stage_review_acceptance_status") != "PASSED"
        or subject.get("s18_p1_started") is not True
        or subject.get("s18_p1_completed") is not False
        or subject.get("s18_p2_entry_allowed") is not False
        or subject.get("s18_p2_started") is not False
        or subject.get("s18_p3_started") is not False
        or subject.get("raw_root_access_count") != 0
        or subject.get("live_source_read_count") != 0
        ...
        or subject.get("source_data_write_count") != 0
        or subject.get("fact_layer_write_count") != 0
        or subject.get("payment_execution_count") != 0
        or subject.get("automatic_customer_contact_count") != 0
        ...
        raise ValidationError("validation subject must be pending S18-P1 with later and release work closed")
    ...
    for name, command in checker.EXPECTED_VALIDATIONS:
        ...
        receipt = {
            "schema_version": "kmfa.v015.s18p1.validation_receipt.v1",
...
def main() -> int:
    try:
        ...
        return 1
    print(
        f"PASS: S18-P1 validation receipts run={rows[0]['validation_run_id']} "
