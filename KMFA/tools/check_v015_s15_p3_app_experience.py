#!/usr/bin/env python3
"""严格检查 KMFA v1.5 S15-P3 搜索、通知和个人偏好。"""
...
EXPECTED_VALIDATIONS = (
    (
        "python_compile",
        "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B -c \"import ast,pathlib; [ast.parse(pathlib.Path(p).read_text(encoding='utf-8')) for p in ('KMFA/tools/v015_s15_p3_app_experience.py','KMFA/tools/run_v015_s15_p3_app_experience.py','KMFA/tools/build_v015_s15_p3_app_experience.py','KMFA/tools/check_v015_s15_p3_app_experience.py','KMFA/tools/run_v015_s15_p3_browser_tests.py','KMFA/tools/run_v015_s15_p3_validations.py','KMFA/tools/v015_roadmap_governance_sync.py')]\"",
    ),
    ("focused_kernel_tests", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B -m unittest KMFA.tests.test_v015_s15_p3_app_experience"),
    ("focused_runtime_tests", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B -m unittest KMFA.tests.test_v015_s15_p3_app_experience_runtime"),
    ("focused_browser_tests", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B KMFA/tools/run_v015_s15_p3_browser_tests.py"),
    ("focused_artifact_tests", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B -m unittest KMFA.tests.test_v015_s15_p3_app_experience_artifacts"),
    ("focused_governance_tests", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B -m unittest KMFA.tests.test_v015_s15_p3_app_experience_governance"),
    ("s15_p2_dependency", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B KMFA/tools/check_v015_s15_p3_app_experience.py --dependency-check"),
    ("deterministic_evidence", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B KMFA/tools/build_v015_s15_p3_app_experience.py --check"),
    ("pre_final_phase_checker", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B KMFA/tools/check_v015_s15_p3_app_experience.py --pre-final --skip-validation-receipts"),
    ...
    ("roadmap_sync_pending", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B KMFA/tools/v015_roadmap_governance_sync.py --check --validation-state S15_P3_PENDING_FINAL_VALIDATION"),
    ...
    ("governance_sync", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B KMFA/tools/check_v015_s15_p3_app_experience.py --clean-governance-sync-check"),
    ...
    ("taskpack_source", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B KMFA/tools/check_v015_s15_p3_app_experience.py --taskpack-source-check"),
    ("public_boundary", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B KMFA/tools/check_v015_s15_p3_app_experience.py --public-boundary-check"),
...
ALLOWED_PHASE_PREFIXES = (
    ...
    "KMFA/stage_artifacts/V015_S15_P3_APP_EXPERIENCE/",
    "KMFA/tests/test_v015_roadmap_governance_sync.py",
    "KMFA/tests/test_v015_s15_p3_app_experience.py",
    "KMFA/tests/test_v015_s15_p3_app_experience_runtime.py",
    "KMFA/tests/test_v015_s15_p3_app_experience_browser.py",
    "KMFA/tests/test_v015_s15_p3_app_experience_artifacts.py",
    "KMFA/tests/test_v015_s15_p3_app_experience_governance.py",
    "KMFA/tools/build_v015_s15_p3_app_experience.py",
    "KMFA/tools/check_v015_s15_p3_app_experience.py",
    "KMFA/tools/run_v015_s15_p3_app_experience.py",
    "KMFA/tools/run_v015_s15_p3_browser_tests.py",
    "KMFA/tools/run_v015_s15_p3_validations.py",
    "KMFA/tools/v015_roadmap_governance_sync.py",
    "KMFA/tools/v015_s15_p3_app_experience.py",
    "KMFA/功能清单.md",
    "KMFA/开发记录.md",
    "KMFA/模型参数文件.md",
)
...
def _check_scope() -> None:
    if subprocess.run(
        ...
        raise CheckError("S15-P3 base commit is not an ancestor of HEAD")
    changed: set[str] = set()
    for args in (
    ...
    if unexpected:
        raise CheckError("unexpected S15-P3 path(s): " + ", ".join(unexpected))


def _check_dependency() -> None:
    value = builder.dependency()
    if value.get("acceptance_status") != "PASSED" or value.get("validation_receipt_count") != 20:
        raise CheckError("S15-P2 dependency is not accepted")
    if value.get("s15_p3_entry_allowed") is not True or value.get("s15_p3_started") is not False:
        raise CheckError("S15-P2 did not open exactly S15-P3")
...
def _check_taskpack_source() -> None:
    ...
    expected = [
        ("T01", "实现搜索与最近访问", "结果有来源和权限过滤。", "敏感结果不得泄露。"),
        ("T02", "实现通知中心和待办", "每项有明确处理入口。", "无入口的提醒失败。"),
        ("T03", "实现偏好设置", "偏好不影响事实和他人。", "偏好不得写入 raw 或事实层。"),
    ]
    actual = [
    ...
    if (
        not stage
        or stage.get("name") != "应用外壳、角色权限与多主体上下文"
        or not phase
        or phase.get("name") != "应用基础体验"
        or actual != expected
    ):
        raise CheckError("S15-P3 source contract drifted")
...
def _check_artifacts() -> None:
    ...
    search = _json(builder.SEARCH_CONTRACT_PATH)
    notices = _json(builder.NOTIFICATION_CONTRACT_PATH)
    preferences = _json(builder.PREFERENCE_CONTRACT_PATH)
    browser = _json(builder.BROWSER_CONTRACT_PATH)
    if search.get("search_item_count") != 8 or search.get("search_kind_count") != 4:
        raise CheckError("search contract incomplete")
    if search.get("management_sensitive_result_count") != 0 or search.get("finance_sensitive_result_count") != 1:
        raise CheckError("sensitive search permission contract incomplete")
    if search.get("sensitive_result_leak_count") != 0 or search.get("recent_scope") != "CURRENT_USER_ONLY":
        raise CheckError("search and recent-access isolation incomplete")
    if notices.get("notification_item_count") != 4 or notices.get("notification_category_count") != 4:
        raise CheckError("notification contract incomplete")
    if notices.get("notification_without_action_count") != 0 or notices.get("action_entry_count") != notices.get("finance_visible_count"):
        raise CheckError("notification action contract incomplete")
    if preferences.get("preference_field_count") != 4 or preferences.get("persistence_scope") != "CURRENT_USER_ONLY":
        raise CheckError("preference contract incomplete")
    if preferences.get("other_user_write_allowed") is not False or preferences.get("fact_layer_write_count") != 0 or preferences.get("raw_write_count") != 0:
        raise CheckError("preference isolation contract incomplete")
...
def _check_public_boundary() -> None:
    ...
    files.extend(
        [
            builder.PROJECT_ROOT / "tools/v015_s15_p3_app_experience.py",
            builder.PROJECT_ROOT / "tools/run_v015_s15_p3_app_experience.py",
        ]
    )
    for path in files:
...
def _check_governance_sync() -> None:
    state = (
        ...
        else "S15_P3_PENDING_FINAL_VALIDATION"
    )
...
def check(pre_final: bool = False, skip_validation_receipts: bool = False) -> None:
    ...
    if pre_final:
        ...
        if manifest.get("s15_stage_review_entry_allowed") is not False or manifest.get("s15_stage_review_started") is not False:
            raise CheckError("pre-final S15 review must remain closed")
        ...
        if not final or manifest.get("phase_acceptance_status") != "PASSED" or manifest.get("evidence_validation_status") != "PASS":
            raise CheckError("final S15-P3 acceptance receipts are incomplete")
        if manifest.get("phase_task_accepted_count") != 3 or matrix.get("phase_task_accepted_count") != 3:
            raise CheckError("all three S15-P3 tasks must be accepted")
        ...
        if manifest.get("s15_stage_review_entry_allowed") is not True or manifest.get("s15_stage_review_started") is not False:
            raise CheckError("final state must open but not start S15 review")
        if manifest.get("s16_entry_allowed") is not False or manifest.get("product_implementation_allowed") is not False:
            raise CheckError("only S15 review may open after S15-P3")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="检查 KMFA v1.5 S15-P3 应用基础体验")
    ...
    try:
        ...
        print(f"FAIL: {error}")
        return 1
    print("PASS: S15-P3 app experience " + ("pre-final" if args.pre_final else "check"))
    return 0
