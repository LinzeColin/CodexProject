#!/usr/bin/env python3
"""为 KMFA v1.5 S11 整体复审生成精确绑定的验收回执。"""
...
from KMFA.tools import check_v015_s11_stage_review as checker
...
def run() -> list[dict[str, Any]]:
    ...
    if (
        ...
        or subject.get("s11_stage_review_acceptance_status") != "PENDING_FINAL_VALIDATION"
        or subject.get("s12_p1_entry_allowed") is not False
        or subject.get("s12_p1_started") is not False
        or subject.get("github_upload_performed") is not False
        or subject.get("app_reinstall_performed") is not False
    ):
        raise ValidationError("validation subject must be pending S11 review with S12 and release actions closed")
    ...
    for name, command in checker.EXPECTED_VALIDATIONS:
        ...
        receipt = {
            "schema_version": "kmfa.v015.s11_stage_review.validation_receipt.v1",
...
def main() -> int:
    try:
        ...
        return 1
    print(f"PASS: S11 整体复审验收回执 run={receipts[0]['validation_run_id']} count={len(receipts)} head={receipts[0]['validation_head']}")
    return 0
