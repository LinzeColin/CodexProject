#!/usr/bin/env python3
"""Run and receipt one exact validation set for KMFA v1.5 S08-P3."""
...
from KMFA.tools import check_v015_s08_p3_matching_quality_confirmation as checker
...
def run() -> list[dict[str, Any]]:
    if _git("status", "--porcelain"):
        raise ValidationError("validation requires a clean S08-P3 implementation commit")
    ...
    if (
        ...
        or subject.get("task_accepted_count") != 0
        or subject.get("s08_stage_review_entry_allowed") is not False
        or subject.get("github_upload_performed") is not False
        or subject.get("app_reinstall_performed") is not False
    ):
        raise ValidationError("validation subject must be pending S08-P3 with review and release closed")
    ...
    for name, command in checker.EXPECTED_VALIDATIONS:
        ...
        receipt = {
            "schema_version": "kmfa.v015.s08p3.validation_receipt.v1",
...
def main() -> int:
    try:
        ...
        return 1
    print(
        "PASS: S08-P3 validation receipts "
