#!/usr/bin/env python3
"""Run and receipt one exact validation set for KMFA v1.5 S09-P3."""
...
from KMFA.tools import check_v015_s09_p3_human_readable_audit as checker
...
def run() -> list[dict[str, Any]]:
    if _git("status", "--porcelain"):
        raise ValidationError("validation requires a clean S09-P3 implementation commit")
    ...
    if (
        ...
        or subject.get("s09_stage_review_entry_allowed") is not False
        or subject.get("s09_stage_review_started") is not False
        ...
        raise ValidationError("validation subject must be pending S09-P3 with review, S10, and release closed")
    ...
    for name, command in checker.EXPECTED_VALIDATIONS:
        ...
        receipt = {
            "schema_version": "kmfa.v015.s09p3.validation_receipt.v1",
...
def main() -> int:
    try:
        ...
        return 1
    print(
        "PASS: S09-P3 validation receipts "
