...
EXPECTED_VALIDATIONS = (
    (
        "python_compile",
        "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B -c \"import ast,pathlib; [ast.parse(pathlib.Path(p).read_text(encoding='utf-8')) for p in ('KMFA/tools/v015_s13_p1_indicator_registry.py','KMFA/tools/build_v015_s13_p1_indicator_registry.py','KMFA/tools/check_v015_s13_p1_indicator_registry.py','KMFA/tools/run_v015_s13_p1_validations.py','KMFA/tools/v015_roadmap_governance_sync.py')]\"",
    ),
    ("focused_kernel_tests", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B -m unittest KMFA.tests.test_v015_s13_p1_indicator_registry"),
    ("focused_artifact_tests", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B -m unittest KMFA.tests.test_v015_s13_p1_indicator_registry_artifacts"),
    ("focused_governance_tests", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B -m unittest KMFA.tests.test_v015_s13_p1_indicator_registry_governance"),
    ("s12_stage_review_regression", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B -m unittest KMFA.tests.test_v015_s12_stage_review_contract KMFA.tests.test_v015_s12_stage_review"),
    ("s12_stage_review_dependency", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B KMFA/tools/check_v015_s13_p1_indicator_registry.py --dependency-check"),
    ("function_boundary_self_check", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B -c \"from KMFA.tools.v015_s13_p1_indicator_registry import public_verification; v=public_verification(); assert v['accounting']=={'total':78,'passed':78,'failed':0} and not v['failed_checks']\""),
    ("deterministic_evidence", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B KMFA/tools/build_v015_s13_p1_indicator_registry.py --check"),
    ("pre_final_phase_checker", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B KMFA/tools/check_v015_s13_p1_indicator_registry.py --pre-final --skip-validation-receipts"),
    ...
    ("roadmap_sync_pending", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B KMFA/tools/v015_roadmap_governance_sync.py --check --validation-state S13_P1_PENDING_FINAL_VALIDATION"),
    ...
    ("governance_sync", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B KMFA/tools/check_v015_s13_p1_indicator_registry.py --clean-governance-sync-check"),
    ...
    ("taskpack_source", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B KMFA/tools/check_v015_s13_p1_indicator_registry.py --taskpack-source-check"),
    ("public_boundary", "PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. python3 -B KMFA/tools/check_v015_s13_p1_indicator_registry.py --public-boundary-check"),
...
ALLOWED_PHASE_PREFIXES = (
    ...
    "KMFA/metadata/lineage/v015_s13_p1_",
    "KMFA/metadata/quality/v015_s13_p1_",
    "KMFA/metadata/model_registry.yaml",
    "KMFA/metadata/project/project.yaml",
    "KMFA/metadata/stage_status.jsonl",
    "KMFA/stage_artifacts/V015_S13_P1_INDICATOR_REGISTRY/",
    "KMFA/tests/test_v015_roadmap_governance_sync.py",
    "KMFA/tests/test_v015_s13_p1_indicator_registry.py",
    "KMFA/tests/test_v015_s13_p1_indicator_registry_artifacts.py",
    "KMFA/tests/test_v015_s13_p1_indicator_registry_governance.py",
    "KMFA/tools/build_v015_s13_p1_indicator_registry.py",
    "KMFA/tools/check_v015_s13_p1_indicator_registry.py",
    "KMFA/tools/run_v015_s13_p1_validations.py",
    "KMFA/tools/v015_roadmap_governance_sync.py",
    "KMFA/tools/v015_s13_p1_indicator_registry.py",
    "KMFA/功能清单.md",
    "KMFA/开发记录.md",
    "KMFA/模型参数文件.md",
)
...
def _check_scope() -> None:
    if subprocess.run(["git", "merge-base", "--is-ancestor", builder.PHASE_BASE_COMMIT, "HEAD"], cwd=REPO_ROOT, check=False).returncode:
        raise CheckError("S13-P1 base commit is not an ancestor of HEAD")
    changed: set[str] = set()
    for args in (
    ...
    if unexpected:
        raise CheckError("unexpected S13-P1 path(s): " + ", ".join(unexpected))


def _check_dependency() -> None:
    value = builder.dependency()
    if value.get("acceptance_status") != "PASSED" or value.get("validation_receipt_count") != 24:
        raise CheckError("S12 Stage Review dependency is not accepted")
    if value.get("s13_p1_entry_allowed") is not True or value.get("s13_p1_started") is not False:
        raise CheckError("S12 Stage Review did not open exactly S13-P1")
...
def _check_taskpack_source() -> None:
    ...
    if (stage or {}).get("name") != "经营指标、健康模型与行动优先级" or (phase or {}).get("name") != "指标注册表":
        raise CheckError("S13-P1 source phase drift")
    tasks = (phase or {}).get("tasks", [])
    if [row.get("id") for row in tasks] != ["T01", "T02", "T03"]:
        raise CheckError("S13-P1 source task drift")
    if [row.get("stop") for row in tasks] != ["无来源指标不得显示。", "前端不得直接改生产参数。", "异常不得被默默吞掉。"]:
        raise CheckError("S13-P1 source stop condition drift")


def _public_paths() -> list[Path]:
    paths = [path for path in builder.OUTPUT_ROOT.rglob("*") if path.is_file()]
    paths.extend((builder.INDICATOR_REGISTRY_PATH, builder.PARAMETER_VERSIONS_PATH, builder.FUNCTION_CONTRACT_PATH))
    return paths
...
def _check_public_boundary() -> None:
    ...
    for pattern in (r"/Users/", r"/Volumes/", r"/home/", r"file://", r"KMFA_MetaData", r"private://"):
        if re.search(pattern, text, re.IGNORECASE):
            raise CheckError(f"public S13-P1 evidence contains forbidden material: {pattern}")
    for forbidden in ("raw_value", "original_value", "plaintext_value", "private_hash", "bank_account_number", "identity_document_number"):
        if forbidden in text:
            raise CheckError(f"public S13-P1 evidence contains forbidden field: {forbidden}")
    for path in paths:
        if path.suffix == ".json":
            _json(path)
...
def _check_governance_sync_in_clean_worktree() -> None:
    with tempfile.TemporaryDirectory(prefix="kmfa-s13p1-governance-") as temp_dir:
...
def _check_evidence() -> None:
    ...
    indicator = _json(builder.INDICATOR_REGISTRY_PATH)
    parameters = _json(builder.PARAMETER_VERSIONS_PATH)
    functions = _json(builder.FUNCTION_CONTRACT_PATH)
    verification = _json(builder.VERIFICATION_PATH)
    tasks = _json(builder.TASK_MATRIX_PATH)
    if source.get("roadmap_phase_id") != "S13-P1" or source.get("task_count") != 3:
        raise CheckError("source contract drift")
    if indicator.get("indicators") != kernel.indicator_registry():
        raise CheckError("indicator registry drift")
    if parameters.get("parameters") != kernel.parameter_versions():
        raise CheckError("parameter registry drift")
    if functions.get("functions") != kernel.function_contracts():
        raise CheckError("function contract drift")
    if verification.get("accounting") != {"total": 78, "passed": 78, "failed": 0} or verification.get("failed_checks") != []:
        raise CheckError("public verification accounting drift")
    if tasks.get("task_count") != 3:
        raise CheckError("task matrix drift")
    if any(row.get("display_without_source_allowed") is not False for row in indicator["indicators"]):
        raise CheckError("source display gate drift")
    if parameters.get("frontend_write_allowed") is not False or parameters.get("production_direct_write_allowed") is not False:
        raise CheckError("parameter write gate drift")
    if functions.get("silent_exception_allowed") is not False:
        raise CheckError("silent exception gate drift")
...
def _check_manifest(*, pre_final: bool) -> dict[str, Any]:
    ...
    expected = {
        ...
        "overall_accepted_phase_count": 34 if pre_final else 35,
        "decision": "REMAIN_IN_S13_P1_FINAL_VALIDATION" if pre_final else "CONTINUE_TO_S13_P2_ONLY",
        "indicator_count": 8,
        "indicator_domain_count": 8,
        "parameter_version_count": 8,
        "function_contract_count": 5,
        "result_status_count": 6,
        ...
        "s13_p2_entry_allowed": not pre_final,
    ...
    if mismatches:
        raise CheckError("S13-P1 manifest mismatch: " + ", ".join(mismatches))
    return manifest
...
def _check_governance(*, pre_final: bool) -> None:
    ...
    for relative in ("docs/governance/project.yaml", "metadata/project/project.yaml", "docs/governance/roadmap.yaml"):
        ...
        for token in (
            ...
            f"s13_p2_entry_allowed: {str(not pre_final).lower()}",
            "s13_p2_started: false",
            "s13_p3_entry_allowed: false",
            "s13_stage_review_entry_allowed: false",
            "github_upload_performed: false",
    ...
    for text in (registry, mirror):
        if "kmfa_v015_s13_p1_indicator_registry" not in text or "MOD-KMFA-COST-001" not in text:
            raise CheckError("S13-P1 model registry entry missing")
    if "FORM-KMFA-V015-S13-P1-INDICATOR-REGISTRY-001" not in formula:
        raise CheckError("S13-P1 formula registry entry missing")
    for number in range(2240, 2254):
        if f"PARAM-KMFA-{number}" not in parameters:
            raise CheckError(f"S13-P1 parameter missing: PARAM-KMFA-{number}")
...
def _check_receipts(manifest: dict[str, Any]) -> None:
    ...
    if manifest.get("validation_head") != head or manifest.get("validation_run_id") != run_id or manifest.get("validation_receipt_count") != len(rows):
        raise CheckError("S13-P1 manifest validation binding drift")
    if _git("rev-parse", "HEAD^") != head:
        raise CheckError("final S13-P1 evidence commit must be the immediate child of validation head")
...
def main() -> int:
    ...
    try:
        ...
        return 1
    print("PASS: S13-P1 strict receipt-bound checks")
    return 0
