#!/usr/bin/env python3
"""Run and receipt one exact validation set for KMFA v1.5 S09-P1."""
...
from KMFA.tools import check_v015_s09_p1_scope_rule_modeling as checker
...
def run() -> list[dict[str, Any]]:
    if _git("status", "--porcelain"):
        raise ValidationError("validation requires a clean S09-P1 implementation commit")
    ...
    if (
        ...
        or subject.get("s09_p2_entry_allowed") is not False
        or subject.get("s09_p2_started") is not False
        or subject.get("github_upload_performed") is not False
        or subject.get("app_reinstall_performed") is not False
    ):
        raise ValidationError("validation subject must be pending S09-P1 with later work and release closed")
    ...
    for name, command in checker.EXPECTED_VALIDATIONS:
        ...
        receipt = {
            "schema_version": "kmfa.v015.s09p1.validation_receipt.v1",
...
def main() -> int:
    try:
        ...
        return 1
    print(
        "PASS: S09-P1 validation receipts "
