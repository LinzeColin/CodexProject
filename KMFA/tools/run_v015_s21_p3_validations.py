#!/usr/bin/env python3
"""Run and bind the single formal KMFA v1.5 S21-P3 validation set."""
...
from KMFA.tools import check_v015_s21_p3_report_workflow as checker
...
class ValidationError(RuntimeError):
    """Formal S21-P3 validation cannot continue."""
...
def _require_clean_subject() -> None:
    ...
    for args in (("diff", "--quiet"), ("diff", "--cached", "--quiet")):
        if subprocess.run(["git", *args], cwd=REPO_ROOT, check=False).returncode:
            raise ValidationError("validation requires a clean tracked S21-P3 implementation commit")
    untracked = [
...
def run() -> list[dict[str, Any]]:
    ...
    zero_keys = (
        "history_overwrite_count", "raw_root_access_count", "raw_write_count",
        "external_network_request_count", "external_publication_count", "public_share_link_count",
        "cross_company_access_success_count",
    )
    if (
        subject.get("phase_acceptance_status") != "PENDING_FINAL_VALIDATION"
        or subject.get("phase_task_accepted_count") != 0
        or subject.get("s21_p2_acceptance_status") != "PASSED"
        or subject.get("s21_p3_started") is not True
        or subject.get("s21_p3_completed") is not False
        or subject.get("s21_stage_review_entry_allowed") is not False
        or subject.get("s21_stage_review_started") is not False
        or subject.get("s22_entry_allowed") is not False
        or subject.get("s22_p1_started") is not False
        or subject.get("unexplained_difference_count") != 0
        or subject.get("internal_approval_count") != 1
        or subject.get("internal_publication_count") != 1
        ...
        raise ValidationError("validation subject must be pending S21-P3 with review release raw and external work closed")
    ...
    for name, command in checker.EXPECTED_VALIDATIONS:
        ...
        result = subprocess.run(
            command, cwd=REPO_ROOT, shell=True, executable="/bin/zsh", env=environment,
            capture_output=True, text=True, check=False,
        )
        combined = result.stdout + result.stderr
        receipt = {
            "schema_version": "kmfa.v015.s21p3.validation_receipt.v1",
    ...
    sync = subprocess.run(
        [str(BUNDLED_PYTHON), "-B", "KMFA/tools/v015_roadmap_governance_sync.py", "--write", "--validation-state", "S21_P3_PASSED"],
        cwd=REPO_ROOT,
...
def main() -> int:
    try:
        ...
        print(f"FAIL: {error}")
        return 1
    print(f"PASS: S21-P3 validation receipts run={rows[0]['validation_run_id']} count={len(rows)} head={rows[0]['validation_head']}")
    return 0
