...
3. 是否确认仅使用 `main` 作为持久分支，并退役 `macdata-airM2`？
...
8. 是否确认不自动清理 Docker、Homebrew、系统缓存、项目缓存，只清理 macdata 自身临时缓存和 3 天以前记录？
9. 如果预检发现设备型号、芯片、内存、Git remote 或 main-only 事务策略和预期不同，是否需要继续？若继续，请明确差异和授权。
