...
9. 是否确认远程上传验证成功后，自动关闭/删除本 automation 或 Codex 创建且已合入 `main` 的临时 PR、临时 branch、带 managed marker 的 issue，并保护 `main` 与 `macdata-proM2`？
10. 如果预检发现设备型号、芯片、内存、Git remote、归档分支和预期不同，是否需要继续？若继续，请明确差异和授权。
