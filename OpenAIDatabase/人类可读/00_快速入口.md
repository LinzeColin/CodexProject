...
这是 Memory Atlas v1.2 的人类入口。当前阶段是 S05 P3：证据引用
已完成。任务 ID 为 `MA-V12-S05P3`，验收 ID 为 `ACC-MA-V12-S05P3`，
状态为 `phase_s05_p3_evidence_refs_completed_pending_s05_review`。
...
`derived_ref` 或 missing reason。下一步只允许进入 S05 Review。
...
下一步只允许进入 S05 Review。整体 S01-S14 全部完成前不上传 GitHub main。
