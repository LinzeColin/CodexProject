...
这是 Memory Atlas v1.2 的人类入口。当前阶段是 S10 P3。任务 ID 为
`MA-V12-S10P3`，验收 ID 为 `ACC-MA-V12-S10P3`，状态为
`phase_s10_p3_machine_detail_folding_completed_pending_s10_review`。Validator 为
`validate:v1.2-s10-p3`。当前 S10 P3 已完成
`machine_detail_folding.v1_2_s10_p3`，机器字段默认折叠并通过中文“高级详情”入口保留
agent 核验能力。下一步只允许进入 pending S10 Review。
