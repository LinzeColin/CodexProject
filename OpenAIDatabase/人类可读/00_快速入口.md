...
这是 Memory Atlas v1.2 的人类入口。当前阶段是 S04 P1：ChatGPT 只读同步已完成。
任务 ID 为 `MA-V12-S04P1`，验收 ID 为 `ACC-MA-V12-S04P1`，状态为
`phase_s04_p1_chatgpt_sync_completed_pending_s04_p2`。
...
ChatGPT 只读同步和 official export fallback。下一步只允许进入 S04 P2。
...
下一步只允许进入 S04 P2。S04 P2 才能处理 Codex local sync 和后续 agent adapter。
