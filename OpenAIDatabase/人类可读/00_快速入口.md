...
这是 Memory Atlas v1.2 的人类入口。当前阶段是 S06 P3：机会发现和 为什么不是现在
卡片已完成。任务 ID 为 `MA-V12-S06P3`，验收 ID 为
...
为什么不是现在 卡片，输出
`data/derived/behavior_intelligence/opportunities.json`。下一步只允许进入 S06 Review。
...
下一步只允许进入 S06 Review。整体 S01-S14 全部完成前不上传 GitHub main。
