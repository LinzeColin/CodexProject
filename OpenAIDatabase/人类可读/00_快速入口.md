...
这是 Memory Atlas v1.2 的人类入口。当前阶段是 S11 P2。任务 ID 为
`MA-V12-S11P2`，验收 ID 为 `ACC-MA-V12-S11P2`，状态为
`phase_s11_p2_economic_like_visuals_completed_pending_s11_p3`。Validator 为
`validate:v1.2-s11-p2`。当前 S11 P2 已完成 Economic-like visuals：`task_treemap`、
`automation_vs_augmentation`、`roi_scatter` 和 `opportunity_radar` 均已绑定中文
insight header、human question、action value 与 `source/time/project/task` 过滤。
下一步是 S11 P3。
...
Economic-like visuals。下一步只允许进入 S11 P3。
