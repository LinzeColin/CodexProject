...
这是 Memory Atlas v1.2 的人类入口。当前阶段是 S04 P3：GitHub backup
dry-run/apply 本地控制面已完成。任务 ID 为 `MA-V12-S04P3`，
...
backup dry-run/apply，apply 只做本地 commit，不执行远端 push。下一步只允许进入 S04 Review。
...
下一步只允许进入 S04 Review。S04 Review 通过前不能进入 S05。
