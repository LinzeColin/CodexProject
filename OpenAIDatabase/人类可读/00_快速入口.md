...
这是 Memory Atlas v1.2 的人类入口。当前阶段是 S12 P1 已完成。任务 ID 为
`MA-V12-S12P1`，验收 ID 为 `ACC-MA-V12-S12P1`，状态为
...
`command_palette.v1_2_s12_p1`，`generate_personalization_prompt` 只提供 ChatGPT、Codex、
other agent 的 dry-run 合同，不自动发送，不写 raw。下一步是 S12 P2。
