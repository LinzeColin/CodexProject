...
这是 Memory Atlas v1.2 的人类入口。当前阶段是 S03 P1：公开 raw 路径已定义。
任务 ID 为 `MA-V12-S03P1`，验收 ID 为 `ACC-MA-V12-S03P1`，状态为
...
append-only 规则和 hash drift fail 规则。下一步只允许进入 S03 P2。
...
- 不重装 app。
- 不导入真实 transcript。
- 不实现 S03 P2 credential gate。
- 不生成 S03 P3 manifest ledger。
- 不修改或删除已有 raw。

## 下一步

下一步只允许进入 S03 P2。S03 P2 需要继续遵守每次 run 最多一个 phase、
