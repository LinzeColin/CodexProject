...
这是 Memory Atlas v1.2 的人类入口。当前阶段是 S04 Review：自动同步 MVP
整体复审已通过。任务 ID 为 `MA-V12-S04-REVIEW`，验收 ID 为
`ACC-MA-V12-S04-REVIEW`，状态为
`stage_s04_review_passed_pending_s05_no_github_main_upload`。
...
下一步只允许进入 S05 P1。整体 S01-S14 全部完成前不上传 GitHub main。
