...
这是 Memory Atlas v1.2 的人类入口。当前阶段是 S08 P1。任务 ID 为
`MA-V12-S08P1`，验收 ID 为 `ACC-MA-V12-S08P1`，状态为
...
系统，不修改 raw，不定义授权 apply 边界。下一步只允许进入 S08 P2。
...
- 不进入 S08 P2 以外的后续 phase。

## 下一步

下一步只允许进入 S08 P2。整体 S01-S14 全部完成前不上传 GitHub main。
