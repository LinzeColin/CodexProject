...
这是 Memory Atlas v1.2 的人类入口。当前阶段是 S06 Review。任务 ID 为
`MA-V12-S06-REVIEW`，验收 ID 为 `ACC-MA-V12-S06-REVIEW`，状态为
`stage_s06_review_passed_pending_s07_no_github_main_upload`。
...
主题簇、低价值循环和机会线索。下一步只允许进入 S07 P1。
...
下一步只允许进入 S07 P1。整体 S01-S14 全部完成前不上传 GitHub main。
