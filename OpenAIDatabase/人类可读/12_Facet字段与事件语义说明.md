...
S05 P2 已完成 Memory Atlas v1.2 的 facet extractor。它从已有 public raw、
processed manifest 和 derived snapshot 中抽取 canonical behavior events，输出到
`data/derived/behavior_intelligence/events.json`。

任务 ID 为 `MA-V12-S05P2`，验收 ID 为 `ACC-MA-V12-S05P2`，状态为
`phase_s05_p2_facet_extractor_completed_pending_s05_p3`。

机器契约文件仍是 `机器治理/数据契约/facet_event_schema.v1_2_s05_p1.json`。
抽取脚本是 `scripts/extract_memory_atlas_facets.py`，CLI 入口是
`scripts/atlasctl.py analyze --stage facets`。下一步只允许进入 S05 P3：补强
evidence refs 和 review。
...
下一步只允许进入 S05 P3：补强 evidence refs 和 review。S05 P2 之后仍不得生成
fake events，不得修改 raw，不得把机器字段直接堆到首屏。
