...
- 存储 series SHA-256：`f6ec1331a97b64d41047c91884de03b1bf8674c2d93802c9f47b79d2e3e45f01`
...
存储 series hash 算法为：
`sha256(concat(natural-sort(filename UTF-8 + gzip file bytes)))`。解压后 hash 使用去掉
`.gz` 的数字文件名和解压后的 patch bytes。
...
2. 在迁移完成后的隔离、干净 checkout 中，按 `COMMIT_LEDGER.tsv` 顺序逐 Phase
   解压对应 `patches/<sequence>.gz`，再使用 `git apply --check` 或
   `git am --3way` 预演。
