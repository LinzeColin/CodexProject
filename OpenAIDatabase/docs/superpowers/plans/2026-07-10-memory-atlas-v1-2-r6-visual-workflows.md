...
- [x] Write a failing unit test that creates synthetic Human Question Map, event and
  ...
  flags.
- [x] Run `python3 -m unittest tests.test_memory_atlas_visual_workflows -q` and confirm it
  fails because the new snapshot fields are absent.
- [x] Add failing assertions that transcript/message/credential/token/raw payload keys
  cannot appear in any exported facet event or Formula summary.
- [x] Implement `HUMAN_QUESTION_MAP_SOURCE`, `BEHAVIOR_EVENT_SOURCE`,
...
- [x] Implement the bounded Formula summary from the tracked preview, preserving
  defaults, bounds, baseline signals, formula source, scenarios and safety.
- [x] Extend `MemoryAtlas` TypeScript types with the exact optional read-only contracts.
- [x] Re-run the focused unit test until green.
- [x] Rebuild `data/derived/visualization/memory_atlas.json`, parse it with
  `python3 -m json.tool`, and confirm raw/private files are unchanged.
- [x] Commit the data contract locally.
...
- [x] Write the model validator first using synthetic events with distinct sources,
  dates, projects and tasks plus a synthetic Formula preview.
- [x] Run `npm run validate:v1.2-visual-models` and confirm failure because the module or
  functions do not exist.
- [x] Require every filter axis independently and conjunctively changes the exact event
  IDs, with time anchored to the latest valid event.
- [x] Require invalid dates to fail a bounded time window and `all` to preserve them.
- [x] Require deterministic option ordering and filter signatures.
- [x] Require a bounded weight change to alter the Formula score, out-of-range values to
  clamp and Reset/default weights to restore the baseline result.
- [x] Implement the pure module with no DOM, file, network or runtime API dependency.
- [x] Run the model validator and TypeScript lint until green.
- [x] Commit the pure model locally.
...
- [x] Create the Playwright validator before UI changes. It must build the current app,
...
- [x] Run `npm run validate:v1.2-visual-workflows` and confirm expected failure because
  the R6 workbench and literal project/task filters are absent.
- [x] Add visual-only filter state and four native selects populated from the derived
  facet option sets. Keep global R2 controls unchanged.
- [x] Filter the facet events once and rebuild all three visual family models from that
  same result. Render matched count, human summary and shared signature.
- [x] Add one keyboard-operable datum/control to each of the twelve visual cards. Chart
  activation updates selected state and the inline evidence region before navigation.
- [x] Add the shared evidence region with question, action, next route, event metadata,
  bounded evidence refs and collapsed machine paths.
- [x] Convert every rendered opportunity item to a button and add inline detail with
  evidence, next step, half-life, defer/Why Not Now and no-pressure boundary.
- [x] Add bounded Formula range inputs for time saved, reuse and skill compounding,
  visible proxy score/delta and reset; perform no POST or file write.
- [x] Add responsive styles without nested decorative cards, side-stripe accents,
  fluid type or new routes. Preserve keyboard focus and reduced-motion behavior.
- [x] Run the browser validator until every required viewport passes.
- [x] Visually inspect all generated screenshots and fix any overlap, clipping,
  illegible text or incoherent hierarchy.
- [x] Commit the rendered workflow locally.
...
- [x] Run focused Python/model tests, `npm run lint`, production build and
  `npm run validate:v1.2-visual-workflows` with tracked R6 evidence output.
- [x] Re-run R1 Home multiviewport, R3 command, R4 proposal, R5 Owner Daily, Stage 7
  visual and privacy gates.
- [x] Run the existing S06/S07/S11 validators only as regression evidence; do not use
  their source markers as R6 acceptance.
- [x] Obtain an independent correctness/security/UX review. Reproduce every High/Medium
  finding with a failing regression before fixing and re-review until none remain.
- [x] Promote only `S06-AC03`, `S07-AC02`, `S07-AC04`, `S11-AC01`, `S11-AC02`,
  `S11-AC03` and `S11-AC04`. Expected aggregate: `VERIFIED 48 / PARTIAL 5 /
  FAILED 4 / NOT_VERIFIED 1`.
- [x] Record online/installed app unchanged and release status
  `FAIL_REMEDIATION_REQUIRED`.
- [x] Mark R5 and R6 plan checkboxes accurately, commit R6 records locally, remove only
