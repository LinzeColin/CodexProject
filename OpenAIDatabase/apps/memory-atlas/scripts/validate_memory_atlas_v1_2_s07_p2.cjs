...
function validateDocsAndRecords() {
  ...
  assertCondition(
    hasAll(quick, [taskId, acceptanceId, status, "当前阶段是 S07 P2", "Information ROI", "下一步只允许进入 S07 P3"]),
    "s07p2_quick_entry",
    "Quick entry records S07 P2 state and next S07 P3 gate",
    "Quick entry is missing S07 P2 state",
  );
  assertCondition(
    hasAll(overview, ["S07 P2 已完成", "Visual ROI Gate", outputPath, "下一步是 S07 P3"]),
    "s07p2_overview",
    "Overview records S07 P2 state and output",
    "Overview is missing S07 P2 state",
  );
  assertCondition(
    hasAll(machine, ["当前为 S07 P2", taskId, acceptanceId, validatorName, "下一步是 S07 P3"]),
    "s07p2_machine_readme",
    "Machine README records S07 P2 identity and next gate",
    "Machine README is missing S07 P2 state",
  );
  assertCondition(
    hasAll(formula, ["当前 S07 P2 已完成", "information_roi.v1_2_s07_p2.json", "information_roi_score", "visual_roi_gate", "下一步是 S07 P3"]),
    "s07p2_formula_readme",
    "Formula README records S07 P2 formula registry",
    "Formula README is missing S07 P2 state",
  );
  assertCondition(
    hasAll(visual, ["当前 S07 P2 已完成", "visual_roi_gate.v1_2_s07_p2.json", "没有决策价值的图表不进 P0", "下一步是 S07 P3"]),
    "s07p2_visual_readme",
    "Visualization README records S07 P2 Visual ROI Gate",
    "Visualization README is missing S07 P2 state",
  );
  assertCondition(
    hasAll(dataContract, ["当前 S07 P2 已完成", outputPath, "roi_items", "visual_roi_gate", "下一步是 S07 P3"]),
    "s07p2_data_contract",
  ...
  assertCondition(
    hasAll(runGate, ["当前阶段是 S07 P2", taskId, acceptanceId, validatorName, reviewPath, "下一步是 S07 P3"]),
    "s07p2_run_gate",
