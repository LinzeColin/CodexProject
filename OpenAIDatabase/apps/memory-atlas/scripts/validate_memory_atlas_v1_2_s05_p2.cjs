...
function validateExtractorRuntime() {
  ...
  assertCondition(
    ...
    "atlasctl analyze --stage facets --dry-run returns the S05 P2 no-write extractor contract",
...
function validateEventsPayload() {
  ...
  assertCondition(
    ...
    "events.json records S05 P2 identity, schema ref, output path and raw/no-fake boundary",
...
function validateDocsAndRecords() {
  ...
  assertCondition(
    hasAll(quick, [taskId, acceptanceId, status, "当前阶段是 S05 P2", "facet extractor", "下一步只允许进入 S05 P3"]),
    "s05p2_quick_entry",
    "Human quick entry records S05 P2 state and next S05 P3 gate",
    "Human quick entry is missing S05 P2 state",
  );
  assertCondition(
    hasAll(overview, ["S05 P2 已完成", "facet extractor", eventsPath, "下一步是 S05 P3"]),
    "s05p2_overview",
    "Human overview records S05 P2 state and next S05 P3 gate",
    "Human overview is missing S05 P2 state",
  );
  assertCondition(
    hasAll(machine, ["当前为 S05 P2", taskId, acceptanceId, validatorName, extractorPath, eventsPath, "下一步是 S05 P3"]),
    "s05p2_machine_readme",
    "Machine README records S05 P2 identity, extractor and next gate",
    "Machine README is missing S05 P2 state",
  );
  assertCondition(
    hasAll(dataContract, ["当前 S05 P2 已完成", eventsPath, "ChatGPT", "Codex", "future_agent", "下一步是 S05 P3"]),
  ...
  assertCondition(
    hasAll(behavior, ["当前 S05 P2 已完成", "facet extractor", eventsPath, "不生成 fake events", "下一步是 S05 P3"]),
  ...
  assertCondition(
    hasAll(runGate, ["当前阶段是 S05 P2", taskId, acceptanceId, validatorName, reviewPath, "下一步是 S05 P3"]),
    "s05p2_run_gate",
