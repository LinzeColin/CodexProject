...
const taskId = "MA-V117-S1-REVIEW";
const acceptanceId = "ACC-MA-V117-S1-REVIEW";
const status = "stage_1_review_passed_pending_stage2_no_github_main_upload";
const validatorName = "validate:v1.1.7-stage1";
const allowedReviewChangePaths = [
  "OpenAIDatabase/CHANGELOG.md",
  "OpenAIDatabase/apps/memory-atlas/package.json",
  "OpenAIDatabase/apps/memory-atlas/scripts/validate_memory_atlas_v1_1_7_stage1.cjs",
  "OpenAIDatabase/docs/MEMORY_ATLAS_DELIVERY_RECORD.md",
  "OpenAIDatabase/docs/MEMORY_ATLAS_PROJECT_MODEL_PARAMETERS.md",
  "OpenAIDatabase/docs/reviews/memory_atlas_v1_1_7_stage1_review.md",
...
function validatePhaseValidators() {
  const validators = [
    ["stage0_review_validator", "validate_memory_atlas_v1_1_7_stage0.cjs", "ACC-MA-V117-S0-REVIEW"],
    ["stage1_phase1_validator", "validate_memory_atlas_v1_1_7_stage1_phase1.cjs", "ACC-MA-V117-S1P01"],
    ["stage1_phase2_validator", "validate_memory_atlas_v1_1_7_stage1_phase2.cjs", "ACC-MA-V117-S1P02"],
    ["stage1_phase3_validator", "validate_memory_atlas_v1_1_7_stage1_phase3.cjs", "ACC-MA-V117-S1P03"],
    ["stage1_phase4_validator", "validate_memory_atlas_v1_1_7_stage1_phase4.cjs", "ACC-MA-V117-S1P04"],
  ];
  ...
  if (changed.length > 0) {
    ...
    assertCondition(
      outside.length === 0,
      "stage1_phase_validator_deferred_scope",
      "Phase validator execution is deferred only because the open diff is limited to Stage 1 review gate files",
    ...
    pass(
      "stage1_phase_validators_deferred_until_clean_tree",
      "Existing phase validators enforce their own changed-path scopes; run this Stage 1 validator again after commit to execute them on a clean tree",
      { changed },
    );
    return;
  }
...
function validateReviewArtifact() {
  const reviewPath = "docs/reviews/memory_atlas_v1_1_7_stage1_review.md";
  ...
  assertCondition(
    ...
    "stage1_review_artifact",
    "Stage 1 review artifact records phase coverage, validation, boundaries and next gate",
    "Stage 1 review artifact is incomplete",
  );
}
...
function validateRecords() {
  ...
  assertCondition(
    packageJson.scripts?.[validatorName] === "node scripts/validate_memory_atlas_v1_1_7_stage1.cjs",
    "stage1_package_script",
  ...
  for (const [name, source] of records) {
    assertCondition(
      ...
      `stage1_records_${name}`,
      `${name} registers Stage 1 review status, acceptance, validator and no-upload boundary`,
      `${name} is missing Stage 1 review tokens`,
    );
  }
}
...
function validateChangeScope() {
  ...
  assertCondition(
    outside.length === 0,
    "stage1_review_change_scope",
    "Current OpenAIDatabase changes are limited to Stage 1 review gate, validator, package script and records",
    "Unexpected files changed outside Stage 1 review scope",
    { changed, outside },
  );
}

function validateBoundary() {
  const review = readRepoFile("docs/reviews/memory_atlas_v1_1_7_stage1_review.md");
  assertCondition(
    review.includes("No Stage 2 work") &&
      review.includes("No proposal write") &&
      review.includes("No agent apply") &&
      review.includes("No GitHub main upload before the whole Stage 0-10 project is complete"),
    "stage1_review_boundary",
    "No Stage 2 proposal editor, raw/private read, direct writeback, proposal write, agent apply, build, deploy, app install or GitHub main upload is included",
    "Stage 1 review boundary violation detected",
  );
}
