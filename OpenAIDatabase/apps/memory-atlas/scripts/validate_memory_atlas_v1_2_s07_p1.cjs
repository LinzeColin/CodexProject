...
function validateDocsAndRecords() {
  ...
  assertCondition(
    hasAll(quick, [taskId, acceptanceId, status, "当前阶段是 S07 P1", "Personal Economic Proxy", "下一步只允许进入 S07 P2"]),
    "s07p1_quick_entry",
    "Quick entry records S07 P1 state and next S07 P2 gate",
    "Quick entry is missing S07 P1 state",
  );
  assertCondition(
    hasAll(overview, ["S07 P1 已完成", "Personal Economic Proxy", outputPath, "下一步是 S07 P2"]),
    "s07p1_overview",
    "Overview records S07 P1 state and economic proxy output",
    "Overview is missing S07 P1 state",
  );
  assertCondition(
    hasAll(machine, ["当前为 S07 P1", taskId, acceptanceId, validatorName, "下一步是 S07 P2"]),
    "s07p1_machine_readme",
    "Machine README records S07 P1 identity and next gate",
    "Machine README is missing S07 P1 state",
  );
  assertCondition(
    hasAll(formula, ["当前 S07 P1 已完成", "Personal Economic Proxy", formulaConfigPath, "time_saved_proxy", "automation_enhancement_ratio_proxy", "下一步是 S07 P2"]),
    "s07p1_formula_readme",
    "Formula README records S07 P1 formula registry and next gate",
    "Formula README is missing S07 P1 state",
  );
  assertCondition(
    hasAll(dataContract, ["当前 S07 P1 已完成", outputPath, "score_cards", "external_economic_database", "下一步是 S07 P2"]),
    "s07p1_data_contract",
    "Data contract README records S07 P1 economic proxy output",
    "Data contract README is missing S07 P1 state",
  );
  assertCondition(
    hasAll(behavior, ["当前 S07 P1 已完成", "S06", "Personal Economic Proxy", "下一步是 S07 P2"]),
  ...
  assertCondition(
    hasAll(runGate, ["当前阶段是 S07 P1", taskId, acceptanceId, validatorName, reviewPath, "下一步是 S07 P2"]),
    "s07p1_run_gate",
