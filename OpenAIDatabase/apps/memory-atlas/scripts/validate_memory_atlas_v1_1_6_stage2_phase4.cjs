...
function validateProductContract() {
  ...
  assertCondition(
    ...
    "stage2_phase4_product_contract",
    "Topic classification lane contract covers information hierarchy, fields, grouping, sorting, badges, expansion, comparison, Inspector handoff, safety and future-phase boundaries",
    "Topic classification lane visibility contract is incomplete",
  );
}
...
function validateAcceptanceContract() {
  ...
  assertCondition(
    ...
    "stage2_phase4_acceptance_contract",
    "Topic classification lane acceptance covers hierarchy, fields, grouping, sorting, badges, interactions, Inspector handoff, states, screenshots, safety failures, deliverables and rollback",
    "Topic classification lane visibility acceptance is incomplete",
  );
}
...
function validateRecords() {
  ...
  assertCondition(
    ...
    "delivery_record_stage2_phase4",
    "Delivery record captures Stage 2 Phase 4 scope, artifacts, status, boundaries, and next step",
    "Delivery record is missing Stage 2 Phase 4 state",
  );

  assertCondition(
    hasAll(model, [
      "v1.1.6 Stage 2 Phase 4 topic classification lane 可见性门槛",
      "PARAM-MA-V116-S2P04-001",
      "PARAM-MA-V116-S2P04-010",
      "topic_classification_lane_visibility_contract.md",
    ]),
    "model_parameters_stage2_phase4",
    "Model parameters record the topic classification lane visibility thresholds and evidence references",
    "Project model parameters are missing Stage 2 Phase 4 gates",
  );

  assertCondition(
    hasAll(features, [
      "FEAT-MA-V116-S2P04",
      "EVID-MA-V116-S2P04-TOPIC-LANE-CONTRACT",
      "EVID-MA-V116-S2P04-TOPIC-LANE-ACCEPTANCE",
      "topic classification lane 可见性",
    ]),
    "feature_list_stage2_phase4",
    "Feature list records Stage 2 Phase 4 feature and evidence",
    "Feature list is missing Stage 2 Phase 4 entries",
  );

  assertCondition(
    hasAll(dev, [
      "MA-V116-S2P04",
      "Topic Classification Lane Visibility Contract",
      "ACC-MA-V116-S2P04",
      "phase_2_4_contract_created_pending_stage_review",
      "Stage 2 整体复审未执行",
    ]),
    "development_record_stage2_phase4",
    "Development record captures Stage 2 Phase 4 task, acceptance, status, and stop boundary",
    "Development record is missing Stage 2 Phase 4",
  );
  ...
  assertCondition(
    ...
    "model_index_stage2_phase4",
    "Root model parameter file records Stage 2 Phase 4 model and parameters",
    "Root model parameter file is missing Stage 2 Phase 4",
  );

  assertCondition(
    hasAll(changelog, [
      "Memory Atlas v1.1.6 Stage 2 Phase 4",
      "topic_classification_lane_visibility_contract.md",
      "validate:v1.1.6-stage2-phase4",
    ...
    "changelog_stage2_phase4",
    "Changelog records Stage 2 Phase 4 artifacts, validator and non-goal boundaries",
    "Changelog is missing Stage 2 Phase 4 entry",
  );

  assertCondition(
    packageSource.includes('"validate:v1.1.6-stage2-phase4": "node scripts/validate_memory_atlas_v1_1_6_stage2_phase4.cjs"'),
    "package_script_stage2_phase4",
    "Package script exposes validate:v1.1.6-stage2-phase4",
    "Package script validate:v1.1.6-stage2-phase4 is missing",
  );
}
...
function validateChangeScope() {
  ...
  assertCondition(
    outside.length === 0,
    "stage2_phase4_change_scope",
    "Current OpenAIDatabase changes are limited to Stage 1 and Stage 2 Phase 1-4 contracts, acceptance, records, reviews, validators, and package script",
...
try {
  ...
  validateRecords();
  validateChangeScope();
  pass("stage2_phase4_boundary", "No runtime UI/CSS/raw-private/direct-writeback/search-review-data-map/proposal-editor/agent-apply/upload work is required or present in this phase");
  console.log(JSON.stringify({ status: "PASS", stage: "v1.1.6-stage2-phase4", checks }, null, 2));
} catch (error) {
  console.error(JSON.stringify({
    status: "FAIL",
    stage: "v1.1.6-stage2-phase4",
    error: error.message,
