...
function validateDocsAndRecords() {
  ...
  assertCondition(
    ...
    "Human quick entry records S04 Review state and next S05 P1 gate",
    "Human quick entry is missing S04 Review state",
  );
  assertCondition(
    hasAll(overview, ["S04 Review 已通过", "S04 整体复审已通过", "ChatGPT 只读同步", "Codex local sync", "GitHub backup dry-run/apply", "下一步是 S05 P1"]),
    "s04_review_overview",
    "Human overview records S04 Review result and next S05 P1 gate",
    "Human overview is missing S04 Review state",
  );
  assertCondition(
    ...
    "Machine README records S04 Review identity, validator and next gate",
    "Machine README is missing S04 Review state",
  );
  assertCondition(
  ...
  assertCondition(
    ...
    "s04_review_run_gate",
    "Run gate README records S04 Review validator and next gate",
    "Run gate README is missing S04 Review state",
  );
  assertCondition(
    hasAll(review, [
      taskId,
