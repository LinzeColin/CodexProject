...
const taskId = "MA-V117-S5-REVIEW";
const acceptanceId = "ACC-MA-V117-S5-REVIEW";
const status = "stage_5_review_passed_pending_stage6_no_github_main_upload";
const validatorName = "validate:v1.1.7-stage5";
const reviewPath = "docs/reviews/memory_atlas_v1_1_7_stage5_review.md";
const branchName = "codex/memory-atlas-v117-stage0-10-local";

const phaseValidators = [
  ["stage4_review_validator", "validate_memory_atlas_v1_1_7_stage4.cjs", "validate:v1.1.7-stage4", "ACC-MA-V117-S4-REVIEW"],
  [
    "stage5_phase1_validator",
    "validate_memory_atlas_v1_1_7_stage5_phase1.cjs",
    "validate:v1.1.7-stage5-phase1",
    "ACC-MA-V117-S5P01",
  ],
  [
    "stage5_phase2_validator",
    "validate_memory_atlas_v1_1_7_stage5_phase2.cjs",
    "validate:v1.1.7-stage5-phase2",
    "ACC-MA-V117-S5P02",
  ],
  [
    "stage5_phase3_validator",
    "validate_memory_atlas_v1_1_7_stage5_phase3.cjs",
...
const allowedReviewChangePaths = [
  ...
  "OpenAIDatabase/apps/memory-atlas/scripts/validate_memory_atlas_v1_1_7_stage5.cjs",
...
function validatePhaseValidators() {
  ...
  if (changed.length > 0) {
    ...
    assertCondition(
      outside.length === 0,
      "stage5_review_phase_validator_deferred_scope",
      "Phase validator execution is deferred only because the open diff is limited to Stage 5 review gate files",
    ...
    pass(
      "stage5_phase_validators_deferred_until_clean_tree",
      "Existing phase validators enforce their own changed-path scopes; run this Stage 5 validator again after commit to execute them on a clean tree",
      { changed },
    );
    return;
  }
...
function validateReviewArtifact() {
  ...
  assertCondition(
    ...
    "Stage 5 review artifact records phase coverage, browser evidence, validation, boundaries and next gate",
...
function validateRecords() {
  ...
  assertCondition(
    packageJson.scripts?.[validatorName] === "node scripts/validate_memory_atlas_v1_1_7_stage5.cjs",
    "stage5_package_script",
  ...
  for (const [name, source] of records) {
    assertCondition(
      ...
      `stage5_records_${name}`,
      `${name} registers Stage 5 review status, acceptance, validator and no-upload boundary`,
      `${name} is missing Stage 5 review tokens`,
    );
  }
}
...
function validateChangeScope() {
  ...
  assertCondition(
    ...
    "Current OpenAIDatabase changes are limited to Stage 5 review gate, validator, package script and records",
    "Unexpected files changed outside Stage 5 review scope",
    { changed, outside },
  );
}
...
function validateBoundary() {
  const review = readRepoFile(reviewPath);
  assertCondition(
    review.includes("No Stage 6 work") &&
      review.includes("No Data Map work") &&
    ...
    "No Stage 6 work, Data Map work, raw/private read, direct writeback, agent apply, deploy or GitHub main upload is included",
    "Stage 5 review boundary violation detected",
  );
}
