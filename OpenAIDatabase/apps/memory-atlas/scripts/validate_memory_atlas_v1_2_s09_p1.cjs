...
function validateCurrentStateDocs() {
  ...
  assertCondition(
    hasAll(quick, ["当前阶段是 S09 P1", taskId, acceptanceId, "下一步只允许进入 S09 P2"]) &&
      hasAll(overview, ["S09 P1 已完成", "latent_signals.json", "下一步是 S09 P2"]) &&
      hasAll(machine, ["当前为 S09 P1", taskId, validatorName, "下一步是 S09 P2"]) &&
      ...
      hasAll(runGate, ["当前阶段是 S09 P1", taskId, acceptanceId, validatorName]),
    "s09p1_current_state_docs",
    "Current human and machine docs now point to S09 P1 and pending S09 P2",
    "Current state docs do not point to S09 P1",
  );
}
