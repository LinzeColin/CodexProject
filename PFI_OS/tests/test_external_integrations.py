...
def test_holdings_are_loaded_from_consumer_and_pfi_os_sources():
    with TemporaryDirectory() as tmp:
        consumer = Path(tmp) / "consumer"
        pfi_os = Path(tmp) / "pfi_os"
        consumer.mkdir()
        pfi_os.mkdir()
        pd.DataFrame(
            [
        ...
        ).to_excel(pfi_os / "holdings.xlsx", index=False)

        holdings = load_holdings_frame(consumer_dirs=[consumer], pfi_os_dirs=[pfi_os])
...
def test_external_system_status_reports_ready_and_needs_config():
    with TemporaryDirectory() as tmp:
        ...
        status = external_system_status(industry_report_root=industry, consumer_dirs=[consumer], pfi_os_dirs=[pfi_os])
