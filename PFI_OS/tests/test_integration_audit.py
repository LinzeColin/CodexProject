...
from pfi_os.integrations.research_bus_api import submit_chat_input
from pfi_os.system.integration_audit import build_pfi_os_integration_audit, write_pfi_os_integration_audit


def test_pfi_os_integration_audit_passes_when_all_layers_have_evidence(tmp_path: Path) -> None:
    ...
    assert payload["schema"] == "PFIOSIntegrationAuditV1"
...
def test_pfi_os_integration_audit_reviews_missing_artifacts(tmp_path: Path) -> None:
    root = _integration_root(tmp_path)
    report_root = tmp_path / "reports"
    report_root.mkdir()
    db_path = tmp_path / "ResearchBus.sqlite"
...
def test_write_pfi_os_integration_audit_outputs_json_and_markdown(tmp_path: Path) -> None:
    root = _integration_root(tmp_path)
    report_root = tmp_path / "reports"
    report_root.mkdir()
    db_path = tmp_path / "ResearchBus.sqlite"
    ...
    assert json_path.name == "PFIOSIntegrationAudit_06062026.json"
    assert saved["schema"] == "PFIOSIntegrationAuditV1"
    assert markdown_path.read_text(encoding="utf-8").startswith("# PFIOS Integration Audit")
...
def _write_report_metadata(report_root: Path, *, with_evidence: bool) -> None:
    ...
    if with_evidence:
        payload["report_evidence"] = {"schema": "PFIOSReportEvidenceV1", "evidence_status": "ContinueResearch"}
