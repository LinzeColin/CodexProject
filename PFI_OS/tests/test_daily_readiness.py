...
def test_daily_readiness_reports_ready_when_core_gates_pass(tmp_path: Path) -> None:
    ...
    assert payload["schema"] == "PFIOSDailyReadinessV1"
...
def test_write_daily_readiness_outputs_json_markdown_and_pdf(tmp_path: Path) -> None:
    ...
    assert json_path.name == "PFIOSDailyReadiness_07062026.json"
    assert saved["schema"] == "PFIOSDailyReadinessV1"
    assert markdown_path.read_text(encoding="utf-8").startswith("# PFIOS Daily Readiness")
