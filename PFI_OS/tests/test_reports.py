...
from pfi_os.reports import export_backtest_docx, report_filename, unique_report_path
from pfi_os.reports.export import (
...
class ReportTests(unittest.TestCase):
    ...
    def test_report_evidence_summary_downgrades_when_critical_evidence_is_missing(self):
        ...
        self.assertEqual(evidence["schema"], "PFIOSReportEvidenceV1")
    ...
    def test_report_bootstrap_simulation_count_has_10000_floor(self):
        ...
        original = os.environ.pop("PFI_TEST_BOOTSTRAP_SIMULATIONS", None)
        try:
            self.assertEqual(_report_bootstrap_simulations(), 10000)
            os.environ["PFI_TEST_BOOTSTRAP_SIMULATIONS"] = "300"
            self.assertEqual(_report_bootstrap_simulations(), 10000)
        finally:
            if original is not None:
                os.environ["PFI_TEST_BOOTSTRAP_SIMULATIONS"] = original
            else:
                os.environ.pop("PFI_TEST_BOOTSTRAP_SIMULATIONS", None)
