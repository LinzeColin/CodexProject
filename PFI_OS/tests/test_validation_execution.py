...
from pfi_os.research import build_validation_task_execution, write_validation_task_execution
...
def test_validation_task_execution_runs_cross_source_and_writes_outputs(tmp_path: Path) -> None:
    ...
    with (
        patch("pfi_os.research.validation_execution._default_providers_for_market", return_value=["Yahoo Finance", "Polygon"]),
        patch("pfi_os.research.validation_execution.validate_close_across_sources", return_value=result),
    ):
    ...
    assert payload["schema"] == "PFIOSValidationTaskExecutionV1"
...
def test_validation_task_execution_blocks_when_provider_coverage_is_insufficient(tmp_path: Path) -> None:
    ...
    with patch("pfi_os.research.validation_execution._default_providers_for_market", return_value=["Yahoo Finance"]):
