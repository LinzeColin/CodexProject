...
from pfi_os.integrations.research_bus import (
    ...
    push_bus_validation_tasks_to_pfi_os_queue,
...
from pfi_os.research.validation_queue import load_validation_tasks
...
class ResearchBusTest(unittest.TestCase):
    def test_industry_report_sync_generates_validation_tasks_and_queue_items(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            pushed = push_bus_validation_tasks_to_pfi_os_queue(db_path, queue_path)
    ...
    def test_pfi_os_results_are_written_to_bus_and_ai_outbox(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            ...
            result = sync_pfi_os_results_to_bus(report_root, db_path, ai_research_root=ai_root)
            frame = bus_pfi_os_results_frame(db_path)
            outbox = ai_root / "data" / "report_artifacts" / "pfi_os_bridge" / "PFIOSResults.json"

            self.assertEqual(result.pfi_os_results, 1)
    ...
    def test_holding_symbol_mappings_sync_writes_shared_proxy_table_and_snapshot(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(state["entity_registry_schema"], "PFIOSEntityRegistryV1")
    ...
    def test_pdf_parser_warns_when_text_and_ocr_are_unavailable(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch.dict(sys.modules, {"pypdf": fake_module}), patch(
                "pfi_os.integrations.research_bus._ocr_runtime_available",
                return_value=False,
            ):
                parsed = parse_report_file(pdf_path)
