...
from pfi_os.integrations.research_bus_api import process_pending_bus_requests, submit_bus_request, submit_chat_input
...
class SystemOrchestratorTest(unittest.TestCase):
    ...
    def test_default_child_roots_can_be_configured_by_environment(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            with patch.dict(os.environ, {"PFI_AI_RESEARCH_ROOT": str(root / "ai"), "PFI_GOVERNMENT_POLICY_ROOT": str(root / "policy")}):
    ...
    def test_execute_orchestration_is_denied_without_explicit_gate(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            db_path = Path(tmp) / "ResearchBus.sqlite"
            with patch.dict(os.environ, {"PFI_ORCHESTRATOR_EXECUTE_ALLOWED": ""}, clear=False):
