...
def test_moomoo_check_script_is_quote_diagnostic_entrypoint():
    ...
    assert "pfi_os.examples.validate_moomoo" in script


def test_verify_script_does_not_lower_bootstrap_simulation_count():
    script = (PROJECT_ROOT / "scripts" / "verifyPFIOS.sh").read_text(encoding="utf-8")

    assert "PFI_TEST_BOOTSTRAP_SIMULATIONS=300" not in script
    assert "scripts/finalAcceptanceCheck.sh" in script
    assert "-p no:cacheprovider" in script
    assert "src/pfi_os/storage.py" in script
    assert "src/pfi_os/approvals/registry.py" in script
    assert "src/pfi_os/research/reviews.py" in script
    assert "src/pfi_os/research/validation_queue.py" in script
    assert "src/pfi_os/research/report_gap_tasks.py" in script
    assert "src/pfi_os/reports/decision_support.py" in script
    assert "src/pfi_os/examples/report_gap_tasks.py" in script
    assert "src/pfi_os/examples/validation_priority_plan.py" in script
    assert "src/pfi_os/examples/validation_task_execution.py" in script
    ...
    assert "src/pfi_os/research/validation_priority.py" in script
    assert "src/pfi_os/research/validation_execution.py" in script
    assert "src/pfi_os/strategies/custom_builder.py" in script
    assert "src/pfi_os/strategies/profiles.py" in script
    assert "PYTHONPYCACHEPREFIX" in script
    assert "PFI_REPORT_DIR" in script
...
def test_heavy_smoke_scripts_require_explicit_confirmation():
    ...
    for script in (final_acceptance, ci_smoke):
        assert "PFI_OS_ALLOW_HEAVY_SMOKE" in script
        assert "scripts/devReadyCheck.sh --summary-json" in script
        assert "exit 64" in script

    assert 'PFI_OS_ALLOW_HEAVY_SMOKE: "1"' in workflow
...
def test_sensitive_holdings_paths_are_gitignored():
    ...
    assert "data/systemAudit/PFI007ResearchPolicyAcceptance*.json" in patterns
    ...
    assert "data/systemAudit/PFI012MVPReleaseGate*.json" in patterns
...
def test_quiet_start_uses_stable_local_streamlit_settings():
    script = (PROJECT_ROOT / "scripts" / "startPFIOS.sh").read_text(encoding="utf-8")
    runtime = (PROJECT_ROOT / "scripts" / "pfiRuntime.sh").read_text(encoding="utf-8")
    stop_script = (PROJECT_ROOT / "scripts" / "stopPFIOS.sh").read_text(encoding="utf-8")
...
def test_report_package_exports_are_lazy_for_light_ui_startup():
    reports_init = (PROJECT_ROOT / "src" / "pfi_os" / "reports" / "__init__.py").read_text(encoding="utf-8")
    streamlit_app = (PROJECT_ROOT / "src" / "pfi_os" / "app" / "streamlit_app.py").read_text(encoding="utf-8")

    assert "__getattr__" in reports_init
    assert "_EXPORTS" in reports_init
    assert "from pfi_os.reports.export import" not in reports_init
    assert "from pfi_os.reports import export_backtest_docx" not in streamlit_app.split("from pfi_os.reports.catalog", maxsplit=1)[0]
...
def test_streamlit_lifecycle_panel_uses_allowlisted_local_scripts_only():
    ...
    assert "PFIOSHotspotQuickPreflightV1" in streamlit_app
    ...
    assert "PFIOSParameterScanPreflightV1" in streamlit_app
    assert "LIFECYCLE_SCRIPT_ALLOWLIST" in streamlit_app
    assert '"scripts/statusPFIOS.sh": 8' in streamlit_app
    assert '"scripts/stopPFIOS.sh": 20' in streamlit_app
...
def test_final_acceptance_check_covers_product_artifacts_without_opening_browser():
    ...
    assert path.exists()
    assert "scripts/verifyPFIOS.sh" in script
    ...
    assert "PFIOSVectorizedResearchBatchV1" in script
    assert "PFIOSHotspotRuntimeSummaryV1" in script
    assert "PFIOSHotspotCacheStatusV1" in script
    assert "PFIOSHotspotRequestTraceV1" in script
    ...
    assert "Lifecycle controls only manage the local PFI_OS/PFIOS app process" in script
    ...
    assert 'APP_BUNDLE_NAME="PFI_OS"' in script
    ...
    assert "PFI_OS" in script
    assert "PFIOSAppIcon.icns" in script
    ...
    assert "PFIOSMacOSAppAcceptanceLiteV1" in script
    assert "PFIOSMacOSLifecycleReadinessV1" in script
    assert "DevReadyScriptExecutable" in script
    assert "PFIOSMacOSRuntimeAcceptanceV1" in script
    assert "MacOSAcceptancePublicSummary_latest.json" in script
    assert "MacOSPublicAcceptanceSummary.md" in script
    assert "Contents/MacOS/PFI_OS" in script
    assert "Contents/MacOS/applet" not in script
    assert "check_absent" in script
    assert "check_text_absent" in script
    assert "PFI_OS_PROJECT_ROOT" in script
    assert "PFI_OS_APP_LAUNCH_DRY_RUN" in script
    ...
    assert "PFIOSRuntimeSummaryRefreshV1" in script
    ...
    assert "PFIOSReportValidationHubV1" in script
...
def test_report_validation_hub_is_default_low_token_entrypoint():
    ...
    module = (PROJECT_ROOT / "src" / "pfi_os" / "system" / "report_validation_hub.py").read_text(encoding="utf-8")
    cli = (PROJECT_ROOT / "src" / "pfi_os" / "examples" / "report_validation_hub.py").read_text(encoding="utf-8")
    streamlit_app = (PROJECT_ROOT / "src" / "pfi_os" / "app" / "streamlit_app.py").read_text(encoding="utf-8")
    docs = (PROJECT_ROOT / "docs" / "ReportValidationHub.md").read_text(encoding="utf-8")

    assert "pfi_os.examples.report_validation_hub" in script
    assert "set -- --mode daily --summary-json" in script
    assert "PFIOSReportValidationHubV1" in module
...
def test_dev_ready_check_is_default_light_gate_without_heavy_release_steps():
    ...
    module = (PROJECT_ROOT / "src" / "pfi_os" / "system" / "dev_readiness.py").read_text(encoding="utf-8")
    cli = (PROJECT_ROOT / "src" / "pfi_os" / "examples" / "dev_ready_check.py").read_text(encoding="utf-8")

    assert "pfi_os.examples.dev_ready_check" in script
    assert "PFIOSDevReadyCheckV1" in module
...
def test_command_center_prefers_compact_runtime_summary_sources():
    command_center = (PROJECT_ROOT / "src" / "pfi_os" / "executive" / "command_center.py").read_text(encoding="utf-8")
...
def test_cashflow_command_script_is_business_layer_entrypoint():
    ...
    example = (PROJECT_ROOT / "src" / "pfi_os" / "examples" / "cashflow_command.py").read_text(encoding="utf-8")
    reviewed_example = (PROJECT_ROOT / "src" / "pfi_os" / "examples" / "cashflow_reviewed_input_refresh.py").read_text(encoding="utf-8")
    cashflow = (PROJECT_ROOT / "src" / "pfi_os" / "business" / "cashflow.py").read_text(encoding="utf-8")
    reviewed = (PROJECT_ROOT / "src" / "pfi_os" / "business" / "cashflow_reviewed_input.py").read_text(encoding="utf-8")
    streamlit_app = (PROJECT_ROOT / "src" / "pfi_os" / "app" / "streamlit_app.py").read_text(encoding="utf-8")
    schema = (PROJECT_ROOT / "shared" / "schema" / "company_cashflow_reviewed_input.schema.json").read_text(encoding="utf-8")

    assert "pfi_os.examples.cashflow_command" in script
    assert "pfi_os.examples.cashflow_reviewed_input_refresh" in reviewed_script
    assert "PYTHONPYCACHEPREFIX" in script
    assert "PFI_PYTHON" in script
    assert "command -v python3" in script
    assert "summary-json" in example
    assert "PFIOSCompanyCashFlowReviewedInputRefreshV1" in reviewed_example
    ...
    assert "PFIOSCompanyCashFlowRuntimeSummaryV1" in cashflow
...
def test_policy_radar_script_is_policy_layer_entrypoint():
    ...
    example = (PROJECT_ROOT / "src" / "pfi_os" / "examples" / "policy_radar.py").read_text(encoding="utf-8")
    reviewed_example = (PROJECT_ROOT / "src" / "pfi_os" / "examples" / "policy_reviewed_input_refresh.py").read_text(
        encoding="utf-8"
    )
    policy = (PROJECT_ROOT / "src" / "pfi_os" / "policy" / "radar.py").read_text(encoding="utf-8")
    reviewed = (PROJECT_ROOT / "src" / "pfi_os" / "policy" / "reviewed_input.py").read_text(encoding="utf-8")
    streamlit_app = (PROJECT_ROOT / "src" / "pfi_os" / "app" / "streamlit_app.py").read_text(encoding="utf-8")
    schema = (PROJECT_ROOT / "shared" / "schema" / "policy_reviewed_input.schema.json").read_text(encoding="utf-8")

    assert "pfi_os.examples.policy_radar" in script
    assert "pfi_os.examples.policy_reviewed_input_refresh" in reviewed_script
    assert "PYTHONPYCACHEPREFIX" in script
    assert "PFI_PYTHON" in script
    assert "command -v python3" in script
    assert "summary-json" in example
    assert "PFIOSPolicyReviewedInputRefreshV1" in reviewed_example
    assert "data/private/policy/PolicyReviewedInput.json" in reviewed
    assert "PolicyReviewedInput.example.json" in reviewed
    assert "Local reviewed JSON input only" in reviewed
    assert "PFI_OS Policy Reviewed Input" in schema
    assert "PFIOSPolicyIntelligenceRuntimeSummaryV1" in policy
...
def test_consumption_guard_script_is_consumption_layer_entrypoint():
    ...
    example = (PROJECT_ROOT / "src" / "pfi_os" / "examples" / "consumption_guard.py").read_text(encoding="utf-8")
    reviewed_example = (PROJECT_ROOT / "src" / "pfi_os" / "examples" / "consumption_reviewed_input_refresh.py").read_text(
        encoding="utf-8"
    )
    consumption = (PROJECT_ROOT / "src" / "pfi_os" / "consumption" / "guard.py").read_text(encoding="utf-8")
    reviewed = (PROJECT_ROOT / "src" / "pfi_os" / "consumption" / "reviewed_input.py").read_text(encoding="utf-8")
    streamlit_app = (PROJECT_ROOT / "src" / "pfi_os" / "app" / "streamlit_app.py").read_text(encoding="utf-8")
    schema = (PROJECT_ROOT / "shared" / "schema" / "consumption_guard_reviewed_input.schema.json").read_text(encoding="utf-8")

    assert "pfi_os.examples.consumption_guard" in script
    assert "pfi_os.examples.consumption_reviewed_input_refresh" in reviewed_script
    assert "PYTHONPYCACHEPREFIX" in script
    assert "PFI_PYTHON" in script
    assert "command -v python3" in script
    assert "summary-json" in example
    assert "PFIOSConsumptionGuardReviewedInputRefreshV1" in reviewed_example
    ...
    assert "Local reviewed JSON input only" in reviewed
    assert "PFI_OS Consumption Guard Reviewed Input" in schema
    assert "PFIOSConsumptionGuardRuntimeSummaryV1" in consumption
...
def test_hotspot_runtime_summary_script_is_sample_only_smoke_entrypoint():
    ...
    verify = (PROJECT_ROOT / "scripts" / "verifyPFIOS.sh").read_text(encoding="utf-8")
    example = (PROJECT_ROOT / "src" / "pfi_os" / "examples" / "hotspot_runtime_summary.py").read_text(encoding="utf-8")

    assert "pfi_os.examples.hotspot_runtime_summary" in script
    ...
    assert "src/pfi_os/examples/hotspot_runtime_summary.py" in verify
...
def test_site52etf_snapshot_script_is_read_only_reference_entrypoint():
    ...
    example = (PROJECT_ROOT / "src" / "pfi_os" / "examples" / "site52etf_snapshot.py").read_text(encoding="utf-8")
    integration = (PROJECT_ROOT / "src" / "pfi_os" / "integrations" / "site52etf.py").read_text(encoding="utf-8")
    streamlit_app = (PROJECT_ROOT / "src" / "pfi_os" / "app" / "streamlit_app.py").read_text(encoding="utf-8")
    gitignore = (PROJECT_ROOT / ".gitignore").read_text(encoding="utf-8")

    assert "pfi_os.examples.site52etf_snapshot" in script
    assert "PYTHONPYCACHEPREFIX" in script
    assert "PFIOS52ETFPublicSnapshotV1" in integration
...
def test_macos_app_installer_builds_standard_app_bundle():
    launcher = (PROJECT_ROOT / "macos" / "PFI_OS_launcher.c").read_text(encoding="utf-8")
    wrapper = (PROJECT_ROOT / "scripts" / "installMacAppLaunchers.sh").read_text(encoding="utf-8")
    installer = (PROJECT_ROOT / "scripts" / "installPFIOSEntryApps.sh").read_text(encoding="utf-8")

    assert "PFI_OS_PROJECT_ROOT" in launcher
    assert "PFI_OS_APP_LAUNCH_DRY_RUN" in launcher
    assert "running_url" in launcher
    assert "./StartPFIOS.command" in launcher
    ...
    assert "PFI_OS local project was not found" in launcher
    assert "github.com/LinzeColin/PFI_OS" not in launcher
    assert "tell application \"Terminal\"" not in launcher
    assert "installPFIOSEntryApps.sh" in wrapper
    assert 'LAUNCHER_SOURCE="$ROOT_DIR/macos/PFI_OS_launcher.c"' in installer
    assert 'clang -O2 -Wall -Wextra' in installer
    assert 'SOURCE_APP="$ROOT_DIR/macos/PFI_OS.app"' in installer
    assert 'DESKTOP_APP="$HOME/Desktop/PFI_OS.app"' in installer
    assert 'DOWNLOADS_APP="$HOME/Downloads/PFI_OS.app"' in installer
    assert 'APPLICATIONS_APP="/Applications/PFI_OS.app"' in installer
    assert 'mktemp -d "${TMPDIR:-/tmp}/pfi_os_app.XXXXXX"' in installer
    ...
    assert 'codesign --force --deep --sign - "$staging/PFI_OS.app"' in installer
    assert "PFI_OS_ENTRY_APPS: installed" in installer
...
def test_macos_app_acceptance_lite_script_is_read_only_entrypoint():
    ...
    example = (PROJECT_ROOT / "src" / "pfi_os" / "examples" / "macos_app_acceptance_lite.py").read_text(
        encoding="utf-8"
    )
    module = (PROJECT_ROOT / "src" / "pfi_os" / "system" / "macos_acceptance.py").read_text(encoding="utf-8")
    dashboard = (PROJECT_ROOT / "src" / "pfi_os" / "app" / "dashboard.py").read_text(encoding="utf-8")

    assert "pfi_os.examples.macos_app_acceptance_lite" in script
    assert "PYTHONPYCACHEPREFIX" in script
    assert "PFIOSMacOSAppAcceptanceLiteV1" in module
    ...
    assert "PFI_OS_APP_LAUNCH_DRY_RUN" in module
    assert "github.com/LinzeColin/PFI_OS" in module
    assert "summary-json" in example
...
def test_macos_lifecycle_readiness_script_is_read_only_entrypoint():
    ...
    example = (PROJECT_ROOT / "src" / "pfi_os" / "examples" / "macos_lifecycle_readiness.py").read_text(
        encoding="utf-8"
    )
    module = (PROJECT_ROOT / "src" / "pfi_os" / "system" / "macos_lifecycle.py").read_text(encoding="utf-8")
    dashboard = (PROJECT_ROOT / "src" / "pfi_os" / "app" / "dashboard.py").read_text(encoding="utf-8")

    assert "pfi_os.examples.macos_lifecycle_readiness" in script
    assert "PYTHONPYCACHEPREFIX" in script
    assert "PFIOSMacOSLifecycleReadinessV1" in module
    assert "build_macos_lifecycle_readiness" in module
    assert "write_macos_lifecycle_readiness" in module
    assert "finalAcceptanceCheck.sh" in module
    assert "ciSmoke.sh" in module
    assert "pfi_os.system.shutdown_monitor" in module
...
def test_macos_runtime_acceptance_script_is_controlled_terminal_entrypoint():
    ...
    example = (PROJECT_ROOT / "src" / "pfi_os" / "examples" / "macos_runtime_acceptance.py").read_text(
        encoding="utf-8"
    )
    module = (PROJECT_ROOT / "src" / "pfi_os" / "system" / "macos_runtime_acceptance.py").read_text(
        encoding="utf-8"
    )
    dashboard = (PROJECT_ROOT / "src" / "pfi_os" / "app" / "dashboard.py").read_text(encoding="utf-8")
    streamlit_app = (PROJECT_ROOT / "src" / "pfi_os" / "app" / "streamlit_app.py").read_text(encoding="utf-8")

    assert "pfi_os.examples.macos_runtime_acceptance" in script
    assert "PYTHONPYCACHEPREFIX" in script
    assert "PFIOSMacOSRuntimeAcceptanceV1" in module
    assert "run_macos_runtime_acceptance" in module
    assert "write_macos_runtime_acceptance" in module
    assert "scripts/startPFIOS.sh" in module
    assert "scripts/stopPFIOS.sh" in module
    ...
    assert "scripts/macosRuntimeAcceptance.sh --launch-method app --app-path ~/Downloads/PFI_OS.app --summary-json" in dashboard
...
def test_macos_public_acceptance_summary_is_sanitized_and_lightweight():
    ...
    assert "PFIOSMacOSPublicAcceptanceSummaryV1" in module
...
def test_macos_acceptance_hub_is_user_friendly_default_entry():
    ...
    module = (PROJECT_ROOT / "src" / "pfi_os" / "system" / "macos_acceptance_hub.py").read_text(encoding="utf-8")
    cli = (PROJECT_ROOT / "src" / "pfi_os" / "examples" / "macos_acceptance_hub.py").read_text(encoding="utf-8")
    docs = (PROJECT_ROOT / "docs" / "MacOSAcceptanceHub.md").read_text(encoding="utf-8")

    assert "PFIOSMacOSAcceptanceHubV1" in module
...
def test_clean_cache_refuses_running_app_and_preserves_install_metadata():
    ...
    assert "pfi_os_is_running" in script
...
def test_research_bus_background_scripts_are_headless_and_dropbox_aware():
    ...
    assert "Application Support/PFIOS" in installer
...
def test_research_bus_webhook_script_is_local_only_entrypoint():
    ...
    server = (PROJECT_ROOT / "src" / "pfi_os" / "examples" / "research_bus_webhook.py").read_text(encoding="utf-8")

    assert "pfi_os.examples.research_bus_webhook" in script
