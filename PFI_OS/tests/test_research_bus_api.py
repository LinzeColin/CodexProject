...
from pfi_os.integrations.research_bus import bus_validation_task_frame, holding_update_candidates_frame


AI_RESEARCH_ROOT = Path(os.environ.get("PFI_AI_RESEARCH_ROOT", "")).expanduser() if os.environ.get("PFI_AI_RESEARCH_ROOT") else Path.home() / "Documents" / "PFI_OS_Workspace" / "systems" / "industry_research"
...
def _load_ai_research_bridge():
    ...
    spec = importlib.util.spec_from_file_location("ai_research_bus_bridge_for_pfi_os_test", bridge_path)
...
class ResearchBusApiTest(unittest.TestCase):
    ...
    def test_image_attachment_without_ocr_runtime_stays_review_candidate(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch(
                "pfi_os.integrations.research_bus_api._image_ocr_runtime_status",
    ...
    def test_image_ocr_json_text_auto_parses_and_confirms_candidate(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            ), patch("pfi_os.integrations.research_bus_api._ocr_image_text", return_value=ocr_json):
                submit_chat_input(
                    "这是今天的支付宝持仓截图",
                    source_system="AI-Research-Chat",
                    attachments=[{"path": image_path, "media_type": "image/png"}],
