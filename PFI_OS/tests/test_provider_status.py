...
from pfi_os.data import market_symbol_examples, provider_status_rows
from pfi_os.data.provider_status import _moomoo_status
...
class ProviderStatusTests(unittest.TestCase):
    ...
    def test_token_status_reflects_environment(self):
        ...
        old_env_file = os.environ.get("PFI_ENV_FILE")
        os.environ["PFI_ENV_FILE"] = "/tmp/pfi_os-test-missing-env-file"
        try:
            rows = provider_status_rows()
            ...
            if old_env_file is None:
                os.environ.pop("PFI_ENV_FILE", None)
            else:
                os.environ["PFI_ENV_FILE"] = old_env_file
    ...
    @patch("pfi_os.data.provider_status._package_available", return_value=False)
    def test_moomoo_status_reports_missing_package(self, _mock_package):
        self.assertEqual(_moomoo_status(), "NeedsPackage")

    @patch("pfi_os.data.provider_status._port_reachable", return_value=False)
    @patch("pfi_os.data.provider_status._package_available", return_value=True)
    def test_moomoo_status_reports_missing_opend(self, _mock_package, _mock_port):
        self.assertEqual(_moomoo_status(), "NeedsOpenD")

    @patch("pfi_os.data.provider_status._port_reachable", return_value=True)
    @patch("pfi_os.data.provider_status._package_available", return_value=True)
