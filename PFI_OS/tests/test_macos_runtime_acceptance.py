...
import pfi_os.system.macos_runtime_acceptance as runtime
...
def test_runtime_acceptance_runs_controlled_start_stop_flow(monkeypatch, tmp_path: Path) -> None:
    ...
    def fake_run_script(_root: Path, args: list[str], *, timeout_seconds: int):
        ...
        if command == "scripts/cleanCache.sh" and "--dry-run" in args:
            return {
                "returncode": 0,
                "stdout": '{"schema":"PFICacheCleanupReportV1","candidate_count":2,"candidate_file_count":3,"candidate_kb":4.5}',
                "stderr": "",
            }
    ...
    monkeypatch.setattr(runtime, "_app_acceptance_summary", lambda _root: {"schema": "PFIOSMacOSAppAcceptanceLiteV1", "status": "Pass", "summary": {"fail": 0}})

    payload = runtime.run_macos_runtime_acceptance(project_root=tmp_path, start_timeout_seconds=1, stop_timeout_seconds=1)

    assert payload["schema"] == "PFIOSMacOSRuntimeAcceptanceV1"
    ...
    assert timeouts[("scripts/statusPFIOS.sh", "default")] >= runtime.SUPPORT_SCRIPT_TIMEOUT_FLOOR_SECONDS
...
def test_runtime_acceptance_supports_app_open_launch_method(monkeypatch, tmp_path: Path) -> None:
    ...
    def fake_launch_app(_root: Path, _app_path: Path | str | None):
        state["running"] = True
        state["launched"] = True
        return {"returncode": 0, "stdout": "opened=/tmp/PFI_OS.app", "stderr": ""}
