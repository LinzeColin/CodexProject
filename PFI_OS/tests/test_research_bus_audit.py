...
from pfi_os.integrations.research_bus_audit import REQUIRED_AI_AUTOMATION_FILES, REQUIRED_AI_BRIDGE_FILES, run_research_bus_interop_audit
...
def test_research_bus_interop_audit_passes_complete_temp_bus(monkeypatch):
    with tempfile.TemporaryDirectory() as tmp:
        ...
        inbox.mkdir()
        monkeypatch.setattr("pfi_os.integrations.research_bus_audit.CHAT_DROPBOX_DIR", inbox)
...
def test_research_bus_interop_audit_warns_when_runtime_records_are_missing(monkeypatch):
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert "PFIOS 回测结论回写行研系统" in warnings
...
def _seed_complete_bus(db_path: Path) -> None:
    with sqlite3.connect(db_path) as conn:
        ...
        conn.execute(
            ...
            VALUES ('mapping_1', 'PFIOS', 'Apple', 'US', 'AAPL', 'AAPL', 'Apple',
