...
from pfi_os.system.macos_lifecycle import build_macos_lifecycle_readiness, write_macos_lifecycle_readiness
...
def test_macos_lifecycle_readiness_passes_fixture_without_full_smoke(tmp_path: Path) -> None:
    ...
    assert payload["schema"] == "PFIOSMacOSLifecycleReadinessV1"
    assert payload["status"] == "Pass"
    assert payload["summary"]["fail"] == 0
    assert payload["cache_preview"]["schema"] == "PFICacheCleanupReportV1"
...
def test_macos_lifecycle_readiness_blocks_missing_heartbeat_ui(tmp_path: Path) -> None:
    _fixture_lifecycle(root=tmp_path)
    streamlit_app = tmp_path / "src" / "pfi_os" / "app" / "streamlit_app.py"
...
def _fixture_lifecycle(*, root: Path) -> None:
    ...
    app_dir = root / "src" / "pfi_os" / "app"
    system_dir = root / "src" / "pfi_os" / "system"
    app_dir.mkdir(parents=True)
    system_dir.mkdir(parents=True)

    _write_executable(
        root / "StartPFIOS.command",
        """
#!/bin/zsh
LOCK_DIR="$PROJECT_DIR/data/cache/pfi_os_launch.lockdir"
...
"$PYTHON_BIN" -m streamlit run src/pfi_os/app/streamlit_app.py --server.address 127.0.0.1 --server.fileWatcherType none --browser.gatherUsageStats false &
STREAMLIT_PID=$!
"$PYTHON_BIN" -m pfi_os.system.shutdown_monitor --streamlit-pid "$STREAMLIT_PID"
""",
    )
    _write_executable(
...
""",
    ...
    _write_executable(scripts / "statusPFIOS.sh", "#!/usr/bin/env zsh\necho stopped\n")
