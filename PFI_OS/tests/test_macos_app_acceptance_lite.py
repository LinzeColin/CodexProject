...
from pfi_os.system.macos_acceptance import AppTarget, build_macos_app_acceptance_lite, write_macos_app_acceptance_lite
...
def test_macos_app_acceptance_lite_passes_fixture_apps_without_heavy_smoke(tmp_path: Path) -> None:
    ...
    assert payload["schema"] == "PFIOSMacOSAppAcceptanceLiteV1"
    ...
    assert all("github.com/LinzeColin/PFI_OS" not in row["evidence"] for row in payload["checks"])


def test_macos_app_acceptance_lite_blocks_missing_project_binding(tmp_path: Path) -> None:
    app_paths = _fixture_apps(tmp_path)
    (app_paths["Desktop"] / "Contents" / "Resources" / "PFI_OS_PROJECT_ROOT").unlink()
    ...
    assert "installPFIOSEntryApps.sh" in payload["next_action"]
...
def _fixture_apps(root: Path) -> dict[str, Path]:
    (root / "StartPFIOS.command").write_text("#!/usr/bin/env zsh\nexit 0\n", encoding="utf-8")
    ...
    app_paths = {
        ...
        "Desktop": root / "Desktop" / "PFI_OS.app",
        "Downloads": root / "Downloads" / "PFI_OS.app",
        "Applications": root / "Applications" / "PFI_OS.app",
    }
...
def _write_app(app: Path, root: Path, *, write_binding: bool) -> None:
    ...
    (macos / "PFI_OS").write_text("#!/usr/bin/env zsh\necho 'PFI_OS_APP_LAUNCH: project=/tmp command=./StartPFIOS.command mode=open-command'\n", encoding="utf-8")
    os.chmod(macos / "PFI_OS", 0o755)
    if write_binding:
        (resources / "PFI_OS_PROJECT_ROOT").write_text(str(root), encoding="utf-8")
