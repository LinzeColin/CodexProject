...
from pfi_os.analysis import build_hotspot_history, default_hotspot_universe
...
def test_fetch_52etf_public_snapshot_uses_curl_fallback_after_urlopen_failure():
    ...
    with patch("pfi_os.integrations.site52etf.urlopen", side_effect=URLError("certificate verify failed")):
        with patch("pfi_os.integrations.site52etf.subprocess.run", return_value=completed) as run:
