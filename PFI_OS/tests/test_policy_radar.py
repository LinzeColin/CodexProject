...
def test_policy_radar_promotes_only_reviewed_authoritative_evidence(tmp_path: Path) -> None:
    ...
    assert payload["schema"] == "PFIOSPolicyIntelligenceRadarV1"
    ...
    assert payload["runtime_summary"]["schema"] == "PFIOSPolicyIntelligenceRuntimeSummaryV1"
...
def test_write_policy_radar_outputs_json_csv_markdown_pdf_and_latest(tmp_path: Path) -> None:
    ...
    assert saved["schema"] == "PFIOSPolicyIntelligenceRadarV1"
    assert runtime_summary["schema"] == "PFIOSPolicyIntelligenceRuntimeSummaryV1"
    ...
    assert markdown_path.read_text(encoding="utf-8").startswith("# PFI_OS Policy Intelligence Radar")
...
def test_policy_runtime_summary_blocks_bad_schema_and_stays_compact(tmp_path: Path) -> None:
    ...
    assert summary["schema"] == "PFIOSPolicyIntelligenceRuntimeSummaryV1"
