...
from pfi_os.integrations.permissions import authorize_system_action
from pfi_os.integrations.system_orchestrator import orchestrate_child_system


class SystemPermissionsTest(unittest.TestCase):
    def test_pfi_os_can_write_finance_ledger_scope(self) -> None:
        decision = authorize_system_action("PFI_OS", "finance_ledger", "ledger.write", action="write")
    ...
    def test_undeclared_scope_is_denied_by_default(self) -> None:
        decision = authorize_system_action("PFI_OS", "finance_ledger", "broker.order.submit", action="write")

        self.assertFalse(decision["allowed"], decision)

    def test_execute_requires_approval_id(self) -> None:
        decision = authorize_system_action("PFI_OS", "finance_ledger", "ledger.write", action="write", execute=True)
    ...
    def test_orchestrator_execute_is_denied_for_cross_domain_child(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch.dict(
                os.environ,
                {
                    "PFI_ORCHESTRATOR_EXECUTE_ALLOWED": "1",
                    "PFI_ORCHESTRATOR_EXECUTE_CALLERS": "finance_ledger",
                },
                clear=False,
            ):
