...
def build_data_home_boundary_contract(project_root: Path | str, data_home: Path | str | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "required": [
            "$PFI_OS_DATA_HOME must be outside the public repository.",
            "Operational SQLite must stay under $PFI_OS_DATA_HOME/private/operational.",
    ...
    if _is_relative_to(resolved_data_home, resolved_root):
        findings.append(
            DataHomeAuditFinding(
                ...
                message="$PFI_OS_DATA_HOME must not live inside the public repository.",
            )
        )
    if _is_relative_to(operational_db, resolved_root):
        findings.append(
            DataHomeAuditFinding(
                ...
                message="Operational SQLite must stay outside public Git under $PFI_OS_DATA_HOME/private/operational.",
            )
        )
    ...
    if lower_name.endswith(FORBIDDEN_RUNTIME_SUFFIXES):
        findings.append(
            DataHomeAuditFinding(
                ...
                path=display_path,
                message="Runtime database files belong under $PFI_OS_DATA_HOME, not public Git.",
            )
        )
