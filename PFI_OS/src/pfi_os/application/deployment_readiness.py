...
REQUIRED_REPO_SURFACES: tuple[tuple[str, str], ...] = (
    ...
    ("start_script", "scripts/startPFIOS.sh"),
    ("status_script", "scripts/statusPFIOS.sh"),
    ("macos_app_template", "macos/PFI_OS.app"),
)
...
def build_phase_d_deployment_readiness_contract(
    ...
    return {
        ...
        "backup_restore_policy": {
            "backup_dir": "$PFI_OS_DATA_HOME/runtime/backups",
            "restore_staging_dir": "$PFI_OS_DATA_HOME/runtime/restore_staging",
...
def _data_home_checks(project_root: Path, data_home: Path) -> list[dict[str, Any]]:
    ...
    return [
        _check(
            ...
            required=True,
            message="$PFI_OS_DATA_HOME must stay outside the public repository.",
        ),
        _check(
            category="data_boundary",
            ...
            required=True,
            message="Operational SQLite must resolve under $PFI_OS_DATA_HOME/private/operational.",
        ),
        _check(
            category="data_boundary",
    ...
    for code, path in paths.items():
        checks.append(
            _check(
                ...
                evidence=str(path),
                required=True,
                message="Path plan is under $PFI_OS_DATA_HOME; readiness check does not create it.",
                exists=path.exists(),
            )
        )
    return checks
...
def _local_model_summary(env: dict[str, str]) -> dict[str, Any]:
    provider = str(env.get("PFI_OS_LLM_PROVIDER", "DisabledProvider") or "DisabledProvider").strip()
    return {
        "provider": provider,
