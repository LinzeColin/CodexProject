...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run a lightweight macOS PFI_OS app acceptance check without full smoke.")
    ...
    parser.add_argument("--skip-dry-run", action="store_true", help="Skip PFI_OS_APP_LAUNCH_DRY_RUN launcher checks.")
    parser.add_argument("--skip-status-script", action="store_true", help="Skip scripts/statusPFIOS.sh execution.")
    ...
    print(
        "PFI_OS_MACOS_APP_ACCEPTANCE_LITE: "
    ...
    print(f"PFI_OS_MACOS_APP_ACCEPTANCE_NEXT_ACTION: {payload['next_action']}")
    if payload.get("outputs"):
        print(f"PFI_OS_MACOS_APP_ACCEPTANCE_OUTPUTS: {payload['outputs']}")
