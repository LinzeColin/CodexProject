    ...
    *,
    ...
    try:
        ...
        return Site52EtfSnapshot(
            ...
            risk_note="52ETF reference could not be read; keep the PFI_OS hotspot workflow on local/provider data and retry later.",
            error=str(exc),
        )
    ...
    *,
    ...
    return {
        ...
        "safety_boundary": (
            ...
            "and no replacement for PFI_OS local/provider data-quality gates."
        ),
    }
...
def site52etf_summary_rows(snapshot: Site52EtfSnapshot | dict[str, object]) -> list[dict[str, str]]:
    ...
    return [
        ...
        {"项目": "覆盖板块", "状态": ", ".join(payload.get("boards", ()) or ()), "说明": "用于对照 PFI_OS 自有热点对象池。"},
...
def build_site52etf_hotspot_comparison(
    ...
    *,
    ...
    return {
        ...
        "comparison_note": (
            "52ETF public page is an A-share market-cloud interaction reference; PFI_OS hotspot analysis uses local/provider "
...
def site52etf_comparison_rows(comparison: dict[str, object]) -> list[dict[str, str]]:
    ...
    return [
        ...
        },
        {
            ...
            "说明": f"52ETF 当前公开页是 A 股云图；当前 PFI_OS 市场为 {comparison.get('market', '')}。",
        },
        {
            "项目": "板块映射",
            "状态": f"{comparison.get('matched_board_count', 0)}/{comparison.get('board_count', 0)}",
            "说明": matched_text or "未找到可直接映射的 PFI_OS 热点对象。",
        },
        {
            "项目": "PFI热点切片",
...
def _fetch_html(source_url: str, timeout_seconds: int) -> str:
    request = Request(source_url, headers={"User-Agent": "PFI_OS/0.1 read-only research reference"})
    try:
...
def _fetch_html_with_curl(source_url: str, timeout_seconds: int) -> str:
    result = subprocess.run(
        [
            ...
            "--user-agent",
            "PFI_OS/0.1 read-only research reference",
            source_url,
        ],
        check=True,
        capture_output=True,
