...
APP_BUNDLE_NAME = "PFI_OS.app"
APP_EXECUTABLE_NAME = "PFI_OS"
...
def _app_checks(
    ...
    *,
    ...
    root_file = app_path / "Contents" / "Resources" / "PFI_OS_PROJECT_ROOT"
    checks = [
...
def _project_checks(root: Path) -> list[dict[str, Any]]:
    return [
        _check("Project", "StartCommandExecutable", "Pass" if _is_executable(root / "StartPFIOS.command") else "Fail", "StartPFIOS.command"),
        _check("Project", "StatusScriptExecutable", "Pass" if _is_executable(root / "scripts" / "statusPFIOS.sh") else "Fail", "scripts/statusPFIOS.sh"),
...
def _runtime_status(root: Path, *, run_status_script: bool) -> dict[str, Any]:
    ...
    if not run_status_script:
        ...
        return payload
    script = root / "scripts" / "statusPFIOS.sh"
...
def _github_fallback_check(label: str, executable: Path) -> dict[str, Any]:
    ...
    found = b"github.com/LinzeColin/PFI_OS" in content
...
def _dry_run_check(label: str, executable: Path, timeout_seconds: int) -> dict[str, Any]:
    ...
    env = dict(os.environ)
    env["PFI_OS_APP_LAUNCH_DRY_RUN"] = "1"
...
def _next_action(summary: dict[str, int]) -> str:
    if summary.get("fail", 0):
        return "Run scripts/installPFIOSEntryApps.sh, then rerun scripts/macosAppAcceptanceLite.sh --json."
    return "Use PFI_OS.app for normal launch; reserve finalAcceptanceCheck.sh for deliberate full release gates."
