...
def collect_health_checks(project_root: Path | None = None, report_root: Path | None = None) -> list[HealthCheck]:
    ...
    return [
        ...
        _path_check("桌面 PFI_OS 入口", "Desktop PFI_OS Launcher", app_paths["desktop"], must_exist=True),
        _path_check("下载目录 PFI_OS 入口", "Downloads PFI_OS Launcher", app_paths["downloads"], must_exist=True),
        _path_check("Applications PFI_OS 入口", "Applications PFI_OS Launcher", app_paths["applications"], must_exist=True),
        _path_check("项目内部启动文件", "Internal Start Launcher", root / "StartPFIOS.command", must_exist=True),
        _path_check("双击停止文件", "Double-Click Stopper", root / "StopPFIOS.command", must_exist=True),
        _path_check("启动脚本", "Start Script", root / "scripts" / "startPFIOS.sh", must_exist=True),
        _path_check("停止脚本", "Stop Script", root / "scripts" / "stopPFIOS.sh", must_exist=True),
        _path_check("状态脚本", "Status Script", root / "scripts" / "statusPFIOS.sh", must_exist=True),
        _path_check("验收脚本", "Verification Script", root / "scripts" / "verifyPFIOS.sh", must_exist=True),
