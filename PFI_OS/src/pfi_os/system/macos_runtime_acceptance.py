    ...
    try:
        if pre_ports:
            ...
            if method == "app":
                ...
                checks.append(_check("Start", "StartCommandLaunched", "Pass", "scripts/startPFIOS.sh launched in controlled background mode"))
        ...
        if started_by_acceptance:
            _run_script(root, ["scripts/stopPFIOS.sh"], timeout_seconds=max(10, stop_timeout_seconds))
...
def _payload(
    ...
    *,
    ...
    return {
        ...
        "checks": checks,
        "runtime_contract": {
            "start": "Launches scripts/startPFIOS.sh or opens a configured PFI_OS.app in controlled mode when no existing service is running.",
            ...
            "stop": "Runs scripts/stopPFIOS.sh and verifies local health disappears.",
        ...
        "safety_boundary": (
            ...
            "PFI_OS service is already running."
        ),
...
def _start_process(root: Path) -> subprocess.Popen[str]:
    return subprocess.Popen(
        [str(root / "scripts" / "startPFIOS.sh")],
        cwd=root,
...
def _launch_app(root: Path, app_path: Path | str | None) -> dict[str, Any]:
    target = Path(app_path).expanduser() if app_path else Path.home() / "Downloads" / "PFI_OS.app"
    if not target.is_dir():
        fallback = root / "macos" / "PFI_OS.app"
    ...
    if completed.returncode == 0 and not _app_launch_observed(root, wait_seconds=12):
        executable = target / "Contents" / "MacOS" / "PFI_OS"
