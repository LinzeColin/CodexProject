...
def build_macos_lifecycle_readiness(
    *,
    ...
    return {
        ...
        "checks": checks,
        "lifecycle_contract": {
            "start": "StartPFIOS.command or scripts/startPFIOS.sh starts local Streamlit on 127.0.0.1 and reuses an existing service.",
            "stop": "scripts/stopPFIOS.sh only stops Streamlit processes for this checkout.",
            "auto_shutdown": "StartPFIOS.command runs pfi_os.system.shutdown_monitor; the UI sends sanitized localhost heartbeat requests.",
        ...
        "safety_boundary": (
            "Read-only readiness inspection. It may run statusPFIOS.sh and build dry-run cache/app summaries, "
...
def _static_lifecycle_checks(root: Path) -> list[dict[str, Any]]:
    start_command = root / "StartPFIOS.command"
    start_script = root / "scripts" / "startPFIOS.sh"
    stop_script = root / "scripts" / "stopPFIOS.sh"
    status_script = root / "scripts" / "statusPFIOS.sh"
    ...
    checks = [
        _executable_check("Command", "StartPFIOSExecutable", start_command),
