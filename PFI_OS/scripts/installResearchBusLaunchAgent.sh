...
LABEL="com.pfi_os.researchbus.sync"
PLIST_PATH="${HOME}/Library/LaunchAgents/${LABEL}.plist"
INTERVAL_SECONDS="${RESEARCH_BUS_LAUNCH_INTERVAL_SECONDS:-60}"
RUNNER_DIR="${HOME}/Library/Application Support/PFIOS"
...
run_step "PFIOS chat dropbox" "\$PYTHON_BIN" -m pfi_os.examples.research_bus_api process-dropbox --min-age-seconds 0 --limit 100 --json
run_step "PFIOS heartbeat" "\$PYTHON_BIN" -m pfi_os.examples.research_bus_api heartbeat --system-name ResearchBus --status Ready --capability sync_all --capability chat_input --capability chat_dropbox --capability independent_validation --capability local_webhook
run_step "PFIOS request processing" "\$PYTHON_BIN" -m pfi_os.examples.research_bus_api process --system-name ResearchBus --limit 100 --json
run_step "PFIOS research bus sync" "\$PYTHON_BIN" -m pfi_os.examples.sync_research_bus --json
