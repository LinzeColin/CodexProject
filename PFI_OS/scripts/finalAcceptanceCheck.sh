...
if [[ "${1:-}" == "--allow-heavy-smoke" ]]; then
  export PFI_OS_ALLOW_HEAVY_SMOKE=1
  shift
fi

if [[ "${PFI_OS_ALLOW_HEAVY_SMOKE:-}" != "1" ]]; then
  cat >&2 <<'EOF'
PFI_OS heavy SmokeTest is blocked by default.
Use scripts/devReadyCheck.sh --summary-json for normal development.
Run deliberately only for release gates:
  PFI_OS_ALLOW_HEAVY_SMOKE=1 scripts/finalAcceptanceCheck.sh
EOF
  exit 64
fi
...
APP_BUNDLE_NAME="PFI_OS"
APP_DISPLAY_NAME="PFI_OS"
...
echo "PFIOS final acceptance check"
...
check_executable "$PROJECT_DIR/StartPFIOS.command" "Project double-click launcher is executable"
check_executable "$PROJECT_DIR/StopPFIOS.command" "Project stop launcher is executable"
check_dir "$DESKTOP_DIR/$APP_BUNDLE_NAME.app" "Desktop PFI_OS app exists"
check_dir "$DOWNLOADS_DIR/$APP_BUNDLE_NAME.app" "Downloads PFI_OS app exists"
check_dir "$APP_INSTALL_DIR/$APP_BUNDLE_NAME.app" "Applications PFI_OS app exists"
check_executable "$DESKTOP_DIR/$APP_BUNDLE_NAME.app/Contents/MacOS/PFI_OS" "Desktop PFI_OS app launcher is executable"
check_executable "$DOWNLOADS_DIR/$APP_BUNDLE_NAME.app/Contents/MacOS/PFI_OS" "Downloads PFI_OS app launcher is executable"
check_executable "$APP_INSTALL_DIR/$APP_BUNDLE_NAME.app/Contents/MacOS/PFI_OS" "Applications PFI_OS app launcher is executable"
check_file "$PROJECT_DIR/macos/PFI_OS.app/Contents/MacOS/PFI_OS" "Source PFI_OS app launcher exists"
check_file "$PROJECT_DIR/macos/PFI_OS_launcher.c" "Source PFI_OS native launcher source exists"
check_file "$DESKTOP_DIR/$APP_BUNDLE_NAME.app/Contents/Resources/PFI_OS_PROJECT_ROOT" "Desktop PFI_OS app local project binding exists"
check_file "$DOWNLOADS_DIR/$APP_BUNDLE_NAME.app/Contents/Resources/PFI_OS_PROJECT_ROOT" "Downloads PFI_OS app local project binding exists"
check_file "$APP_INSTALL_DIR/$APP_BUNDLE_NAME.app/Contents/Resources/PFI_OS_PROJECT_ROOT" "Applications PFI_OS app local project binding exists"
check_app_signature "$DESKTOP_DIR/$APP_BUNDLE_NAME.app" "Desktop PFI_OS app signature is valid"
check_app_signature "$DOWNLOADS_DIR/$APP_BUNDLE_NAME.app" "Downloads PFI_OS app signature is valid"
check_app_signature "$APP_INSTALL_DIR/$APP_BUNDLE_NAME.app" "Applications PFI_OS app signature is valid"
check_plist_value "$DESKTOP_DIR/$APP_BUNDLE_NAME.app" "CFBundleDisplayName" "$APP_DISPLAY_NAME" "Desktop PFI_OS display name is correct"
check_plist_value "$DOWNLOADS_DIR/$APP_BUNDLE_NAME.app" "CFBundleDisplayName" "$APP_DISPLAY_NAME" "Downloads PFI_OS display name is correct"
check_plist_value "$APP_INSTALL_DIR/$APP_BUNDLE_NAME.app" "CFBundleDisplayName" "$APP_DISPLAY_NAME" "Applications PFI_OS display name is correct"
check_file "$PROJECT_DIR/assets/PFIOSAppIconConfig.json" "macOS app icon config exists"
check_file "$PROJECT_DIR/assets/PFIOSAppIcon.icns" "macOS app icon exists"
check_file "$PROJECT_DIR/assets/PFIOSAppIconPreview.png" "macOS app icon preview exists"
...
check_executable "$PROJECT_DIR/scripts/startPFIOS.sh" "Start script is executable"
check_executable "$PROJECT_DIR/scripts/stopPFIOS.sh" "Stop script is executable"
check_executable "$PROJECT_DIR/scripts/installMacAppLaunchers.sh" "macOS app installer is executable"
check_executable "$PROJECT_DIR/scripts/statusPFIOS.sh" "Status script is executable"
...
check_executable "$PROJECT_DIR/scripts/verifyPFIOS.sh" "Verification script is executable"
...
check_text "$PROJECT_DIR/macos/PFI_OS_launcher.c" "PFI_OS_PROJECT_ROOT" "Source PFI_OS app resolves local project root"
check_text "$PROJECT_DIR/macos/PFI_OS_launcher.c" "PFI_OS_APP_LAUNCH_DRY_RUN" "Source PFI_OS app launch can be dry-run verified"
check_text "$PROJECT_DIR/macos/PFI_OS_launcher.c" "./StartPFIOS.command" "Source PFI_OS app opens local command launcher"
check_text "$PROJECT_DIR/macos/PFI_OS_launcher.c" "mode=spawn-command" "Source PFI_OS app uses direct command launch mode"
check_text "$PROJECT_DIR/macos/PFI_OS_launcher.c" "posix_spawn" "Source PFI_OS app uses native launcher process spawning"
check_text "$PROJECT_DIR/macos/PFI_OS_launcher.c" "\"/bin/zsh\"" "Source PFI_OS app invokes local command launcher through zsh without Terminal"
check_text_absent "$PROJECT_DIR/macos/PFI_OS_launcher.c" "github.com/LinzeColin/PFI_OS" "Source PFI_OS app does not fall back to GitHub"
check_text_absent "$PROJECT_DIR/macos/PFI_OS_launcher.c" "tell application \"Terminal\"" "Source PFI_OS app does not require Terminal automation"
check_text_absent "$PROJECT_DIR/macos/PFI_OS_launcher.c" "open_command_in_terminal" "Source PFI_OS app does not route command launch through Terminal"
check_text_absent "$PROJECT_DIR/macos/PFI_OS_launcher.c" "\"Terminal\"" "Source PFI_OS app does not name Terminal"
check_text_absent "$DESKTOP_DIR/$APP_BUNDLE_NAME.app/Contents/MacOS/PFI_OS" "github.com/LinzeColin/PFI_OS" "Desktop PFI_OS app does not fall back to GitHub"
check_text_absent "$DOWNLOADS_DIR/$APP_BUNDLE_NAME.app/Contents/MacOS/PFI_OS" "github.com/LinzeColin/PFI_OS" "Downloads PFI_OS app does not fall back to GitHub"
check_text_absent "$APP_INSTALL_DIR/$APP_BUNDLE_NAME.app/Contents/MacOS/PFI_OS" "github.com/LinzeColin/PFI_OS" "Applications PFI_OS app does not fall back to GitHub"
...
check_text "$PROJECT_DIR/src/pfi_os/system/macos_acceptance.py" "PFI_OS_APP_LAUNCH_DRY_RUN" "macOS App Acceptance Lite uses launcher dry-run"
...
check_text "$PROJECT_DIR/src/pfi_os/app/dashboard.py" "Lifecycle controls only manage the local PFI_OS/PFIOS app process" "macOS lifecycle safety policy is present"
...
check_text "$PROJECT_DIR/shared/schema/company_cashflow_reviewed_input.schema.json" "PFI_OS Company CashFlow Reviewed Input" "Company CashFlow reviewed input JSON schema title is present"
...
check_text "$PROJECT_DIR/shared/schema/policy_reviewed_input.schema.json" "PFI_OS Policy Reviewed Input" "Policy reviewed input JSON schema title is present"
...
check_text "$PROJECT_DIR/shared/schema/consumption_guard_reviewed_input.schema.json" "PFI_OS Consumption Guard Reviewed Input" "Consumption reviewed input JSON schema title is present"
...
if "$PROJECT_DIR/scripts/verifyPFIOS.sh"; then
...
echo "PFIOS final acceptance check completed."
