...
| PFI-003 | Supervisor, durable Job Store, lifecycle, backup/restore, crash recovery | Partial, stronger | Existing start/stop/status, launch lock, shutdown monitor, Phase C scheduler, Phase D backup/restore tests; `DurableJobStore` covers idempotency, atomic claim, lease, heartbeat, bounded retry, cancel/resume, expired-lease recovery, and dead letter; `scripts/pfiSupervisor.sh` exposes contract/status/doctor/lifecycle commands plus double-worker and crash-recovery smoke; PFI-003 durable job tests passed 8/8 and supervisor CLI tests passed 6/6 | Sleep/wake, real TERM/KILL process harness, network recovery, launchd throttle/log rotation, backup manifest, private-log scan, and Web Shell/read-model integration still need release evidence |
