...
| PFI-003 | Supervisor, durable Job Store, lifecycle, backup/restore, crash recovery | Partial, stronger | Existing start/stop/status, launch lock, shutdown monitor, Phase C scheduler, Phase D backup/restore tests; `DurableJobStore` now covers idempotency, atomic claim, lease, heartbeat, bounded retry, cancel/resume, expired-lease recovery, and dead letter; `tests/contract/test_pfi003_durable_jobs.py` passed 8/8 | CLI/status/doctor surface, double start/double worker script proof, sleep/wake, TERM/KILL, network recovery, launchd throttle/log rotation, backup manifest, and private-log scan still need release evidence |
