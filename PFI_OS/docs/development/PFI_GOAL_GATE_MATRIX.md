...
| Gate 3 | Four vertical slices each have data, domain, API, UI, tasks, evidence, tests, rollback | Partial: Markets slice strong | Phase B workflow contracts; PFI-006 Markets has acceptance script, Golden metrics, same-shell UI controls, alerts/saved views, portfolio overlay, task/evidence records, and rollback proof | Add equivalent closure for PFI-007 Research/Policy, PFI-008 Portfolio, and PFI-009 Strategy |
...
Continue with Gate 3 / PFI-007 Research + Policy vertical slice. Gate 1, Gate
2, and the Markets part of Gate 3 are closed for the current evidence scope
and must be re-run in the final Gate 7 release package.
