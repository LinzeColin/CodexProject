...
| Repository root evidence | 当前 checkout 的仓库根有 `README.md`，未发现仓库根 `AGENTS.md` 或 `governance/projects.yaml`；本地治理合同落在 `PFI_OS/AGENTS.md`。 |
...
| Command | Result |
| --- | --- |
| `PYTHONDONTWRITEBYTECODE=1 PYTEST_ADDOPTS='-p no:cacheprovider' .venv/bin/python -m pytest tests/contract/test_pfi_v02_stage0_compatibility.py tests/test_pfi_product_contracts.py -q` | `16 passed` |
...
| Excluded-literal grep scoped to Stage 0 touched files | Pass, no output |
