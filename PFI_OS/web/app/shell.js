...
const FUNCTION_VIEWS = {
  ...
  pit_backtest: functionView(
    ...
    ["可用：bar_checksum、reproducibility_hash 和 next-bar 执行模型", "验收：公司行动和退市样本必须显式记录", "边界：回测不是交易信号"],
    { legacyView: "single" },
  ),
  train_test_validation: functionView(
    ...
    ["可用：split_datetime、train_rows、test_rows 和 generalization_ratio", "验收：训练窗口不得覆盖测试窗口", "边界：验证结果只进入人工复核"],
    { legacyView: "scan" },
  ),
  walk_forward_validation: functionView(
    ...
    ["可用：window_count、pass_count、average_generalization_ratio", "验收：每个滚动窗口都必须没有未来数据", "边界：滚动通过也不允许自动下单"],
    { legacyView: "scan" },
  ),
  strategy_registry: functionView(
    ...
    ["可用：model_id、strategy_version、train_test_status、walk_forward_status", "验收：order_enabled=false 且 live_signal_enabled=false", "边界：注册不等于上线，不提交订单"],
    { legacyView: "library" },
  ),
