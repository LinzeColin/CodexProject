...
# ---------- 6.5 R2 免费额度(守卫判定) ----------
# 规则见 /opt/STORAGE_POLICY.md「R2 零付费红线」与 Private-Database OPS/AGENT_ONBOARDING.md §9.7。
# 守卫 /usr/local/bin/linze-r2-free-tier-guard.py 每 6h 跑一次并写判定文件;
# 这里只读判定,不重跑 —— 重跑一遍纯属白花 API 调用,而且守卫已经能自己熔断。
# 接进这里的理由:守卫的 CRIT 原本只落在日志和 JSON 里,没人看 = 等于没报。
GUARD=/srv/linze/apps/status/data/r2_free_tier_guard.json
if [ ! -s "$GUARD" ] || ! jq -e . "$GUARD" >/dev/null 2>&1; then
  gap "R2 免费额度守卫判定缺失或非法 JSON($GUARD) —— 本轮 R2 计费风险未核(取不到 != 没问题)"
else
  g_at=$(jq -r '.generated_at // empty' "$GUARD")
  g_sev=$(jq -r '.severity // "UNKNOWN"' "$GUARD")
  g_age=$(( ( $(date +%s) - $(date -d "${g_at:-1970-01-01}" +%s 2>/dev/null || echo 0) ) / 86400 ))
  if [ "$g_age" -gt 1 ] 2>/dev/null; then
    add now "r2-guard-stale" "R2 免费额度守卫已 ${g_age} 天没出新判定" \
      "判定日期 ${g_at:-无} · cron /etc/cron.d/linze-r2-freetier-guard · 守卫不跑=计费风险裸奔"
  fi
  g_ratio=$(jq -r '"ClassA \(.usage.class_a.projected_ratio*100|floor)% / ClassB \(.usage.class_b.projected_ratio*100|floor)% / 存储 \(.usage.storage_gb.projected_ratio*100|floor)%"' "$GUARD" 2>/dev/null)
  case "$g_sev" in
    CRIT) g_lvl=now ;;
    WARN) g_lvl=later ;;
    *)    g_lvl="" ;;
  ...
  if [ -n "$g_lvl" ]; then
    jq -r '.findings[]? | "\(.code)\t\(.severity)\t\(.zh)"' "$GUARD" | \
    while IFS=$'\t' read -r g_code g_fsev g_zh; do
      [ -z "$g_code" ] && continue
      add "$g_lvl" "r2-freetier-$g_code" "R2 计费风险:$g_zh" \
        "守卫判定 $g_at · severity=$g_fsev · 免费额度投影占比 $g_ratio · 详情 $GUARD"
    done
  fi
  # 熔断动作即使不告警也要留痕:自动改回 Standard 说明有人/有东西建错了桶
  jq -r '.actions_taken[]? | .zh' "$GUARD" 2>/dev/null | while read -r g_act; do
    [ -z "$g_act" ] && continue
    add now "r2-freetier-fused" "R2 守卫触发过熔断:$g_act" \
      "守卫判定 $g_at · 有人建了非 Standard 存储类的桶,已被自动改回,但要查是谁建的"
  done
fi
