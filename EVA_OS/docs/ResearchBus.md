...
日常用户先读 `QuickStart.md`。本文件用于排查和维护 QuantLab、行研系统、消费行为/持仓数据、独立验证系统、FIFA/TAB 研究系统、政府文件/政策解读系统之间的输入、输出和数据互通。

最常用三件事：

| 目标 | 命令 |
| --- | --- |
| 一次性同步全部系统 | `$EVA_OS_HOME/scripts/syncResearchSystemsOnce.sh` |
| 处理任意聊天框提交的待办 | `$EVA_OS_HOME/scripts/researchBusApi.sh process --system-name ResearchBus --limit 100 --json` |
| 查看共享库健康状态 | `$EVA_OS_HOME/scripts/syncResearchBus.sh --json` |
| 审计跨系统互通是否完整 | `$EVA_OS_HOME/scripts/auditResearchBusInterop.sh --json` |
| 查看或调度子系统 | `$EVA_OS_HOME/scripts/orchestrateSystems.sh status --json` |

总系统开发协调入口：

```text
$EVA_OS_HOME/docs/SystemCoordinationPlan.md
...
```text
$EVA_OS_HOME/docs/PublicResearchUpgradePlan.md
$EVA_OS_HOME/data/researchBus/PublicResearchSourceLog_20260605.json
$EVA_OS_HOME/data/researchBus/MarketFeelResearchSourceLog_20260605.json
...
统一研究数据总线用于让 QuantLab、行研系统、消费行为/持仓数据、独立验证系统、FIFA/TAB 研究系统和政府文件/政策解读系统共享同一份可审计状态。

它解决这些问题：

- 共享数据库：`data/researchBus/ResearchBus.sqlite`
- JSON 快照：`data/researchBus/ResearchBusSnapshot.json`
- 母系统系统注册表：QuantLab 记录每个子系统的根目录、独立命令、健康检查命令、同步命令和能力
- 子系统产物索引：FIFA、政策系统、独立验证和行研系统的报告/JSON/dashboard 可被统一检索
- 行研报告正文解析成待验证任务
- QuantLab 回测结论回写给行研系统
- 持仓主数据和独立验证运行状态跨系统同步
...
```text
$EVA_OS_HOME/docs/ResearchBusSchema.json
```
...
| `system_artifacts` | 子系统产物索引，保存 FIFA、政策、独立验证、行研和 QuantLab 产出的报告、JSON、dashboard 路径 |
| `orchestration_runs` | QuantLab 母系统调度子系统的 dry-run 或实际执行记录 |
| `research_reports` | 行研 PDF、Word、Markdown、文本报告索引 |
| `validation_tasks` | 从行研报告、政策、新闻或人工研究拆出的待验证任务 |
| `quantlab_results` | QuantLab 回测结果、风险闸门、决策质量和报告路径 |
...
| `bus_heartbeats` | QuantLab、行研系统、独立验证系统的心跳和能力声明 |
...
```python
from quantlab.integrations import workflow_inputs_frame
...
回测 Word 报告和 RunMetadata JSON 现在包含 `QuantLabReportEvidenceV1` 摘要。它把研究结论前置绑定到证据链：
...
| `ReportEvidence` | 新报告 RunMetadata 是否包含 `QuantLabReportEvidenceV1`。 |
...
`holding_symbol_mappings` 同步时会同时生成 `QuantLabEntityRegistryV1` 摘要并写入 `system_state.summary_json`。实体状态分为：
...
互通审计是判断 QuantLab、行研系统、消费行为系统和独立验证系统是否真正互通的机器可读检查。它不会下单，不会修改持仓，只读取共享库和行研桥接目录；不加 `--no-write` 时会把审计结果写入固定文件。

```bash
$EVA_OS_HOME/scripts/auditResearchBusInterop.sh --json
```

输出文件：

```text
$EVA_OS_HOME/data/researchBus/ResearchBusInteropAudit.json
```
...
QuantLab 是母系统，ResearchBus 是控制平面。子系统可以被母系统调度，也可以完全绕过 QuantLab 独立运行。

注册所有默认子系统：

```bash
$EVA_OS_HOME/scripts/orchestrateSystems.sh register --json
```

同步子系统产物索引：

```bash
$EVA_OS_HOME/scripts/orchestrateSystems.sh sync-artifacts --json
```

查看系统注册表、产物和最近调度记录：

```bash
$EVA_OS_HOME/scripts/orchestrateSystems.sh status --json
```

dry-run 调度 FIFA 子系统健康检查，只记录母系统将调用的命令，不打开页面：

```bash
$EVA_OS_HOME/scripts/orchestrateSystems.sh run --system FIFA-Research-System --action health --json
```

确实需要由母系统实际运行子系统时，显式加 `--execute`：

```bash
$EVA_OS_HOME/scripts/orchestrateSystems.sh run --system GovernmentPolicySystem --action health --execute --timeout-seconds 120 --json
```
...
```text
$EVA_OS_HOME/data/researchBus/chatInbox
```
...
```bash
$EVA_OS_HOME/scripts/researchBusApi.sh process-dropbox --json
```
...
QuantLab 同步共享库、回写行研系统、批量推送验证任务：

```bash
$EVA_OS_HOME/scripts/syncResearchBus.sh --json
```

行研系统同步共享库、解析报告正文、拉取 QuantLab 和独立验证状态：
...
```bash
$EVA_OS_HOME/scripts/syncResearchBus.sh --mode consumer --json
```

独立验证百亿行 dry-run 分片登记：

```bash
$EVA_OS_HOME/scripts/runIndependentValidation.sh run --synthetic-rows 10000000000 --rows-per-shard 100000000 --json
```
...
```bash
$EVA_OS_HOME/scripts/runIndependentValidation.sh run --synthetic-rows 10000000000 --rows-per-shard 1000000000 --mode checksum --worker-count 4 --json
```
...
```bash
$EVA_OS_HOME/scripts/researchBusApi.sh submit-chat \
...
```bash
$EVA_OS_HOME/scripts/researchBusApi.sh confirm-holding-candidate \
...
```bash
$EVA_OS_HOME/scripts/researchBusApi.sh submit-chat --text "请运行十亿行独立验证，每片1亿行" --source-system ExternalChat --json
```
...
```bash
cd $EVA_AI_RESEARCH_ROOT
python3 -m src.cli research-bus-submit --text "同步 QuantLab 和行研系统状态" --json
...
跨系统一次性总控同步入口。该入口会串行处理投递箱、ResearchBus 请求、QuantLab 同步、行研系统请求和行研系统同步，运行完即退出：
...
$EVA_OS_HOME/scripts/installResearchBusLaunchAgent.sh status
```

安装器会生成：

```text
~/Library/Application Support/QuantLab/researchBusSyncRunner.sh
~/Library/LaunchAgents/com.quantlab.researchbus.sync.plist
```
...
- QuantLab 总集成审计：`Pass`，`6 Pass / 0 Review / 0 Fail`。
...
- 审计表计数：`system_registry=5`，`system_artifacts=253`，`orchestration_runs=1`，`research_reports=47`，`validation_tasks=10149`，`quantlab_results=30`，`holdings_master=28`，`holding_symbol_mappings=28`，`portfolio_transactions=1117`，`consumer_behavior_state=2`，`independent_validation_runs=4`，`independent_validation_shards=130`。
- 子系统注册表：已登记 `QuantLab`、`AI-Research-System`、`FIFA-Research-System`、`GovernmentPolicySystem` 和 `IndependentValidation`。
...
- QuantLab 同步：行研报告 18 份，QuantLab 结果 30 条，持仓 28 条，交易流水 23 条。
- 行研系统同步：发布报告 18 份，拉取 QuantLab 结果 30 条，拉取验证任务 1000 条，拉取独立验证运行 2 条，拉取消费行为状态 2 条，拉取持仓候选 0 条。
...
- 结构化附件自动解析：QuantLab CLI 和行研 CLI 提交 CSV 附件均已临时库验证，候选 parser 状态为 `Parsed`，确认后状态 `Applied`。
...
- 研究总线页面：QuantLab 已新增 `研究总线` 功能区，可查看请求、对话输入、心跳、系统状态和投递箱路径。
...
- 双向 API smoke：QuantLab 侧 `submit-chat` 自动生成验证任务并完成请求；行研侧 `pull_quantlab_results` 请求完成并回写响应。
- PDF 正文解析：QuantLab venv 已加入 `pypdf`，本机同步实测 `warnings=[]`，验证任务从 PDF 正文生成。
- 对话触发独立验证 smoke：`请运行十亿行独立验证，每片1亿行` 自动生成 `1,000,000,000` 行、10 个分片的 dry-run 计划。
- 准实时 watch smoke：QuantLab 和行研系统 watch 脚本并发各跑 1 轮通过；SQLite 已设置 `busy_timeout=30000` 降低并发写入锁冲突。
- 本轮新增测试：QuantLab 侧 ResearchBus/API/持仓簿相关测试 `41 passed in 83.09s`。
...
- 行研系统 `research-bus-submit --text` 现在会把 checksum 和规模参数写入与 QuantLab 相同的 `bus_api_requests.payload_json`。
...
```bash
rm -rf $EVA_OS_HOME/data/researchBus
rm -rf $EVA_OS_HOME/data/independentValidation
rm -rf $EVA_AI_RESEARCH_ROOT/data/report_artifacts/research_bus_bridge
rm -rf $EVA_AI_RESEARCH_ROOT/data/report_artifacts/quantlab_bridge
```
