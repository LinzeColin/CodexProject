...
Market Event Layer 是 EVA_OS 事件驱动行情层的第一块本地底座。它把 OHLCV bar 转成稳定、可排序、可去重、可落盘的 `BarClosed` 事件，供后续数据湖、回放、三模式回测/模拟内核和实时研究工作流复用。
...
批量事件日志使用 `EVAOSMarketEventLogV1`，包含：
...
```bash
$EVA_OS_HOME/scripts/marketEventLayer.sh --symbol SPY --market US --interval 1d --start 2026-01-01 --end 2026-01-10 --output-dir data/marketEvents
```

从本地 CSV 转换：

```bash
$EVA_OS_HOME/scripts/marketEventLayer.sh --symbol SPY --market US --interval 1d --source csv-import --input-csv path/to/bars.csv --output-dir data/marketEvents
```
