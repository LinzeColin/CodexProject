...
当所选数据源不原生支持 `1w`、`1m`、`1q` 或 `1y` 时，QuantLab 会尝试从日线或月线本地重采样。

When the selected provider does not natively support `1w`, `1m`, `1q`, or `1y`, QuantLab attempts local resampling from daily or monthly bars.
...
`.env` 文件放在 `$EVA_OS_HOME/.env`。

The `.env` file should be placed at `$EVA_OS_HOME/.env`.
...
如果状态是 `NeedsPackage`，需要在 QuantLab 环境安装 `futu-api`。

If the status is `NeedsPackage`, install `futu-api` in the QuantLab environment.
...
打开 moomoo 桌面程序不等于启动 OpenD；只有 `127.0.0.1:11111` 端口可连接时，QuantLab 才能读取 Moomoo 行情。

Opening the moomoo desktop app is not the same as starting OpenD. QuantLab can read Moomoo quote data only when `127.0.0.1:11111` is reachable.
