...
During PFI-001 the working directory remains `EVA_OS`; that name is legacy
...
- One product, one user-facing entry. Internal services may exist, but the user
  should not have to operate QuantLab, ResearchBus, policy, cashflow, or other
...
- No mandatory local LLM before product architecture and data boundaries are
  correct.
- No preservation of low-value EVA features for nostalgia.
