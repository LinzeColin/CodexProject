...
| `MacOSAppAcceptanceLite.md` | macOS app 入口轻量验收：检查 Desktop、Downloads、Applications 的 `EVA_OS.app`，不运行完整 smoke。 |
...
停止使用：双击 `$EVA_OS_HOME/StopQuantLab.command`。

报告目录：`~/Downloads/量化回测分析`。

快速生成样例报告：`$EVA_OS_HOME/scripts/createSampleReport.sh`。

日常检查：`$EVA_OS_HOME/scripts/dailyCheck.sh`。

macOS 统一验收入口：`$EVA_OS_HOME/scripts/macosAcceptance.sh`。

日常开发轻量检查：`$EVA_OS_HOME/scripts/devReadyCheck.sh --summary-json`。

macOS app 入口轻量验收：`$EVA_OS_HOME/scripts/macosAppAcceptanceLite.sh --summary-json`。

macOS 生命周期只读验收：`$EVA_OS_HOME/scripts/macosLifecycleReadiness.sh --summary-json`。

macOS 受控运行验收：`$EVA_OS_HOME/scripts/macosRuntimeAcceptance.sh --summary-json`。

macOS UI 可见性验收：`$EVA_OS_HOME/scripts/uiVisualAcceptance.sh --summary-json`。

macOS 公开验收摘要：`$EVA_OS_HOME/scripts/macosPublicAcceptanceSummary.sh`。
...
政策雷达快照：`$EVA_OS_HOME/scripts/policyRadar.sh --output-dir data/policy`。

消费守卫快照：`$EVA_OS_HOME/scripts/consumptionGuard.sh --output-dir data/consumption`。

总控报告：`$EVA_OS_HOME/scripts/commandCenter.sh --output-dir data/commandCenter`。

行情事件日志：`$EVA_OS_HOME/scripts/marketEventLayer.sh --output-dir data/marketEvents`。

数据湖 Manifest：`$EVA_OS_HOME/scripts/dataLakeManifest.sh --output-dir data/dataLake`。
...
报告验证工作台：`$EVA_OS_HOME/scripts/reportValidation.sh`。

报告证据索引高级产物：`$EVA_OS_HOME/scripts/reportDecisionSupport.sh --output-dir data/reportDecision`。

报告补证据任务预览：`$EVA_OS_HOME/scripts/reportGapTasks.sh --dry-run --output-dir data/reportDecision`。

报告补证据任务入队：`$EVA_OS_HOME/scripts/reportGapTasks.sh --output-dir data/reportDecision`。

验证任务优先级计划：`$EVA_OS_HOME/scripts/validationPriorityPlan.sh --output-dir data/validationQueue`。

执行最高优先级验证任务：`$EVA_OS_HOME/scripts/runValidationTask.sh --output-dir data/validationQueue`。

最终成品验收：`EVA_OS_ALLOW_HEAVY_SMOKE=1 $EVA_OS_HOME/scripts/finalAcceptanceCheck.sh`。仅在明确发布闸门或完整验收时运行；日常 agent 工作不要运行。

只读证据审计：`PYTHONPATH=src .venv/bin/python -m quantlab.examples.data_trust_audit --output-dir /private/tmp/quantlab-data-trust`。

联网验证：`$EVA_OS_HOME/scripts/dailyCheck.sh --network`。

Moomoo 只读行情诊断：`$EVA_OS_HOME/scripts/checkMoomoo.sh`。

持仓簿：`$EVA_OS_HOME/data/holdings/HoldingsBook.json`。

持仓导入目录：`$EVA_OS_HOME/data/holdings/imports`。

统一研究数据总线：`$EVA_OS_HOME/scripts/syncResearchBus.sh --json`。

独立验证系统：`$EVA_OS_HOME/scripts/runIndependentValidation.sh run --synthetic-rows 1000000000 --rows-per-shard 100000000 --json`。
