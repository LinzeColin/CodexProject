...
from quantlab.research.validation_queue import VALIDATION_QUEUE_PATH, create_validation_task
...
def _classified_gaps(text: str) -> list[str]:
    ...
    for item in parts:
        lowered = item.lower()
        if "quantlabreportevidencev1" in lowered:
...
def _signal_for_gap(gap: str) -> str:
    return {
        "ReportEvidence": "重新导出包含 QuantLabReportEvidenceV1 的 Word 报告和 RunMetadata。",
