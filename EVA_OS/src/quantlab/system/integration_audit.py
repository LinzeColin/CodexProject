...
from quantlab.integrations.research_bus import RESEARCH_BUS_DB_PATH
from quantlab.integrations.research_bus_audit import run_research_bus_interop_audit
from quantlab.storage import atomic_write_json, atomic_write_text
from quantlab.system.data_trust import build_data_trust_audit


@dataclass(frozen=True)
class QuantLabIntegrationAuditItem:
    layer: str
    status: str
    summary: str
...
def build_quantlab_integration_audit(
    *,
    as_of: str | None = None,
    ...
    return {
        ...
        "assumptions": [
            "The integration audit is read-only unless write_quantlab_integration_audit is called.",
...
def write_quantlab_integration_audit(
    *,
    as_of: str | None = None,
    project_root: Path | str = PROJECT_ROOT,
    report_root: Path | str = REPORT_ROOT_DIR,
    ...
    json_path = target / f"QuantLabIntegrationAudit_{stamp}.json"
    markdown_path = target / f"QuantLabIntegrationAudit_{stamp}.md"
...
def _data_trust_item(payload: dict[str, Any]) -> QuantLabIntegrationAuditItem:
    status = str(payload.get("audit_status") or "")
    mapped = "Pass" if status == "Pass" else "Fail" if status == "Blocked" else "Review"
    return QuantLabIntegrationAuditItem(
        layer="DataTrust",
        status=mapped,
...
def _entity_registry_item(root: Path) -> QuantLabIntegrationAuditItem:
    ...
    schema_ok = payload.get("schema") == "QuantLabEntityRegistryV1"
...
def _workflow_inputs_item(db_path: Path) -> QuantLabIntegrationAuditItem:
...
def _report_evidence_item(report_root: Path) -> QuantLabIntegrationAuditItem:
    ...
    for path in metadata_files:
        ...
        if isinstance(evidence, dict) and evidence.get("schema") == "QuantLabReportEvidenceV1":
            evidence_count += 1
    if unreadable:
        return QuantLabIntegrationAuditItem(
            layer="ReportEvidence",
    ...
    return QuantLabIntegrationAuditItem(
        ...
        next_action="旧报告可保留；新报告必须包含 QuantLabReportEvidenceV1。" if status == "Pass" else "重新导出报告以写入 report_evidence。",
    )


def _research_bus_item(payload: dict[str, Any]) -> QuantLabIntegrationAuditItem:
...
def _no_live_trading_item(root: Path) -> QuantLabIntegrationAuditItem:
