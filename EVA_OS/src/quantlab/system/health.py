...
from quantlab.config import PROJECT_ROOT, REPORT_ROOT_DIR, get_env_value
from quantlab.system.eva_identity import app_bundle_paths
...
def collect_health_checks(project_root: Path | None = None, report_root: Path | None = None) -> list[HealthCheck]:
    ...
    return [
        ...
        _path_check("桌面 EVA_OS 入口", "Desktop EVA_OS Launcher", app_paths["desktop"], must_exist=True),
        _path_check("下载目录 EVA_OS 入口", "Downloads EVA_OS Launcher", app_paths["downloads"], must_exist=True),
        _path_check("Applications EVA_OS 入口", "Applications EVA_OS Launcher", app_paths["applications"], must_exist=True),
        _path_check("项目内部启动文件", "Internal Start Launcher", root / "StartQuantLab.command", must_exist=True),
        _path_check("双击停止文件", "Double-Click Stopper", root / "StopQuantLab.command", must_exist=True),
        _path_check("启动脚本", "Start Script", root / "scripts" / "startQuantLab.sh", must_exist=True),
        _path_check("停止脚本", "Stop Script", root / "scripts" / "stopQuantLab.sh", must_exist=True),
        _path_check("状态脚本", "Status Script", root / "scripts" / "statusQuantLab.sh", must_exist=True),
        _path_check("验收脚本", "Verification Script", root / "scripts" / "verifyQuantLab.sh", must_exist=True),
