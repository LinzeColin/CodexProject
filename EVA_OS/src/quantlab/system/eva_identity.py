...
MASTER_SYSTEM_ID = "EVA_OS"
MASTER_DISPLAY_NAME = "EVA_OS"
...
MASTER_EXPANSION = "Evidence-Value-Automation Operating System"
...
EVA_FOUNDATION_LAYERS: tuple[EvaFoundationLayer, ...] = (
    EvaFoundationLayer("evidence", "Evidence Layer", "证据层", "所有输入必须进入证据层。"),
...
def eva_manifest() -> dict[str, object]:
    return {
        ...
        "entry_system": "QuantLab",
        "research_only": True,
        "no_live_trading": True,
        "subsystems": [asdict(item) for item in EVA_SUBSYSTEMS],
        "foundation_layers": [asdict(item) for item in EVA_FOUNDATION_LAYERS],
        "rules": [
            "所有输入必须进入证据层。",
            "所有结论必须经过风控层。",
            "所有系统必须经过 Codex 工程层。",
