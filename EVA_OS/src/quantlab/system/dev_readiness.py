...
DEV_READY_CHECK_SCHEMA = "EVAOSDevReadyCheckV1"
...
def _runtime_status(root: Path, *, run_status_script: bool) -> dict[str, Any]:
    ...
    status = "Running" if "QuantLab running:" in stdout else "Stopped"
...
def _cache_preview_check(payload: dict[str, Any]) -> dict[str, Any]:
    ok = payload.get("schema") == "EVACacheCleanupReportV1" and payload.get("failed_count", 0) == 0
    evidence = (
