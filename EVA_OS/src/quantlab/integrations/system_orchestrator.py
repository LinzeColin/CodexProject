...
from quantlab.config import DATA_DIR, PROJECT_ROOT
from quantlab.integrations.permissions import assert_system_permission
from quantlab.integrations.research_bus import initialize_research_bus, research_bus_db_path
from quantlab.integrations.workspace_systems import WorkspaceSystemSummary, load_workspace_system_summaries
from quantlab.storage import atomic_write_text
...
def _workspace_root() -> Path:
    return _configured_path("EVA_WORKSPACE_ROOT", DEFAULT_WORKSPACE_ROOT)


def _systems_root() -> Path:
    return _configured_path("EVA_SYSTEMS_ROOT", _workspace_root() / "systems")


def _report_root() -> Path:
    return _configured_path("EVA_REPORT_ROOT", _workspace_root() / "reports")
...
def default_child_systems() -> tuple[ChildSystemDefinition, ...]:
    fifa_root = _configured_path("EVA_FIFA_ROOT", _systems_root() / "fifa_research")
    fifa_output_root = _configured_path("EVA_FIFA_OUTPUT_ROOT", _report_root() / "fifa_research")
    government_policy_root = _configured_path("EVA_GOVERNMENT_POLICY_ROOT", _systems_root() / "policy_intelligence")
    ai_research_root = _configured_path("EVA_AI_RESEARCH_ROOT", _systems_root() / "industry_research")
    legacy_systems = (
        ChildSystemDefinition(
            system_name="QuantLab",
            role="MotherSystem",
            root_path=str(PROJECT_ROOT),
            standalone_command=("./scripts/startQuantLab.sh",),
            health_command=("./scripts/statusQuantLab.sh",),
        ...
        ChildSystemDefinition(
            ...
            capabilities=("industry_research", "report_bridge", "validation_task_publish", "quantlab_result_pull"),
            output_globs=("data/report_artifacts/research_bus_bridge/*.json", "data/report_artifacts/quantlab_bridge/*.json"),
    ...
    *,
    ...
    with _connect(target_db) as conn:
        for system in systems:
            ...
            registered += 1
        _record_event(conn, "system_registry_sync", "QuantLab", "ResearchBus", "success", f"registered_systems={registered}", {"warnings": warnings})
...
def sync_child_system_artifacts(
    ...
    *,
    ...
    with _connect(target_db) as conn:
        ...
        _record_event(conn, "system_artifacts_sync", "QuantLab", "ResearchBus", "success", f"system_artifacts={artifact_count}", {"warnings": warnings})
...
def _assert_execution_allowed(
    ...
    *,
    ...
    approval_id = str(approval_token or os.environ.get("EVA_ORCHESTRATOR_APPROVAL_ID", "")).strip()
    ...
    if os.environ.get("EVA_ORCHESTRATOR_EXECUTE_ALLOWED", "").strip().lower() not in TRUTHY_ENV_VALUES:
        raise PermissionError(
            "Execution denied: set EVA_ORCHESTRATOR_EXECUTE_ALLOWED=1 only for an approved local run."
        )
    expected_token = os.environ.get("EVA_ORCHESTRATOR_APPROVAL_TOKEN", "").strip()
...
def _allowed_execute_callers() -> set[str]:
    raw = os.environ.get("EVA_ORCHESTRATOR_EXECUTE_CALLERS", "QuantLab,EVA_OS,ResearchBus")
...
def _upsert_system_state(conn: sqlite3.Connection, system: ChildSystemDefinition, *, status: str, now: str) -> None:
    summary = {"role": system.role, "capabilities": list(system.capabilities), "orchestrated_by": "QuantLab"}
    if system.payload:
