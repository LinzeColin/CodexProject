...
DEFAULT_QUANTLAB_HOLDINGS_DIRS = (
    PROJECT_ROOT / "data" / "holdings",
    PROJECT_ROOT / "data" / "external" / "quantlabHoldings",
)
...
def load_holdings_frame(
    ...
    quantlab_dirs: list[str | Path] | tuple[str | Path, ...] | None = None,
) -> pd.DataFrame:
    rows = []
    rows.extend(_load_holding_sources(_configured_dirs("QUANTLAB_CONSUMER_HOLDINGS_DIR", consumer_dirs or DEFAULT_CONSUMER_HOLDINGS_DIRS), "消费行为分析系统"))
    rows.extend(_load_holding_sources(_configured_dirs("QUANTLAB_HOLDINGS_DIR", quantlab_dirs or DEFAULT_QUANTLAB_HOLDINGS_DIRS), "量化回测系统"))
...
def external_system_status(
    ...
    for path in _configured_dirs("QUANTLAB_CONSUMER_HOLDINGS_DIR", consumer_dirs or DEFAULT_CONSUMER_HOLDINGS_DIRS):
        rows.append(_status_row("消费行为分析系统持仓", path, _holding_path_status(path), "CSV/XLSX/JSON 持仓数据"))
    for path in _configured_dirs("QUANTLAB_HOLDINGS_DIR", quantlab_dirs or DEFAULT_QUANTLAB_HOLDINGS_DIRS):
...
def _industry_report_root(root: Path | str | None = None) -> Path:
    configured = os.getenv("QUANTLAB_INDUSTRY_REPORT_DIR", "").strip()
    if root is not None:
...
def _profile_risks(summary: dict[str, Any], runs: pd.DataFrame, reviews: pd.DataFrame, validation_tasks: pd.DataFrame) -> list[dict[str, str]]:
    ...
    if summary["holding_count"] <= 0:
        risks.append({"风险": "持仓数据缺失", "说明": "未读取到消费行为分析系统或 QuantLab 持仓数据，无法判断真实组合集中度。"})
...
def _profile_suggestions(summary: dict[str, Any], runs: pd.DataFrame, reviews: pd.DataFrame, validation_tasks: pd.DataFrame) -> list[dict[str, str]]:
    suggestions = []
    if summary["holding_count"] <= 0:
        suggestions.append({"建议": "配置持仓数据", "行动": "把消费行为分析系统或 QuantLab 持仓 CSV/XLSX/JSON 放入 data/external/consumerHoldings 或 data/holdings。"})
