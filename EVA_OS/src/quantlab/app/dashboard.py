...
def vectorized_research_shell_summary(payload: dict | None) -> dict:
    if not payload or payload.get("schema") != "EVAOSVectorizedResearchBatchV1":
        return {
...
def macos_runtime_evidence_summary(payload: dict | None) -> dict:
    if not payload or payload.get("schema") != "EVAOSMacOSRuntimeAcceptanceV1":
        return {
            ...
            "commands": [
                ...
                "scripts/macosRuntimeAcceptance.sh --launch-method app --app-path ~/Downloads/EVA_OS.app --summary-json",
            ],
    ...
    *,
    ...
    actions = [
        ...
        _lifecycle_action(
            ...
            True,
            "只读检查当前 8501-8510 端口和本地 QuantLab 进程。",
            "Read-only port and local QuantLab process check.",
        ),
        _lifecycle_action(
            "开发检查",
            "Dev Ready Check",
        ...
        _lifecycle_action(
            ...
            not is_running,
            "从终端或 EVA_OS.app 启动；当前页面内不自启动新服务，避免重复进程。",
            "Start from Terminal or EVA_OS.app; the running page does not start another service.",
        ...
        _lifecycle_action(
            ...
            "scripts/macosRuntimeAcceptance.sh --launch-method app --app-path ~/Downloads/EVA_OS.app --summary-json",
            not is_running,
            "真实打开 Downloads 的 EVA_OS.app，验证 health、缓存保护和停止闭环；可能打开默认浏览器，必须在 Terminal 运行。",
            "Opens the Downloads EVA_OS.app, then verifies health, cache guard, and stop loop; may open the default browser, run from Terminal.",
            disabled_reason="服务已在运行，避免误停当前会话" if is_running else "",
        ),
        _lifecycle_action(
            "最终验收",
    ...
    return {
        ...
        "actions": actions,
        "safety_policy": "Lifecycle controls only manage the local EVA_OS/QuantLab app process and caches; they do not refresh market data, place orders, or send broker instructions.",
    }
...
def quick_actions() -> list[QuickAction]:
    return [
        QuickAction(
            ...
            "先看 EVA_OS 是否可继续研究：就绪检查、集成审计、Token ROI、最新报告和行动队列。",
            "Check whether EVA_OS is ready for research: readiness, integration audit, Token ROI, latest report, and action queue.",
            "总控驾驶舱",
            "Executive Command Center",
            "command",
        ),
        ...
        QuickAction(
            "同步当前持仓",
            "Sync Holdings",
            "读取行研、消费分析、支付宝账本和 QuantLab 导入文件，形成正式持仓簿。",
            "Read industry research, consumer analytics, Alipay ledger, and QuantLab import files into a canonical holdings book.",
            "持仓",
            "Holdings",
            "holdings",
        ),
...
def command_center_next_actions(payload: dict | None = None) -> dict:
    ...
    return {
        "schema": "EVAOSCommandCenterActionRouterV1",
...
def usage_guide_sections() -> list[UsageGuideSection]:
    return [
        UsageGuideSection(
            title="总控驾驶舱",
            target_key="command",
            purpose="作为 EVA_OS 日常入口，集中判断当前系统能否继续研究，哪些证据缺失，哪些事项必须先处理。",
            best_for="每天第一次打开系统、准备生成报告、准备使用回测结论前，先做一次总控复核。",
            steps=(
                "先看总控状态。可继续研究表示核心证据闭合；需要复核表示存在缺失证据；阻断表示不应继续使用下游结论。",
                "查看核心状态表，确认 Daily Readiness、Integration Audit、Risk Gates、Token ROI Ledger 和 Latest Report 是否通过。",
                "查看行动队列，优先处理 P0，再处理 P1，P2 通常是价值量化或长期完善事项。",
                "查看证据来源，确认 Daily Readiness、Integration Audit、Data Trust、Token ROI 和最新报告是否都有实际文件路径。",
                ...
                "生成总控报告后复跑 Token ROI Ledger，可把新产物纳入价值台账。",
            ),
            checks=(
                "总控状态是否为可继续研究；如果不是，先不要把研究结论用于交易前参考。",
                "行动队列是否存在 P0；P0 必须优先处理。",
                "证据来源是否缺失；缺失时说明结论不可追溯。",
                "Token ROI 是否只登记事实产物，没有伪造收益或节省成本。",
            ),
            outputs=(
                "页面总览：总控状态、待处理事项、Token ROI 记录和最新报告。",
                "正式产物：`data/commandCenter/EVACommandCenter_DDMMYYYY.json/md/pdf`。",
                "最新指针：`data/commandCenter/EVACommandCenter_latest.json/md/pdf`。",
                "可被 Token ROI Ledger 登记的系统可靠性证据。",
            ),
            risks=(
        ...
        UsageGuideSection(
            title="Token ROI",
            target_key="token_roi",
            purpose="把 AI token、人工时间、可复用产物和真实经济价值放入同一套可复核台账，判断系统投入是否产生可解释回报。",
            best_for="完成一次系统升级、报告生成、研究验证或自动化优化后，登记证据并区分待复核价值和已确认价值。",
            steps=(
                "先看台账记录、已量化、未量化、确认收益/节省和确认成本，判断当前价值闭环是否只是产物登记还是已经有真实金额。",
                "打开公式与边界，确认本页只做价值复核，不自动推断收益、不连接实盘、不付款。",
                "在人工价值录入区填写运行日期、子系统、任务目标和证据标题，先把这次 run 的目的记录清楚。",
                "填写证据链接或本地产物路径，确保后续能回到报告、JSON、CSV、截图或源码变更复核。",
                "如果金额尚未确认，保持 PendingReview；如果有真实收入、节省成本、避免损失或资产复用价值，再改为 Reviewed。",
                "填写 AI 成本、人工时间成本、token 估算、节省时间和复用次数，用同一口径比较不同 run 的投入产出。",
                "保存价值证据后点击生成 Token ROI 台账，输出 latest 文件供总控驾驶舱和后续复盘读取。",
            ),
            checks=(
                "Reviewed 是否真的有证据链接或可复核本地产物。",
                "金额是否来自真实账单、节省时间、可比外包成本或明确业务收益，而不是主观感觉。",
                "PendingReview 是否没有进入已量化汇总，避免污染 Token ROI。",
                "ROI 为负时是否只是成本记录，还是说明该类 run 应减少或合并。",
            ),
            outputs=(
                "人工价值证据：`data/value/TokenROIManualEntries.json`。",
                "正式台账：`data/value/EVATokenROILedger_DDMMYYYY.json/csv/md/pdf`。",
                "最新指针：`data/value/EVATokenROILedger_latest.json/csv/md/pdf`。",
                "总控驾驶舱可读取的 Token ROI 汇总。",
            ),
            risks=(
                "没有证据的金额只能保留为 PendingReview，不能算作已量化收益。",
                "节省时间和资产复用价值需要统一估值口径，否则跨 run 比较会失真。",
                "Token ROI 只衡量研究和系统产出价值，不证明策略盈利或实盘可执行性。",
            ),
        ),
        UsageGuideSection(
            title="现金流",
        ...
        UsageGuideSection(
            ...
            best_for="政策报告、政府公告、监管变化、产业支持、税务优惠、合规风险和项目申报机会进入 QuantLab 前的第一层证据整理。",
            steps=(
        ...
        UsageGuideSection(
            ...
            best_for="把行研报告系统、消费行为分析系统、支付宝账本和 QuantLab 本地导入的持仓放到同一持仓簿。",
            steps=(
                ...
                "打开同步来源，确认支付宝账本、行研上传、消费分析系统和 QuantLab 导入目录是否 Ready。",
        ...
        UsageGuideSection(
            title="行研报告",
            target_key="industry",
            purpose="按日期检索本地行研报告，并把报告、验证任务、QuantLab 研究产物和回测元数据放到同一视图。",
            best_for="从盘前、盘中、盘后、K 线分析或行业研究报告出发，形成待验证问题并联动量化验证。",
            steps=(
                "确认行研报告目录。默认目录是当前用户 `~/Downloads/行研报告`，也可用 `QUANTLAB_INDUSTRY_REPORT_DIR` 覆盖。",
                "选择开始日期和结束日期，按报告日期筛选。",
                "输入关键词，按报告名称、类型、目录或路径搜索。",
                "在报告表格中查看报告日期、类型、区间目录、大小和路径。",
                "选择报告后可以打开报告或打开所在目录。",
                "查看最近 QuantLab Word 报告，把行研结论与回测验证报告对应起来。",
                "查看验证任务队列，把行研中的假设拆成待验证信号。",
            ),
            checks=(
                "报告日期是否与研究日期一致。",
                "报告类型是否符合本次研究目的，例如盘前、盘中、盘后或 K 线。",
                "是否已有对应 QuantLab 验证报告。",
                "验证任务是否记录了样本区间、成本假设和基准。",
            ),
            outputs=(
                "按日期筛选后的行研报告列表。",
                "行研报告路径和打开入口。",
                "最近 QuantLab 报告和验证任务联动表。",
            ),
            risks=(
        ...
        UsageGuideSection(
            ...
            purpose="统一监控 QuantLab、行研系统、消费行为系统、持仓主数据和独立验证系统的输入、输出、请求、心跳和同步状态。",
            ...
            steps=(
                ...
                "点击同步一次，串行同步行研报告、QuantLab 回测结果、持仓主数据和消费行为系统状态。",
                "打开系统状态页，确认 QuantLab、AI-Research-System、ConsumptionAnalysisSystem、HoldingsMaster 和 ResearchBus 都是 Ready。",
        ...
        UsageGuideSection(
            title="大数据模拟",
            target_key="big_data",
            purpose="调用独立验证系统生成大规模数据测试计划、分片记录和 checksum 校验摘要，确认 QuantLab、行研系统和自动化入口能共享同一验证结果。",
            ...
            outputs=(
                ...
                "可被 QuantLab、AI-Research-System、automation 和其他 agent 读取的 ResearchBus 共享状态。",
            ),
            risks=(
        ...
        UsageGuideSection(
            title="个人画像",
            target_key="profile",
            purpose="综合消费行为分析系统持仓、QuantLab 持仓、回测记录、复盘记录和验证任务，形成行为习惯、风险和优化方向。",
            best_for="日常复盘自己的研究纪律、风险暴露、证据习惯、情绪偏差和待验证积压。",
            steps=(
                "先看顶部持仓总市值、持仓数量、最大单一权重、前三权重和 HHI。",
                "打开数据连接状态，确认消费行为分析系统和 QuantLab 持仓数据是否 Ready。",
...
def _ordered_close_series(data: pd.DataFrame) -> pd.Series:
    ...
    if "datetime" in frame.columns:
        frame = frame.assign(_quantlab_datetime=pd.to_datetime(frame["datetime"], errors="coerce"))
        frame = frame.sort_values("_quantlab_datetime", kind="mergesort")
