    ...
    class _MissingStreamlit:
        ...
        def __getattr__(self, name):
            raise ModuleNotFoundError(
                "streamlit is required for the EVA_OS UI runtime. "
...
from quantlab.backtest import BacktestConfig, BacktestEngine, PortfolioBacktestEngine
...
from quantlab.integrations.workspace_systems import compact_workspace_system_payload
...
from quantlab.research.experiments import analyze_parameter_stability
from quantlab.reports.catalog import (
    WORD_REPORT_TYPES,
    artifact_counts,
    cleanup_report_junk,
...
from quantlab.system import MASTER_SHORT_TITLE, MASTER_DISPLAY_NAME, collect_health_checks
from quantlab.system.cache_cleanup import build_cache_cleanup_report
from quantlab.system.report_validation_hub import build_report_validation_hub, report_validation_hub_summary
...
TERM_HELP = {
    ...
    "52ETF热点对照": "把 52ETF 公开 A 股云图板块与 EVA_OS 当前热点对象做只读覆盖对照，用于检查 UI 和对象池，不作为交易信号。",
...
def hotspot_quick_preflight(
    *,
    ...
    return {
        "schema": "EVAOSHotspotQuickPreflightV1",
        "status": status,
        "label": label,
...
RESEARCH_CHART_MODEBAR_CONFIG = {
    ...
    "toImageButtonOptions": {
        "format": "png",
        "filename": "eva_os_research_chart",
        "height": 720,
        "width": 1280,
        "scale": 2,
    },
...
def research_chart_config(filename: str = "eva_os_research_chart") -> dict[str, object]:
...
def apply_research_chart_ux(
    ...
    *,
    ...
    fig.update_layout(
        ...
        hovermode=hovermode,
        dragmode=dragmode,
        uirevision="eva_os_research_chart_v1",
...
def parameter_scan_preflight(
    *,
    ...
    return {
        "schema": "EVAOSParameterScanPreflightV1",
        "status": status,
        "label": label,
...
def main() -> None:
    st.set_page_config(page_title=MASTER_DISPLAY_NAME, page_icon="EVA", layout="wide", initial_sidebar_state="expanded")
    ...
    st.markdown(
        ...
        <div class="ql-hero">
          <div>
            ...
            <p class="ql-hero-cn">QuantLab 是主入口，统一承接量化研究、行研验证、消费持仓、独立验证和多系统证据流。</p>
          </div>
        </div>
        """,
        unsafe_allow_html=True,
...
def render_vectorized_research_panel() -> None:
    ...
    if summary["status"] == "Missing":
        ...
        if not chart_rows.empty and go is not None:
            ...
            st.plotly_chart(fig, width="stretch", config=research_chart_config("eva_os_vectorized_research"))
        if not rows.empty:
            st.dataframe(rows, width="stretch", hide_index=True)
...
LIFECYCLE_SCRIPT_ALLOWLIST = {
    "scripts/statusQuantLab.sh": 8,
    "scripts/stopQuantLab.sh": 20,
...
def render_macos_lifecycle_panel() -> None:
    runtime = _run_lifecycle_script("scripts/statusQuantLab.sh")
    is_running = "QuantLab running:" in runtime["stdout"]
    ...
    if control_cols[0].button("刷新状态", key="macos_lifecycle_status"):
        refreshed = _run_lifecycle_script("scripts/statusQuantLab.sh")
    ...
    if control_cols[1].button("停止服务", key="macos_lifecycle_stop", disabled=not (is_running and confirm_stop)):
        result = _run_lifecycle_script("scripts/stopQuantLab.sh")
        st.code(_command_output(result), language="text")
        st.warning("如果当前页面断开，说明本地 Streamlit 服务已停止。重新打开 EVA_OS.app 即可启动。")
...
def single_backtest_view() -> None:
    ...
    with col_body:
        ...
        show_tables(result)
        from quantlab.reports import export_backtest_docx
...
def industry_research_view() -> None:
        ...
        try:
            ...
            st.session_state["research_bus_sync_message"] = {
                ...
                "text": (
                    ...
                    f"QuantLab 结果 {result.quantlab_results} 条，持仓 {result.holdings} 条。"
                ),
    ...
    if reports.empty:
        st.warning("未找到行研报告。默认目录为当前用户 `~/Downloads/行研报告`；也可以通过环境变量 `QUANTLAB_INDUSTRY_REPORT_DIR` 指定。")
...
def research_bus_monitor_view() -> None:
    st.subheader("研究总线")
    st.caption("集中查看 QuantLab、行研系统、消费行为系统、持仓主数据和独立验证系统的互通状态。")
    ...
    if action_cols[2].button("同步一次"):
        try:
            result = sync_all_research_bus(push_validation_queue=True)
            st.success(f"同步完成：报告 {result.reports}，验证任务 {result.validation_tasks}，回测结果 {result.quantlab_results}，持仓 {result.holdings}。")
...
def holdings_view() -> None:
    st.subheader("持仓")
    st.caption("读取、保存并同步 QuantLab、行研报告系统、消费行为分析系统和支付宝账本中的持仓数据；待确认订单不会计入正式持仓。")
    ...
    with tab_current:
        if holdings.empty:
            ...
            st.code(
                ...
                "~/Downloads/行研报告，或通过 QUANTLAB_INDUSTRY_REPORT_DIR 指定",
                language="text",
            )
        else:
...
def render_site52etf_public_reference() -> dict[str, object]:
    ...
    if snapshot.get("status") == "Unavailable":
        st.warning("52ETF 公开参考暂不可用；EVA_OS 本地热点分析仍可继续运行。")
    else:
...
def render_site52etf_hotspot_comparison(snapshot: dict[str, object], history: pd.DataFrame, market: str, snapshot_time: str) -> None:
    ...
    st.markdown(f'<h4 title="{_escape_html(TERM_HELP["52ETF热点对照"])}">52ETF 与 EVA 热点对照</h4>', unsafe_allow_html=True)
    st.caption("只读对照公开 A 股云图板块和 EVA_OS 当前热点对象池；用于检查覆盖与交互口径，不进入回测、不触发交易。")
    st.dataframe(pd.DataFrame(site52etf_comparison_rows(comparison)), width="stretch", hide_index=True)
    if comparison.get("status") != "Pass":
        st.caption("当前对照为 Review：可能是 52ETF 不可用、市场不是 CN，或 EVA_OS 对象池与公开云图板块映射不足。")
...
def render_hotspot_request_trace(trace_summary: dict[str, object] | object) -> None:
    if not isinstance(trace_summary, dict) or trace_summary.get("schema") != "EVAOSHotspotRequestTraceV1":
...
def show_hotspot_timeline(history: pd.DataFrame) -> None:
    ...
    st.plotly_chart(fig, width="stretch", config=research_chart_config("eva_os_hotspot_timeline"))
...
def show_hotspot_heatmap(history: pd.DataFrame, snapshot_time: str) -> None:
    ...
    st.plotly_chart(fig, width="stretch", config=research_chart_config("eva_os_hotspot_heatmap"))
...
def show_hotspot_bubble_chart(history: pd.DataFrame, snapshot_time: str) -> None:
    ...
    st.plotly_chart(fig, width="stretch", config=research_chart_config("eva_os_hotspot_bubble"))
...
def personal_profile_view() -> None:
    ...
    if holdings.empty:
        st.warning("未读取到消费行为分析系统或 QuantLab 持仓数据。页面会先基于回测、复盘和验证任务生成画像；持仓集中度需要配置后才准确。")
    else:
        st.markdown("#### 持仓数据")
...
def show_validation_queue_panel(artifacts: pd.DataFrame) -> None:
    st.markdown("#### 验证任务队列")
    st.caption("这里承接行研、政策、新闻或手工研究问题。任务只用于安排 QuantLab 验证，不代表实盘操作。默认先看“证据索引”页的报告验证工作台，再决定是否执行高级动作。")
...
def show_report_decision_support_panel() -> None:
    from quantlab.reports import build_report_decision_support_index, write_report_decision_support_index
...
def show_experiment_detail(summary_path: str) -> None:
    ...
    if st.button("导出实验 Word 报告", key=f"export_experiment_{detail['experiment']}"):
        try:
            from quantlab.reports import export_experiment_docx
