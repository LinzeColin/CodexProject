...
from quantlab.consumption import build_consumption_guard, write_consumption_guard


def main() -> None:
    parser = argparse.ArgumentParser(description="Generate the EVA_OS Consumption Guard snapshot.")
    ...
    parser.add_argument("--summary-json", action="store_true", help="Print only EVAOSConsumptionGuardRuntimeSummaryV1 for low-token checks.")
    ...
    print(f"EVA_OS_CONSUMPTION_RUNTIME_STATUS: {payload.get('runtime_summary', {}).get('status')}")
    print(f"EVA_OS_CONSUMPTION_OUTPUTS: {payload.get('outputs', {})}")
