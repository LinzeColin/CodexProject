...
from quantlab.system import build_quantlab_integration_audit, write_quantlab_integration_audit


def main() -> None:
    parser = argparse.ArgumentParser(description="Run the read-only QuantLab integration audit.")
    parser.add_argument("--date", default=None, help="Audit date in YYYY-MM-DD format. Defaults to today.")
    parser.add_argument("--project-root", default=str(PROJECT_ROOT), help="QuantLab project root.")
    parser.add_argument("--report-root", default=str(REPORT_ROOT_DIR), help="QuantLab report root.")
    ...
    if args.no_write:
        payload = build_quantlab_integration_audit(**common)
    else:
    ...
    if args.json:
        ...
        return
    print(f"QUANTLAB_INTEGRATION_AUDIT: {payload['status']} {payload['as_of']}")
