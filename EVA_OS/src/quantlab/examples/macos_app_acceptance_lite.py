...
from quantlab.system import build_macos_app_acceptance_lite, write_macos_app_acceptance_lite


def main() -> None:
    parser = argparse.ArgumentParser(description="Run a lightweight macOS EVA_OS app acceptance check without full smoke.")
    ...
    parser.add_argument("--skip-dry-run", action="store_true", help="Skip EVA_OS_APP_LAUNCH_DRY_RUN launcher checks.")
    parser.add_argument("--skip-status-script", action="store_true", help="Skip scripts/statusQuantLab.sh execution.")
    parser.add_argument("--json", action="store_true", help="Print EVAOSMacOSAppAcceptanceLiteV1 as JSON.")
    ...
    print(
        "EVA_OS_MACOS_APP_ACCEPTANCE_LITE: "
    ...
    print(f"EVA_OS_MACOS_APP_ACCEPTANCE_NEXT_ACTION: {payload['next_action']}")
    if payload.get("outputs"):
        print(f"EVA_OS_MACOS_APP_ACCEPTANCE_OUTPUTS: {payload['outputs']}")
