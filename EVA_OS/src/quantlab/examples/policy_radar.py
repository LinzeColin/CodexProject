...
from quantlab.policy import build_policy_radar, write_policy_radar


def main() -> None:
    parser = argparse.ArgumentParser(description="Generate the EVA_OS Policy Intelligence Radar snapshot.")
    ...
    parser.add_argument("--summary-json", action="store_true", help="Print only EVAOSPolicyIntelligenceRuntimeSummaryV1 for low-token checks.")
    ...
    print(f"EVA_OS_POLICY_RUNTIME_STATUS: {payload.get('runtime_summary', {}).get('status')}")
    print(f"EVA_OS_POLICY_OUTPUTS: {payload.get('outputs', {})}")
