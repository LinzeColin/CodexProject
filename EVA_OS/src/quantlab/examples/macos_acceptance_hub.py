...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the user-friendly EVA_OS macOS acceptance hub.")
    ...
    parser.add_argument("--json", action="store_true", help="Print EVAOSMacOSAcceptanceHubV1 as JSON.")
    ...
    print(
        "EVA_OS_MACOS_ACCEPTANCE: "
        f"mode={payload['mode']} "
        f"status={payload['status']} "
        f"pass={summary['pass']} "
        f"fail={summary['fail']}"
    )
    print(f"EVA_OS_MACOS_ACCEPTANCE_NEXT_ACTION: {payload['next_action']}")
