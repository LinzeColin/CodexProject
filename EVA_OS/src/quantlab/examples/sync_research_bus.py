...
from quantlab.integrations.consumer_behavior import consumer_behavior_state_frame, sync_consumer_behavior_state
from quantlab.integrations.research_bus_audit import AUDIT_OUTPUT_PATH, run_research_bus_interop_audit


def main() -> None:
    parser = argparse.ArgumentParser(description="Sync QuantLab, AI-Research-System, and holdings into the unified research data bus.")
    ...
    parser.add_argument("--report-root", default="", help="Optional QuantLab report directory.")
    parser.add_argument("--ai-research-root", default="", help="Optional AI-Research-System root for QuantLab result outbox.")
    parser.add_argument(
        "--mode",
        choices=["all", "industry", "quantlab-results", "holdings", "consumer", "status", "snapshot", "audit"],
        default="all",
    )
    parser.add_argument("--no-push-validation-queue", action="store_true", help="Do not push generated validation tasks into QuantLab JSON queue.")
    ...
    elif args.mode == "quantlab-results":
        result = sync_quantlab_results_to_bus(report_root, db_path, ai_research_root=ai_root).to_dict()
