...
from quantlab.business import build_cashflow_command, write_cashflow_command
from quantlab.config import PROJECT_ROOT


def main() -> None:
    parser = argparse.ArgumentParser(description="Generate the EVA_OS Company CashFlow Command snapshot.")
    ...
    parser.add_argument("--summary-json", action="store_true", help="Print only EVAOSCompanyCashFlowRuntimeSummaryV1 for low-token checks.")
    ...
    print(f"EVA_OS_CASHFLOW_RUNTIME_STATUS: {payload.get('runtime_summary', {}).get('status')}")
    print(f"EVA_OS_CASHFLOW_OUTPUTS: {payload.get('outputs', {})}")
