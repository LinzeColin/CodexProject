...
from quantlab.system import run_macos_runtime_acceptance, write_macos_runtime_acceptance
...
def main() -> None:
    ...
    parser.add_argument("--launch-method", choices=["script", "app"], default="script", help="Use quiet script start or real EVA_OS.app open.")
    parser.add_argument("--app-path", default=None, help="Optional EVA_OS.app path when --launch-method app is used.")
    parser.add_argument(
        "--allow-existing-service",
        action="store_true",
        help="Allow using and stopping a pre-existing local service. Default is fail-closed.",
    )
    parser.add_argument("--json", action="store_true", help="Print EVAOSMacOSRuntimeAcceptanceV1 as JSON.")
    ...
    print(
        "EVA_OS_MACOS_RUNTIME_ACCEPTANCE: "
    ...
    print(f"EVA_OS_MACOS_RUNTIME_NEXT_ACTION: {payload['next_action']}")
    if payload.get("outputs"):
        print(f"EVA_OS_MACOS_RUNTIME_OUTPUTS: {payload['outputs']}")
