...
## 一句话结论
EVA 经营与估值操作系统，当前重点是把产品状态、模型证据、运行入口和下一步验收放在首屏，降低业务 Owner 的理解成本。
...
## 证据与验证
- active_model_count: `16`
- active_formula_count: `16`
- active_parameter_count: `189`
- 总模型数：`16`；总公式数：`16`；总参数数：`189`。
...
- 最小验证路径：进入 `EVA_OS/`，运行 `python -m pytest tests/test_eva_identity.py -q`。
...
QuantLab is a personal platform for quantitative research, analysis, backtesting, and experiment tracking.
...
Machine sources of truth live under `docs/governance/` for model, formula, parameter, delivery, development, version and traceability records.
...
```text
$EVA_OS_HOME/docs/QuickStart.md
```
...
```bash
$EVA_OS_HOME/scripts/statusQuantLab.sh
$EVA_OS_HOME/scripts/stopQuantLab.sh
```

总控报告：

```bash
$EVA_OS_HOME/scripts/commandCenter.sh --output-dir data/commandCenter
```

行情事件日志：

```bash
$EVA_OS_HOME/scripts/marketEventLayer.sh --output-dir data/marketEvents
```

数据湖 Manifest：

```bash
$EVA_OS_HOME/scripts/dataLakeManifest.sh --output-dir data/dataLake
```
...
```bash
$EVA_OS_HOME/scripts/cashFlowCommand.sh --output-dir data/cashflow
```
...
```bash
$EVA_OS_HOME/scripts/cashFlowCommand.sh --summary-json
```

政策雷达快照：

```bash
$EVA_OS_HOME/scripts/policyRadar.sh --output-dir data/policy
```
...
```bash
$EVA_OS_HOME/scripts/policyRadar.sh --summary-json
```

消费守卫快照：

```bash
$EVA_OS_HOME/scripts/consumptionGuard.sh --output-dir data/consumption
```
...
```bash
$EVA_OS_HOME/scripts/consumptionGuard.sh --summary-json
```

报告验证工作台。默认只读合并报告证据、补证据候选和验证优先级，低 token 输出，不入队、不执行验证：

```bash
$EVA_OS_HOME/scripts/reportValidation.sh
```

高级动作。确认要写入产物、入队或执行验证时再使用：

```bash
$EVA_OS_HOME/scripts/reportDecisionSupport.sh --output-dir data/reportDecision
```

```bash
$EVA_OS_HOME/scripts/reportGapTasks.sh --output-dir data/reportDecision
```

```bash
$EVA_OS_HOME/scripts/validationPriorityPlan.sh --output-dir data/validationQueue
```

```bash
$EVA_OS_HOME/scripts/runValidationTask.sh --output-dir data/validationQueue
```

## Integrated Research Role

QuantLab 在行研报告、政府文件解读和交易策略建议系统中只承担验证层职责。它应输出回测对象、样本区间、成本假设、参数稳定性、样本外验证、walk-forward 结果、研究风险闸门、失效环境和停用条件。

如果行研或政策系统引用某个 QuantLab 结果，必须同时引用数据质量、多源交叉校验、成本假设和风险闸门状态。验证不足时，结论只能作为观察或待验证线索，不能升级为实盘交易建议。

QuantLab 现在增加 Decision Quality Score，用来判断一次研究结论是否完整。状态分为 `ContinueResearch`、`WatchOnly`、`NeedsMoreEvidence` 和 `DoNotUse`。
...
统一研究数据总线已接入 QuantLab、行研系统、消费行为系统、独立验证系统、FIFA/TAB 研究系统和政府文件/政策解读系统。共享 SQLite 数据库位于 `data/researchBus/ResearchBus.sqlite`，JSON 快照位于 `data/researchBus/ResearchBusSnapshot.json`。它会同步行研报告索引、行研正文拆出的待验证任务、QuantLab 回测结果、统一持仓主数据、消费行为系统内部状态、独立验证运行状态、子系统注册表和子系统产物索引。

The unified research data bus now connects QuantLab, AI-Research-System, the independent validation system, FIFA/TAB research, and the government policy interpretation system. The shared SQLite database is `data/researchBus/ResearchBus.sqlite`, and the JSON snapshot is `data/researchBus/ResearchBusSnapshot.json`.

共享 schema 合约保存在 `docs/ResearchBusSchema.json`。后续新聊天、其他 agent 或 automation 增删字段时，应先对齐该 schema，再修改 QuantLab 和行研系统两侧的 bridge。

The shared schema contract is `docs/ResearchBusSchema.json`.

跨系统互通审计可用 `scripts/auditResearchBusInterop.sh --json` 一键检查。审计结果保存到 `data/researchBus/ResearchBusInteropAudit.json`，覆盖共享 schema、共享 SQLite、系统注册表、子系统产物索引、双向 API、任意聊天框输入、行研报告解析、QuantLab 结果回写、消费行为同步、统一持仓、独立验证入口、独立验证两级架构、系统心跳、行研桥接输出和行研 automation 保留状态。
...
只读证据审计可用 `PYTHONPATH=src .venv/bin/python -m quantlab.examples.data_trust_audit --output-dir /private/tmp/quantlab-data-trust` 生成。它会检查项目控制文件、数据源 Provider、策略库、持仓、ResearchBus、验证任务、独立验证、参数实验和报告目录，把文件分为 `RECONCILED`、`NEEDS_REVIEW`、`PARSED_CANDIDATE`、`REJECTED` 等状态，避免把候选数据、损坏文件或不完整实验误当成可用研究结论。

The read-only Data Trust audit is available through `quantlab.examples.data_trust_audit`; it classifies project controls, providers, strategies, holdings, ResearchBus, validation tasks, independent validation, experiments, and reports into evidence statuses.
...
母系统编排入口是 `scripts/orchestrateSystems.sh`。QuantLab 作为母系统登记每个子系统的根目录、独立启动命令、健康检查命令、同步命令、能力和输出产物；子系统仍可直接运行自己的脚本，不依赖 QuantLab 页面。FIFA/TAB 系统默认使用 `scripts/run_tab_fifa_daily_automation.sh` 和 `scripts/verify_fifa_automation_readiness.sh`；政府文件/政策系统默认使用 `scripts/run_policy_report.sh` 和 `python3 -m source_registry ... status --json`。
...
行研系统通过 `research-bus-sync` 写入报告索引并拉取 QuantLab 结果；QuantLab 通过 `scripts/syncResearchBus.sh` 读取共享库、批量推送验证任务并把回测结论回写给行研系统。独立验证系统通过 `scripts/runIndependentValidation.sh` 生成 manifest 和分片运行记录，支持百亿级数据量的 dry-run 调度登记，也支持 `checksum` 模式逐片流式校验和本机 worker pool 并行校验。

The AI research system uses `research-bus-sync`; QuantLab uses `scripts/syncResearchBus.sh`; independent validation uses `scripts/runIndependentValidation.sh`.
...
```bash
bash $EVA_OS_HOME/scripts/auditQuantLabIntegration.sh --no-write
```
...
通用聊天投递箱位于 `data/researchBus/chatInbox`。把 `.txt`、`.md` 或 `.json` 文件放入该目录后，`process-dropbox` 或后台同步脚本会自动写入 ResearchBus，成功文件移到 `processed`，失败文件移到 `failed` 并生成错误记录。这个入口适合任何聊天框、快捷指令或自动化系统把文本投递给 QuantLab/行研系统。
...
功能导航和 `使用指导` 已统一放到左侧侧栏，方便边看步骤、检查点和术语说明边操作。`总控驾驶舱` 另有 `统一下一步`，用只读路由把热点快速预检、参数扫描预检、报告验证和 macOS 日常验收聚合成少数入口，减少在多个页面之间试错。侧栏会显示当前功能区的用途、适用场景、最短操作路径、优先检查点、产出、风险和常用术语悬停解释。功能区包含 `情绪分析`、`热点分析`、`盘感训练`、`持仓`、`行研报告` 和 `个人画像`。行研报告默认读取 `~/Downloads/行研报告`，个人画像会优先读取 QuantLab 持仓簿，再综合回测元数据、复盘记录和验证任务生成行为习惯、风险画像和优化方向。

Navigation and `使用指导` now live in the sidebar so you can read steps, checkpoints, and term help while operating the page. `总控驾驶舱` also includes `统一下一步`, a read-only action router that consolidates hotspot preflight, parameter-scan preflight, report validation, and macOS daily acceptance into a few next-step links. Workspace areas include `情绪分析`, `热点分析`, `盘感训练`, `持仓`, `行研报告`, and `个人画像`. Industry reports default to `~/Downloads/行研报告`; the personal profile first reads the QuantLab holdings book, then combines run metadata, review records, and validation tasks into behavior habits, risk profile, and improvement actions.
...
持仓页面会同步支付宝持仓账本、行研报告上传目录、消费行为分析系统目录和 QuantLab 本地导入目录。正式持仓永久保存到 `$EVA_OS_HOME/data/holdings/HoldingsBook.json`，同步历史保存到 `HoldingsImportHistory.json`，待确认订单单独显示，不计入正式持仓。

The holdings page syncs the Alipay ledger, industry-report upload directories, consumer-analysis directories, and QuantLab local import directories. Confirmed holdings are permanently saved to `$EVA_OS_HOME/data/holdings/HoldingsBook.json`; sync history is saved to `HoldingsImportHistory.json`; pending orders are shown separately and are not counted as confirmed holdings.

外部支付宝账本和跨系统私有目录必须通过 `.env` 显式配置 `QUANTLAB_ALIPAY_LEDGER_DIR`。默认只扫描项目内 `data/private/alipay`，且正式持仓文件、导入目录和私有目录已加入 `.gitignore`。

External Alipay ledgers and cross-system private directories must be explicitly configured with `QUANTLAB_ALIPAY_LEDGER_DIR` in `.env`. By default QuantLab only scans project-local `data/private/alipay`, and confirmed holdings, imports, and private folders are ignored by Git.
...
The market-feel training page explains technical structure with MA20/MA60, support/resistance, RSI, MACD, Bollinger, ATR, volatility, drawdown, and volume. It supports three levels: beginner direction judgement, intermediate direction plus return range, and expert direction plus precise return estimate. After a challenge starts, the page shows a live countdown. QuantLab hides the future answer window, locks the pre-result technical analysis first, then reveals the actual path and multi-dimensional review. It never outputs live trading instructions.
...
最快启动方式：双击 Desktop、Downloads 或 Applications 里的 `EVA_OS.app`。
...
```bash
$EVA_OS_HOME/scripts/dailyCheck.sh
```
...
```bash
$EVA_OS_HOME/scripts/dailyCheck.sh --output-dir data/systemAudit
```

输出文件包括 `QuantLabDailyReadiness_DDMMYYYY.json`、`QuantLabDailyReadiness_DDMMYYYY.md` 和 `QuantLabDailyReadiness_DDMMYYYY.pdf`。

The outputs include `QuantLabDailyReadiness_DDMMYYYY.json`, `QuantLabDailyReadiness_DDMMYYYY.md`, and `QuantLabDailyReadiness_DDMMYYYY.pdf`.
...
```bash
$EVA_OS_HOME/scripts/runIndependentValidation.sh run --synthetic-rows 1000000000 --rows-per-shard 100000000 --json
```
...
```bash
$EVA_OS_HOME/scripts/runIndependentValidation.sh run --synthetic-rows 1000000000 --rows-per-shard 100000000 --mode checksum --json
```
...
```bash
$EVA_OS_HOME/scripts/researchBusApi.sh submit-chat --text "请验证 600000 的 RSI 均线策略是否有效" --source-system ExternalChat --json
```
...
```bash
$EVA_OS_HOME/scripts/researchBusApi.sh submit-chat --text "请运行千万行独立验证 checksum 校验，每片100万行" --source-system ExternalChat --json
$EVA_OS_HOME/scripts/researchBusApi.sh process --system-name ResearchBus --limit 100 --json
```
...
```bash
$EVA_OS_HOME/scripts/researchBusWebhook.sh --port 8765
...
```bash
RESEARCH_BUS_WATCH_INTERVAL_SECONDS=30 $EVA_OS_HOME/scripts/watchResearchBus.sh
```
...
```bash
$EVA_OS_HOME/scripts/syncResearchSystemsOnce.sh
```
...
```bash
$EVA_OS_HOME/scripts/installResearchBusLaunchAgent.sh install
```
...
```bash
$EVA_OS_HOME/scripts/installResearchBusLaunchAgent.sh load
```
...
当前安装器会把 runner 写入 `~/Library/Application Support/QuantLab/researchBusSyncRunner.sh`，但共享库和项目数据仍位于当前工程目录；如果 macOS 后台权限不足，仍可能被 TCC 拦截写入 `Documents`。
...
```bash
$EVA_OS_HOME/scripts/installResearchBusLaunchAgent.sh unload
```

macOS 应用入口会自动打开浏览器。`/Applications/EVA_OS.app` 可以放入 Dock，也会出现在 Launchpad。

The macOS app launcher opens the browser automatically. `/Applications/EVA_OS.app` can be kept in the Dock and appears in Launchpad.

Moomoo 是优先真实数据入口，需要你本机启动 Moomoo OpenD；QuantLab 只使用行情数据，不接实盘交易。

Moomoo is the primary real-data entry and requires local Moomoo OpenD; QuantLab uses quote data only and does not connect to live trading.

如果 QuantLab 已经在运行，双击入口会刷新旧服务后重新打开工作台，避免旧进程保留过期代码状态。

If QuantLab is already running, the launcher refreshes the old service and reopens the workspace to avoid stale code state.
...
`.app` 启动不会弹出 Terminal 窗口。启动日志保存在 `data/cache/quantlab_macos_app.log`。

The `.app` launcher does not open a Terminal window. Launch logs are saved to `data/cache/quantlab_macos_app.log`.

如果要重新生成三个 `.app` 或替换图标，先修改 `assets/QuantLabAppIconConfig.json`，再运行安装脚本。
...
```bash
$EVA_OS_HOME/scripts/installMacAppLaunchers.sh
```
...
```bash
$EVA_OS_HOME/scripts/startQuantLab.sh
```
...
```text
$EVA_OS_HOME/StopQuantLab.command
```
...
```bash
$EVA_OS_HOME/scripts/macosAcceptance.sh
```
...
```bash
$EVA_OS_HOME/scripts/macosAcceptance.sh --mode ui --summary-json
```
...
```bash
$EVA_OS_HOME/scripts/macosAcceptance.sh --mode public-summary --summary-json
```
...
```bash
$EVA_OS_HOME/scripts/verifyQuantLab.sh
```
...
```bash
EVA_OS_ALLOW_HEAVY_SMOKE=1 $EVA_OS_HOME/scripts/finalAcceptanceCheck.sh
```
...
```bash
$EVA_OS_HOME/scripts/createSampleReport.sh
```
...
```bash
$EVA_OS_HOME/scripts/setupEnv.sh
```
...
```bash
$EVA_OS_HOME/scripts/validateRealData.sh
```
...
```bash
$EVA_OS_HOME/scripts/checkMoomoo.sh
```
...
```bash
$EVA_OS_HOME/scripts/validateCrossSource.sh
```
...
If a real data provider does not natively support weekly, monthly, quarterly, or yearly bars, QuantLab fetches an available lower interval first, resamples it locally, and records the source interval in the data quality notes.
...
Create custom strategies from `策略库 Strategy Library`: enter a Chinese name, choose strategy logic, indicator combination, and parameter settings, then QuantLab automatically generates the English name, strategy id, category, return sources, thesis, failure regime, no-code executable strategy code, profile, runnable spec JSON, and Pending approval record.
...
编辑后系统会同步更新 `src/quantlab/strategies/custom/*.py` 中对应策略代码文件，避免 JSON 规格和代码版本不一致。

After editing, QuantLab synchronizes the matching strategy code file under `src/quantlab/strategies/custom/*.py`, preventing spec and code version drift.
