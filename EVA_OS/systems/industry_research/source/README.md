...
系统主线：数据采集 -> 事实整理 -> 行研判断 -> 仓位操作建议 -> 风险提示 -> QuantLab 验证队列 -> 报告生成 -> 纸面跟踪 -> 复盘迭代。
...
python3 -m src.cli quantlab-refresh --date 2026-06-03
...
- QuantLab 验证附件保存在 `data/report_artifacts/对应周文件夹/_quantlab/`，包括 `thesis_queue_YYYY-MM-DD.csv`、`validation_results_YYYY-MM-DD.csv` 和 `validation_summary_YYYY-MM-DD.json`。
- 每周标准产出 22 份 PDF：5 个交易日 * 4 份日报/K线报告 + 周一报告 + 周五报告。

## Data Trust Layer

`data-trust-audit` 是只读审计入口，用来检查行研系统现有 source log、报告伴随文件、支付宝持仓/交易文件、ResearchBus bridge、QuantLab bridge、policy bridge 和 automation health 的证据状态。
...
`reconciliation-audit` 是只读自动对账入口，用来检查 Data Trust、source log、Markdown、PDF、ResearchBus bridge、QuantLab bridge、policy bridge、automation health 和 `HANDOFF.md` 之间是否互相支持。
...
- 每日仓位操作报告：仓位操作建议、盘前/盘中/盘后对比复盘、关键事实事件、市场结构、研究可信度、QuantLab 验证、操作纪律、反方校验和末尾账户附图。
...
- 研究可信度与 QuantLab 验证：全自选池进入基础验证队列；估值缺失必须使用 ETF/指数代理估值或个股 PE/PB/行业估值分位补齐口径；信号质量矩阵放在本板块，和研究可信度合并阅读，不再占用仓位操作建议空间。
- 操作纪律与反方校验：检查待确认订单、现金缓冲、情绪化追涨补跌、反方观点和推翻条件。
- 复合判断质量：周报不得把收盘价、涨跌幅、成交额作为独立结论表；这些字段只能进入量价证据，并必须同时展示复合质量分、策略胜率代理、概率等级、高概率盈利条件、事件原文核验、QuantLab 风险闸门、操作策略和失败动作。
...
每日、周度和 K 线报告必须引用行研结论、政策催化、账户/行情证据或 QuantLab 验证需求。买入/卖出建议必须同时显示 Volume、持仓依据、风险点和反方校验；证据不足的对象降为等待确认或观望。
...
QuantLab 在本系统中只作为验证层：需要输出回测对象、样本区间、成本假设、参数稳定性、样本外验证、walk-forward 结果和研究风险闸门状态。验证不足时，研究结论必须降级为观察或待验证。
...
共享 SQLite：`<EVA_OS_HOME>/data/researchBus/ResearchBus.sqlite`

行研系统 outbox：`data/report_artifacts/research_bus_bridge/`

QuantLab 回写 outbox：`data/report_artifacts/quantlab_bridge/QuantLabResults.json`

该同步会发布行研报告索引，解析 PDF、Markdown 和 TXT 正文生成待验证任务，拉取 QuantLab 回测结果，并拉取独立验证系统运行状态。

QuantLab schema 合约：`<EVA_OS_HOME>/docs/ResearchBusSchema.json`

该 schema 是统一研究数据总线的字段和请求类型来源。后续新聊天、其他 agent 或 automation 修改字段时，应同时更新 QuantLab 与本系统 bridge。
...
真正执行独立验证请求由 QuantLab / ResearchBus 处理：
...
- 报告生成前应先运行 `quantlab-refresh`，让可信度评分和验证队列表引用同一天的验证结果。
...
- 正式报告是否包含仓位操作建议、对比复盘、事实/推论/观点、研究可信度与 QuantLab 验证、反方观点、持仓纪律检查和末尾持仓附图。
...
- QuantLab 动作闸门是否生效：同一标的若为 `NeedsMoreEvidence`、`DataQualityReview`、`DoNotUse` 或风险闸门 `Blocked`，买入类建议不得出现非零 `Volume`，最终规则必须是取消、观望、等待或暂停，而不是继续保留可执行买入。
...
- Research Confidence Score 是否被 QuantLab 状态正确压低。
- `_quantlab` 队列、结果和 summary 是否存在。
...
该脚本会固定 Australia/Sydney 日期、选择包含 `certifi` / `matplotlib` / `reportlab` 的 Python 运行时、记录 automation log、运行自动化健康检查、检查 OpenD 监听状态、刷新支付宝更新状态和 QuantLab，再生成对应报告并只补当天已到期的缺失或质量失败报告。即使 automation 被误提前触发，CLI 到点闸门也会阻断同日未到点报告和未来日期报告。

自动化任务内部顺序：

1. 打开/同步 moomoo 自选池。
2. 运行 `python3 -m src.cli quantlab-refresh --date 当日日期`。
