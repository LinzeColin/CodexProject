...
class AutomationHealthTest(unittest.TestCase):
    def test_automation_prompt_check_passes_current_required_terms(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            prompt = "Volume依据; fixed 8% style sizing; Translate internal QuantLab labels to Chinese; No internal local paths in PDF body; local request/report paths; Do not backfill or regenerate reports dated before TODAY"
    ...
    def test_automation_prompt_check_rejects_stale_prompt_terms(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            current_prompt = "Volume依据; fixed 8% style sizing; Translate internal QuantLab labels to Chinese; No internal local paths in PDF body; local request/report paths; Do not backfill or regenerate reports dated before TODAY"
