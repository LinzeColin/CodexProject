...
from src.quantlab.engine import _to_yahoo_symbol, build_thesis_queue, run_quantlab_validation
from src.reporting.analysis import research_confidence_table


class QuantLabValidationTest(unittest.TestCase):
    def test_validation_requires_minimum_simulation_and_reruns(self) -> None:
        queue = self._queue("2026-01-15")
        with self.assertRaises(ValueError):
            run_quantlab_validation("2026-01-15", queue, [], monte_carlo_runs=99_999, pipeline_reruns=2)
        with self.assertRaises(ValueError):
            run_quantlab_validation("2026-01-15", queue, [], monte_carlo_runs=100_000, pipeline_reruns=1)

    def test_successful_validation_records_required_quantlab_evidence(self) -> None:
        as_of = "2026-01-15"
    ...
    def test_research_confidence_uses_saved_quantlab_result(self) -> None:
        as_of = "2026-01-17"
        factors = self._factors()
        advice = self._advice()
        queue = build_thesis_queue(as_of, factors, advice, [])
        run_quantlab_validation(
            as_of,
            queue,
        ...
        self.assertNotIn("QuantLab未验证", rendered)
