    ...
    report = render_template(
        ROOT / "01_templates" / "K线分析报告模板.md",
        {
            ...
            "learning_notes": (
                ...
                "- 执行标准：MA/EMA/BOLL 负责趋势与区间，MACD/RSI/KDJ 负责动能，VOL 负责确认。三类至少两类同向，只能保留研究候选；账户、QuantLab、尾盘价格和成交额全部通过后才重算候选 Volume。\n"
...
def _kline_persuasion(row: dict[str, object], validation: dict[str, str]) -> str:
    ...
    if gate == "Blocked":
        return f"中低：{_quantlab_status_display(status)} 且风险闸门阻断，只能观察或暂停，Volume=0。"
    return f"中：技术信号{quality}，验证状态{_quantlab_status_display(status)}，需等待样本确认。"
...
def _kline_risk(row: dict[str, object], validation: dict[str, str]) -> str:
    ...
    gate = str(validation.get("risk_gate") or "Pending")
    return _compact(
        f"{_quantlab_status_display(status)}/{_risk_gate_display(gate)}；失效条件：{row.get('if_failed_action', '样本不满足则取消方向。')}",
        78,
    )
        ...
        output.append(
            {
                ...
                "name": row.get("name", ""),
                "status": _quantlab_status_display(status),
...
def _why_evidence_needed(status: str, validation: dict[str, str]) -> str:
    ...
    if status == "NeedsMoreEvidence":
        return str(validation.get("conclusion") or "QuantLab风险闸门未通过或稳定性不足。")
    return "验证未否定当前方向。"
...
def _operation_for_evidence_status(row: dict[str, object], status: str) -> str:
    ...
    if status == "NeedsMoreEvidence":
        if "买入" in action:
            return "证据不足，买入不进入执行；Volume=0，补足样本、账户和QuantLab验证后重算。"
        if "卖出" in action:
...
def _counter_conclusion(row: dict[str, object]) -> str:
    ...
    if "买入" in action and quality == "Low":
        return "推翻买入，改观望。"
    if "买入" in action:
        return "保留买入候选；账户、QuantLab和尾盘量价通过后重算Volume。"
