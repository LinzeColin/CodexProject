...
from src.quantlab.engine import latest_result_by_symbol
...
def position_action_recommendations(
    ...
    quantlab_by_symbol = latest_result_by_symbol(as_of) if as_of else {}
    ...
    for row in sorted_advice:
        ...
        validation = quantlab_by_symbol.get(symbol, {})
        status = str(validation.get("validation_status") or _quantlab_status(factor, row, validation))
        composite_quality = _weekly_composite_score(factor, row, events or [], quantlab_by_symbol)
...
def decision_signal_quality_matrix(
    ...
    for row in sorted(advice, key=lambda item: (_action_priority(str(item.get("Position", ""))), -float(item.get("Volume") or 0))):
        ...
        symbol = str(factor.get("symbol") or row.get("symbol") or "")
        validation = quantlab_by_symbol.get(symbol, {})
        status = validation.get("validation_status") or _quantlab_status(factor, row, validation)
        rows.append(
            {
                "Name": row.get("Name", ""),
...
def core_trading_summary(
    ...
    return (
        ...
        "- 研究原则：先做事实整理和风险识别，再进入 QuantLab 验证；验证不足时只输出观察或待验证。\n"
...
def daily_session_analysis(session: str, factors: list[dict[str, object]], advice: list[dict[str, object]]) -> str:
    ...
    return (
        ...
        "- 纪律提醒：次日仍需先确认事实、反方观点和 QuantLab 状态。"
    )
        ...
        rows.append(
            {
                ...
                "quantlab_gate": validation.get("risk_gate") or _quantlab_operation_behavior(row, factor, validation),
        ...
        rows = [
            {
                ...
                "evidence_gap": "继续刷新行情、账户、事件和QuantLab结果。",
                "quantlab_gate": "无可验证线索",
                "final_rule": "明确结论：观望。",
            }
        ]
    return (
        ...
        + table(
            rows,
            ["Name", "Position", "window", "account_gate", "quality", "pass_condition", "fail_action", "evidence_gap", "quantlab_gate", "final_rule"],
            ["Name", "Position", "检查时间", "账户闸门", "信号质量", "成立条件", "不成立动作", "证据缺口", "QuantLab闸门", "最终规则"],
        )
    )
...
def fact_inference_opinion(
    ...
    return (
        ...
        "- 买卖建议只在行情、账户、事件与 QuantLab 证据可串联时保留；当日支付宝流水缺失或仅为待确认候选数据时只能保留买/卖研究候选，执行金额必须为0。\n"
...
def research_confidence_table(
        ...
        rows.append(
            {
                ...
                "validation_status": _quantlab_status_display(_quantlab_status(item, advice_row, validation)),
    ...
    return table(
        ...
        ["Name", "主题", "Research Confidence Score", "研究等级", "数据完整性", "QuantLab状态", "已处理/待跟踪事项"],
    )
...
def discipline_check(account_summary: dict[str, object], advice: list[dict[str, object]], exposure: dict[str, object] | None = None) -> str:
    ...
    rows = [
        ...
        {"item": "情绪化追涨/补跌", "status": "需检查", "detail": "任何单日涨跌触发的线索必须经过反方观点和 QuantLab 验证"},
        {"item": "现实现金流影响", "status": "需人工确认", "detail": "报告系统无法判断生活现金流，必须由用户确认"},
    ]
    return table(rows, ["item", "status", "detail"], ["纪律项", "状态", "说明"])


def quantlab_validation_queue(factors: list[dict[str, object]], advice: list[dict[str, object]], as_of: str | None = None) -> str:
    factor_by_name = {str(item.get("name")): item for item in factors}
    quantlab_by_symbol = latest_result_by_symbol(as_of) if as_of else {}
    rows = []
    for row in advice:
        ...
        rows.append(
            {
                "Name": row.get("Name"),
                "status": _quantlab_status_display(str(validation.get("validation_status") or _quantlab_status(factor, row))),
                "target": symbol,
                "sample": _quantlab_sample_text(validation),
                "cost": validation.get("cost_assumption") or "申赎费/管理费/滑点/15:00收盘确认",
                "monte_carlo": _quantlab_monte_carlo_text(validation),
                "rerun": _quantlab_rerun_text(validation),
                "risk_gate": _risk_gate_display(str(validation.get("risk_gate") or "")),
                "needed": _decision_evidence_needed(row, factor, str(validation.get("validation_status") or _quantlab_status(factor, row)), validation),
                "operation": _quantlab_operation_behavior(row, factor, validation),
                "conclusion": _quantlab_conclusion_display(validation),
            }
        )
    if not rows:
...
def comparison_backtest(session: str, factors: list[dict[str, object]], advice: list[dict[str, object]], as_of: str | None = None) -> str:
    ...
    return (
        ...
        "- 复盘结论：收益来自单一主题时，次日降低主题集中；收益来自多主题且 QuantLab 风险闸门未否定时，保留为次日优先观察。\n"
...
def weekly_operation_synthesis(
    ...
    quantlab_by_symbol = latest_result_by_symbol(as_of)
    pass_count = sum(1 for row in quantlab_by_symbol.values() if str(row.get("risk_gate") or "") == "Pass")
    blocked_count = sum(1 for row in quantlab_by_symbol.values() if str(row.get("risk_gate") or "") == "Blocked")
    return (
        f"- 综合结论：本周只把 {_names(leaders)} 作为强势观察线索，把 {_names(weak)} 作为风险/承接校验线索；单独收盘价、涨跌幅、成交额不构成周报结论。\n"
        f"- 操作结构：{_counts_text(counts)}；买入必须满足尾盘止跌、成交额不恶化、事件原文无负面扩散和 QuantLab 未阻断，卖出优先处理上涨兑现、转弱降仓和现仓集中风险。\n"
        f"- 事件核验：已完成原文核验事件 {len(verified_events)} 条；未核验政策/新闻只作为风险背景，不增加买入分、买入金额或 Volume。\n"
        f"- QuantLab闸门：通过 {pass_count} 个，阻断 {blocked_count} 个；被阻断的买入自动取消或观望，卖出/风控优先。\n"
...
def weekly_composite_strategy_table(
    ...
    ranked = sorted(
        factors,
        key=lambda item: _weekly_composite_score(item, advice_by_name.get(str(item.get("name")), {}), events, quantlab_by_symbol),
        reverse=True,
    )
    for item in ranked[:16]:
        ...
        validation = quantlab_by_symbol.get(symbol, {})
        score = _weekly_composite_score(item, row, events, quantlab_by_symbol)
    ...
    return (
        "- 本表不把收盘价、涨跌幅、成交额单独作为结论；它们只进入“量价证据”。排序依据为复合质量分、策略胜率代理、QuantLab 风险闸门、政策/事件支持、持仓逻辑和反方触发条件。\n"
        "- 策略胜率代理 = 1 - QuantLab 蒙特卡洛亏损概率；若样本不足，则用数据完整性、量价一致性和持仓约束给出低权重代理，不能视为历史真实胜率，也不能保证盈利。\n"
        "- 高概率盈利条件必须同时满足量价、持仓纪律、事件原文核验和 QuantLab 风险闸门；任一关键条件缺失，操作策略自动降额、取消或观望。\n"
        + table(
            rows,
            [
                "Name",
...
def theme_tracking_table(factors: list[dict[str, object]], events: list[dict[str, str]]) -> str:
    ...
    return table(
        ...
        ["主题", "今日强弱", "成交额", "主要催化剂", "主要风险", "相关标的", "QuantLab", "研究状态"],
    )
...
def section_explanation_weekly() -> str:
    return (
        ...
        "- 复合判断质量表：把量价证据、QuantLab、政策事件、持仓约束和失败动作合成操作策略，不把裸行情当结论。\n"
...
def _weekly_composite_score(
    ...
    row: dict[str, object],
    events: list[dict[str, str]],
    quantlab_by_symbol: dict[str, dict[str, str]],
) -> float:
    symbol = str(item.get("symbol") or row.get("symbol") or "")
    validation = quantlab_by_symbol.get(symbol, {})
    data_score = 0
    ...
    status = str(validation.get("validation_status") or _quantlab_status(item, row, validation))
    ...
    raw = data_score + signal_score + quantlab_score + win_score + policy_score + holding_score - pending_penalty
...
def _persuasion_summary(
    ...
    if status == "ContinueResearch":
        evidence.append("QuantLab通过")
    elif status in {"NeedsMoreEvidence", "DataQualityReview", "DoNotUse"}:
        evidence.append(f"QuantLab{_short_quantlab_status(status)}")
    if risk_gate == "Blocked":
...
def _short_quantlab_status(status: str) -> str:
...
def _accuracy_basis(score: float, win_rate: float, validation: dict[str, str]) -> str:
    ...
    status = _quantlab_status_display(raw_status) if raw_status else "基础代理"
...
def _high_probability_profit_condition(
    ...
    action = str(row.get("Position") or "观望")
    quantlab = str(validation.get("risk_gate") or "")
    volume = float(row.get("Volume") or 0)
    if "账户待更新" in action:
...
def _weekly_risk_gate_label(validation: dict[str, str]) -> str:
    ...
    if status:
        return f"{_quantlab_status_display(status)}：只作为观察权重。"
    return "基础代理：未拿到完整QuantLab指标。"
...
def _weekly_operation_strategy(
    ...
    if _is_buy_action(action):
        ...
        if score >= 70 and win_rate >= 0.56:
            return "买入候选保留；14:30-14:55尾盘仍止跌、量价不背离且账户/QuantLab通过后重算Volume。"
        if score >= 55:
            return "证据不足时不执行买入；账户、QuantLab和量价同时通过后重算候选Volume。"
        return "买入取消，转观望。"
    if _is_sell_action(action):
        if score >= 55:
...
def _research_confidence(
    ...
    if advice_row.get("holding_amount", 0):
        score += 10
    quantlab_status = _quantlab_status(item, advice_row, validation or {})
    if quantlab_status == "ContinueResearch":
        score += 15
    elif quantlab_status == "ValidationQueued":
        score += 6
        validation_plan.append("QuantLab排队")
    else:
        validation_plan.append(_quantlab_gap(quantlab_status, validation or {}))
    if advice_row.get("risk_note"):
        score += 10
    level = _cap_confidence_level(_confidence_level_from_score(score), quantlab_status)
    return score, level, "；".join(validation_plan[:3]) or "已完成基础行情/估值/QuantLab校验"


def _quantlab_status(item: dict[str, object], advice_row: dict[str, object], validation: dict[str, str] | None = None) -> str:
...
def _quantlab_gap(status: str, validation: dict[str, str]) -> str:
    if status == "NeedsMoreEvidence":
        if str(validation.get("risk_gate", "")) == "Blocked":
            return "QuantLab风险闸门未通过"
        return "QuantLab证据不足"
    if status == "DataQualityReview":
        rows = validation.get("sample_rows")
        return f"QuantLab样本不足({rows}条)" if rows not in {"", None} else "QuantLab数据质量待核验"
    if status == "DoNotUse":
        return "QuantLab停用"
    if status == "WatchOnly":
        return "QuantLab仅观察"
    if status == "ValidationQueued":
        return "QuantLab排队待验证"
    if status == "NotValidated":
        return "已补入QuantLab基础覆盖队列"
    return "QuantLab状态待确认"


def _quantlab_status_display(status: str) -> str:
    mapping = {
...
def _quantlab_conclusion_display(validation: dict[str, str]) -> str:
    ...
    return "排队验证；等待 QuantLab 生成可审计结果，未完成前不升级为执行依据。"
...
def _cap_confidence_level(level: str, quantlab_status: str) -> str:
    order = {
    ...
    cap = caps.get(quantlab_status, "Watch Only")
    return level if order[level] <= order[cap] else cap


def _quantlab_sample_text(validation: dict[str, str]) -> str:
    if not validation:
...
def _quantlab_monte_carlo_text(validation: dict[str, str]) -> str:
...
def _quantlab_rerun_text(validation: dict[str, str]) -> str:
...
def _volume_confirmation(row: dict[str, object], factor: dict[str, object]) -> str:
    ...
    return "VOL结论：一般确认；保留候选Volume上限，执行前仍需账户和QuantLab闸门通过。"
...
def _mixed_signal_conclusion(row: dict[str, object], factor: dict[str, object], status: str) -> str:
    ...
    if not _is_buy_action(action) and not _is_sell_action(action):
        return "观察状态：不执行候选Volume；只有连续1-2日强弱、成交额、事件和QuantLab同向才升级。"
    quality = _decision_signal_quality(row, factor, status)
    if quality == "High":
        return "三类信号基本同向：趋势/动能/VOL支持；保留候选Volume上限，执行前仍需账户和QuantLab闸门通过。"
    if quality == "Medium":
...
def _decision_evidence_needed(
    ...
    if status == "NeedsMoreEvidence":
        if _is_buy_action(action):
            return "需要尾盘止跌、成交额不继续异常放大、无负面事件、QuantLab风险闸门通过。"
        if _is_sell_action(action):
            return "需要尾盘不能放量突破、上涨缺乏持续事件支持、QuantLab风险闸门不否定卖出。"
        return "需要连续1-2日强弱排序、成交额和事件面同向后再升级。"
    if status == "ValidationQueued":
        return "等待QuantLab完成>=100,000次模拟、全流程>=2次重跑和样本外检查。"
...
def _decision_operation_conclusion(row: dict[str, object], factor: dict[str, object], status: str) -> str:
    ...
    if "账户待更新" in action:
        if _is_buy_action(action):
            return "明确结论：等待账户确认；当前买入动作取消为可执行项，Volume 0；账户与QuantLab闸门通过后重新评估。"
        if _is_sell_action(action):
    ...
    if _is_buy_action(action):
        if quality == "High":
            return "明确结论：买入候选保留；账户、QuantLab和尾盘量价通过后重算Volume，跌破关键区间则取消。"
        if quality == "Medium":
            return "明确结论：证据不足时不买；账户、QuantLab和尾盘量价同时通过后重算候选Volume。"
        return "明确结论：不买，改观望。"
    if _is_sell_action(action):
    ...
    return "明确结论：观望；只有连续1-2日强弱排序改善、成交额>0且事件/QuantLab同向时才升级，否则不新增买卖。"
...
def _account_gate_text(blocked: bool, reason: str, row: dict[str, object]) -> str:
    ...
    return "账户闸门通过：仍受尾盘样本、事件和QuantLab约束。"
...
def _daily_pass_condition(row: dict[str, object], factor: dict[str, object], status: str) -> str:
    ...
    if _is_buy_action(action):
        return "买入成立：尾盘跌幅收窄或止跌，成交额不继续异常放大，无新增负面事件，QuantLab未阻断。"
    if _is_sell_action(action):
    ...
    return "观察成立：当前不占用资金；升级条件为连续1-2日强弱排序改善、成交额>0、事件面与QuantLab不反向。"
...
def _daily_fail_action(row: dict[str, object], factor: dict[str, object]) -> str:
    ...
    return "维持观望；若成交额缺失、价格未转强或QuantLab风险闸门阻断，不因单日行情升级。"


def _quantlab_operation_behavior(row: dict[str, object], factor: dict[str, object], validation: dict[str, str]) -> str:
    status = str(validation.get("validation_status") or _quantlab_status(factor, row, validation))
    if status == "ContinueResearch":
...
def _counter_clear_conclusion(row: dict[str, object], factor: dict[str, object]) -> str:
    ...
    return "结论：观望；只有方向突破、成交额确认且事件/QuantLab同向时升级，否则维持观望。"
...
def _counter_operation(row: dict[str, object], factor: dict[str, object]) -> str:
    ...
    return "若反方触发且成交额/事件/QuantLab同向：升级到重点观察；否则维持观望且不占用资金。"
