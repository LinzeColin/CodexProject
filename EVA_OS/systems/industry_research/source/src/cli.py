...
from src.quantlab.engine import build_thesis_queue, run_quantlab_validation
...
from src.reporting.paths import excel_dir, quantlab_dir
...
def quantlab_refresh(args: argparse.Namespace) -> None:
    context = load_research_context(args.date, sync_moomoo=not args.no_sync_moomoo)
    _run_quantlab_for_context(args.date, context, args.monte_carlo_runs, args.pipeline_reruns)
...
def generate_due_reports(args: argparse.Namespace) -> None:
    ...
    if not due_rows:
        ...
        return
    context = load_research_context(args.date, sync_moomoo=not args.no_sync_moomoo)
    _run_quantlab_for_context(args.date, context, 100000, 2)
    for row in due_rows:
...
def _run_quantlab_for_context(
    ...
    result = run_quantlab_validation(
        as_of,
        queue,
        context["market_rows"],
    ...
    print(f"QuantLab queue: {len(result['queue'])}")
    print(f"QuantLab results: {summary['status_counts']}")
    print(quantlab_dir(as_of))
...
def research_bus_sync(args: argparse.Namespace) -> None:
    ...
    print(f"quantlab_result_count: {payload['quantlab_result_count']}")
    print(f"quantlab_results_path: {payload['quantlab_results_path']}")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    quantlab = sub.add_parser("quantlab-refresh")
    quantlab.add_argument("--date", required=True)
    quantlab.add_argument("--monte-carlo-runs", type=int, default=100000)
    quantlab.add_argument("--pipeline-reruns", type=int, default=2)
    _add_sync_controls(quantlab)
    quantlab.set_defaults(func=quantlab_refresh)
