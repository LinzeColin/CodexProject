...
def _bridge_checks(root: Path) -> list[ReconciliationCheck]:
    ...
    checks.append(
        _boolean_check(
            ...
            "同步 QuantLab ResearchBus schema 后重新拉取。",
        )
    )
    quantlab_path = root / "data" / "report_artifacts" / "quantlab_bridge" / "QuantLabResults.json"
    quantlab = _load_json(quantlab_path, {})
    results = quantlab.get("results") if isinstance(quantlab, dict) and isinstance(quantlab.get("results"), list) else []
    weak = [
        row
        for row in results
    ...
    checks.append(
        _check(
            ...
            "weak_rows=0",
            [quantlab_path],
            "QuantLab bridge contains Review/NeedsMoreEvidence rows." if weak else "",
            "报告中读取这些结果时必须降级为观察或待验证。" if weak else "QuantLab bridge 当前无弱证据状态。",
        )
    )
