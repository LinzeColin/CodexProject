...
- 对外同步边界：本系统向 QuantLab/ResearchBus 同步消费行为状态、现金流视图、分类/风险标签、待复核隔离和周期统计；不主导 ResearchBus schema、投资/回测、行研报告生成或政策采集。
