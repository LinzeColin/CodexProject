...
| macOS EVA_OS entry apps | `macos/EVA_OS.app`, `macos/EVA_OS_launcher.c`, `scripts/installEVAOSEntryApps.sh` |
| macOS App Acceptance Lite | `EVAOSMacOSAppAcceptanceLiteV1`, `scripts/macosAppAcceptanceLite.sh`, `docs/MacOSAppAcceptanceLite.md` |
| macOS Lifecycle Readiness | `EVAOSMacOSLifecycleReadinessV1`, `scripts/macosLifecycleReadiness.sh`, `docs/MacOSLifecycleReadiness.md` |
...
| Workspace system manifest adapter | `src/quantlab/integrations/workspace_systems.py`, `scripts/syncWorkspaceSystemSummaries.sh` |
| Unified UI Shell status | `src/quantlab/app/dashboard.py`, `src/quantlab/app/streamlit_app.py`, `tests/test_workspace_shell.py` |
| macOS runtime acceptance | `scripts/quantlabRuntime.sh`, `scripts/startQuantLab.sh`, `StartQuantLab.command`, `scripts/finalAcceptanceCheck.sh` |
| UI Shell lifecycle panel | `macos_lifecycle_summary()`, `render_macos_lifecycle_panel()` |
| Vectorized Research Mode | `src/quantlab/research/vectorized.py`, `scripts/vectorizedResearch.sh`, `docs/VectorizedResearchMode.md` |
| Hotspot Runtime Summary | `hotspot_runtime_summary()`, `scripts/hotspotRuntimeSummary.sh`, `EVAOSHotspotRuntimeSummaryV1` |
| Hotspot Quick Preflight | `hotspot_quick_preflight()`, `render_hotspot_preflight()`, `EVAOSHotspotQuickPreflightV1` |
| Hotspot Persisted Cache | `EVAOSHotspotPersistedCacheV1`, `data/cache/hotspots/`, `--use-persisted-cache` |
| Parameter Scan Preflight | `parameter_scan_preflight()`, `render_parameter_scan_preflight()`, `EVAOSParameterScanPreflightV1` |
| Command Center Action Router | `command_center_next_actions()`, `render_command_center_action_router()`, `EVAOSCommandCenterActionRouterV1` |
| 52ETF Read-Only Reference | `EVAOS52ETFPublicSnapshotV1`, `EVAOS52ETFHotspotComparisonV1`, `scripts/site52etfSnapshot.sh`, `render_site52etf_hotspot_comparison()` |
...
- Heavy SmokeTest scripts are guarded locally and require `EVA_OS_ALLOW_HEAVY_SMOKE=1`; this is intentional to prevent accidental repeated SmokeTest Fail noise.
...
./scripts/macosAcceptance.sh --summary-json returned EVAOSMacOSAcceptanceHubV1 status=Pass mode=daily pass=2 fail=0 info=0 starts_service=false opens_browser=false heavy_smoke=false
...
./scripts/devReadyCheck.sh --summary-json returned status=Pass pass=40 fail=0 info=1
./scripts/cleanCache.sh --dry-run --json returned candidate_count=0 candidate_file_count=0 candidate_dir_count=0 candidate_kb=0.0
Post-push ./scripts/macosAcceptance.sh --summary-json returned EVAOSMacOSAcceptanceHubV1 status=Pass mode=daily pass=2 fail=0 info=0; starts_service=false opens_browser=false heavy_smoke=false
No scripts/finalAcceptanceCheck.sh, scripts/ciSmoke.sh, full pytest, browser automation, browser-open action, market refresh, broker connection, order, payment, or holdings write was run in this step
```
...
```text
./scripts/reportValidation.sh returned EVAOSReportValidationHubV1 status=Pass mode=daily report_records=32 needs_more=32 gap_tasks=170 validation_queue_candidates=7058 prioritized=120
...
```text
./scripts/macosAcceptance.sh returned EVAOSMacOSAcceptanceHubV1 status=Pass mode=daily pass=2 fail=0
...
post-run scripts/statusQuantLab.sh returned QuantLab is not running on ports 8501-8510 and pgrep found no streamlit_app.py process
...
```text
Desktop, Downloads, and Applications EVA_OS.app refreshed from repo template
...
scripts/statusQuantLab.sh detected the running service
...
scripts/statusQuantLab.sh detected http://localhost:8501
...
scripts/verifyQuantLab.sh completed; full pytest 367 passed, 2 skipped, 3 subtests passed
...
github_ready/EVA_OS mirror rsync transferred 15 files
```
...
EVAOS52ETFPublicSnapshotV1 writes local latest snapshot without raw HTML and can feed EVAOS52ETFHotspotComparisonV1
...
```text
py_compile passed for src/quantlab/app/streamlit_app.py
...
```text
py_compile passed for src/quantlab/business/cashflow.py, src/quantlab/examples/cashflow_command.py, and src/quantlab/app/streamlit_app.py
tests/test_cashflow_command.py tests/test_scripts.py tests/test_app_dashboard.py: 70 passed
scripts/cashFlowCommand.sh --summary-json returned EVAOSCompanyCashFlowRuntimeSummaryV1 status=Blocked cashflow_status=MissingBalance entry_count=0 and no full entries
...
```text
py_compile passed for src/quantlab/policy/radar.py, src/quantlab/examples/policy_radar.py, and src/quantlab/app/streamlit_app.py
tests/test_policy_radar.py tests/test_scripts.py tests/test_app_dashboard.py: 71 passed
scripts/policyRadar.sh --summary-json returned EVAOSPolicyIntelligenceRuntimeSummaryV1 status=Blocked policy_status=MissingPolicyEvidence opportunity_count=0 and no full opportunities
...
```text
py_compile passed for src/quantlab/consumption/guard.py, src/quantlab/examples/consumption_guard.py, and src/quantlab/app/streamlit_app.py
tests/test_consumption_guard.py tests/test_scripts.py tests/test_app_dashboard.py: 72 passed
scripts/consumptionGuard.sh --summary-json returned EVAOSConsumptionGuardRuntimeSummaryV1 status=Blocked guard_status=MissingConsumptionEvidence event_count=0 and no full events
...
scripts/statusQuantLab.sh after stop reported not running on ports 8501-8510
...
```text
scripts/cashFlowReviewedInputRefresh.sh --as-of 2026-06-16 --json returned EVAOSCompanyCashFlowReviewedInputRefreshV1 status=Blocked cashflow_status=MissingReviewedInput with outputs={}
scripts/cashFlowReviewedInputRefresh.sh --as-of 2026-06-16 --entry-path data/cashflow/CompanyCashFlowReviewedInput.example.json --output-dir /tmp/eva_cashflow_reviewed_smoke returned status=Pass cashflow_status=Stable balance=18000.0 net=3840.0 runway_days=1500.0
scripts/refreshRuntimeSummaries.sh --project-root /tmp/eva_runtime_summary_cashflow_smoke --cashflow-entry-path <repo>/data/cashflow/CompanyCashFlowReviewedInput.example.json returned Company CashFlow runtime_status=Pass
scripts/commandCenter.sh --project-root /tmp/eva_runtime_summary_cashflow_smoke loaded Company CashFlow from runtime_summary with Stable status and valid PDF output
...
```text
scripts/policyReviewedInputRefresh.sh missing default private input returned EVAOSPolicyReviewedInputRefreshV1 status=Blocked policy_status=MissingReviewedInput and wrote no outputs
...
macos/EVA_OS_launcher.c now builds a native Mach-O launcher; installed Desktop, Downloads, and Applications apps bind Contents/Resources/EVA_OS_PROJECT_ROOT to the current local checkout
EVA_OS_APP_LAUNCH_DRY_RUN=1 on installed apps points to local StartQuantLab.command and github_fallback=absent
open -n ~/Downloads/EVA_OS.app returned 0, started local EVA_OS, /_stcore/health returned ok, and service was stopped afterward
```

Latest Consumption reviewed-input evidence:

```text
scripts/consumptionReviewedInputRefresh.sh missing default private input returned EVAOSConsumptionGuardReviewedInputRefreshV1 status=Blocked guard_status=MissingReviewedInput and wrote no outputs
...
- Normal app launch installs/reuses `.[app]` only through `scripts/quantlabRuntime.sh`.
- Verification scripts install `.[test]` only when pytest is missing.
- `.venv/.quantlab_app_ready` is a local marker and must not be committed.
...
Previous verified engineering step: `EVA_OS app identity, icon, and legacy launcher cleanup`.
...
- Native `EVA_OS.app` launcher uses `/bin/zsh -f` for local `StartQuantLab.command`.
- The launcher no longer names Terminal or GitHub fallback and uses short health-port curl timeouts.
- `scripts/installEVAOSEntryApps.sh` signs in a temporary staging directory, then installs to Desktop, Downloads, and Applications with `ditto --norsrc --noextattr --noacl`.
- `scripts/stopQuantLab.sh` now stops project-scoped launcher processes as well as Streamlit.
...
syntax checks: StartQuantLab.command, stopQuantLab.sh, installEVAOSEntryApps.sh, macosRuntimeAcceptance.sh, finalAcceptanceCheck.sh passed
...
- `src/quantlab/analysis/market_hotspots.py` now exposes `EVAOSHotspotCacheStatusV1`, `EVAOSHotspotCacheDirectorySummaryV1`, current request cache status, directory summary, and current request invalidation.
- `src/quantlab/app/streamlit_app.py` now renders `热点缓存` before generation with current request state, age, remaining TTL, directory file count, and a button to clear only the active request-key cache.
- `src/quantlab/examples/hotspot_runtime_summary.py` and `scripts/hotspotRuntimeSummary.sh` support CLI-only `--cache-status` and `--invalidate-cache`.
...
CLI cache status: EVAOSHotspotCacheStatusV1 returned without market-bar loading
...
- `EVAOSHotspotRequestTraceV1` was added to `src/quantlab/analysis/market_hotspots.py`.
...
syntax checks: hotspotRuntimeSummary.sh and finalAcceptanceCheck.sh passed
CLI trace summary: EVAOSHotspotRequestTraceV1 request_count=5 success_count=5 failed_count=0
...
- Added `src/quantlab/system/dev_readiness.py`, `src/quantlab/examples/dev_ready_check.py`, and `scripts/devReadyCheck.sh`.
- `scripts/devReadyCheck.sh --summary-json` now emits `EVAOSDevReadyCheckV1` for normal development checks.
...
- `scripts/cleanCache.sh` now prevents repo-local pycache creation while checking or deleting cache by exporting `PYTHONDONTWRITEBYTECODE=1` and `PYTHONPYCACHEPREFIX=/private/tmp/quantlab-pycache`.
...
```text
devReadyCheck summary: EVAOSDevReadyCheckV1 status=Pass pass=26 fail=0 info=1
...
- `src/quantlab/app/streamlit_app.py` now allowlists `scripts/devReadyCheck.sh` and renders a `开发检查` button in the `macOS 生命周期` panel.
- `src/quantlab/app/dashboard.py` now includes `Dev Ready Check` as a UI-mode lifecycle action.
- `src/quantlab/system/macos_lifecycle.py` now checks `DevReadyScriptExecutable` and UI allowlist coverage for `scripts/devReadyCheck.sh`.
...
devReadyCheck summary: EVAOSDevReadyCheckV1 status=Pass pass=26 fail=0 info=1
macOS lifecycle readiness summary: EVAOSMacOSLifecycleReadinessV1 status=Pass pass=29 fail=0
...
- `src/quantlab/app/dashboard.py` now exposes `macos_runtime_evidence_summary(payload)` with schema `MacOSRuntimeEvidenceSummaryV1`.
- `src/quantlab/app/streamlit_app.py` now renders a read-only `运行时验收证据` section in the `macOS 生命周期` panel.
...
- Result was `EVAOSMacOSRuntimeAcceptanceV1 status=Pass pass=10 fail=0 info=0`.
...
- Ran Downloads `EVA_OS.app` open-path runtime acceptance from Terminal:
...
- Added `scripts/uiVisualAcceptance.sh` with schema `EVAOSUIVisualAcceptanceV1`.
- The script verifies the local Streamlit workbench with headless Chrome, checks `EVA_OS`, `工作台状态`, `macOS 生命周期`, `运行时验收证据`, cache preview text, and visible lifecycle buttons.
...
EVAOSUIVisualAcceptanceV1 status=Pass pass=16 fail=0 screenshot_bytes=278744 started_by_acceptance=true
...
- Added `scripts/macosPublicAcceptanceSummary.sh` and schema `EVAOSMacOSPublicAcceptanceSummaryV1`.
- Added `src/quantlab/system/macos_public_acceptance.py` and CLI `quantlab.examples.macos_public_acceptance`.
...
- Added schema `EVAOSMacOSAcceptanceHubV1`, module `src/quantlab/system/macos_acceptance_hub.py`, and CLI `quantlab.examples.macos_acceptance_hub`.
