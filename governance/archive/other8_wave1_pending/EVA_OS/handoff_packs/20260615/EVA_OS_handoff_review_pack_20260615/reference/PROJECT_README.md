...
1. 双击 `EVA_OS.app`。
2. 在 `总控驾驶舱` 检查系统状态、现金流、政策、消费、证据来源、行动队列和最新报告。
...
热点分析现在默认使用 `快速预览` 工作台模式，限制对象数和时间切片数，减少点击“生成热点分析”后的等待时间；需要报告前复盘时可切换到 `标准分析` 或 `完整复盘`。页面也提供可选的 `52ETF 公开参考`，只读取 `https://52etf.site/` 公开页面的板块和交互提示，用于对照市场云图表达，不作为行情、回测或交易证据。

Market hotspots now default to `快速预览`, limiting object count and time slices to reduce wait time after clicking generate. Switch to `标准分析` or `完整复盘` for deeper review. The optional `52ETF 公开参考` reads only the public `https://52etf.site/` page for market-cloud reference and is not used as market data, backtest evidence, or trading input.
