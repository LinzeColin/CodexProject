...
def test_release_manager_preflight_is_fail_closed_for_repository_state() -> None:
    ...
    assert heartbeat["progress_status"] == "FAILED_WINDOW"
    assert heartbeat["counts_as_release_ready"] is False
    assert heartbeat["release_gate_closed_by_background_heartbeat"] is False
    assert heartbeat["windows_completed"] == 129
    assert heartbeat["windows_failed"] == 1
    assert heartbeat["operator_process_status"] == "NOT_RUNNING"
    assert heartbeat["watchdog_process_status"] == "NOT_RUNNING"
