...
- 2026-06-26 T1303 external release operator input kit adds `PARAM-093` for the template-kit schema version; it creates template-only A202/A210/A026-A027/A209 worksheets under `artifacts/operator_input_kit/` and validates that template paths are distinct from signed `artifacts/operator_inputs/` targets, with all release gates still blocked.
- 2026-06-27 T1303 external release operator input status adds `PARAM-094` for the status-manifest schema version; it reports missing, rejected template-copy and present-but-unvalidated operator input states under `artifacts/operator_inputs/` while keeping release-manager and MVP release refresh blocked. EVENT-20260627-016 exposes the same fail-closed status through `/v1/release/operator-input-status` and `/development-status` without changing scoring formulas, active parameter values, release-manager activation or publication policy. EVENT-20260627-017 advances the `external-release-operator-input-status` profile binding to `@2` by adding per-input validator ids, expected artifacts, success statuses and fail-closed validator states; the `PARAM-094` value remains `eei-external-release-operator-input-status-v1`.
- 2026-06-27 T1303 operator-input submission preflight adds `PARAM-095` for `eei-operator-input-submission-preflight-v1`. EVENT-20260627-018 exposes `/v1/release/operator-input-submission-preflight` as a dry-run/manual-command-only dispatch plan for registered A202/A210/A026/A027/A209 inputs; it does not write operator files, execute validators, certify evidence, activate release-manager refresh or close release gates.
- 2026-06-27 T1303 operator-input submission receipt adds `PARAM-096` for `eei-operator-input-submission-receipt-v1`. EVENT-20260627-019 exposes `/v1/release/operator-input-submission-receipt` as a hash-bound server response for placed operator input targets; it does not write files, execute validators, certify evidence, activate release-manager refresh or close release gates.
- 2026-06-27 T1303 operator-input receipt ledger adds `PARAM-097` for `eei-operator-input-submission-receipt-ledger-v1`. EVENT-20260627-020 upgrades `/v1/release/operator-input-submission-receipt` to persist receipt attempts in a controlled ledger and adds `/v1/release/operator-input-submission-receipts` for readback; repeated identical receipts are idempotent, stale `expected_previous_receipt_id` values fail closed, and the ledger does not modify operator inputs, execute validators, certify evidence or close release gates.
- 2026-06-27 T1303 operator receipt ledger frontend readback keeps `PARAM-097` unchanged. EVENT-20260627-022 binds the committed ledger artifact and `GET /v1/release/operator-input-submission-receipts` hydration into `/development-status`; this is read-only visibility and does not upload files, mutate evidence, execute validators, change formulas, activate release-manager refresh or close release gates.
...
- Purpose: Provide non-scoring operational thresholds for refresh, visual coverage, motion timing, calibration controls, soak runner execution windows, fail-closed A202 review-packet gates, A202 release-decision intake, signed-intake preflight, A202 operator intake gap-packet and signed-intake source-boundary gates, A202/A210 release-decision bundle schema validation/publication binding, A026/A027 gold-quality/intake gates, A209 background heartbeat controls, A210 brand-clearance intake/source-boundary gates, T1303 external release operator intake packet gates, the T1303 template-only operator input kit, fail-closed operator input status visibility, operator input submission preflight/receipt contracts, receipt-ledger persistence and read-only receipt-ledger frontend readback.
- Owner: model owner
- Status: `active`
...
- Evidence references: EEI/data/parameter_catalog.csv:43-84, EEI/config/thresholds/default-v2.json:1, EEI/config/model_runtime_defaults.yaml:1, EEI/artifacts/tests/a209/t1307_operator_soak_readiness.json, EEI/artifacts/tests/a209/t1307_operator_soak_background_progress.json, EEI/artifacts/tests/a202/t1301_operator_review_packet_contract.json, EEI/artifacts/tests/a202/t1301_a202_release_decision_intake_template.json, EEI/artifacts/tests/a202/t1301_a202_signed_intake_preflight.json, EEI/artifacts/tests/a202/t1301_a202_operator_intake_gap_packet.json, EEI/artifacts/tests/a202/t1301_a202_a210_release_decision_bundle_contract.json, EEI/artifacts/tests/a026/t904_a026_a027_production_gold_label_intake_template.json, EEI/artifacts/tests/a026/t904_entity_resolution_gold_evaluation_contract.json, EEI/artifacts/tests/a027/t904_relationship_gold_evaluation_contract.json, EEI/artifacts/tests/a210/t1309_brand_clearance_preflight_contract.json, EEI/artifacts/tests/a210/t1309_brand_clearance_intake_template.json, EEI/artifacts/tests/a205/t1303_external_release_operator_intake_packet.json, EEI/artifacts/operator_input_kit/operator_input_kit_manifest.json
...
- Natural language explanation: Operational deterministic lookup for threshold, motion, visual, refresh, calibration, frontend hydration guard, gold-quality gate, operator-input release blocker visibility and receipt-ledger frontend readback parameters without a scoring formula.
