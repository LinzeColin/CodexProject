...
- 2026-06-27 T1303 external release operator input status adds `PARAM-094` for the status-manifest schema version; it reports missing, rejected template-copy and present-but-unvalidated operator input states under `artifacts/operator_inputs/` while keeping release-manager and MVP release refresh blocked.
