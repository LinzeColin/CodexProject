...
- 2026-06-27 T1303 external release operator input status adds `PARAM-094` for the status-manifest schema version; it reports missing, rejected template-copy and present-but-unvalidated operator input states under `artifacts/operator_inputs/` while keeping release-manager and MVP release refresh blocked. EVENT-20260627-016 exposes the same fail-closed status through `/v1/release/operator-input-status` and `/development-status` without changing scoring formulas, active parameter values, release-manager activation or publication policy.
