      ...
      <div>
        <section class="panel">
          ...
          <div class="cards">
            ...
            <a class="card" id="dataAccessHubLink"><strong>数据接入与回测入口</strong><span>像量化回测入口一样，集中给 QuantLab、行研、赛事分析读取账本视图和 API。</span></a>
...
def _usage_guide_html() -> str:
    glossary = [
        ...
        ("只读视图", "给 QuantLab、行研、赛事分析等系统读取的 SQLite view，不允许下游直接改账本。"),
...
def _data_access_hub_html(
    ...
    *,
    ...
    consumers = [
        {"system": "QuantLab / 量化回测", "use": "读取日度现金流、金融资金流和待复核隔离状态，避免把生活账单误当成交易信号。", "entry": "v_mart_daily_cashflow"},
...
<body>
  <header>
    <h1>数据接入与回测入口</h1>
    <div class="sub">这是给 QuantLab、行研、赛事分析和其他本地子系统使用的账本数据入口。它复用同一个 SQLite 和报告产物，不复制数据库、不重复建设代码，也不默认公网开放。</div>
  </header>
  <main>
    <section class="metrics">
    ...
    data = {
        ...
        "actions": [
            ...
            },
            {
                ...
                "title": "数据接入与回测入口",
                "status": "可打开",
                "target": "像量化回测入口一样给 QuantLab、行研和赛事系统读取账本视图",
