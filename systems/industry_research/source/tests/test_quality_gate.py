...
class ReportQualityGateTest(unittest.TestCase):
    ...
    def test_caps_confidence_when_quantlab_needs_more_evidence(self) -> None:
        content = self._base_daily_content().replace(
            "| 测试ETF | 测试主题 | 0.700 | Medium Confidence Research | 完整 | 验证通过-可继续研究 | QuantLab风险闸门通过 |",
            "| 测试ETF | 测试主题 | 0.700 | High Confidence Research | 完整 | 证据不足-禁止执行买入 | QuantLab风险闸门未通过 |",
        )
        issues = quality_issues_for_content(content, "pre_open")
        self.assertTrue(any("Research confidence exceeds QuantLab cap" in issue for issue in issues))
        self.assertTrue(any("cannot support nonzero buy Volume" in issue for issue in issues))

    def test_rejects_raw_quantlab_internal_status_in_report_body(self) -> None:
    ...
    def test_core_action_table_rejects_zero_holding_background_rows(self) -> None:
        content = self._base_daily_content().replace(
            "| 测试ETF | 建议买入-下跌承接 | 4.000% | 60.000% | 中：可观察但需尾盘确认 | 400.00 | 1000.00 | -3.000% | 10.000% | 0.00 | -2.000% | 14:30-14:55 Australia/Sydney | 明确结论：买入候选保留；账户、QuantLab和尾盘量价通过后重算Volume | 跌幅项0.150%、亏损项0.350%、低仓项1.500%、放量扣减0.000%、目标/风险上限5.000%。 | 下跌承接 | 放量下跌 |",
    ...
    def test_signal_quality_matrix_rejects_original_volume_for_non_executable_positions(self) -> None:
        content = self._base_daily_content().replace(
            "| 测试ETF | 建议买入-下跌承接 | Medium | 趋势区间结论：下跌承接可观察 | 动能结论：中性 | VOL结论：成交量确认 | 两类信号可用 | 需要尾盘止跌 | 明确结论：证据不足时不买；账户、QuantLab和尾盘量价同时通过后重算候选Volume |",
...
"""
    ...
    def test_requires_holding_discipline_core_items(self) -> None:
        content = self._base_daily_content().replace("| 情绪化追涨/补跌 | 需检查 | 单日涨跌触发必须经过反方和QuantLab验证 |", "")
        issues = quality_issues_for_content(content, "pre_open")
...
"""
    ...
    def test_quantlab_blocked_rejects_nonzero_buy_volume_and_buy_downsize_final_rule(self) -> None:
        content = (
            self._base_daily_content()
            .replace("Medium Confidence Research | 完整 | 验证通过-可继续研究 | QuantLab风险闸门通过", "Watch Only | 完整 | 证据不足-禁止执行买入 | QuantLab风险闸门未通过")
            ...
            .replace("| 测试ETF | 建议买入-下跌承接 | 14:30-14:55 Australia/Sydney | 账户闸门通过 | Medium | 尾盘止跌且无负面事件 | 取消买入 | 需要尾盘止跌 | 通过 | 明确结论：买入候选保留；账户、QuantLab和尾盘量价通过后重算Volume |", "| 测试ETF | 建议买入-下跌承接 | 14:30-14:55 Australia/Sydney | 账户闸门通过 | Medium | 尾盘止跌且无负面事件 | 取消买入 | 需要尾盘止跌 | 阻断 | 明确结论：买入降额 |")
        )
        issues = quality_issues_for_content(content, "pre_open")
    ...
    def test_daily_quality_rejects_low_density_watch_decisions(self) -> None:
        content = self._base_daily_content().replace(
            "明确结论：买入候选保留；账户、QuantLab和尾盘量价通过后重算Volume",
            "明确结论：观望；只记录强弱变化。",
        )
        issues = quality_issues_for_content(content, "pre_open")
    ...
    def test_daily_quality_accepts_watch_decisions_with_trigger_metrics(self) -> None:
        content = self._base_daily_content().replace(
            "明确结论：买入候选保留；账户、QuantLab和尾盘量价通过后重算Volume",
            "明确结论：观望；只有成交额>0、事件/QuantLab同向且触发升级条件时才进入重点观察，否则不新增买卖。",
        )
        issues = quality_issues_for_content(content, "pre_open")
...
- 主题强弱需要成交额验证。
    ...
    - 图表说明：热力图使用浅色背景和深色文字，按研究分组聚合，热力图每格列出该板块下明确对象名称、平均涨跌和对象数量；气泡图底部横轴=当日涨跌幅，顶部横轴=左侧下跌承接/风险释放、右侧上涨兑现/趋势延续，左侧纵轴=复合质量分（量价、QuantLab、风险闸门、成交额），右侧纵轴=质量分区（0-40低质量、40-60观察、60+重点跟踪），气泡大小=成交额，颜色=涨跌方向（红涨绿跌），气泡标注=对象名称。
    ### 板块对象明细
...
| 待确认订单 | 正常 | 无 |
| 现金缓冲 | 正常 | 30.000% |
| 情绪化追涨/补跌 | 需检查 | 单日涨跌触发必须经过反方和QuantLab验证 |
## 六、技术面、基本面、价值面综合结论
- 技术面、基本面和价值面均已给出结论。
## 七、收盘执行规则与风控
| Name | Position | 检查时间 | 账户闸门 | 信号质量 | 成立条件 | 不成立动作 | 证据缺口 | QuantLab闸门 | 最终规则 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 测试ETF | 建议买入-下跌承接 | 14:30-14:55 Australia/Sydney | 账户闸门通过 | Medium | 尾盘止跌且无负面事件 | 取消买入 | 需要尾盘止跌 | 通过 | 明确结论：买入候选保留；账户、QuantLab和尾盘量价通过后重算Volume |
## 八、持仓与支付宝历史交易附图
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| T1 | 标的1 | 建议买入技术候选 | 建议买入-下跌承接 | Medium | 买入候选保留 | 1 | 1 | 0 | 2 | 账户/QuantLab/尾盘量价通过后重算Volume |
## 三、训练问题答案与分析逻辑
