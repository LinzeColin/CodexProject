...
DEFAULT_QUANTLAB_BUS_DB = ROOT.parents[2] / "data" / "researchBus" / "ResearchBus.sqlite"
...
def research_bus_db_path(path: Path | str | None = None) -> Path:
    ...
    return DEFAULT_QUANTLAB_BUS_DB
...
def sync_research_bus(
    ...
    *,
    ...
    quantlab_results_path, quantlab_result_count = pull_quantlab_results(target_db, limit=result_limit)
    ...
    with _connect(target_db) as conn:
        _upsert_system_state(
            ...
            str(ROOT),
            {
                ...
                "pulled_quantlab_results": quantlab_result_count,
        ...
        _record_event(
            ...
            "ResearchBus",
            "success",
            f"reports={report_count}; quantlab_results={quantlab_result_count}; validation_tasks={validation_task_count}; independent_validation_runs={validation_run_count}; consumer_behavior_state={consumer_state_count}; holdings={holdings_count}; holding_symbol_mappings={holding_mapping_count}; portfolio_transactions={portfolio_transaction_count}; holding_update_candidates={holding_candidate_count}",
        )
    return {
        ...
        "published_reports": report_count,
        "quantlab_results_path": str(quantlab_results_path),
        "quantlab_result_count": quantlab_result_count,
...
def pull_quantlab_results(db_path: Path | str | None = None, *, limit: int = 500) -> tuple[Path, int]:
    target_db = initialize_research_bus(db_path)
    with _connect(target_db) as conn:
        rows = conn.execute(
            """
    ...
    payload = {
        ...
        "purpose": "行研系统读取 QuantLab 回测结论、决策质量、门禁状态和报告路径。",
        "results": [_row_to_payload(row) for row in rows],
    }
    path = BRIDGE_DIR / "QuantLabResultsFromBus.json"
...
def pull_validation_tasks(db_path: Path | str | None = None, *, limit: int = 500) -> tuple[Path, int]:
    ...
    payload = {
        ...
        "source_system": "ResearchBus",
        "target_system": "AI-Research-System",
        "purpose": "行研系统读取 QuantLab 自动生成和人工维护的待验证研究问题。",
...
def pull_holdings_master(db_path: Path | str | None = None, *, limit: int = 500) -> tuple[Path, int]:
    ...
    payload = {
        ...
        "source_system": "ResearchBus",
        "target_system": "AI-Research-System",
        "purpose": "行研系统读取 QuantLab/消费系统/支付宝导入后的统一持仓主数据。",
...
def _validation_tasks_from_report(payload: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if not paragraphs and symbols:
        paragraphs = [f"{payload.get('title', '')} 提到 {', '.join(symbols[:6])}，需要独立验证系统和 QuantLab 校验。"]
    tasks = []
            ...
            tasks.append(
                {
                    ...
                    "sample_period": "由独立验证系统或 QuantLab 选择；建议覆盖完整市场周期。",
...
def _handle_ai_request(db_path: Path, request_type: str, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if request_type == "pull_quantlab_results":
        path, count = pull_quantlab_results(db_path, limit=int(payload.get("limit") or 500))
        return {"quantlab_results_path": str(path), "quantlab_result_count": count}
...
def _classify_chat_input(text: str) -> str:
    ...
    if any(keyword in lowered for keyword in ["同步", "sync", "刷新", "互通"]):
        return "sync_request"
    if any(keyword in lowered for keyword in ["验证", "回测", "quantlab", "策略", "信号", "rsi", "macd", "均线", "估值"]):
