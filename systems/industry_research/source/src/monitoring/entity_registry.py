...
def _collect_research_bus_entities(builder: _RegistryBuilder, root: Path) -> None:
    ...
    quantlab = _load_json(root / "data" / "report_artifacts" / "quantlab_bridge" / "QuantLabResults.json", {})
    results = quantlab.get("results", []) if isinstance(quantlab, dict) else []
    for row in results if isinstance(results, list) else []:
        builder.add_entity(
            "Strategy",
            row.get("strategy_id") or row.get("result_id") or "",
            source_system="QuantLab",
            source_path=row.get("metadata_path") or root / "data" / "report_artifacts" / "quantlab_bridge" / "QuantLabResults.json",
            ...
            issues="QuantLab row requires downgrade or review" if row.get("status") == "Review" else "",
        )
    for system_name in ["AI-Research-System", "ResearchBus", "QuantLab", "IndependentValidation", "PolicyBridge", "Alipay"]:
