...
def _json_records(as_of: str, root: Path) -> list[DataTrustRecord]:
    patterns = [
        ...
        "data/report_artifacts/quantlab_bridge/*.json",
        "data/report_artifacts/**/_quantlab/validation_summary_*.json",
    ]
    records = []
    seen: set[Path] = set()
...
def _classify_json(path: Path, payload: Any) -> tuple[str, str, str, list[str], int]:
    ...
    if name == "QuantLabResults.json":
        ...
        if weak:
            return "NEEDS_REVIEW", "Watch", "QuantLab 结果含 Review/NeedsMoreEvidence，报告只能降级使用。", [f"weak quantlab rows={len(weak)}"], len(results)
        return "RECONCILED", "Actionable", "QuantLab 结果无明显弱证据状态。", issues, len(results)
