...
from src.reporting.paths import quantlab_dir
...
def run_quantlab_validation(
    ...
    if monte_carlo_runs < DEFAULT_MONTE_CARLO_RUNS:
        raise ValueError("QuantLab simulation requires at least 100,000 Monte Carlo runs when simulation is used.")
    if pipeline_reruns < DEFAULT_PIPELINE_RERUNS:
        raise ValueError("QuantLab validation requires full pipeline rerun at least 2 times.")
    results = [
        _validate_one(as_of, row, price_rows, monte_carlo_runs, pipeline_reruns)
        for row in queue
    ]
    summary = _summary(as_of, results)
    out_dir = quantlab_dir(as_of)
...
def load_quantlab_results(as_of: str) -> list[dict[str, str]]:
    path = quantlab_dir(as_of) / f"validation_results_{as_of}.csv"
    if not path.exists():
        return []
...
def latest_result_by_symbol(as_of: str) -> dict[str, dict[str, str]]:
    return {row.get("symbol", ""): row for row in load_quantlab_results(as_of)}
