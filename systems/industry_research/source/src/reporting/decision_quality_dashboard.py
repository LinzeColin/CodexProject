...
def decision_quality_dashboard(
    ...
    return (
        ...
        "该图用于检查研究线索是否值得进入 QuantLab 验证，不作为最终账户操作依据。"
    )
...
def _plot_dashboard(
    ...
    _plot_research_quality(axes[1][0], factors, events, advice, quantlab_by_symbol)
    _plot_holding_risk(axes[0][1], factors, advice, account_summary, exposure)
    _plot_review_dashboard(axes[1][1], factors, advice, quantlab_by_symbol)
...
def _plot_research_quality(
    ...
    levels = [_confidence_level(item, events, _advice_for(item, advice), quantlab_by_symbol) for item in factors]
    ...
    validation_gaps = sum(
        1
        for item in factors
        if _validation_status(item, _advice_for(item, advice), quantlab_by_symbol)
...
def _plot_review_dashboard(
    ...
    queued = sum(1 for row in active if _validation_status(_factor_for(row, factors), row, quantlab_by_symbol) == "ValidationQueued")
    weak_evidence = sum(
        1
        for row in active
        if _validation_status(_factor_for(row, factors), row, quantlab_by_symbol)
...
def _confidence_level(
    ...
    if _float(advice_row.get("holding_amount")) > 0:
        score += 10
    validation_status = _validation_status(item, advice_row, quantlab_by_symbol)
...
def _validation_status(
    item: dict[str, object],
    advice_row: dict[str, object],
    quantlab_by_symbol: dict[str, dict[str, str]] | None = None,
) -> str:
    symbol = str(item.get("symbol") or advice_row.get("symbol") or "")
    validation = (quantlab_by_symbol or {}).get(symbol, {})
