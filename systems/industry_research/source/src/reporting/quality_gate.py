...
from src.reporting.paths import markdown_path, pdf_path, quantlab_dir, source_log_path, weekly_report_dir
...
FORBIDDEN_PHRASES = [
    ...
    "等待基本面",
    "观望但记录强弱变化",
    "低：账户缺口，执行归零；行情可核、成交额可核、QuantLab=NeedsMoreEvidence、风险闸门阻断",
    "可保留原Volume上限",
    "量价背离则降到50%",
    "买入降额到50% Volume",
    "只执行 50% Volume",
...
def run_report_quality_gate(as_of: str, report_kind: str, strict_week_folder: bool = True) -> list[str]:
    ...
    issues.extend(_quantlab_issues(as_of, content))
...
def quality_issues_for_content(content: str, report_kind: str) -> list[str]:
    ...
    issues.extend(_quantlab_action_gate_issues(content))
...
def _decision_density_issues(content: str, report_kind: str) -> list[str]:
    ...
    if not decision_text:
        return issues
    conditional_markers = ["成交额", "QuantLab", "事件", "触发", "否则", "取消", "降额", "暂停", "升级", "Volume"]
...
def _confidence_table_issues(content: str) -> list[str]:
    ...
    for row in _markdown_rows(content):
        ...
        if _LEVEL_ORDER[level] > _LEVEL_ORDER[cap]:
            issues.append(f"Research confidence exceeds QuantLab cap: level={level}, status={status}, row={' | '.join(row[:3])}")
    return issues
...
def _quantlab_action_gate_issues(content: str) -> list[str]:
    blocked = _quantlab_blocked_names(content)
    if not blocked:
        return []
    issues = []
    for table_rows in _markdown_tables(content):
        ...
        for row in table_rows[1:]:
            ...
            if _is_buy_action_text(position) and volume_idx is not None and volume_idx < len(row) and _numeric_cell_value(row[volume_idx]) > 0:
                issues.append(
                    "QuantLab blocked/insufficient validation cannot support nonzero buy Volume: "
                    f"name={name}, position={position}, volume={row[volume_idx]}, reason={reason}"
                )
            if _is_buy_action_text(position) and _quantlab_final_rule_keeps_buy(final_rule):
                issues.append(
                    "QuantLab blocked/insufficient validation buy row must end in cancel/watch/wait, not buy-downsize: "
...
def _signal_matrix_consistency_issues(content: str) -> list[str]:
    section = _section_text(content, "研究可信度与 QuantLab 验证")
...
def _quantlab_blocked_names(content: str) -> dict[str, str]:
    ...
    for table_rows in _markdown_tables(content):
        ...
        status_idx = _first_index(headers, ["QuantLab状态", "验证状态"])
        gate_idx = _first_index(headers, ["风险闸门", "QuantLab闸门"])
...
def _quantlab_final_rule_keeps_buy(value: str) -> bool:
    if not value:
        return False
...
def _daily_execution_rule_issues(content: str) -> list[str]:
    ...
    issues = []
    for phrase in ["检查时间", "账户闸门", "成立条件", "不成立动作", "证据缺口", "QuantLab闸门", "最终规则"]:
        if phrase not in section:
...
def _local_path_leak_issues(content: str) -> list[str]:
    ...
    patterns = [
        ...
        (r"\b_source_logs\b|\b_markdown\b|\b_excel_outputs\b|\b_quantlab\b", "internal artifact directory"),
...
def _quantlab_issues(as_of: str, content: str) -> list[str]:
    issues = []
    if "QuantLab 验证队列" not in content and "研究可信度与 QuantLab 验证" not in content:
        return issues
    summary_path = quantlab_dir(as_of) / f"validation_summary_{as_of}.json"
    results_path = quantlab_dir(as_of) / f"validation_results_{as_of}.csv"
    queue_path = quantlab_dir(as_of) / f"thesis_queue_{as_of}.csv"
    for path in [summary_path, results_path, queue_path]:
        if not path.exists():
            issues.append(f"Missing QuantLab artifact: {path}")
    if summary_path.exists():
        payload = json.loads(summary_path.read_text(encoding="utf-8"))
        if int(payload.get("total") or 0) <= 0:
            issues.append("QuantLab summary has no validation rows.")
        if "100,000" not in str(payload.get("rule", "")) and "100000" not in str(payload.get("rule", "")):
            issues.append("QuantLab summary does not state the >=100,000 simulation rule.")
    if "100000次" not in content and "100,000" not in content:
        issues.append("Report does not show the QuantLab 100,000 simulation requirement/result.")
    if "全流程2次" not in content and "全流程>=2次" not in content:
        issues.append("Report does not show the QuantLab full-pipeline rerun requirement/result.")
    return issues
...
def _remediation_for(status: str, expected: dict[str, str], issues: list[str]) -> dict[str, str]:
    ...
    if status in {"missing", "quality_fail"}:
        ...
        return {
            "next_action": action,
            "repair_command": f"python3 -m src.cli quantlab-refresh --date {report_date} && {generator}",
