...
This directory contains the migrated source package for the EVA_OS policy intelligence and government document interpretation system.
...
Or run the consolidated EVA_OS smoke:
...
Build a read-only EVA_OS adapter that publishes policy queue readiness, external-reference gaps, quality-gate status, and source authority summaries to ResearchBus. Do not publish raw document text, cookies, API keys, or local platform-auth paths.
