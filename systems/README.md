# EVA_OS Systems Workspace

This directory is the controlled landing zone for child systems that will be migrated into EVA_OS one at a time.
...
| --- | --- | --- | --- |
| Finance Ledger / Consumption Analysis | `systems/finance_ledger` | source/tests/docs migrated | add EVA_OS adapter and run macOS acceptance after unified UI Shell |
| Industry Research / Trading Strategy Advice | `systems/industry_research` | source/tests/docs migrated | add EVA_OS adapter and keep report generation fail-closed |
| Policy Intelligence | `systems/policy_intelligence` | source/tests/docs migrated | add EVA_OS adapter and restore private-runtime automation freshness |

Each system must keep a `SYSTEM_MANIFEST.json` aligned with `shared/schema/system_manifest.schema.json`.

`ResearchBus` consumes these manifests through `quantlab.integrations.workspace_systems`.
