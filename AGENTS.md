...
模型名、错误信息可保留英文。项目级 `AGENTS.md` 可以追加规则，但不能
弱化本文件；细节以 `docs/governance/STANDARD.md` 为准。

## 永久规则

- Every active project must be registered in `governance/projects.yaml`.
- Every active project must expose project-root `功能清单.md`, `开发记录.md`,
...
Model means math/statistical/ML models, ranking/scoring, backtests, risk,
salary/business formulas, rule engines, heuristics, and LLM routing/fallbacks.
Tech stack names are not models; no-model projects still need `MODEL_SPEC.md`
with evidence-backed `NOT_APPLICABLE`.
