...
static int running_url(char *out, size_t out_size) {
  char command[PATH_MAX + 128];
  for (int port = 8501; port <= 8510; port++) {
    snprintf(
        command,
        sizeof(command),
        "/usr/bin/curl --connect-timeout 0.15 --max-time 0.3 -fsS http://127.0.0.1:%d/_stcore/health >/dev/null 2>&1",
        port);
    if (run_quiet(command)) {
...
static int open_target(const char *target) {
  char *const argv[] = {"open", (char *)target, NULL};
  return spawn_detached("/usr/bin/open", argv) == 0;
}
...
int main(void) {
  ...
  join_path(root_file, sizeof(root_file), contents_dir, "Resources/EVA_OS_PROJECT_ROOT");

  char candidates[8][PATH_MAX];
  int candidate_count = 0;
  add_candidate(candidates, &candidate_count, getenv("EVA_OS_HOME"));
  ...
  if (home != NULL && home[0] != '\0') {
    ...
    const char *workspace_root = getenv("EVA_WORKSPACE_ROOT");
    if (workspace_root != NULL && workspace_root[0] != '\0') {
      snprintf(workspace_candidate, sizeof(workspace_candidate), "%s/systems/eva_os", workspace_root);
    } else {
      snprintf(workspace_candidate, sizeof(workspace_candidate), "%s/Documents/EVA_OS_Workspace/systems/eva_os", home);
    }
    add_candidate(candidates, &candidate_count, workspace_candidate);

    char codex_candidate[PATH_MAX];
    snprintf(codex_candidate, sizeof(codex_candidate), "%s/Documents/Codex/EVA_OS", home);
    add_candidate(candidates, &candidate_count, codex_candidate);
  }

  int dry_run = getenv("EVA_OS_APP_LAUNCH_DRY_RUN") != NULL && strcmp(getenv("EVA_OS_APP_LAUNCH_DRY_RUN"), "1") == 0;
  char existing_url[128];
  if (running_url(existing_url, sizeof(existing_url))) {
    if (dry_run) {
      printf("EVA_OS_APP_LAUNCH: existing_url=%s mode=open-url\n", existing_url);
      return 0;
    }
    return open_target(existing_url) ? 0 : 1;
  }

  for (int i = 0; i < candidate_count; i++) {
    char command_path[PATH_MAX];
    if (!join_path(command_path, sizeof(command_path), candidates[i], "StartQuantLab.command")) {
      continue;
    }
    ...
    if (dry_run) {
      printf(
          "EVA_OS_APP_LAUNCH: project=%s command=./StartQuantLab.command command_path=%s mode=spawn-command\n",
          candidates[i],
          command_path);
      return 0;
    }
  ...
  if (dry_run) {
    printf("EVA_OS_APP_LAUNCH: missing_project\n");
    return 1;
  }

  if (file_exists(root_file)) {
    show_error("display dialog \"EVA_OS local project was not found or StartQuantLab.command is not executable. Reinstall EVA_OS.app from a local checkout.\" buttons {\"OK\"} default button \"OK\" with icon caution");
  } else {
    show_error("display dialog \"EVA_OS local project binding is missing. Reinstall EVA_OS.app from a local checkout.\" buttons {\"OK\"} default button \"OK\" with icon caution");
  }
  return 1;
}
