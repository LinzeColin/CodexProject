...
from quantlab.strategies.custom_builder import CUSTOM_STRATEGY_SPEC_PATH, custom_strategy_spec_from_payload, save_custom_strategy_spec


CUSTOM_STRATEGY_DIR = PROJECT_ROOT / "src" / "quantlab" / "strategies" / "custom"
...
def _load_strategy_class(path: Path):
    ...
    module_name = f"quantlab_custom_smoke_{path.stem}"
...
from quantlab.strategies.base import StrategyResult
from quantlab.strategies.custom_builder import CustomNoCodeStrategy, get_custom_strategy_spec
...
默认状态为 `Pending`。完成研究假设、代码审查、数据验证和风险说明后，再在 QuantLab 中确认审批。

## Approval Status

Default status is `Pending`. Confirm approval in QuantLab only after thesis, code review, data validation, and risk notes are complete.
"""
