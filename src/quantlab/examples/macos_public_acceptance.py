...
def main() -> None:
    ...
    parser.add_argument("--json", action="store_true", help="Print EVAOSMacOSPublicAcceptanceSummaryV1 as JSON.")
    ...
    print(
        "EVA_OS_MACOS_PUBLIC_ACCEPTANCE: "
        f"status={payload['status']} "
        f"sources_pass={summary['sources_pass']}/{summary['sources_total']} "
        f"runtime={summary['runtime_status']} "
        f"ui={summary['ui_status']}"
    )
    print(f"EVA_OS_MACOS_PUBLIC_ACCEPTANCE_NEXT_ACTION: {payload['next_action']}")
    if payload.get("outputs"):
        print(f"EVA_OS_MACOS_PUBLIC_ACCEPTANCE_OUTPUTS: {payload['outputs']}")
