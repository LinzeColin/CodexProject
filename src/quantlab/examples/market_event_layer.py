...
from quantlab.data.market_events import build_market_event_log, write_market_event_log
from quantlab.data.models import BarDataRequest
from quantlab.data.providers.sample_provider import SampleDataProvider


def main() -> None:
    parser = argparse.ArgumentParser(description="Build a local EVA_OS market event log from sample bars or a CSV file.")
    ...
    if payload.get("outputs"):
        print(f"EVA_OS_MARKET_EVENT_LAYER_OUTPUTS: {payload['outputs']}")
