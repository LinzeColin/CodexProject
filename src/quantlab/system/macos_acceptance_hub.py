...
MACOS_ACCEPTANCE_HUB_SCHEMA = "EVAOSMacOSAcceptanceHubV1"
...
MODE_SPECS: dict[str, dict[str, Any]] = {
    ...
    "app-entry": {
        "label": "App 入口检查",
        "description": "只读检查 Desktop、Downloads、Applications 的 EVA_OS.app 入口。",
        "actions": (
    ...
    "app-runtime": {
        "label": "App 打开路径验收",
        "description": "通过 Downloads EVA_OS.app 打开路径做真实启动/停止闭环；必须显式选择。",
        "actions": (
            AcceptanceAction(
                "app_runtime",
                "App 打开路径验收",
...
def _compact_payload(payload: dict[str, Any] | None) -> dict[str, Any]:
    ...
    if payload.get("schema") == "EVAOSDevReadyCheckV1":
    ...
    if payload.get("schema") == "EVAOSMacOSPublicAcceptanceSummaryV1":
    ...
    if payload.get("schema") == "EVAOSUIVisualAcceptanceV1":
