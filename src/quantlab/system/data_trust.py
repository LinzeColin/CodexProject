...
from quantlab.config import DATA_DIR, PROJECT_ROOT, REPORT_ROOT_DIR
...
def build_data_trust_audit(
    *,
    ...
    records: list[QuantLabDataTrustRecord] = []
    ...
    return {
        "schema": "QuantLabDataTrustAuditV1",
        "system": "QuantLab",
        ...
        "assumptions": [
            "This audit is read-only and only classifies local QuantLab files.",
...
def write_data_trust_audit(
    *,
    ...
    stem = f"QuantLabDataTrustAudit_{stamp}"
...
def _project_records(root: Path) -> list[QuantLabDataTrustRecord]:
    records = [
...
def _data_provider_records(root: Path) -> list[QuantLabDataTrustRecord]:
    provider_dir = root / "src" / "quantlab" / "data" / "providers"
    provider_files = [
    ...
    records.append(_required_path_record(root / "src" / "quantlab" / "data" / "quality.py", root, "Data Providers", "python", "数据质量检查模块存在。", "回测结论前必须结合缺口、重复值、异常价格和覆盖率检查。"))
    records.append(_required_path_record(root / "src" / "quantlab" / "data" / "validation.py", root, "Data Providers", "python", "数据校验模块存在。", "真实研究前必须运行跨源校验或记录缺失原因。"))
...
def _strategy_records(root: Path) -> list[QuantLabDataTrustRecord]:
    strategy_root = root / "src" / "quantlab" / "strategies"
    records: list[QuantLabDataTrustRecord] = [
...
def _holding_records(root: Path) -> list[QuantLabDataTrustRecord]:
    ...
    if history_payload.exists and history_payload.valid and isinstance(history_payload.payload, dict) and isinstance(history_payload.payload.get("history"), list):
        ...
        records.append(_path_record(history, root, "Holdings", "json", "NEEDS_REVIEW", "FACT", "Watch", "持仓同步历史 JSON 可读但 schema 不符合 QuantLabHoldingsImportHistoryV1。", "保留文件，人工确认是否需要迁移旧历史结构。", user_confirmation_required=True))
    else:
...
def _research_bus_records(root: Path) -> list[QuantLabDataTrustRecord]:
...
def _validation_queue_records(root: Path) -> list[QuantLabDataTrustRecord]:
...
def _independent_validation_records(root: Path, *, max_runs: int) -> list[QuantLabDataTrustRecord]:
...
def _cache_records(root: Path) -> list[QuantLabDataTrustRecord]:
...
def _raw_processed_records(root: Path) -> list[QuantLabDataTrustRecord]:
    records: list[QuantLabDataTrustRecord] = []
...
def _experiment_records(root: Path) -> list[QuantLabDataTrustRecord]:
...
def _report_catalog_records(root: Path, report_root: Path) -> list[QuantLabDataTrustRecord]:
...
def _json_record(path: Path, root: Path, group: str, *, expected_type: type | tuple[type, ...] | None, status_if_valid: str, issue_if_valid: str, next_action: str) -> QuantLabDataTrustRecord:
    payload = _load_json(path)
...
def _csv_record(path: Path, root: Path, group: str, *, status_if_valid: str, issue_if_valid: str, next_action: str, user_confirmation_required: bool = False) -> QuantLabDataTrustRecord:
    if not path.exists():
...
def _missing_or_rejected_record(path: Path, root: Path, group: str, issue: str, next_action: str) -> QuantLabDataTrustRecord:
...
def _path_record(
    ...
    *,
    ...
    return QuantLabDataTrustRecord(
...
def _required_path_record(path: Path, root: Path, group: str, source_type: str, issue: str, next_action: str) -> QuantLabDataTrustRecord:
    if path.exists():
...
def _advisory_path_record(path: Path, root: Path, group: str, source_type: str, next_action: str) -> QuantLabDataTrustRecord:
    if path.exists():
...
def _pid_file_status(path: Path) -> tuple[str, str]:
    ...
    if proc_cmdline.exists():
        try:
            ...
            cmdline = ""
        if "quantlab" in cmdline or "streamlit_app.py" in cmdline or "streamlit" in cmdline:
            return "NEEDS_REVIEW", "PID 文件对应 QuantLab/Streamlit 相关进程仍存在，需要确认是否为当前服务。"
        return "ARCHIVED", "PID 文件对应进程存在但不是 QuantLab/Streamlit，按旧运行残留归档。"

    return "NEEDS_REVIEW", "PID 文件对应进程仍存在，需要确认是否为当前 QuantLab 服务。"
...
def _audit_markdown(audit: dict[str, Any]) -> str:
    lines = [
        f"# QuantLab Data Trust Audit {audit['as_of']}",
        "",
        "## Summary",
...
def _write_records_csv(path: Path, rows: list[dict[str, Any]]) -> None:
    ...
    columns = list(QuantLabDataTrustRecord.__dataclass_fields__)
