...
MACOS_RUNTIME_ACCEPTANCE_SCHEMA = "EVAOSMacOSRuntimeAcceptanceV1"
    ...
    try:
        if pre_ports:
            ...
            if method == "app":
                ...
                checks.append(_check("Start", "StartCommandLaunched", "Pass", "scripts/startQuantLab.sh launched in controlled background mode"))
        ...
        if not healthy_after_start:
            stop_after_failed_start = _run_script(root, ["scripts/stopQuantLab.sh"], timeout_seconds=max(10, stop_timeout_seconds))
            checks.append(_script_check("Stop", "StopAfterFailedStart", stop_after_failed_start, expected_text="QuantLab stop command completed."))
            return _payload(
                root,
                checks,
                pre_ports,
        ...
        running_status = _run_script(root, ["scripts/statusQuantLab.sh"], timeout_seconds=support_timeout)
        checks.append(_script_check("Runtime", "StatusSeesRunning", running_status, expected_text="QuantLab running:"))
        ...
        stop_result = _run_script(root, ["scripts/stopQuantLab.sh"], timeout_seconds=max(10, stop_timeout_seconds))
        checks.append(_script_check("Stop", "StopScriptRuns", stop_result, expected_text="QuantLab stop command completed."))
        ...
        stopped_status = _run_script(root, ["scripts/statusQuantLab.sh"], timeout_seconds=support_timeout)
        ...
        cache_preview_ok = cache_preview["returncode"] == 0 and "EVACacheCleanupReportV1" in cache_preview["stdout"]
        checks.append(
            _check(
                "Cache",
        ...
        if started_by_acceptance:
            _run_script(root, ["scripts/stopQuantLab.sh"], timeout_seconds=max(10, stop_timeout_seconds))
...
def _payload(
    ...
    *,
    ...
    return {
        ...
        "checks": checks,
        "runtime_contract": {
            "start": "Launches scripts/startQuantLab.sh or opens a configured EVA_OS.app in controlled mode when no existing service is running.",
            ...
            "stop": "Runs scripts/stopQuantLab.sh and verifies local health disappears.",
        ...
        "safety_boundary": (
            ...
            "EVA_OS service is already running."
        ),
...
def _start_process(root: Path) -> subprocess.Popen[str]:
    return subprocess.Popen(
        [str(root / "scripts" / "startQuantLab.sh")],
        cwd=root,
...
def _launch_app(root: Path, app_path: Path | str | None) -> dict[str, Any]:
    target = Path(app_path).expanduser() if app_path else Path.home() / "Downloads" / "EVA_OS.app"
    if not target.is_dir():
        fallback = root / "macos" / "EVA_OS.app"
    ...
    if completed.returncode == 0 and not _app_launch_observed(root, wait_seconds=12):
        executable = target / "Contents" / "MacOS" / "EVA_OS"
...
def _app_launch_observed(root: Path, *, wait_seconds: int) -> bool:
    log_path = root / "data" / "cache" / "quantlab_macos_app.log"
