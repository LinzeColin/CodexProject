...
MACOS_ACCEPTANCE_LITE_SCHEMA = "EVAOSMacOSAppAcceptanceLiteV1"
APP_BUNDLE_NAME = "EVA_OS.app"
APP_EXECUTABLE_NAME = "EVA_OS"
...
def _app_checks(
    ...
    *,
    ...
    root_file = app_path / "Contents" / "Resources" / "EVA_OS_PROJECT_ROOT"
    checks = [
...
def _project_checks(root: Path) -> list[dict[str, Any]]:
    return [
        _check("Project", "StartCommandExecutable", "Pass" if _is_executable(root / "StartQuantLab.command") else "Fail", "StartQuantLab.command"),
        _check("Project", "StatusScriptExecutable", "Pass" if _is_executable(root / "scripts" / "statusQuantLab.sh") else "Fail", "scripts/statusQuantLab.sh"),
...
def _plist_check(label: str, plist_path: Path) -> dict[str, Any]:
    ...
    status = "Pass" if display == "EVA_OS" and executable == APP_EXECUTABLE_NAME else "Fail"
...
def _github_fallback_check(label: str, executable: Path) -> dict[str, Any]:
    ...
    found = b"github.com/LinzeColin/EVA_OS" in content
...
def _dry_run_check(label: str, executable: Path, timeout_seconds: int) -> dict[str, Any]:
    ...
    env = dict(os.environ)
    env["EVA_OS_APP_LAUNCH_DRY_RUN"] = "1"
    ...
    ok = completed.returncode == 0 and (
        ...
        or "mode=open-url" in output
        or "existing_url=" in output
    )
...
def _next_action(summary: dict[str, int]) -> str:
    if summary.get("fail", 0):
        return "Run scripts/installEVAOSEntryApps.sh, then rerun scripts/macosAppAcceptanceLite.sh --json."
    return "Use EVA_OS.app for normal launch; reserve finalAcceptanceCheck.sh for deliberate full release gates."
