...
from quantlab.data import provider_status_rows
from quantlab.reports.catalog import artifact_counts, latest_report_artifact, report_artifacts_frame
...
from quantlab.system.integration_audit import build_quantlab_integration_audit
...
def write_daily_readiness(
    *,
    ...
    stem = f"QuantLabDailyReadiness_{_date_stamp(str(payload['as_of']))}"
...
def daily_readiness_markdown(payload: dict[str, Any]) -> str:
    lines = [
        f"# QuantLab Daily Readiness {payload.get('as_of', '')}",
        "",
        "## Summary",
...
def _gate_rows(data_trust: dict[str, Any], integration: dict[str, Any], counts: dict[str, int], latest_report: dict[str, Any]) -> list[dict[str, str]]:
    ...
    return [
        ...
        },
        {
            ...
            "next_action": "Run scripts/auditQuantLabIntegration.sh --no-write if not Pass.",
        },
        {
