...
from quantlab.system.macos_acceptance import build_macos_app_acceptance_lite


MACOS_LIFECYCLE_READINESS_SCHEMA = "EVAOSMacOSLifecycleReadinessV1"
...
def build_macos_lifecycle_readiness(
    *,
    ...
    return {
        ...
        "checks": checks,
        "lifecycle_contract": {
            "start": "StartQuantLab.command or scripts/startQuantLab.sh starts local Streamlit on 127.0.0.1 and reuses an existing service.",
            "stop": "scripts/stopQuantLab.sh only stops Streamlit processes for this checkout.",
            "auto_shutdown": "StartQuantLab.command runs quantlab.system.shutdown_monitor; the UI sends sanitized localhost heartbeat requests.",
        ...
        "safety_boundary": (
            "Read-only readiness inspection. It may run statusQuantLab.sh and build dry-run cache/app summaries, "
...
def _static_lifecycle_checks(root: Path) -> list[dict[str, Any]]:
    start_command = root / "StartQuantLab.command"
    start_script = root / "scripts" / "startQuantLab.sh"
    stop_script = root / "scripts" / "stopQuantLab.sh"
    status_script = root / "scripts" / "statusQuantLab.sh"
    ...
    streamlit_app = root / "src" / "quantlab" / "app" / "streamlit_app.py"
    shutdown_monitor = root / "src" / "quantlab" / "system" / "shutdown_monitor.py"
    cache_cleanup = root / "src" / "quantlab" / "system" / "cache_cleanup.py"
    checks = [
        _executable_check("Command", "StartQuantLabExecutable", start_command),
        ...
        _text_check("Start", "RuntimeResolver", start_command, ("quantlab_ensure_app_python",)),
        ...
        _text_check("Start", "LaunchLock", start_command, ("quantlab_launch.lockdir", "LOCK_PID_FILE")),
        _text_check("Start", "AutoShutdownMonitor", start_command, ("quantlab.system.shutdown_monitor", "STREAMLIT_PID")),
        _text_check("Start", "HeartbeatUrl", start_command, ("QUANTLAB_HEARTBEAT_URL", "HEARTBEAT_PORT")),
        _text_check("Start", "HeartbeatTimeoutDefault", start_command, ("QUANTLAB_HEARTBEAT_TIMEOUT:-120",)),
        _text_check("QuietStart", "RuntimeResolver", start_script, ("quantlab_ensure_app_python",)),
        _text_check("QuietStart", "LocalhostBinding", start_script, ("--server.address", "127.0.0.1")),
        _text_check("Stop", "ScopedStop", stop_script, ("process_cwd", 'cwd_path" == "$PROJECT_DIR"', "src/quantlab/app/streamlit_app.py")),
        ...
        _text_check("Cache", "ScopedRunningDetection", clean_script, ("quantlab_is_running", "process_cwd", 'cwd_path" == "$PROJECT_DIR"')),
        _text_check("Cache", "DryRunMode", clean_script, ("--dry-run", "quantlab.system.cache_cleanup")),
        _text_check("Cache", "PreservesArtifacts", cache_cleanup, ("Reports, holdings", "market bar caches are not deleted")),
        _text_check("Heartbeat", "UIHeartbeatInstalled", streamlit_app, ("install_shutdown_heartbeat()", "QUANTLAB_HEARTBEAT_URL", "fetch(heartbeatUrl")),
...
def _runtime_status(root: Path, *, run_status_script: bool) -> dict[str, Any]:
    ...
    if not run_status_script:
        ...
        return payload
    script = root / "scripts" / "statusQuantLab.sh"
...
def _cache_preview_check(preview: dict[str, Any]) -> dict[str, Any]:
    ok = preview.get("schema") == "EVACacheCleanupReportV1" and preview.get("failed_count", 0) == 0
...
def _app_acceptance_check(summary: dict[str, Any]) -> dict[str, Any]:
    ok = summary.get("schema") == "EVAOSMacOSAppAcceptanceLiteV1" and summary.get("status") == "Pass"
