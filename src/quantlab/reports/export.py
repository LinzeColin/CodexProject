...
from quantlab.app.dashboard import backtest_result_judgements, buy_and_hold_metrics
from quantlab.analysis import bootstrap_equity_robustness, build_strategy_diagnostics, robustness_summary_rows
from quantlab.backtest import BacktestResult
from quantlab.config import report_date_dir
from quantlab.data.validation import CrossSourceValidationResult
from quantlab.risk import DecisionQualityResult, RiskGateResult, evaluate_decision_quality, evaluate_research_risk_gates
from quantlab.storage import atomic_write_json
from quantlab.strategies.profiles import RETURN_SOURCE_TAXONOMY, get_strategy_profile
...
def export_experiment_docx(
    ...
    from quantlab.reports.catalog import load_experiment_detail
...
def _add_title(doc: Document) -> None:
    ...
    run = title.add_run("QuantLab 回测研究报告")
    run.bold = True
    run.font.size = Pt(21)
    run.font.color.rgb = RGBColor(32, 36, 43)
    subtitle = doc.add_paragraph()
    subtitle.alignment = WD_ALIGN_PARAGRAPH.CENTER
    subtitle.add_run("QuantLab Backtest Research Report").italic = True


def _add_experiment_title(doc: Document, experiment_name: str) -> None:
    title = doc.add_paragraph()
    title.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run = title.add_run("QuantLab 实验研究报告")
    run.bold = True
    run.font.size = Pt(21)
    run.font.color.rgb = RGBColor(32, 36, 43)
    subtitle = doc.add_paragraph()
    subtitle.alignment = WD_ALIGN_PARAGRAPH.CENTER
    subtitle.add_run("QuantLab Experiment Research Report").italic = True
...
def _add_strategy_review_title(doc: Document, strategy_id: str) -> None:
    title = doc.add_paragraph()
    title.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run = title.add_run("QuantLab 候选策略审查报告")
    run.bold = True
    run.font.size = Pt(21)
    run.font.color.rgb = RGBColor(32, 36, 43)
    subtitle = doc.add_paragraph()
    subtitle.alignment = WD_ALIGN_PARAGRAPH.CENTER
    subtitle.add_run("QuantLab Candidate Strategy Review Report").italic = True
    name = doc.add_paragraph()
    name.alignment = WD_ALIGN_PARAGRAPH.CENTER
...
def _report_bootstrap_simulations() -> int:
    value = os.environ.get("QUANTLAB_TEST_BOOTSTRAP_SIMULATIONS")
    if value:
        try:
