...
from quantlab.config import DATA_DIR, REPORT_ROOT_DIR
from quantlab.integrations.external_systems import collect_industry_reports
from quantlab.integrations.holding_symbol_map import build_entity_registry, holdings_symbol_proxy_frame
from quantlab.integrations.holdings_book import load_current_holdings
from quantlab.research.validation_queue import VALIDATION_QUEUE_PATH, create_validation_task, load_validation_tasks
from quantlab.storage import atomic_write_json, locked_json_update
...
DEFAULT_AI_RESEARCH_ROOT = Path("$EVA_AI_RESEARCH_ROOT")
...
class ResearchBusSyncResult:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "quantlab_results": self.quantlab_results,
...
def research_bus_db_path(path: Path | str | None = None) -> Path:
    ...
    configured = os.getenv("QUANTLAB_RESEARCH_BUS_DB", "").strip()
    if configured:
...
def sync_all_research_bus(
    ...
    *,
    ...
    try:
        from quantlab.integrations.system_orchestrator import register_default_systems, sync_default_system_artifacts
    ...
    results_result = sync_quantlab_results_to_bus(report_root, target_db, ai_research_root=ai_research_root)
    ...
    try:
        from quantlab.integrations.consumer_behavior import sync_consumer_behavior_state
    ...
    return ResearchBusSyncResult(
        ...
        quantlab_results=results_result.quantlab_results,
...
def sync_industry_reports_to_bus(
    ...
    *,
    ...
    if push_validation_queue:
        pushed = push_bus_validation_tasks_to_quantlab_queue(target_db)
    return ResearchBusSyncResult(
        synced_at=_now(),
        db_path=str(target_db),
...
def sync_quantlab_results_to_bus(
    report_root: Path | str | None = None,
    db_path: Path | str | None = None,
    *,
    ai_research_root: Path | str | None = None,
) -> ResearchBusSyncResult:
    from quantlab.reports.catalog import run_metadata_summaries_frame
    ...
    with _connect(target_db) as conn:
        ...
        for row in runs.to_dict("records"):
            ...
            result_id = _stable_id("quantlabResult", str(metadata_path.resolve()) if metadata_path.exists() else json.dumps(row, sort_keys=True, ensure_ascii=False))
            _upsert_quantlab_result(conn, result_id, row, payload)
            exported_payloads.append({"result_id": result_id, **row, "payload": payload})
            result_count += 1
        _record_event(conn, "quantlab_results_sync", "QuantLab", "ResearchBus", "success", f"results={result_count}")
    bridge_path = ""
    if ai_research_root is not None:
        bridge_path = str(write_quantlab_results_for_ai_research(exported_payloads, ai_research_root))
    elif DEFAULT_AI_RESEARCH_ROOT.exists():
        bridge_path = str(write_quantlab_results_for_ai_research(exported_payloads, DEFAULT_AI_RESEARCH_ROOT))
    else:
        warnings.append("未找到行研系统目录，已只写入共享研究数据总线。")
    return ResearchBusSyncResult(
        synced_at=_now(),
        db_path=str(target_db),
        quantlab_results=result_count,
        events=1,
    ...
    with _connect(target_db) as conn:
        ...
        if not frame.empty:
            ...
            for row in frame.to_dict("records"):
                ...
                holding_count += 1
        _record_event(conn, "holdings_sync", "QuantLab", "ResearchBus", "success", f"holdings={holding_count}")
...
def sync_holding_symbol_mappings_to_bus(
    ...
    with _connect(target_db) as conn:
        ...
        conn.execute("DELETE FROM holding_symbol_mappings WHERE source_system='QuantLab'")
...
def push_bus_validation_tasks_to_quantlab_queue(
    db_path: Path | str | None = None,
...
def extract_validation_candidates(
    ...
    if not paragraphs and symbols:
        paragraphs = [f"{path.name} 中提到 {', '.join(symbols[:6])}，需由 QuantLab 补充验证。"]
            ...
            candidates.append(
                {
                    ...
                    "sample_period": "由 QuantLab 回测页面选择；建议至少覆盖一轮牛熊或完整行业周期。",
                    "cost_assumption": "使用 QuantLab 当前手续费、滑点、冲击成本设置；报告中必须披露公式和假设。",
...
def research_bus_status_frame(db_path: Path | str | None = None) -> pd.DataFrame:
    ...
    with _connect(target_db) as conn:
        ...
        counts = {
            ...
            "quantlab_results": _table_count(conn, "quantlab_results"),
...
def bus_quantlab_results_frame(db_path: Path | str | None = None) -> pd.DataFrame:
    target_db = initialize_research_bus(db_path)
    with _connect(target_db) as conn:
        rows = conn.execute(
            """
...
def write_quantlab_results_for_ai_research(
    results: list[dict[str, Any]],
    ai_research_root: Path | str,
) -> Path:
    root = Path(ai_research_root).expanduser()
    out_path = root / "data" / "report_artifacts" / "quantlab_bridge" / "QuantLabResults.json"
    payload = {
        "schema": RESEARCH_BUS_SCHEMA_VERSION,
        "exported_at": _now(),
        "source_system": "QuantLab",
        "purpose": "供行研系统读取 QuantLab 回测、门禁、决策质量和报告路径。",
        "results": results[-500:],
    }
...
def _create_schema(conn: sqlite3.Connection) -> None:
    conn.executescript(
        ...
        CREATE TABLE IF NOT EXISTS orchestration_runs (
            ...
            parent_system TEXT NOT NULL DEFAULT 'QuantLab',
        ...
        CREATE TABLE IF NOT EXISTS holding_symbol_mappings (
            ...
            source_system TEXT NOT NULL DEFAULT 'QuantLab',
        ...
        CREATE INDEX IF NOT EXISTS idx_results_strategy ON quantlab_results(strategy_id);
...
def _upsert_quantlab_result(conn: sqlite3.Connection, result_id: str, row: dict[str, Any], payload: dict[str, Any]) -> None:
...
def _default_transaction_roots() -> tuple[Path, ...]:
    configured = os.getenv("QUANTLAB_PORTFOLIO_TRANSACTION_DIR", "").strip()
    if configured:
