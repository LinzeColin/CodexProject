...
from quantlab.integrations.external_systems import holding_columns
from quantlab.integrations.holdings_book import (
    HOLDINGS_IMPORT_DIR,
    HOLDINGS_BOOK_PATH,
...
from quantlab.integrations.independent_validation import run_independent_validation
...
def _handle_request(
    ...
    *,
    ...
    if request_type == "sync_quantlab_results":
        return sync_quantlab_results_to_bus(db_path=db_path).to_dict()
    ...
    for symbol in symbols[:12]:
        ...
        conn.execute(
            ...
            (
                ...
                market,
                _truncate(text, 280),
                "由 QuantLab 或独立验证系统选择样本区间；正式报告必须记录样本范围。",
