...
from quantlab.integrations.research_bus import DEFAULT_AI_RESEARCH_ROOT, research_bus_db_path
from quantlab.integrations.research_bus_api import CHAT_DROPBOX_DIR
...
def run_research_bus_interop_audit(
    ...
    *,
    ...
    items.append(_quantlab_result_item(table_counts, target_ai_root))
    ...
    if missing_tables or missing_requests:
        return [
            AuditItem(
                ...
                "更新 docs/ResearchBusSchema.json，并同步 QuantLab 和行研 bridge。",
            )
        ]
...
def _quantlab_result_item(table_counts: dict[str, int], ai_root: Path) -> AuditItem:
    count = table_counts.get("quantlab_results", 0)
    bridge_path = ai_root / "data" / "report_artifacts" / "research_bus_bridge" / "QuantLabResultsFromBus.json"
    direct_path = ai_root / "data" / "report_artifacts" / "quantlab_bridge" / "QuantLabResults.json"
    if count and (bridge_path.exists() or direct_path.exists()):
        return AuditItem("QuantLab 回测结论回写行研系统", "Pass", f"quantlab_results={count}；桥接文件存在。")
    if count:
        return AuditItem(
            "QuantLab 回测结论回写行研系统",
            "Warn",
            f"quantlab_results={count}；未找到行研桥接文件。",
            "在行研系统运行 python3 -m src.cli research-bus-sync --json。",
        )
    return AuditItem("QuantLab 回测结论回写行研系统", "Warn", "quantlab_results=0。", "生成或同步至少一份 QuantLab 回测报告。")
