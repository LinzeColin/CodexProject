...
from quantlab.integrations.external_systems import (
...
HOLDINGS_HISTORY_SCHEMA = "QuantLabHoldingsImportHistoryV1"
...
def default_holding_source_specs() -> list[HoldingSourceSpec]:
    ...
    return [
        ...
        HoldingSourceSpec(
            ...
            paths=tuple(_configured_paths("QUANTLAB_INDUSTRY_HOLDINGS_DIR", (DEFAULT_INDUSTRY_REPORT_DIR, DATA_DIR / "external" / "industryHoldings"))),
            description="行研报告对话或目录中上传的 CSV/XLSX/JSON 持仓文件。",
        ),
        HoldingSourceSpec(
            source_system="消费行为分析系统",
            paths=tuple(_configured_paths("QUANTLAB_CONSUMER_HOLDINGS_DIR", DEFAULT_CONSUMER_HOLDINGS_DIRS)),
            description="消费行为分析系统同步的持仓文件。",
        ),
        HoldingSourceSpec(
            source_system="量化回测系统导入",
            paths=tuple(_configured_paths("QUANTLAB_HOLDINGS_DIR", (HOLDINGS_IMPORT_DIR, *DEFAULT_QUANTLAB_HOLDINGS_DIRS))),
            description="QuantLab 本地导入目录中的持仓文件。",
        ),
    ]


def default_alipay_ledger_dirs() -> tuple[Path, ...]:
    configured = os.getenv("QUANTLAB_ALIPAY_LEDGER_DIR", "").strip()
    if configured:
