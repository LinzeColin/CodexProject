...
from quantlab.data.providers import AlphaVantageProvider, CSVProvider, MoomooProvider, PolygonProvider, SampleDataProvider, TushareProvider
...
def make_provider(name: str, csv_path: Path | str | None = None):
    ...
    from quantlab.data.providers import AKShareProvider, YahooFinanceProvider

    if normalized in {"akshare", "ak_share"}:
        if AKShareProvider is None:
            raise RuntimeError("AKShare provider is unavailable. Install quantlab[data].")
        return AKShareProvider()
    if normalized in {"yahoo", "yahoo_finance", "yfinance"}:
        if YahooFinanceProvider is None:
            raise RuntimeError("Yahoo Finance provider is unavailable. Install quantlab[data].")
