...
BASE_FILES = {
    "功能清单": ["当前速读", "用户第一眼应看到的信息", "已实现功能", "明确禁止或尚未完成"],
    "开发记录": ["当前速读", "安全边界", "当前运行路线", "近期开发事件"],
    "模型参数文件": ["当前速读", "模型总览", "公式逻辑", "参数表", "Owner 需要关注的参数"],
}
...
def test_owner_base_files_use_exact_contract_paths_and_are_chinese():
    for filename, required_sections in BASE_FILES.items():
        ...
        assert _chinese_ratio(text) >= 0.20, f"{filename} 中文占比不足"
