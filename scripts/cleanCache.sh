...
if [[ "$DRY_RUN" == "0" ]] && quantlab_is_running; then
  echo "QuantLab appears to be running. Stop it before cleaning cache."
  exit 2
fi

PYTHON_BIN="${QUANTLAB_CLEANUP_PYTHON:-${QUANTLAB_PYTHON:-${EVA_PYTHON:-python3}}}"
...
PYTHONPATH="$PROJECT_DIR/src" "$PYTHON_BIN" -m quantlab.system.cache_cleanup "${ARGS[@]}"
