...
LABEL="com.quantlab.researchbus.sync"
PLIST_PATH="${HOME}/Library/LaunchAgents/${LABEL}.plist"
INTERVAL_SECONDS="${RESEARCH_BUS_LAUNCH_INTERVAL_SECONDS:-60}"
RUNNER_DIR="${HOME}/Library/Application Support/QuantLab"
...
AI_ROOT="\${RESEARCH_BUS_AI_ROOT:-$EVA_AI_RESEARCH_ROOT}"
...
run_step "QuantLab chat dropbox" "\$PYTHON_BIN" -m quantlab.examples.research_bus_api process-dropbox --min-age-seconds 0 --limit 100 --json
run_step "QuantLab heartbeat" "\$PYTHON_BIN" -m quantlab.examples.research_bus_api heartbeat --system-name ResearchBus --status Ready --capability sync_all --capability chat_input --capability chat_dropbox --capability independent_validation --capability local_webhook
run_step "QuantLab request processing" "\$PYTHON_BIN" -m quantlab.examples.research_bus_api process --system-name ResearchBus --limit 100 --json
run_step "QuantLab research bus sync" "\$PYTHON_BIN" -m quantlab.examples.sync_research_bus --json

if [[ -d "\$AI_ROOT" ]]; then
  run_step "AI research heartbeat" /bin/bash -lc "cd '\$AI_ROOT' && PYTHONDONTWRITEBYTECODE=1 python3 -m src.cli research-bus-heartbeat --system-name AI-Research-System --status Ready --capability publish_reports --capability pull_quantlab_results --capability pull_validation_tasks --capability pull_independent_validation --capability pull_consumer_behavior_state"
