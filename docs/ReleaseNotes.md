# QuantLab Release Notes

## 2026-06-17 macOS App Runtime Acceptance Hardening

- `EVAOSMacOSRuntimeAcceptanceV1` now uses wider bounded timeouts for support scripts during real `.app` open acceptance, reducing false failures from slow `statusQuantLab.sh` or `cleanCache.sh --dry-run --json` on a busy Mac.
...
- Final-code real app-open acceptance passed for `~/Desktop/EVA_OS.app`, `~/Downloads/EVA_OS.app`, and `/Applications/EVA_OS.app`: each returned `status=Pass pass=10 fail=0 launch_method=app`.

## 2026-06-17 Command Center Action Router

- `总控驾驶舱` 新增 `EVAOSCommandCenterActionRouterV1`，把行动队列、热点快速预检、参数扫描预检、报告验证和 macOS 日常验收聚合为 `统一下一步`。
...
- 热点分析新增 `EVAOSHotspotQuickPreflightV1` 预检层，生成前先展示缓存状态、对象数、切片上限、预计 provider 请求数和下一步建议。
...
- 参数扫描新增 `EVAOSParameterScanPreflightV1` 预检层，运行前先展示组合数、最大组合数、上限占用、策略类型和下一步建议。
...
- 新增 `EVAOSReportValidationHubV1` 和 `scripts/reportValidation.sh`，把报告证据索引、补证据候选和验证优先级计划合并成默认只读入口。
...
- Hotspot generation now records `EVAOSHotspotRequestTraceV1` compact per-symbol timing and error diagnostics so slow providers/symbols can be identified before recomputing.
- 新增 `EVAOSMacOSRuntimeAcceptanceV1` 和 `scripts/macosRuntimeAcceptance.sh`，用于受控真实启动本地服务、检查 health、验证运行中缓存清理拒绝、停止服务并复核停止状态。
- `scripts/macosRuntimeAcceptance.sh --launch-method app --app-path ~/Downloads/EVA_OS.app --summary-json` 可通过真实 `.app` 打开路径做启动、health、缓存保护和停止闭环验收。
- Native launcher 现在通过 `posix_spawn("/bin/zsh", "-f", StartQuantLab.command)` 启动本地脚本，不再通过 `open -a Terminal` 兜转启动。
- `scripts/installEVAOSEntryApps.sh` 先在临时目录清理元数据、构建并 ad-hoc 签名 app bundle，再用 `ditto --norsrc --noextattr --noacl` 安装到 Desktop、Downloads 和 Applications，避免目标目录的 Finder/resource metadata 导致 codesign 失败。
...
- `StartQuantLab.command` 的启动锁等待默认降为 30 秒，减少 `.app` 重复唤起或 stale lock 时的等待时间。
- Native launcher 的本地 health 端口探测使用短连接超时，减少无服务时点击 `.app` 和 dry-run 的等待。
- Runtime Acceptance 的 app 模式会先尝试 macOS `open -n`；若短窗口内没有日志或 health，会 fallback 到同一 app bundle 的 `Contents/MacOS/EVA_OS`，并在停止阶段清理本项目残留 launcher 进程。
- `macOS 生命周期` 面板新增 `运行时验收` Terminal 命令展示，但不加入 Streamlit allowlist，避免从页面内停止当前会话。
- `scripts/cleanCache.sh` 改为端口 + 进程 cwd 的 scoped detection，可识别 `startQuantLab.sh` 的相对路径 Streamlit 启动，避免服务运行时误进入 delete mode。
...
- 新增 `EVAOSMacOSLifecycleReadinessV1` 和 `scripts/macosLifecycleReadiness.sh`，用于只读检查启动、停止、heartbeat 自动关闭、缓存清理保护、UI allowlist 和 App 轻量验收。
...
- 新增 `EVAOSMacOSAppAcceptanceLiteV1` 和 `scripts/macosAppAcceptanceLite.sh`，用于日常轻量检查 Desktop、Downloads、Applications 三个 `EVA_OS.app` 入口。
- 轻量验收检查 app bundle、native launcher、Info.plist、`EVA_OS_PROJECT_ROOT` binding、GitHub fallback 缺失、codesign、launcher dry-run、状态脚本和本地 `_stcore/health`。
...
- 新增 `EVAOS52ETFPublicSnapshotV1`，把 `https://52etf.site/` 公开大盘云图页面契约固化为低 token latest snapshot。
- 新增 `scripts/site52etfSnapshot.sh` 和 `src/quantlab/examples/site52etf_snapshot.py`，可输出 `Site52ETFPublicSnapshot_DDMMYYYY.json` 和 `Site52ETFPublicSnapshot_latest.json`。
...
- 新增 `scripts/consumptionReviewedInputRefresh.sh` 和 `src/quantlab/consumption/reviewed_input.py`，把 Consumption Guard 推进到 real-input refresh MVP。
- 默认真实输入路径为 `data/private/consumption/ConsumptionGuardReviewedInput.json`，该路径被 Git 忽略；缺少输入时返回 `EVAOSConsumptionGuardReviewedInputRefreshV1 status=Blocked` 且不写入输出。
...
## 2026-06-16 macOS EVA_OS App Launch Hardening

- Replaced the shell-script app executable with a native Mach-O launcher built from `macos/EVA_OS_launcher.c`.
- `EVA_OS.app` now reads `Contents/Resources/EVA_OS_PROJECT_ROOT`, opens the local `StartQuantLab.command`, and never falls back to GitHub when the local project path is missing.
- `scripts/installEVAOSEntryApps.sh` compiles the native launcher with `clang`, writes the current local project root into Desktop, Downloads, and Applications app bundles, clears xattrs, and ad-hoc signs each app.
- `StartQuantLab.command` now records launch-lock pid metadata and immediately removes stale locks without pid metadata; `scripts/stopQuantLab.sh` removes the launch lock during stop.
- Verified `open -n ~/Downloads/EVA_OS.app` starts local EVA_OS and `/_stcore/health` returns `ok`.

## 2026-06-16 Policy Intelligence Reviewed Input Refresh

- 新增 `scripts/policyReviewedInputRefresh.sh` 和 `src/quantlab/policy/reviewed_input.py`，把 Policy Intelligence 推进到 real-input refresh MVP。
- 默认真实输入路径为 `data/private/policy/PolicyReviewedInput.json`，该路径被 Git 忽略；缺少输入时返回 `EVAOSPolicyReviewedInputRefreshV1 status=Blocked` 且不写入输出。
...
- 新增 `scripts/cashFlowReviewedInputRefresh.sh` 和 `src/quantlab/business/cashflow_reviewed_input.py`，把 Company CashFlow 推进到 real-input refresh MVP。
- 默认真实输入路径为 `data/private/cashflow/CompanyCashFlowReviewedInput.json`，该路径被 Git 忽略；缺少输入时返回 `EVAOSCompanyCashFlowReviewedInputRefreshV1 status=Blocked` 且不写入输出。
...
- Consumption Guard 新增 `EVAOSConsumptionGuardRuntimeSummaryV1` compact 运行摘要，包含守卫状态、计入记录、缺证据、待复核、冲动风险、固定成本、可投资现金流压力和 evidence gate。
...
- Policy Intelligence Radar 新增 `EVAOSPolicyIntelligenceRuntimeSummaryV1` compact 运行摘要，包含政策状态、机会数量、Actionable、来源权威、待溯源、待复核、缺证据和 evidence gate。
...
- Company CashFlow Command 新增 `EVAOSCompanyCashFlowRuntimeSummaryV1` compact 运行摘要，包含现金流状态、余额日期、计入记录、待复核、缺证据、净现金流、Runway、应收应付和 evidence gate。
...
- 热点分析新增 `EVAOSHotspotRuntimeSummaryV1` compact 运行摘要：生成后显示请求指纹、对象覆盖、切片数量、缓存 TTL 和证据状态，帮助判断本次点击是否复用同一计算口径，降低重复解释和排查成本。
- 新增 `scripts/hotspotRuntimeSummary.sh` Sample-only smoke 入口，可在不打开 Streamlit、不联网、不连接券商的情况下验证热点 summary 和证据闸门。
- 热点分析新增 `EVAOSHotspotPersistedCacheV1` 本地派生缓存，写入被 Git 忽略的 `data/cache/hotspots/`；同一请求指纹在 TTL 内可跨工作台重启复用已计算热点结果。
- 52ETF 公开参考升级为 `EVAOS52ETFHotspotComparisonV1` 只读对照摘要，展示公开 A 股云图板块与 EVA_OS 当前热点对象池的映射、口径差异和安全边界。
- 安全边界不变：热点分析仍只作研究观察，不创建订单、不修改持仓、不连接实盘交易。

## 2026-06-13 Event Replay MVP

- 新增事件回放层：`src/quantlab/data/replay.py`。
...
- 新增可复现数据湖 manifest 层：`src/quantlab/data/lake.py`。
...
- 新增本地事件驱动行情层：`src/quantlab/data/market_events.py`。
- 新增 CLI：`scripts/marketEventLayer.sh --output-dir data/marketEvents`。
- 支持把 deterministic sample 或本地 CSV 的 OHLCV bar 转成 `EVAOSMarketEventV1` / `BarClosed` 事件。
...
- 新增 `Consumption Guard` 后端模块：`src/quantlab/consumption/guard.py`。
...
- 新增 `Policy Intelligence Radar` 后端模块：`src/quantlab/policy/radar.py`。
...
- 新增 `Company CashFlow Command` 后端模块：`src/quantlab/business/cashflow.py`。
...
三个启动入口已经统一为标准 macOS `.app` 包，支持 Dock 和 Launchpad。图标配置保存在 `assets/QuantLabAppIconConfig.json`，重新生成入口使用 `scripts/installMacAppLaunchers.sh`。

The three launchers are now standard macOS `.app` bundles with Dock and Launchpad support. Icon configuration is saved in `assets/QuantLabAppIconConfig.json`, and launchers can be rebuilt with `scripts/installMacAppLaunchers.sh`.
...
Custom strategy creation is now centralized in the strategy library. QuantLab infers English name, strategy id, category, return sources, thesis, and failure regime from strategy logic, indicator combination, and parameter settings.
...
编辑自定义策略规格后，系统会自动同步对应 `src/quantlab/strategies/custom/*.py` 策略代码文件，避免规格和代码版本不一致。

After editing a custom strategy spec, QuantLab automatically synchronizes the matching `src/quantlab/strategies/custom/*.py` strategy code file to prevent spec and code version drift.
...
The data layer now includes a fallback for non-native intervals: when a real provider does not support weekly, monthly, quarterly, or yearly bars, QuantLab attempts local OHLCV resampling from daily or monthly bars and writes the source interval into data quality notes.
...
热点页面新增可选 `52ETF 公开参考`。系统只读取 `https://52etf.site/` 公开首页可见的“大盘云图”板块和操作提示，并以 `Review/Pass` 证据状态展示；读取失败时 fail-closed，不阻塞 EVA_OS 本地热点分析。
...
新增 `scripts/devReadyCheck.sh --summary-json` 作为日常开发默认 gate，输出 `EVAOSDevReadyCheckV1`。
...
2026-06-16 本机已完成 Downloads `EVA_OS.app` open-path runtime acceptance：`EVAOSMacOSRuntimeAcceptanceV1 status=Pass pass=10 fail=0 launch_method=app`。原始 evidence 含本机绝对路径，继续只保留在本地 `data/systemAudit/MacOSRuntimeAcceptance*.json`，不提交 GitHub。

## 2026-06-17 macOS UI 可见性验收

新增 `scripts/uiVisualAcceptance.sh --summary-json` 作为轻量 UI visual acceptance 入口，输出 `EVAOSUIVisualAcceptanceV1`。

该脚本会在没有健康本机服务时启动 EVA_OS Streamlit，用 headless Chrome 检查 `EVA_OS`、`工作台状态`、`macOS 生命周期`、`运行时验收证据`、缓存预览和生命周期按钮是否真实可见，并保存本地截图。

该验收不会运行 `scripts/finalAcceptanceCheck.sh`、`scripts/ciSmoke.sh`、full pytest、行情刷新、券商连接、订单、付款或持仓写入；如果脚本自己启动服务，结束时只停止本次启动的服务。

2026-06-17 本机最终验收通过：`EVAOSUIVisualAcceptanceV1 status=Pass pass=16 fail=0 screenshot_bytes=278744`。本地 UI evidence 和截图继续只保留在 `data/systemAudit/UIVisualAcceptance*`，并已通过 `.gitignore` 排除。

## 2026-06-17 macOS 公开验收摘要

新增 `scripts/macosPublicAcceptanceSummary.sh`，输出 `EVAOSMacOSPublicAcceptanceSummaryV1`，默认把本机 `MacOSRuntimeAcceptance_latest.json` 和 `UIVisualAcceptance_latest.json` 转换成 GitHub-safe JSON/Markdown：
...
新增 `scripts/macosAcceptance.sh` 作为用户友好的统一入口，输出 `EVAOSMacOSAcceptanceHubV1`。无参数默认运行 `--mode daily --summary-json`，合并轻量开发就绪和 GitHub-safe 公开验收摘要，减少日常需要记住的脚本数量。
