...
适用范围：QuantLab、AI-Research-System、消费分析系统、独立验证系统、政府政策系统，以及相关 Codex 对话框和 automation。

## 1. 当前判断

QuantLab 暂定为母系统和研究指挥中心。

理由：

- QuantLab 当前保存统一 ResearchBus 主库、schema、互通审计和桌面入口。
- QuantLab 已经能读取行研报告、消费行为状态、持仓主数据、独立验证运行状态和回测结果。
- QuantLab 的定位是研究验证、策略变更确认、报告中心和个人日常工作台，适合作为用户操作入口。
...
| 独立验证系统 | 独立验证系统 | 暂停新长跑和 OpenD 抓取，等待 QuantLab ACK |
...
| 系统 | 主导职责 | 不应主导 |
| --- | --- | --- |
| QuantLab | ResearchBus、统一 schema、主工作台、持仓主数据、回测验证、策略变更确认、报告中心、总协调看板 | 行研报告正文生成、政策原文爬虫、消费账单分类、大规模 worker 长跑 |
| AI-Research-System | 日报/周报/K线报告、报告质量门、行研判断、automation、证据链引用 | ResearchBus 主 schema、QuantLab 策略库写入、消费账单分类 |
| 消费分析系统 | 支付宝/账单入库、经济放血分类、消费周期报告、行为状态输出、用户验收工作台 | 投资建议、回测、行研报告生成、政策采集 |
| 独立验证系统 | 大规模策略验证、manifest/campaign、worker、长跑产物、外部证据包 | QuantLab 数据库写入、策略变更确认、用户界面主入口 |
| 政府政策系统 | 政策原文抓取、政策证据链、政策报告、平台授权 readiness | 行研日报编排、QuantLab 回测、消费分析 |

## 4. 当前 active goal 汇总

| 系统 | 当前 goal | 当前状态 | 主要阻塞 |
| --- | --- | --- | --- |
| QuantLab | 个人量化研究、回测、持仓、报告、ResearchBus 总工作台 | 互通审计 15/15 Pass；盘感训练已增强 | 需要把独立验证 ACK、总协调看板和开发排期产品化 |
...
| 独立验证系统 | QuantLab 外部策略验证层 | QBVS 侧协议和证据包就绪 | QuantLab 真实 ACK 缺失；OpenD 额度和 200 标的真实验证未完成 |
...
- 独立验证系统：暂停新一轮 OpenD 抓取、重复长跑、继续扩充策略目录数量；先完成 QuantLab ACK 和额度友好计划。
- 政府政策系统：暂停继续堆解析器和 dashboard；优先补搜索 API / 平台授权 / scheduler 证据。
- QuantLab：暂停非关键页面美化和新模块扩张；优先完成总协调、ACK、schema 治理和任务看板。

## 6. 最高优先级

P0，必须先做：

1. QuantLab 读取 QBVS 握手请求并生成真实 `quantlab_handshake_ack.json`。
2. 建立 `SystemCoordinationPlan` 和 `SystemCoordinationStatus`，统一显示每个系统当前状态、阻塞、下一步。
3. 固化 ResearchBus schema version、请求类型和字段变更流程。
4. 把各系统 ACK 汇总到 QuantLab 总协调看板。
5. 所有系统进入“冻结非必要功能 + 只做指定高价值任务”的节奏。

P1，随后做：

1. 行研系统只保留报告质量门、automation、政策/QuantLab/持仓引用链路的修复。
2. 消费系统只输出消费状态、行为画像和验收结论给 ResearchBus。
3. 独立验证系统只输出 evidence bundle、campaign 状态、worker readiness，不写 QuantLab。
4. 政策系统只补 P0 凭据和 scheduler 证据，不继续扩张解析器。

P2，稳定后做：

1. QuantLab 首页新增总系统状态板块。
...
| Phase 1：ACK 和 schema 治理 | 1 天 | QuantLab 生成 QBVS ACK；ResearchBus schema/version/request registry 固化 | ACK valid；互通审计仍 Pass |
| Phase 2：总状态看板 | 1-2 天 | QuantLab 显示各系统 goal、阻塞、下一步、暂停项 | 页面/CLI 均可查看；不打开子系统也能判断状态 |
...
4. 任务合并：由 QuantLab 维护跨系统 task queue。
5. UI 合并：QuantLab 只聚合入口和摘要，不复制子系统全部页面。
...
3. QuantLab 能看到每个子系统的 latest status。
4. 子系统可单独运行，不依赖 QuantLab 启动。
...
- 独立验证系统缺 QuantLab 真实 ACK，不能宣称双系统完全闭环。
- 政策系统缺 API key / platform auth，不能宣称全网在线覆盖。
- 消费分析系统工程可验收，但用户主观验收和 ChatGPT 对照仍未完成。

## 12. 下一轮执行计划

下一轮只做一件事：QuantLab 总协调执行层。

任务：

1. 读取 QBVS `qbvs_handshake_request.json`。
2. 在 QuantLab 生成真实 `quantlab_handshake_ack.json`。
3. 新增或更新 QuantLab `System Coordination` 状态数据文件。
