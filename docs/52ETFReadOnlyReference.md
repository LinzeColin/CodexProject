...
`52ETF Read-Only Reference` 是 EVA_OS 热点分析的公开市场云图参考层。它读取 `https://52etf.site/` 首页公开可见的“大盘云图 / A股热力图”页面契约，生成低 token snapshot，用于对照 EVA_OS 自有热点对象池和 UI 交互口径。
...
```bash
$EVA_OS_HOME/scripts/site52etfSnapshot.sh --output-dir data/integrations/site52etf
```

只看低 token 摘要：

```bash
$EVA_OS_HOME/scripts/site52etfSnapshot.sh --summary-json
```

使用本地 HTML fixture 做离线验证：

```bash
$EVA_OS_HOME/scripts/site52etfSnapshot.sh --source-html /path/to/52etf_sample.html --output-dir /tmp/eva_site52etf_snapshot
```
...
- `safety_boundary`: 只读公开参考，不替代 EVA_OS 本地/provider 数据质量闸门。
