...
双击 `EVA_OS.app` 可以启动 EVA_OS / QuantLab。

Double-clicking `EVA_OS.app` starts EVA_OS / QuantLab.

双击 `StopQuantLab.command` 可以停止 QuantLab。

Double-clicking `StopQuantLab.command` stops QuantLab.

`scripts/statusQuantLab.sh` 可以检查运行状态且不会打开浏览器。

`scripts/statusQuantLab.sh` checks runtime status without opening a browser.
...
`scripts/macosAppAcceptanceLite.sh --json` 可以轻量检查 Desktop、Downloads、Applications 的 `EVA_OS.app`、native launcher、Info.plist、project binding、codesign、launcher dry-run、本地 health 和状态脚本；它不得运行 `finalAcceptanceCheck.sh`、`ciSmoke.sh`、full pytest、浏览器自动化、行情刷新、回测、券商连接、订单、付款或持仓写入。

`scripts/macosAppAcceptanceLite.sh --json` checks Desktop, Downloads, and Applications `EVA_OS.app`, native launcher, Info.plist, project binding, codesign, launcher dry-run, local health, and status script; it must not run `finalAcceptanceCheck.sh`, `ciSmoke.sh`, full pytest, browser automation, market refresh, backtests, broker connections, orders, payments, or holdings writes.
...
`scripts/macosRuntimeAcceptance.sh --launch-method app --app-path ~/Downloads/EVA_OS.app --json` 必须能通过真实 `.app` 打开路径完成启动、health、缓存保护、停止和停止后复核；native launcher 必须通过本地 `/bin/zsh` 启动 `StartQuantLab.command`，不得通过 Terminal 或 GitHub fallback 启动。

`scripts/macosRuntimeAcceptance.sh --launch-method app --app-path ~/Downloads/EVA_OS.app --json` must complete start, health, cache guard, stop, and post-stop checks through the real `.app` open path; the native launcher must start local `StartQuantLab.command` through `/bin/zsh`, not route through Terminal or GitHub fallback.
...
`scripts/uiVisualAcceptance.sh --summary-json` 必须能用 headless Chrome 打开本机工作台并验证 `EVA_OS`、`工作台状态`、`macOS 生命周期`、`运行时验收证据` 和生命周期按钮文案可见，同时保存本地截图；它不得运行 final acceptance、CI smoke、full pytest、行情刷新、券商连接、订单、付款或持仓写入。

`scripts/uiVisualAcceptance.sh --summary-json` must open the local workbench with headless Chrome and verify visible `EVA_OS`, workspace status, macOS lifecycle, runtime evidence, and lifecycle action text while saving a local screenshot; it must not run final acceptance, CI smoke, full pytest, market refresh, broker connections, orders, payments, or holdings writes.

`scripts/macosPublicAcceptanceSummary.sh` 必须把 `MacOSRuntimeAcceptance_latest.json` 和 `UIVisualAcceptance_latest.json` 转换成 `EVAOSMacOSPublicAcceptanceSummaryV1`，输出到 `docs/evidence/`，且不得包含 `/Users/`、`/Applications/`、浏览器可执行路径、截图路径、PID、raw logs 或私有数据；它不得启动服务、打开浏览器、运行 final acceptance、CI smoke、full pytest、行情刷新、券商连接、订单、付款或持仓写入。

`scripts/macosPublicAcceptanceSummary.sh` must convert `MacOSRuntimeAcceptance_latest.json` and `UIVisualAcceptance_latest.json` into `EVAOSMacOSPublicAcceptanceSummaryV1` under `docs/evidence/` without `/Users/`, `/Applications/`, browser executable paths, screenshot paths, PIDs, raw logs, or private data; it must not start services, open browsers, run final acceptance, CI smoke, full pytest, market refresh, broker connections, orders, payments, or holdings writes.
...
`scripts/tokenRoiLedger.sh --output-dir data/value` 可以生成 Token ROI Ledger JSON、CSV、Markdown 和 PDF，且未量化金额时不得伪造 ROI。

Token ROI Ledger 必须生成 `EVATokenROIRuntimeSummaryV1` compact 运行摘要，并支持 `scripts/tokenRoiLedger.sh --summary-json` 只输出低 token 摘要；该摘要不得包含完整 records，必须展示 schema、记录数、已量化数、待复核金额假设、证据闸门和研究-only安全边界。

`scripts/tokenRoiReviewedValueRefresh.sh --entry-path data/private/value/TokenROIReviewedValueEvidence.json --output-dir data/value` 可以从本地 reviewed value evidence 生成 Token ROI Ledger full snapshot 和 runtime summary；缺少输入时必须返回 `EVATokenROIReviewedValueEvidenceRefreshV1 status=Blocked` 且不得写入输出。公共仓库只能包含 `TokenROIReviewedValueEvidence.example.json` 和 schema，真实输入必须留在 `data/private/value`，`TokenROIManualEntries.json` 必须保持本地 gitignored。
...
Company CashFlow Command 必须生成 `EVAOSCompanyCashFlowRuntimeSummaryV1` compact 运行摘要，并支持 `scripts/cashFlowCommand.sh --summary-json` 只输出低 token 摘要；该摘要不得包含完整 entries，必须展示余额证据、缺证据记录、待复核记录、Runway、净现金流、证据闸门和 no-external-execution 安全边界。

`scripts/cashFlowReviewedInputRefresh.sh --entry-path data/private/cashflow/CompanyCashFlowReviewedInput.json --output-dir data/cashflow` 可以从本地 reviewed input 生成 Company CashFlow Command full snapshot 和 runtime summary；缺少输入时必须返回 `EVAOSCompanyCashFlowReviewedInputRefreshV1 status=Blocked` 且不得写入输出。公共仓库只能包含 `CompanyCashFlowReviewedInput.example.json` 和 schema，真实输入必须留在 `data/private/cashflow`。
...
Policy Intelligence Radar 必须生成 `EVAOSPolicyIntelligenceRuntimeSummaryV1` compact 运行摘要，并支持 `scripts/policyRadar.sh --summary-json` 只输出低 token 摘要；该摘要不得包含完整 opportunities，必须展示政策证据、来源权威、缺证据记录、待复核记录、证据闸门和 no-external-execution 安全边界。

`scripts/policyReviewedInputRefresh.sh --entry-path data/private/policy/PolicyReviewedInput.json --output-dir data/policy` 可以从本地 reviewed input 生成 Policy Intelligence Radar full snapshot 和 runtime summary；缺少输入时必须返回 `EVAOSPolicyReviewedInputRefreshV1 status=Blocked` 且不得写入输出。公共仓库只能包含 `PolicyReviewedInput.example.json` 和 schema，真实输入必须留在 `data/private/policy`。
...
Consumption Guard 必须生成 `EVAOSConsumptionGuardRuntimeSummaryV1` compact 运行摘要，并支持 `scripts/consumptionGuard.sh --summary-json` 只输出低 token 摘要；该摘要不得包含完整 events，必须展示消费证据、缺证据记录、待复核记录、冲动风险、固定成本、可投资现金流压力、证据闸门和 no-external-execution 安全边界。

`scripts/consumptionReviewedInputRefresh.sh --event-path data/private/consumption/ConsumptionGuardReviewedInput.json --output-dir data/consumption --monthly-investable-budget 1000` 可以从本地 reviewed input 生成 Consumption Guard full snapshot 和 runtime summary；缺少输入时必须返回 `EVAOSConsumptionGuardReviewedInputRefreshV1 status=Blocked` 且不得写入输出。公共仓库只能包含 `ConsumptionGuardReviewedInput.example.json` 和 schema，真实输入必须留在 `data/private/consumption`。

`scripts/refreshRuntimeSummaries.sh --as-of YYYY-MM-DD --token-roi-entry-path data/private/value/TokenROIReviewedValueEvidence.json` 可以只刷新 Token ROI、Company CashFlow、Policy Intelligence 和 Consumption Guard 的 compact runtime summary latest 文件，不重写完整 records、entries、opportunities、events、CSV、Markdown 或 PDF。

Runtime Summary Refresh 必须输出 `EVAOSRuntimeSummaryRefreshV1`，并生成四个 public-safe latest 文件：`EVATokenROIRuntimeSummary_latest.json`、`CompanyCashFlowRuntimeSummary_latest.json`、`PolicyIntelligenceRuntimeSummary_latest.json` 和 `ConsumptionGuardRuntimeSummary_latest.json`；这些 JSON 不得包含本机绝对路径、私密导入、持仓明细、账户凭据或外部执行状态。

`scripts/commandCenter.sh --output-dir data/commandCenter` 可以生成总控驾驶舱 JSON、Markdown 和 PDF，且不刷新行情、不修改持仓、不打开浏览器、不连接实盘。

总控驾驶舱必须聚合 Daily Readiness、Integration Audit、Data Trust、Token ROI、最新报告和行动队列，并把状态降级为 `NeedsReview` 或 `Blocked` 而不是伪造就绪。

总控驾驶舱必须优先读取 `EVATokenROIRuntimeSummary_latest.json`、`CompanyCashFlowRuntimeSummary_latest.json`、`PolicyIntelligenceRuntimeSummary_latest.json` 和 `ConsumptionGuardRuntimeSummary_latest.json`；这些 compact summary 缺失时才 fallback 到 full latest 快照或本地 fail-closed 构建，并在输出中记录 `runtime_summary_sources`。
...
报告决策支持索引必须把缺少 `QuantLabReportEvidenceV1`、数据质量、多源交叉校验、风险闸门或 Decision Quality 的报告降级为 `NeedsMoreEvidence` 或 `DoNotUse`。
...
验证任务执行产物必须进入 Token ROI Ledger，且仍不得伪造 `revenue_generated`、`cost_saved`、`loss_avoided`、`asset_reuse_value` 或 `roi_score`。
...
`scripts/verifyQuantLab.sh` 可以完整验收系统且不会打开浏览器。

`scripts/verifyQuantLab.sh` verifies the system without opening a browser.

`scripts/finalAcceptanceCheck.sh` 可以检查 macOS `.app` 入口、报告目录、核心文档、关键功能源码、完整测试和日常检查，且不会联网或打开浏览器。它是重 SmokeTest/release gate，本地默认被 `EVA_OS_ALLOW_HEAVY_SMOKE=1` 显式确认门保护。

`scripts/finalAcceptanceCheck.sh` checks macOS `.app` launchers, report directory, core docs, key feature source files, full tests, and daily checks without using network access or opening a browser. It is a heavy SmokeTest/release gate and is protected locally by the explicit `EVA_OS_ALLOW_HEAVY_SMOKE=1` confirmation gate.
...
样例报告必须生成包含 `QuantLabReportEvidenceV1` 的 RunMetadata；缺少真实多源交叉校验时应降级为 `NeedsMoreEvidence`。
...
行研报告功能区可以按日期和关键词检索本地行研报告，并显示最近 QuantLab 报告、验证任务和研究门禁状态。

The industry research area can search local industry reports by date and keyword, and show recent QuantLab reports, validation tasks, and research gate status.

持仓功能区可以同步支付宝确认持仓、行研上传持仓、消费行为分析持仓和 QuantLab 本地导入持仓，并永久保存到统一持仓簿。

The holdings area can sync confirmed Alipay holdings, industry-uploaded holdings, consumer-analysis holdings, and QuantLab local imports, then persist them to one holdings book.
...
热点分析生成后必须显示 `EVAOSHotspotRuntimeSummaryV1` compact 运行摘要，包含请求指纹、对象覆盖、切片数量、缓存 TTL、证据状态和只读安全边界；该摘要不得保留原始行情明细、连接券商、创建订单或修改持仓。

After hotspot generation, the page must show an `EVAOSHotspotRuntimeSummaryV1` compact runtime summary with request key, object coverage, slice count, cache TTL, evidence status, and read-only safety boundary; it must not retain raw price frames, connect brokers, create orders, or mutate holdings.

热点分析可以使用 `EVAOSHotspotPersistedCacheV1` 本地派生缓存复用同一请求指纹的已计算结果；缓存必须位于被 Git 忽略的 `data/cache/hotspots/`，必须按 TTL 过期，且不得存储 secrets、原始账户数据、券商 token、订单或持仓修改。

The hotspot area may use `EVAOSHotspotPersistedCacheV1` local derived cache to reuse computed results for the same request key; the cache must live under gitignored `data/cache/hotspots/`, expire by TTL, and must not store secrets, raw account data, broker tokens, orders, or holdings mutations.

热点分析页面必须显示 `EVAOSHotspotCacheStatusV1` 当前请求缓存状态、剩余有效期、缓存目录文件数量，并提供只清除当前请求指纹缓存的按钮；该按钮不得删除市场 bar cache、报告、持仓、SQLite 或其他系统缓存。

The hotspot page must show `EVAOSHotspotCacheStatusV1` current-request cache state, remaining TTL, cache directory file count, and a button that clears only the current request-key cache; it must not delete market bar cache, reports, holdings, SQLite files, or other subsystem caches.

热点分析生成后必须显示 `EVAOSHotspotRequestTraceV1` 每对象请求耗时摘要，至少包含请求数量、成功/失败、总耗时、最慢对象和错误摘要；该 trace 只能保存 compact 诊断字段，不得保存原始 provider payload 或行情明细。

After hotspot generation, the page must show an `EVAOSHotspotRequestTraceV1` per-object request timing summary with request count, success/failure, total elapsed time, slowest objects, and error summaries; this trace may store compact diagnostics only, not raw provider payloads or market-data details.
...
勾选 52ETF 参考后，热点页面必须提供 `EVAOS52ETFHotspotComparisonV1` 只读对照摘要，将公开 A 股云图板块与 EVA_OS 当前热点对象池映射；该对照不得替代本地数据质量闸门，不得写入回测、下单、持仓或交易前证据。

When 52ETF reference is enabled, the hotspot page must provide an `EVAOS52ETFHotspotComparisonV1` read-only comparison summary mapping public A-share market-cloud boards to the current EVA_OS hotspot object pool; it must not replace local data-quality gates or write into backtests, orders, holdings, or pre-trade evidence.

`scripts/site52etfSnapshot.sh --output-dir data/integrations/site52etf` 可以生成 `EVAOS52ETFPublicSnapshotV1` latest JSON，记录 52ETF 公开页面的板块、指标、操作提示、8 秒刷新提示、证据闸门和只读安全边界；该 snapshot 不得保存 raw HTML、cookies、登录态、行情明细、回测输入、订单或持仓修改。热点页面必须优先读取本地 latest snapshot，缺失时才按缓存在线读取公开页面。

`scripts/site52etfSnapshot.sh --output-dir data/integrations/site52etf` generates `EVAOS52ETFPublicSnapshotV1` latest JSON with public boards, metrics, operating notes, 8-second refresh hint, evidence gates, and read-only safety boundary; it must not store raw HTML, cookies, session state, market-data detail, backtest input, orders, or holdings mutations. The hotspot page must prefer local latest snapshot before cached live fetch.
...
```bash
$EVA_OS_HOME/scripts/finalAcceptanceCheck.sh
```
