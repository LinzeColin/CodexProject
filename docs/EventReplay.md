...
Event Replay 是 EVA_OS 的事件回放最小实现。它读取可复现数据湖的 manifest、replay cursor 和本地 `MarketEventLog_*.jsonl`，生成一个按 `event_time`、`event_id` 稳定排序的 replay batch。
...
| 字段 | 含义 |
| --- | --- |
| `schema` | `EVAOSEventReplayBatchV1`。 |
...
Vectorized Research Mode MVP 已落地在 `docs/VectorizedResearchMode.md`、`src/quantlab/research/vectorized.py` 和 `scripts/vectorizedResearch.sh`。它只读取本地 replay 输出，不联网、不刷新行情、不连接券商、不创建订单。
