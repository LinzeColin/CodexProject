...
Company CashFlow Command 是 EVA_OS 的经营现金流子系统。它把公司现金余额、收入、支出、应收和应付记录成可复核证据，并输出现金流状态、近 30 天净现金流、Runway、分类汇总和行动队列。
...
- 人工台账保存到 `data/cashflow/CompanyCashFlowEntries.json`。
- 支持从本地私有 reviewed input 刷新正式快照：`data/private/cashflow/CompanyCashFlowReviewedInput.json`。
- 输出 JSON、CSV、Markdown、PDF 和 latest 指针文件。
- 输出 `EVAOSCompanyCashFlowRuntimeSummaryV1` compact 运行摘要，供 UI、总控驾驶舱和后续 agent 低 token 读取。
...
```bash
$EVA_OS_HOME/scripts/cashFlowCommand.sh --json-only
```
...
```bash
$EVA_OS_HOME/scripts/cashFlowReviewedInputRefresh.sh \
  --entry-path data/private/cashflow/CompanyCashFlowReviewedInput.json \
  --output-dir data/cashflow
```

如果默认私有输入文件不存在，该命令返回 `EVAOSCompanyCashFlowReviewedInputRefreshV1 status=Blocked`，不会写入现金流输出。

刷新全部 compact runtime summaries 时复用同一 reviewed input：

```bash
$EVA_OS_HOME/scripts/refreshRuntimeSummaries.sh \
...
`EVAOSCompanyCashFlowRuntimeSummaryV1` 只保留 compact 状态，不包含完整 `entries`，用于快速判断现金流是否可继续经营复盘。
...
```text
data/cashflow/CompanyCashFlowEntries.json
data/cashflow/CompanyCashFlowCommand_DDMMYYYY.json
data/cashflow/CompanyCashFlowCommand_DDMMYYYY.csv
data/cashflow/CompanyCashFlowCommand_DDMMYYYY.md
data/cashflow/CompanyCashFlowCommand_DDMMYYYY.pdf
data/cashflow/CompanyCashFlowCommand_latest.json
data/cashflow/CompanyCashFlowCommand_latest.csv
data/cashflow/CompanyCashFlowCommand_latest.md
data/cashflow/CompanyCashFlowCommand_latest.pdf
data/cashflow/CompanyCashFlowRuntimeSummary_DDMMYYYY.json
