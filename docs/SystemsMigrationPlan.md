# EVA_OS Systems Migration Plan

Last updated: 2026-06-16 Australia/Sydney

## Goal

Move EVA_OS from a single QuantLab-centered workspace toward a product-grade research operating system with child systems registered under `systems/`, shared permission rules under `shared/security`, and common schemas under `shared/schema`.
...
1. `finance_ledger`
   ...
   - Next acceptance gate: add an EVA_OS adapter that publishes summarized ledger metrics to ResearchBus without raw transaction rows.
...
2. `industry_research`
   ...
   - Next acceptance gate: add an EVA_OS adapter that publishes summarized report readiness and validation tasks to ResearchBus without raw account artifacts.
...
3. `policy_intelligence`
   ...
   - Next acceptance gate: add an EVA_OS adapter that publishes policy readiness, queue, external-reference-gap, and quality-gate summaries to ResearchBus without raw document text or auth paths.
...
1. `/Applications/EVA_OS.app`, `~/Desktop/EVA_OS.app`, and `~/Downloads/EVA_OS.app` launch the current EVA_OS workspace.
