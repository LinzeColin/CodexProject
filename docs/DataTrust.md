...
Data Trust Audit 是 QuantLab 的只读证据审计层，用来回答一个核心问题：当前研究系统里的文件、数据、策略、实验和报告，哪些可以进入研究证据链，哪些只能观察，哪些必须复核或拒绝。
...
```bash
cd $EVA_OS_HOME
PYTHONPYCACHEPREFIX=/private/tmp/quantlab-pycache PYTHONPATH=src .venv/bin/python -m quantlab.examples.data_trust_audit --output-dir data/systemAudit
```

默认输出目录是项目内 `data/systemAudit`。如果当前环境没有写入权限，使用 `/private/tmp/quantlab-data-trust`。

输出文件包括：

| 文件 | 用途 |
| --- | --- |
| `QuantLabDataTrustAudit_DDMMYYYY.json` | 机器可读完整审计 |
| `QuantLabDataTrustAudit_DDMMYYYY.csv` | 表格复核和筛选 |
| `QuantLabDataTrustAudit_DDMMYYYY.md` | 人工阅读摘要 |
| `QuantLabDataTrustAudit_DDMMYYYY.pdf` | 正式轻量审计摘要 |
...
- 正式产物：
  - `data/systemAudit/QuantLabDataTrustAudit_07062026.json`
  - `data/systemAudit/QuantLabDataTrustAudit_07062026.csv`
  - `data/systemAudit/QuantLabDataTrustAudit_07062026.md`
  - `data/systemAudit/QuantLabDataTrustAudit_07062026.pdf`
