...
`macOS Runtime Acceptance` 是 EVA_OS 的受控真实运行验收入口，用于验证本机启动、health、运行中缓存清理保护、停止服务和停止后缓存 dry-run 是否形成闭环。
...
```bash
$EVA_OS_HOME/scripts/macosRuntimeAcceptance.sh --json
```
...
默认情况下，如果 8501-8510 已经存在健康的 EVA_OS / QuantLab 服务，脚本会 fail-closed，不会启动第二个服务，也不会停止现有服务。

如果确实要把已有服务纳入验收，必须显式传入：

```bash
$EVA_OS_HOME/scripts/macosRuntimeAcceptance.sh --allow-existing-service --summary-json
```
...
- `scripts/startQuantLab.sh` 是否能受控后台启动本地服务，或 `--launch-method app` 是否能打开真实 `EVA_OS.app`。
- 8501-8510 是否出现 `/_stcore/health`。
- `scripts/statusQuantLab.sh` 是否能看到运行服务。
- 服务运行时 `scripts/cleanCache.sh --json` 是否返回拒绝清理。
- `scripts/stopQuantLab.sh` 是否完成。
- 停止后 `/_stcore/health` 是否消失。
- 停止后 `scripts/statusQuantLab.sh` 是否显示未运行。
- 停止后 `scripts/cleanCache.sh --dry-run --json` 是否能输出 `EVACacheCleanupReportV1`。
