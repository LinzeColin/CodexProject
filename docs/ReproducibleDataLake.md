...
Reproducible Data Lake 是 EVA_OS 的可复现数据湖最小实现。它不复制数据、不联网、不接实时库，而是先把本地不可变数据资产登记成 manifest：路径、schema、partition、row count、checksum 和 replay cursor 都可复核。
...
主 manifest 使用 `EVAOSReproducibleDataLakeManifestV1`：
...
```bash
$EVA_OS_HOME/scripts/dataLakeManifest.sh --json-only
```

只索引事件层，跳过 bar cache：

```bash
$EVA_OS_HOME/scripts/dataLakeManifest.sh --no-cache --output-dir data/dataLake
```
