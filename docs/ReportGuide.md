...
QuantLab is the validation layer for the integrated industry research, policy interpretation and trading advice workflow. It does not create live trading instructions and does not replace policy or industry research.

When an industry report or trading advice item asks for QuantLab validation, the report must preserve enough context to verify the idea:
...
If any required validation evidence is missing, QuantLab output should be treated as `NeedsMoreEvidence`, not as support for actionable trading advice.
...
注意：QuantLab 禁止实盘交易，因此这里的停止交易是研究层面的停用条件，不是自动下单系统的交易停止指令。

Note: QuantLab prohibits live trading, so stop trading here means a research-level deactivation condition, not an automated order system instruction.
...
`行研报告` 功能区按日期索引本地行研 PDF、Word 或文本报告。默认目录为 `~/Downloads/行研报告`，可用环境变量 `QUANTLAB_INDUSTRY_REPORT_DIR` 覆盖。

The `行研报告` area indexes local industry PDF, Word, or text reports by date. The default directory is `~/Downloads/行研报告`, and it can be overridden with `QUANTLAB_INDUSTRY_REPORT_DIR`.

`持仓` 功能区会同步支付宝确认持仓、行研上传持仓、消费行为分析持仓和 QuantLab 本地导入持仓，并保存到统一持仓簿 `data/holdings/HoldingsBook.json`。待确认订单和截图候选持仓单独显示，不计入正式持仓、权重或集中度。

The `持仓` area syncs confirmed Alipay holdings, industry-uploaded holdings, consumer-analysis holdings, and QuantLab local imports into `data/holdings/HoldingsBook.json`. Pending orders and screenshot candidate holdings are shown separately and are not counted in confirmed holdings, weights, or concentration.
