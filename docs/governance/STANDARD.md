...
**目标：任何一个项目，从建目录的那一刻起就在体系内 —— 不存在「等想起来再登记」的窗口。**

「部署即登记」管**上了云的东西**；「业务流登记」管**已经有治理文件的项目**；
本节补最前面那一环：**还没有治理文件的项目**。这一环缺了，前两条都白搭 ——
因为一个项目在写出治理文件之前，对监控中枢是**彻底隐形**的：
它不在分母里，所以覆盖率不掉、未登记数不涨、看板一切正常。
**隐形不会让任何指标变红**，这是最危险的一类漏。

★ 实测（2026-07-27）全仓扫出 **10 个**这样的目录，其中 `MetaDatabase/CyberBoss`
有 **634 个文件、是 owner 明确在跑的活跃项目**，而监控中枢一个字都看不到它。

1. **有 `README.md` / `AGENTS.md` / `VERSION` 之一的顶层目录，就必须有
   `docs/governance/project.yaml`（强制）。** 这是「项目」的机器判据 ——
   不靠人声明「我这是个项目」，靠它长什么样。
2. **不自动下结论，强制表态。** 扫到的目录只有两条出路：
   补治理文件，或者在监控中枢的豁免名单里**写明它为什么不是项目**。
   **沉默不是选项** —— 沉默正是它们过去能隐形的原因。
3. **豁免必须带理由，且理由要公开可复核。**
   没有理由的豁免过两个月就没人知道当初为什么豁免，等同于永久黑洞。
4. **新建项目的治理文件先于第一行业务代码。** 顺序反了就会出现
   「先写六百个文件、再补登记」，而中间那段时间它是隐形的。
5. **如实优先于好看。** 没跑起来的就写 `not_built`，合成验收通过
   **不等于**真实链路跑通。把没激活的链路填成健康就是造假绿 ——
   宁可一片 `not_built`，不要一格假 `healthy`。

**机器判定**：`tests/governance/test_project_registry_contract.py` 校验本节存在；
扫描与豁免的可执行断言在实现仓
`LinzeHomeHub/status/collector/tests/test_ungoverned_projects.py`
（含「没治理文件的不再检出」「豁免不写理由也放行」两条负控）。
