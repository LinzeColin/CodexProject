...
这份文档是日常使用主入口。`EVA_OS` 是总系统入口，应用显示为 `EVA_OS`；QuantLab 是其中的量化研究与回测主入口。目标是让你不用记很多命令，也能完成启动、更新持仓、运行回测、阅读报告、同步行研系统和触发独立验证。
...
| 步骤 | 做什么 | 正常结果 |
| --- | --- | --- |
| 1 | 双击 `EVA_OS.app` | 浏览器打开 EVA_OS / QuantLab 工作台 |
...
不要把完整 SmokeTest 当作日常检查。`finalAcceptanceCheck.sh` 和 `ciSmoke.sh` 默认需要 `EVA_OS_ALLOW_HEAVY_SMOKE=1`，只给明确 release gate 使用。

常用高级模式：

```bash
$EVA_OS_HOME/scripts/macosAcceptance.sh --mode app-entry --summary-json
$EVA_OS_HOME/scripts/macosAcceptance.sh --mode lifecycle --summary-json
$EVA_OS_HOME/scripts/macosAcceptance.sh --mode app-runtime --summary-json
...
```bash
$EVA_OS_HOME/scripts/devReadyCheck.sh --summary-json
...
$EVA_OS_HOME/scripts/macosRuntimeAcceptance.sh --output-dir data/systemAudit
$EVA_OS_HOME/scripts/macosRuntimeAcceptance.sh --launch-method app --app-path ~/Downloads/EVA_OS.app --summary-json
$EVA_OS_HOME/scripts/uiVisualAcceptance.sh --summary-json
$EVA_OS_HOME/scripts/macosPublicAcceptanceSummary.sh
```
...
```text
$EVA_OS_HOME/data/holdings/HoldingsBook.json
```
...
```text
$EVA_OS_HOME/data/holdings/HoldingSymbolMap.json
```
...
python3 -m src.cli research-bus-submit --text "请把这条研究问题同步到 QuantLab" --json
```

共享数据库：

```text
$EVA_OS_HOME/data/researchBus/ResearchBus.sqlite
```
...
```bash
cd $EVA_SYSTEMS_ROOT/fifa_research
...
```bash
$EVA_OS_HOME/scripts/runIndependentValidation.sh run \
...
| 问题 | 处理方式 |
| --- | --- |
| 页面显示 No connection | 运行 `statusQuantLab.sh`；必要时运行 `stopQuantLab.sh` 后重新双击 app |
| 双击 app 没反应 | 查看 `data/cache/quantlab_macos_app.log`，再运行 `scripts/startQuantLab.sh` 做终端诊断 |
