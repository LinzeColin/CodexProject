...
class ScheduledExecutionTests(unittest.TestCase):
    ...
    def test_daily_run_production_ready_with_real_smtp_and_text_artifact(self) -> None:
        ...
        self.assertIn("【M1 科学与理论前沿】", email_body)
        self.assertIn("【1. 今日真正变化】", email_body)
        self.assertIn("【2. 一分钟决策卡】", email_body)
        self.assertIn("【8. B6 个人影响、ROI 和行动】", email_body)
        self.assertIn("预计 ROI", email_body)
        self.assertIn("时间成本", email_body)
        self.assertIn("置信度", email_body)
        self.assertIn("行动路径", email_body)
        self.assertIn("候选队列摘要", email_body)
        self.assertNotIn("【今天讲透一个问题】", email_body)
