...
class GlobalScanTests(unittest.TestCase):
    ...
    def test_delivery_package_requires_stage1_text_and_queue_summary(self) -> None:
        ...
        self.assertIn("【M1 科学与理论前沿】", package["notification"].body)
        self.assertIn("【1. 今日真正变化】", package["notification"].body)
        self.assertIn("【2. 一分钟决策卡】", package["notification"].body)
        self.assertIn("【3. 主讲深度解释】", package["notification"].body)
        self.assertIn("【4. 前沿 Delta】", package["notification"].body)
        self.assertIn("【5. 证据级别与不确定性】", package["notification"].body)
        self.assertIn("【6. B4 社会与时代影响】", package["notification"].body)
        self.assertIn("【7. B5 风险、反证与失败条件】", package["notification"].body)
        self.assertIn("【8. B6 个人影响、ROI 和行动】", package["notification"].body)
        self.assertIn("【9. 复习问题】", package["notification"].body)
        self.assertIn("【10. 反馈】", package["notification"].body)
        self.assertIn("预计 ROI", package["notification"].body)
        self.assertIn("时间成本", package["notification"].body)
        self.assertIn("置信度", package["notification"].body)
        self.assertIn("行动路径", package["notification"].body)
        self.assertIn("M1 科学与理论前沿 · B1+B4/B5/B6", package["notification"].html_body)
        self.assertIn("候选队列摘要", package["notification"].body)
        self.assertIn("已入队候选", package["notification"].body)
        self.assertNotIn("【今天讲透一个问题】", combined_body)
        self.assertNotIn("为什么值得你看", combined_body)
        ...
        self.assertLessEqual(len(package["notification"].body), 2600)
