...
- Files changed: `local_runner.py`, CLI local-runner commands, scheduled execution local enablement flag, migration packaging, local runner tests, local and Stage 1 migration runbooks, governance task/event/status files, and S1P5T05 manifest. The daily-operation hardening update adds `local-runner readiness` and health/daily/watchdog launchd package generation in the same Stage 1 local runner boundary.
...
- Commands run: `PYTHONDONTWRITEBYTECODE=1 PYTHONPYCACHEPREFIX=/tmp/codex_adp_s1p5t05_focus_now PYTHONPATH=arxiv-daily-push/src python3 -m unittest arxiv-daily-push/tests/test_local_runner.py arxiv-daily-push/tests/test_scheduled_execution.py arxiv-daily-push/tests/test_stage1_migration.py -q`; daily-operation hardening revalidation: `PYTHONDONTWRITEBYTECODE=1 PYTHONPYCACHEPREFIX=/tmp/codex_adp_daily_operation_tests2_pycache PYTHONPATH=arxiv-daily-push/src python3 -m unittest arxiv-daily-push/tests/test_local_runner.py arxiv-daily-push/tests/test_stage1_runtime.py arxiv-daily-push/tests/test_scheduled_execution.py -q`; full ADP unit discovery.
- Test results: 15 tests OK in the original S1P5T05 run; daily-operation hardening revalidation passed 21 targeted tests and 221 full ADP tests.
...
- Remaining risks: Gmail-accepted app password/account alignment and one confirmed local run with `real_smtp_sent=true` still need explicit execution before the owner can safely just wait for daily emails; Stage 2 integrated production remains separately blocked by V7.1 P0/P1 gates.
