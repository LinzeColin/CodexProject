...
| MOD-ADP-037 | Phase 12 M1 learning email frontstage | deterministic human-frontstage email renderer | Render a Chinese M1 learning email plus bounded plain-text fallback from the all-arXiv daily package, with `YYYYMMDD -- Project Name -- arXiv Group -- Theme` subject, V1 learning sections, safe arXiv/PDF/ChatGPT entries, and no foreground ROI, Claim Ledger, reading-time, Delta, Release, or video clutter | active | adp-email-decision-ui-v2 | `src/arxiv_daily_push/global_scan.py`, `src/arxiv_daily_push/lesson.py`, `src/arxiv_daily_push/mail_templates.py`, `src/arxiv_daily_push/smtp_delivery.py` |
...
| MOD-ADP-040 | Review8 Stage 1 B1/arXiv report and email text package | deterministic Chinese teaching report and email renderer | Render B1/M1 teaching report/email previews with supported claim evidence, V1 learning sections, safe arXiv/PDF/ChatGPT entries, candidate queue summary, and no production side effects | active | adp-stage1-b1-report-email-v1 | `src/arxiv_daily_push/stage1_b1_report.py`, `src/arxiv_daily_push/mail_templates.py`, `src/arxiv_daily_push/cli.py` |
...
- FORM-ADP-039 validates the M1 learning email frontstage: owner subject contract, Chinese plain text, responsive HTML, learning frontstage payload, safe source/PDF/ChatGPT entries, q-fin candidate filtering, feedback actions, and hidden backend ROI/Claim Ledger/Release/video/reading-time/Delta foreground details.
...
- Active Phase 12 learning email frontstage parameters: PARAM-ADP-276 through PARAM-ADP-279.
