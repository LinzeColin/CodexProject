...
| MOD-ADP-037 | V7.1 M1-compatible email decision UI | deterministic human-frontstage email renderer | Render the Stage 1 arXiv daily email as a V7.1 M1-compatible Chinese HTML/plain-text frontstage with the required 1-10 section order, expected ROI, time cost, confidence, action path, and B1+B4/B5/B6 board framing while keeping backend score, Release, video, and Claim Ledger clutter out of the foreground | active | adp-email-decision-ui-v7.1-m1-compatible | `src/arxiv_daily_push/global_scan.py`, `src/arxiv_daily_push/lesson.py`, `src/arxiv_daily_push/smtp_delivery.py`, `src/arxiv_daily_push/video.py` |
...
- FORM-ADP-039 validates the V7.1 M1-compatible Stage 1 email frontstage: owner subject contract, Chinese plain text, responsive HTML, frontstage lesson payload, exact 1-10 V7.1 section order, expected ROI, time cost, confidence, action path, B1+B4/B5/B6 framing, q-fin candidate filtering, feedback actions, no old `今天讲透一个问题` template marker, no foreground numeric score labels, and hidden backend ROI/Claim Ledger/Release/video foreground details.
...
- Active V7.1 M1-compatible email decision UI parameters: PARAM-ADP-276 through PARAM-ADP-279; PARAM-ADP-277 active plain-text cap is 2600 characters for the ten-section contract.
