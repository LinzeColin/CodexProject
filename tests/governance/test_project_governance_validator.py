...
class ProjectGovernanceValidatorTests(unittest.TestCase):
    ...
    def test_arxiv_owner_status_uses_latest_event_manifest(self) -> None:
        ...
        self.assertEqual(info["latest_event"]["event_id"], "EVENT-20260622-ADP-069")
        self.assertEqual(info["assurance"]["as_of_event_id"], "EVENT-20260622-ADP-069")
        self.assertEqual(info["product_version"], "0.18.0")
        self.assertEqual(info["current_gate"], "ADP-S1-08-LOCAL-RUNTIME-RECOVERY-READY")
        self.assertEqual(
            info["latest_manifest"]["_path"],
            "governance/run_manifests/ADP-S1-08-LOCAL-RUNTIME-RECOVERY-20260622.json",
        )
        rendered = dashboard.render_owner_status(info)
        self.assertIn("0.18.0", rendered)
        self.assertIn("ADP-S1-08-LOCAL-RUNTIME-RECOVERY-READY", rendered)
        self.assertIn("S1-09-MIGRATION_PACKAGE-001", rendered)
        self.assertIn("迁移包", rendered)
    ...
    def test_arxiv_s1_next_task_priority_does_not_reorder_other_projects(self) -> None:
        ...
        config = dashboard.structural.load_yaml(ROOT / "governance" / "projects.yaml")
        expected = {
            "arxiv-daily-push": "S1-09-MIGRATION_PACKAGE-001",
    ...
    def test_adp_s104_dashboard_sync_manifest_binds_root_views(self) -> None:
        ...
        self.assertIn("| `arxiv-daily-push` | `0.18.0` |", dashboard_text)
        backlog_text = (ROOT / "governance" / "binding_backlog.yaml").read_text(encoding="utf-8")
        adp_backlog = backlog_text.split('project_id: "arxiv-daily-push"', 1)[1].split("next_task:", 1)[0]
        self.assertIn("precommit_pending_events: 16", adp_backlog)
