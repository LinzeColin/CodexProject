...
class ProjectGovernanceValidatorTests(unittest.TestCase):
    ...
    def test_review9_s5pbt05_arxiv_project_yaml_preserves_stage_truth(self) -> None:
        ...
        self.assertEqual(project["fact_level"], "EXTRACTED")
        self.assertEqual(project["current_status"], "stage1_accepted_s2pct07_d2_qualification_ready_s2pdt01_next_no_formal_production")
        ...
        self.assertIn("S2PDT01/S2P3T01 中国官方核心数据源", project["summary"])
        self.assertIn("不得宣称 Stage 2 或 integrated production accepted", project["summary"])
        self.assertEqual(len(project["features"]), 14)
        self.assertEqual(len(project["models"]), 60)
        self.assertEqual(len(project["formulas"]), 62)
        self.assertEqual(len(project["parameters"]), 406)
        self.assertEqual(len(project["strategies"]), 3)
        ...
        formula_semantic_counts = Counter(item["semantic_status"] for item in project["formulas"])
        model_semantic_counts = Counter(item["semantic_status"] for item in project["models"])
        self.assertEqual(parameter_semantic_counts["MACHINE_VERIFIED"], 406)
        self.assertEqual(formula_semantic_counts["MACHINE_VERIFIED"], 62)
        self.assertEqual(model_semantic_counts["EXTRACTED"], 60)

        limitations = " ".join(item["statement"] for item in project["limitations"])
        self.assertIn("406/406 semantically checked active parameters", limitations)
        self.assertIn("62/62 active formulas", limitations)
        self.assertIn("60 个 active models 只从 model_registry EXTRACTED", limitations)
        ...
        self.assertEqual(project["delivery_readiness"]["next_executable_task_id"], "S2PDT01")
        ...
        self.assertEqual(matrix["current_iteration"], "ITER-20260624-ADP-S2PCT07-D2-QUALIFICATION")
        ...
        self.assertEqual(matrix["current_v7_task_id"], "S2PDT01")
        ...
        self.assertIn("next S2PDT01 -> S2P3T01", matrix["current_v7_legacy_alias"])
    ...
    def test_review9_s5pbt05_arxiv_roadmap_tracks_single_project_task(self) -> None:
        ...
        self.assertEqual(roadmap["current_task_id"], "S2PDT01")
        ...
        self.assertEqual(china_task["status"], "planned")
    ...
    def test_arxiv_owner_status_uses_latest_event_manifest(self) -> None:
        ...
        self.assertEqual(info["latest_event"]["event_id"], "EVENT-20260624-ADP-100")
        self.assertEqual(info["assurance"]["as_of_event_id"], "EVENT-20260624-ADP-100")
        ...
        self.assertEqual(
            ...
            "governance/run_manifests/ADP-S2PCT07-D2-QUALIFICATION-EVIDENCE-20260624.json",
        )
        ...
        self.assertIn("S2PDT01", rendered)
        self.assertIn("S2P3T01", rendered)
    ...
    def test_adp_v7_1_root_lock_hashes_stage_gates_and_aliases_are_machine_checkable(self) -> None:
        ...
        self.assertEqual(lock["stage2_boundary"]["current_task_id"], "S2PDT01")
