...
class ProjectGovernanceValidatorTests(unittest.TestCase):
    ...
    def test_review9_s5pbt05_arxiv_project_yaml_preserves_stage_truth(self) -> None:
        ...
        self.assertEqual(project["fact_level"], "EXTRACTED")
        self.assertEqual(project["current_status"], "stage1_accepted_s2pct04_profile_shadow_complete_s2pct05_next_no_formal_production")
        ...
        self.assertEqual(len(project["features"]), 12)
        self.assertEqual(len(project["models"]), 57)
        self.assertEqual(len(project["formulas"]), 59)
        self.assertEqual(len(project["parameters"]), 382)
        self.assertEqual(len(project["strategies"]), 3)
        ...
        formula_semantic_counts = Counter(item["semantic_status"] for item in project["formulas"])
        model_semantic_counts = Counter(item["semantic_status"] for item in project["models"])
        self.assertEqual(parameter_semantic_counts["MACHINE_VERIFIED"], 382)
        self.assertEqual(formula_semantic_counts["MACHINE_VERIFIED"], 59)
        self.assertEqual(model_semantic_counts["EXTRACTED"], 57)

        limitations = " ".join(item["statement"] for item in project["limitations"])
        self.assertIn("382/382 active parameters", limitations)
        self.assertIn("59/59 active formulas", limitations)
        self.assertIn("57 个 active models 只从 model_registry EXTRACTED", limitations)
        ...
        self.assertEqual(project["delivery_readiness"]["next_executable_task_id"], "S2PCT05")
        ...
        self.assertEqual(matrix["current_iteration"], "ITER-20260624-ADP-S2PCT04-JOURNAL-PROFILE")
        ...
        self.assertEqual(matrix["current_v7_task_id"], "S2PCT04")
        self.assertEqual(matrix["current_v6_task_id"], "S2P2T04")
        ...
        self.assertIn("next S2PCT05", matrix["current_v7_legacy_alias"])
    ...
    def test_review9_s5pbt05_arxiv_roadmap_tracks_single_project_task(self) -> None:
        ...
        self.assertEqual(roadmap["current_task_id"], "S2PCT05")
        self.assertEqual(roadmap["next_gate_id"], "S2PC-GATE-V7-CONTRACT-BLOCKED")
        self.assertEqual(roadmap["total_estimated_hours"], 16.5)
        self.assertEqual(roadmap["completed_estimated_hours"], 14.5)
        ...
        self.assertEqual([task["task_id"] for task in tasks], ["S2PBT01", "S2PCT01", "S2PCT02", "S2PCT03", "S2PCT04", "S2PCT05", "S5PBT05"])
        task = tasks[0]
        self.assertEqual(task["status"], "completed")
        ...
        self.assertEqual(engineering_task["status"], "planned")
    ...
    def test_review9_s5pbt05_arxiv_human_files_render_without_drift(self) -> None:
        ...
        self.assertIn("MOD-ADP-057", model_text)
        self.assertIn("FORM-ADP-059", model_text)
        self.assertIn("PARAM-ADP-399", model_text)
        for text in (feature_text, dev_text, model_text):
            self.assertNotIn("docs/governance/", text.splitlines()[0])
    ...
    def test_adp_v7_1_root_lock_hashes_stage_gates_and_aliases_are_machine_checkable(self) -> None:
        ...
        self.assertEqual(lock["stage2_boundary"]["current_task_id"], "S2PCT05")
